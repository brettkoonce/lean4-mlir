#!/usr/bin/env bash
# Per-epoch clock + host-memory guard for the R34 bf16 verified run: one row per checkpoint-marker
# change. The checkpoint is ONE overwritten file, so its mtime history exists only if logged here.
# RssAnon is the leak guard (13d90e68's mimalloc bug showed up as monotone anon growth); the large
...
OUT=runs/2026-09-16-r34-bf16-90ep/epoch_clock.tsv
