...
#
# ⚠ R34's shim is FLIP-ONLY (no AutoAugment, no RandAugment), the lightest in the fleet. If these
# rows stay flat for the whole run, the fault is augmentation-weight-dependent; if they climb, it
# is not. Either way this file is the evidence.
#
...
OUT=runs/2026-09-16-r34-bf16-90ep/loader_rss.tsv
...
while systemctl --user is-active -q r34-bf16; do
  ...
  if [ -n "$n" ] && [ "$n" != "$last" ]; then
    for p in $(pgrep -f generated_resnet34_imagenet_shim | sort -n); do
