...
E=.lake/build/resnet50in160_lambaccdp4x128wxclipbcebf16_ckpt_xla.bin.epoch
OUT=runs/2026-09-23-r50-a3-syncbn-bf16-100ep/loader_rss.tsv
[ -s "$OUT" ] || printf 'utc\tepoch\tpid\tetime_s\trss_gib\tarena_class_gib\tarena_class_maps\tthreads\n' > "$OUT"
last=""
while systemctl --user is-active -q r50-a3; do
  n=$(tr -cd 0-9 < "$E" 2>/dev/null)
  if [ -n "$n" ] && [ "$n" != "$last" ]; then
    for p in $(pgrep -f generated_resnet50_imagenet_short_shim | sort -n); do
