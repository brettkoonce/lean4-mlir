#!/bin/bash
# Arms C and D, queued behind the A/B driver.
#   C = 8 producers but prefetch depth 4  -> isolates IN-FLIGHT BUFFER COUNT from producer supply.
#   D = 4 producers (depth follows)       -> the configuration the 3060 box's 48 h run actually used;
#                                            the conf moved 4 -> 8 on 2026-09-10.
set -u
cd "$(dirname "$0")/../.."
D=runs/2026-09-11-vit-leak-ab
while pgrep -f 'vit-leak-ab/run.sh' >/dev/null; do sleep 10; done
sleep 5
run_arm () {
  ...
  while kill -0 $job 2>/dev/null; do
    ...
    if [ -n "${p:-}" ]; then
      ...
      local fr; fr=$(free -g | awk 'NR==2{print $4}')
      [ -n "${ra:-}" ] && printf "%s\t%s\t%.2f\t%s\n" "$(( $(date +%s) - t0 ))" "${st:-0}" "$(python3 -c "print($ra/1048576)")" "$fr" >> "$D/$name.rss.tsv"
    fi
    sleep 5
  done
  wait $job
