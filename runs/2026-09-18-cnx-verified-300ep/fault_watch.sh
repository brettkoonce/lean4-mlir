#!/usr/bin/env bash
# Durable detector for the ConvNeXt bf16 run. Writes WATCH_EVENT and exits on whichever comes
# first — a SUSTAINED loader-fault signature, or the run ending.
#
# ⚠⚠ SELF-CALIBRATING, unlike R34's, which hardcoded 950 s. ConvNeXt's epoch time on this box is a
# forecast until the run produces it, and a hardcoded threshold derived from a probe is a threshold
# that fires on a forecast error. Baseline = MEDIAN of the epochs in [BASE_FROM, BASE_TO], taken
  ...
  if [ -n "$hit" ]; then
    ...
    if awk -v a="$av" -v g="$AVAIL_GIB" 'BEGIN{exit !(a>g)}'; then
      { echo "⚠⚠ SUSTAINED LOADER FAULT at $(date -u '+%F %T') UTC"
        ...
        echo "⛔ THE RESPAWN WAS ON (LEAN_MLIR_SHIM_RESPAWN_EPOCHS=10) — so this is the mitigation"
        echo "   FAILING IN PRODUCTION, which is a more valuable finding than the top-1. Record it"
        echo "   in RESULTS.md verbatim, then kill right after a checkpoint and resume with"
        echo "   REST_EPOCHS every 40 (REST_SECS=15) — B0's cure, ~2 min each."
