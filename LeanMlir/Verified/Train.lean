...
structure VerifiedNet where
  ...
  name     : String
  /-- **Which directory this net's artifacts live in.** Default `verified_mlir/` — the committed
      corpus, whose contents `scripts/regen_verified_mlir.sh check` pins to exactly the set with a
      literal `IO.FS.writeFile "verified_mlir/…"` writer in LeanMlir/Proofs/Codegen/.

      The width/batch sweep nets (`mlpG`, `cnnG`, `cifar8BnG`) set `.lake/build` instead: they
      render their artifact at run time from argv and train on it at once, so those files are build
      products, not committed renders. Keeping them out of `verified_mlir/` is what lets the writer
      audit pin that directory — the audit greps for a literal path, and these writers interpolate a
      slug.

      A field rather than a global because the read sites are per-net. -/
  ...
  /-- **Does `<slug>_train_step.mlir` return the trailing report-only `%loss` scalar?**
      ...
      A per-render fact, not a driver-wide one: true for the `mlp` and `cnn` renders, which emit
      the loss, and false for the nets whose train step returns parameters only.

      A wrong value here cannot corrupt anything: the G4 arity gate compares the module's real
      output count against what the driver supplies and refuses on any mismatch. -/
  lossSlot : Bool := false
  ...
  /-- **Stochastic-depth keep probabilities**, one per drop site, in the render's signature order.
      Empty = no drop sites. Consumed only by `*drop*` variants (`VerifiedVariant.sdOn`).

      The driver owns the ramp, deliberately: the emitted op is a
      ...
      It is therefore a second hand-list against the renderer's `enetDropIdxs`, like
      ...
      `Verified.Spec` sits downstream of this file, so the renderer cannot share the definition by
      import without inverting the dependency. -/
  dropKeeps : Array Float := #[]
  /-- **Classifier dropout** — `(keep_prob, per-example width)`, or
      ...
      The width is here because the mask is per-element, which is the difference from
  ...
  /-- **The generated ImageNet batch shim this net streams**, as a bare filename under
      ...
      Each net's shim carries its own reference's augmentation (EfficientNet's sets
      `useAutoAugment`, ViT's RandAugment m9/mstd0.5/inc1 + random erasing + repeated aug ×3,
      ConvNeXt's RandAugment + random erasing), so streaming another net's shim trains on the
      wrong recipe.

      There is deliberately no fallback: an empty value on an `.imagenet` net refuses at spawn,
      because a wrong shim fails silently — the wrong augmentation compiles, streams, trains and
      descends. `scripts/gen_shims.sh` writes all five;
...
structure VerifiedConfig where
  ...
  /-- **ConvNeXt `_init_weights` — the verified peer of `TrainConfig.cnxInit`.**
      ...
      reach the weights. It must match the JAX reference's setting
      ([`jax/MainConvNeXtImagenet.lean`](https://github.com/brettkoonce/lean4-mlir/blob/main/jax/MainConvNeXtImagenet.lean)
      sets `cnxInit := true`), or a pair run compares two inits.

      Separate from `vitInit`, deliberately — the same split `TrainConfig` keeps on the JAX side:
      the two specs disagree on the conv path. timm's ViT leaves the patch
  ...
  /-- BatchNorm running-statistic **decay** — the verified peer of `TrainConfig.bnMomentum`,
      ...
      the timm audit; keep the two in step, since a reference ↔ verified gap here is invisible
      in every loss curve.
...
/-- **The 95% Wilson score interval on an accuracy**, as `lo–hi` in percentage points.
    ...
    It sizes the digits an eval line can claim: on a 3,925-image validation set at p ≈ 0.85 the
    95% half-width is ±1.11 pt.
...
-- Measurement behind the cross-thread-free note below: runs/2026-09-11-vit-leak-ab/README.md.
...
/-- Spawn `n` shim processes over disjoint shards of one split, and read them round-robin.
    ...
    One shim process can be slower than a multi-replica step consumes images; several producers
    raise the loader's throughput.
...
/-- Print the startup banner with `%LOWERER%` resolved to the lowerer that actually ran.
    ...
    The blurb carries a placeholder rather than a transport, so the banner states the run, not
    the build; every print site goes through here. -/
...
/-- The eval forward's rendered input shape, `(batch, d0)`, off `%x: tensor<BxWxf32>`.
    ...
    Both numbers come from one parse of one declaration, so they cannot disagree about the
    same tensor.
...
/-- **fp8 (E4M3) Lean trainer** — the low-precision sibling of `trainLinear`.
    ...
    render-tie model of [`Proofs/Float/E4M3Fold.lean`](https://github.com/brettkoonce/lean4-mlir/blob/main/LeanMlir/Proofs/Float/E4M3Fold.lean)). Eval runs the fp32
...
/-- **fp8 (E4M3) packed-params trainer** — the low-precision sibling of
    ...
    **Scope:** host-side prep reaches weights + the *input* activation only. The
    intermediate activations (relu/pool/flatten outputs feeding the deeper matmuls)
    and the backward-chain cotangents are computed *inside* the fused kernel and
    stay fp32 — quantizing them would need in-graph E4M3 ops, not host byte-prep.
    So this is **fp8 weights + fp8 input, fp32 intermediates**. Eval runs the fp32 master.
...
--   (`Verified.Spec` is the import-free DSL; these are the only places a spec meets the runtime)
-- ════════════════════════════════════════════════════════════════
