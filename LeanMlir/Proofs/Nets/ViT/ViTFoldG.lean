...
`ViTFold.lean` makes every parameter output of the SGD-inline `vit_train_step.mlir`
...
the fused file. That is not an ambiguity: the tie (`ViTStepTie.lean`) is what says which forward a
...
* Every lemma is `∀ cot`, so each holds at the actual backward-chain cotangent without naming it.
  Pinning them is the §1a tie (`ViTStepTie.lean`, 200 params); re-pointing that capstone at these
