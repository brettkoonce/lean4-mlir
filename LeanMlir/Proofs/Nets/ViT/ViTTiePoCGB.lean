...
`ViTTiePoC.lean` ties all 200 parameters of the SGD-inline `vit_train_step.mlir`: each fused
...
`ConvNeXtTiePoCGB.lean` (§4b.6) moved for ConvNeXt, and it is that file's transformation applied
...
`ViTFaithfulPoCGB.lean` (§4c-ter) is the fold each conjunct delegates to.
...
Every save, every chain cotangent and every Jacobian witness is `ViTTiePoC.lean`'s, lifted; every
