...
-- between a conv and its ReLU. `convBnReluPC_has_vjp_at` folds the conv, per-channel BN,
-- and ReLU VJPs via `vjp_comp_at`; the 8-conv BN net below chains it. BN is
