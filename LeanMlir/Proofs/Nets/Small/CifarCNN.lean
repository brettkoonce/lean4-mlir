...
`vjpCompAt`, just longer and with **two** maxpool steps. Spatial bookkeeping
...
* `cifarCnnHasVJPAt` — the **structural** whole-network VJP: the composed
  ...
  of `mnistCnnNoBnHasVJPAt`.

* `Tiny.cifarTinyCnnHasVJP_correct` — a **concrete instance** where every
...
/-- **CIFAR 2D CNN (no BN) whole-network VJP at a smooth point.**
    ...
    and the two MaxPools. Built by `vjpCompAt` through
    `convRelu → convRelu → maxPool → convRelu → convRelu → maxPool →
     denseRelu → denseRelu → dense`. The Chapter-4 sibling of
    `mnistCnnNoBnHasVJPAt` (two conv stages, two pools). -/
noncomputable def cifarCnnHasVJPAt
    {ic c1 c2 h w d1 nClasses kH kW : Nat}
    (W₁ : Kernel4 c1 ic kH kW) (b₁ : Vec c1)
    (W₂ : Kernel4 c1 c1 kH kW) (b₂ : Vec c1)
    (W₃ : Kernel4 c2 c1 kH kW) (b₃ : Vec c2)
  ...
  have s4v := convReluHasVJPAt W₃ b₃ zp1 h3
  ...
  have s5v := convReluHasVJPAt W₄ b₄ z3 h4
...
/-- **Unconditional whole-network VJP for a concrete tiny CIFAR CNN.** Every
    smoothness hypothesis of `cifarCnnHasVJPAt` is discharged — the two
...
theorem cifarTinyCnnHasVJP_correct (dy : Vec 2) (i : Fin (1 * (2*(2*1)) * (2*(2*1)))) :
    cifarTinyCnnHasVJPAt.backward dy i =
      ∑ j : Fin 2,
        pdiv (cifarCnnForward K Bz K Bz K Bz K Bz Wd5 Bp Wd6 Bp Wd7 Bz2) X i j * dy j :=
  cifarTinyCnnHasVJPAt.correct dy i
...
-- flat pool out `32·2·2=128`. Built exactly as `cifarCnnHasVJPAt` — a longer
-- `vjpCompAt` chain through `convRelu`×8 + `maxPool`×4 + `denseRelu`×2 + dense.
-- ════════════════════════════════════════════════════════════════
...
noncomputable def cifarCnn8HasVJPAt
  ...
  -- stage 1: conv→relu ×2
  have s1 := convReluHasVJPAt W₁ b₁ x hf1
  have s1d := convRelu_differentiableAt W₁ b₁ x hf1
  have s2v := convReluHasVJPAt W₂ b₂ _ hf2
  ...
  have s4v := convReluHasVJPAt W₃ b₃ zp1 hf3
  ...
  have s5v := convReluHasVJPAt W₄ b₄ z3 hf4
  ...
  have s13v := denseReluHasVJPAt W₉ b₉ zp4 hf9
  have s13d9 := denseRelu_differentiableAt W₉ b₉ zp4 hf9
  have s13 := vjpCompAt _ _ x s12d s13d9 s12 s13v
  have s13d := s13d9.comp x s12d
  set z9 := (relu d1 ∘ dense W₉ b₉) zp4 with hz9
  have s14v := denseReluHasVJPAt Wa ba z9 hfa
  have s14da := denseRelu_differentiableAt Wa ba z9 hfa
  have s14 := vjpCompAt _ _ x s13d s14da s13 s14v
...
--
-- A per-example **per-channel** BatchNorm (`bnPerChannelTensor3`, `m=h·w`: normalize
-- each channel over its own h·w spatial cells, vector γ/β `[oc]` — PerChannelBN.lean)
-- between a conv and its ReLU. `convBnReluPCHasVJPAt` folds the conv, per-channel BN,
-- and ReLU VJPs via `vjpCompAt`; the 8-conv BN net below chains it. BN is
-- differentiable everywhere (ε > 0), so the only new side conditions are `0 < ε` and
-- smoothness at the POST-BN pre-activations.
-- ════════════════════════════════════════════════════════════════

/-- **conv → per-channel-BN → relu block VJP at a smooth point.** The per-channel
    (`bnPerChannelTensor3`, `m=h·w`) sibling of `convBnReluHasVJPAt`: the same two
    `vjpCompAt` chain with `bnPerChannelTensor3` (vector γ/β `[oc]`, its own
    block-diagonal VJP) in place of the scalar-global `bnForward`. -/
noncomputable def convBnReluPCHasVJPAt {ic oc h w kH kW : Nat}
  ...
  stageHasVJPAt (flatConv W b) (bnPerChannelTensor3 oc h w ε γ β) (relu (oc * h * w)) v
    (flatConv_differentiable W b) (HasVJP3.toHasVJP (conv2dHasVJP3 W b))
    (bnPerChannelTensor3_differentiable oc h w ε hε γ β)
    (bnPerChannelTensor3HasVJP oc h w ε hε γ β)
...
noncomputable def cifarCnnBn8HasVJPAt
  ...
  -- stage 1
  have s1 := convBnReluPCHasVJPAt W₁ b₁ ε₁ γ₁ β₁ hε₁ x h1
  have s1d := convBnReluPC_differentiableAt W₁ b₁ ε₁ γ₁ β₁ hε₁ x h1
  have s2v := convBnReluPCHasVJPAt W₂ b₂ ε₂ γ₂ β₂ hε₂ _ h2
  ...
  -- stage 2
  set zp1 := maxPoolFlat c1 (2*(2*(2*h))) (2*(2*(2*w))) z1 with hzp1
  have s4v := convBnReluPCHasVJPAt W₃ b₃ ε₃ γ₃ β₃ hε₃ zp1 h3
  ...
  have s5v := convBnReluPCHasVJPAt W₄ b₄ ε₄ γ₄ β₄ hε₄ z3 h4
  ...
  -- stage 3
  set zp2 := maxPoolFlat c2 (2*(2*h)) (2*(2*w)) z2 with hzp2
  have s7v := convBnReluPCHasVJPAt W₅ b₅ ε₅ γ₅ β₅ hε₅ zp2 h5
  ...
  have s8v := convBnReluPCHasVJPAt W₆ b₆ ε₆ γ₆ β₆ hε₆ z5 h6
  ...
  -- stage 4
  set zp3 := maxPoolFlat c3 (2*h) (2*w) z3p with hzp3
  have s10v := convBnReluPCHasVJPAt W₇ b₇ ε₇ γ₇ β₇ hε₇ zp3 h7
  ...
  have s11v := convBnReluPCHasVJPAt W₈ b₈ ε₈ γ₈ β₈ hε₈ z7 h8
  ...
  -- dense head
  set zp4 := maxPoolFlat c4 h w z4p with hzp4
  have s13v := denseReluHasVJPAt W₉ b₉ zp4 h9
  have s13d9 := denseRelu_differentiableAt W₉ b₉ zp4 h9
  have s13 := vjpCompAt _ _ x s12d s13d9 s12 s13v
  have s13d := s13d9.comp x s12d
  set z9 := (relu d1 ∘ dense W₉ b₉) zp4 with hz9
  have s14v := denseReluHasVJPAt Wa ba z9 ha
