...
⛔ **Two corrections to this header, 2026-09-06 (from `MobileNetV2FoldPaperG.lean`).** It
...
* These folds are at the FUSED `θ − lr·g` ops, which only this SGD-inline render emits.
  ...
  which is batched, AdamW-only and un-fused; `MobileNetV2FoldPaperG.lean` is the fold
  there.
-/
