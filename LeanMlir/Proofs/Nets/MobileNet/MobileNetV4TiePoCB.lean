...
`MobileNetV4FaithfulPoCB.lean` proves every parameter gradient node denotes the certified gradient
...
/-- ⭐⭐ **And it holds** — twelve instantiations of `MobileNetV4FaithfulPoCB`'s `∀ cot` fold with
...
/-- ⭐⭐ **And it holds** — twelve instantiations (the first at the STRIDED depthwise) of `MobileNetV4FaithfulPoCB`'s `∀ cot` fold with
    the freedom removed. Nothing here is new mathematics; what is new is that the cotangents are
    the chain's, not free. -/
