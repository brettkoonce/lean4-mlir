...
`MobileNetV2FaithfulPoCPaperG.lean` (4b.4) makes every parameter GRADIENT node of the batched
...
cotangent. With 4.2b it completes MobileNetV2's T3, and it is `ResNet34TiePoCB.lean`'s peer.
...
`EfficientNetTiePoC.enetHeadTied` spells B0's identically-shaped head. -/
...
/-! Each conjunct is `MobileNetV2FaithfulPoCPaperG`'s `∀ cot` fold instantiated at the cotangent the
...
/-- **Head, tied.** The 1x1 conv's weight and bias, its BatchNorm's γ/β, and the classifier's
    ...
    `x` is, so the statement is `x`-free (the shape `EfficientNetTiePoC`'s bias conjuncts take). -/
