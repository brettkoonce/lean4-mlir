...
`ResNet34FaithfulPoCB.lean` and `EfficientNetFaithfulPoCG.lean` state their folds at op kinds, not
...
## ⛔ Two corrections to `MobileNetV2FaithfulPoCPaper.lean`'s header, found here
...
* Every lemma is `∀ cot`: each holds at the actual backward-chain cotangent without naming it.
  Pinning them is the §1a tie, and it landed 2026-09-06 as `Nets/MobileNet/MobileNetV2TiePoCB.lean`
