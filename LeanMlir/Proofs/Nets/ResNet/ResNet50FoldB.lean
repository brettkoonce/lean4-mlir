...
eight lemmas turned out to be `ResNet34FoldB.lean`'s. For ResNet-50 it is **six of six**:
...
⛔ **ResNet-50 emits no conv BIAS gradient at all.** `ResNet34FoldB.lean` and
`MobileNetV2FoldPaperG.lean` both carry bias conjuncts to cover the `convBias := true`
