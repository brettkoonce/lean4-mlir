...
| file | role |
|---|---|
| `StableHLO/Basic.lean` | the `SHlo` AST, `den` (its ℝ semantics), the per-op `*_faithful` lemmas. A module that only states `den` facts imports this alone. Header has a table of contents |
...
| EfficientNet-B0 | `EfficientNetRender/Basic` | `efficientnetFwdGraphBFull_faithful` | `efficientnet_net_tiedG` | `efficientnetInputGradBFull_eq_efficientnetForwardB_full_vjp` |
