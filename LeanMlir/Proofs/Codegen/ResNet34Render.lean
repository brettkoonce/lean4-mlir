...
private def idBackSgd (B c hh : Nat) (epsStr lrStr p : String) (f : BFwd) (dyName : String)
  ...
  let (cb1, nb1) ← if convBias then pretty B (.convBiasSgd s!"%{p}b1" lrStr zk zT zc 0 (.operand nDn1 zin)) else pure ("", "")
  ...
  let (cb2, nb2) ← if convBias then pretty B (.convBiasSgd s!"%{p}b2" lrStr zk zT zc 0 (.operand nDn2 zin)) else pure ("", "")
  ...
  pure { code := cDa ++ cDn2 ++ cDc2 ++ cDr1 ++ cDn1 ++ cDc1 ++ cDx ++
         ...
         dx := nDx,
         names := [nW1] ++ (if convBias then [nb1] else []) ++ [ng1, nt1] ++
                  [nW2] ++ (if convBias then [nb2] else []) ++ [ng2, nt2] }
...
private def downBackSgd (B cin c hh : Nat) (epsStr lrStr p : String) (f : BFwd) (dyName : String)
  ...
  let (cb1, nb1) ← if convBias then pretty B (.convStridedBiasSgd s!"%{p}b1" lrStr zk1 zinS zc 0 (.operand nDn1 zout)) else pure ("", "")
  ...
  pure { code := cDa ++ cDn2 ++ cDc2 ++ cDr1 ++ cDn1 ++ cDc1 ++ cDnp ++ cDcp ++ cDx ++
         ...
         names := [nW1] ++ (if convBias then [nb1] else []) ++ [ng1, nt1] ++
                  [nW2] ++ (if convBias then [nb2] else []) ++ [ng2, nt2] ++
                  [nWp] ++ (if convBias then [nbp] else []) ++ [ngp, ntp] }
...
def resnet34TrainStepFaithfulV (B nClasses : Nat) (epsStr lrStr : String)
  ...
  let go : StateM Nat String := do
    ...
    let b16 ← idBackSgd   B 512 7 epsStr lrStr "s4b1" blk[15]! nDgi convBias
    let b15 ← idBackSgd   B 512 7 epsStr lrStr "s4b0" blk[14]! b16.dx convBias
    let b14 ← downBackSgd B 256 512 7 epsStr lrStr "d4" blk[13]! b15.dx convBias
    let b13 ← idBackSgd   B 256 14 epsStr lrStr "s3b4" blk[12]! b14.dx convBias
    let b12 ← idBackSgd   B 256 14 epsStr lrStr "s3b3" blk[11]! b13.dx convBias
    let b11 ← idBackSgd   B 256 14 epsStr lrStr "s3b2" blk[10]! b12.dx convBias
    let b10 ← idBackSgd   B 256 14 epsStr lrStr "s3b1" blk[9]! b11.dx convBias
    let b9  ← idBackSgd   B 256 14 epsStr lrStr "s3b0" blk[8]! b10.dx convBias
    let b8  ← downBackSgd B 128 256 14 epsStr lrStr "d3" blk[7]! b9.dx convBias
    let b7  ← idBackSgd   B 128 28 epsStr lrStr "s2b2" blk[6]! b8.dx convBias
    let b6  ← idBackSgd   B 128 28 epsStr lrStr "s2b1" blk[5]! b7.dx convBias
    let b5  ← idBackSgd   B 128 28 epsStr lrStr "s2b0" blk[4]! b6.dx convBias
    let b4  ← downBackSgd B 64 128 28 epsStr lrStr "d2" blk[3]! b5.dx convBias
    let b3  ← idBackSgd   B 64 56 epsStr lrStr "s1b2" blk[2]! b4.dx convBias
    let b2  ← idBackSgd   B 64 56 epsStr lrStr "s1b1" blk[1]! b3.dx convBias
    ...
    let (csb, nsb) ← if convBias then
      pretty B (.convStridedBiasSgd "%sbi" lrStr zSk zx z64 0 (.operand nDsn zSt112))
    else pure ("", "")
    ...
    let outNames : List String :=
      [nsW] ++ (if convBias then [nsb] else []) ++ [nsg, nst] ++
