...
where the Gram/Schatten and LipSDP certificates are quadratic-to-cubic — so it
is the tier that could scale past the h = 16 wall. Every dense instance here is
still at h = 16 (784→16→10). And
...
bracket `mlp2_boxSound`. Any tighter bracket therefore plugs into this tier rather
than forking it. The CROWN bound of `CrownBound.lean` plugs in one rung lower, on the
margin, through `certified_of_marginPos`.

Everything is elementary and 3-axiom clean; instance files:
`LipschitzCert.ScorecardIBP*` (generated by
`scripts/certs/lipschitz_cert_scorecard_ibp.py`). -/
...
/-- `f` is *certified at `L∞` radius ε* on input `x` with class `y`: every
    ...
    `L∞` peer of `CertifiedAt` (`DenseEuclid.lean`). -/
...
/-- **IBP `L∞` certificate for `dense ∘ relu ∘ dense`.** If the propagated
    ...
    the generated instance files (`LipschitzCert.ScorecardIBP*`) cite it in
