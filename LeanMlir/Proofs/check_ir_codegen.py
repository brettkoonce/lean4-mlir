...
def invres_fwd_ref(x, We, ge, beb, Wd, gd, bdb, Wp, gp, bpb, eps):
    """MobileNetV2 inverted residual (stride-1, cin=cout): expand-1×1→BN→relu6 →
    depthwise→BN→relu6 → project-1×1→BN (linear bottleneck) + skip. BN = flat LN;
    conv biases folded into BN."""
    r6 = ACT_REF["relu6"][0]
    c, h, w = x.shape[1:]; cmid = We.shape[0]; Me, Mp = cmid*h*w, c*h*w
    e_r = r6(bn_fwd_ref(_convsame(x, We).reshape(1, Me), ge, beb, eps)).reshape(1, cmid, h, w)
    d_r = r6(bn_fwd_ref(dw_fwd_ref(e_r, Wd, np.zeros(cmid, np.float32)).reshape(1, Me), gd, bdb, eps)).reshape(1, cmid, h, w)
    p_n = bn_fwd_ref(_convsame(d_r, Wp).reshape(1, Mp), gp, bpb, eps).reshape(1, c, h, w)
    return x + p_n

def invres_back_ref(x, We, ge, beb, Wd, gd, bdb, Wp, gp, bpb, eps, dOut):
    """Inverted-residual input gradient dx = dOut + body.back(dOut). relu6' is the
    smooth-point subgradient 1[0<x<6]."""
    r6f, r6b = ACT_REF["relu6"]
    c, h, w = x.shape[1:]; cmid = We.shape[0]; Me, Mp = cmid*h*w, c*h*w
    e_c = _convsame(x, We); e_n = bn_fwd_ref(e_c.reshape(1, Me), ge, beb, eps).reshape(1, cmid, h, w); e_r = r6f(e_n)
    d_c = dw_fwd_ref(e_r, Wd, np.zeros(cmid, np.float32)); d_n = bn_fwd_ref(d_c.reshape(1, Me), gd, bdb, eps).reshape(1, cmid, h, w); d_r = r6f(d_n)
    p_c = _convsame(d_r, Wp)
    dpc = bn_back_ref(p_c.reshape(1, Mp), gp, dOut.reshape(1, Mp), eps).reshape(1, c, h, w)
    ddr = conv_input_vjp_ref(Wp, dpc, cmid, h, w)
    ddc = bn_back_ref(d_c.reshape(1, Me), gd, r6b(d_n, ddr).reshape(1, Me), eps).reshape(1, cmid, h, w)
    des = dw_back_ref(ddc, Wd)
    dec = bn_back_ref(e_c.reshape(1, Me), ge, r6b(e_n, des).reshape(1, Me), eps).reshape(1, cmid, h, w)
    return dOut + conv_input_vjp_ref(We, dec, c, h, w)

def convnext_fwd_ref(x, Wdw, gln, bln, Wex, bex, Wpr, bpr, gls, eps):
    """ConvNeXt block: depthwise(7×7)→LN → expand-1×1+b→gelu → project-1×1+b →
    layerScale(γ⊙) + skip. LN = flat layerNormForward; depthwise bias folded in."""
    gel = ACT_REF["gelu"][0]
    c, h, w = x.shape[1:]; cExp = Wex.shape[0]; M, Mex = c*h*w, cExp*h*w
    ln = bn_fwd_ref(dw_fwd_ref(x, Wdw, np.zeros(c, np.float32)).reshape(1, M), gln, bln, eps).reshape(1, c, h, w)
    ex = _convsame(ln, Wex) + bex[None, :, None, None]
    ge = gel(ex.reshape(1, Mex)).reshape(1, cExp, h, w)
    pr = _convsame(ge, Wpr) + bpr[None, :, None, None]
    return x + gls * pr

def convnext_back_ref(x, Wdw, gln, bln, Wex, bex, Wpr, bpr, gls, eps, dOut):
    """ConvNeXt input gradient dx = dOut + body.back(dOut). Everything smooth:
    layerScale-back (γ⊙) → project VJP → gelu-back → expand VJP → LN-back → dw VJP."""
    gelb = ACT_REF["gelu"][1]
    c, h, w = x.shape[1:]; cExp = Wex.shape[0]; M, Mex = c*h*w, cExp*h*w
    d_c = dw_fwd_ref(x, Wdw, np.zeros(c, np.float32))
    ln = bn_fwd_ref(d_c.reshape(1, M), gln, bln, eps).reshape(1, c, h, w)
    ex = _convsame(ln, Wex) + bex[None, :, None, None]
    dge = conv_input_vjp_ref(Wpr, gls * dOut, cExp, h, w)              # layerScale + project VJP
    dex = gelb(ex.reshape(1, Mex), dge.reshape(1, Mex)).reshape(1, cExp, h, w)
    dln = conv_input_vjp_ref(Wex, dex, c, h, w)                        # expand VJP
    ddc = bn_back_ref(d_c.reshape(1, M), gln, dln.reshape(1, M), eps).reshape(1, c, h, w)
    return dOut + dw_back_ref(ddc, Wdw)                                # LN-back folded above, dw VJP
...
# ── MobileNetV2 inverted residual (linear bottleneck, relu6): fwd + dx ──
iv_args = [mbx, mbWe, mbge, mbbe, mbWd, mbgd, mbbd, mbWp, mbgp, mbbp]   # (MBConv params minus SE)
outs, nb = compile_run("/tmp/invres_fwd.mlir", "invres_fwd", iv_args)
e = max_err(outs, [invres_fwd_ref(*iv_args, EPS)]); ok &= e < TOL
print(f"invres_fwd      ({nb}B): max_err={e:.2e}  {'PASS' if e < TOL else 'FAIL'}  (expand→dw→project + skip; relu6)")
outs, nb = compile_run("/tmp/invres_back.mlir", "invres_back", iv_args + [mbdO])
e = max_err(outs, [invres_back_ref(*iv_args, EPS, mbdO)]); ok &= e < TOL
print(f"invres_back     ({nb}B): max_err={e:.2e}  {'PASS' if e < TOL else 'FAIL'}  (dx: relu6 smooth-point + dw/conv input-VJP)")

# ── ConvNeXt block (dw7×7→LN→inverted MLP w/ gelu→layerScale + skip): fwd + dx ──
xc, xe, xh, xw, xk = 2, 8, 4, 4, 7
cnx  = rng.standard_normal((1, xc, xh, xw)).astype(np.float32)
cnWdw = rng.standard_normal((xc, xk, xk)).astype(np.float32); cngln, cnbln = _msc(), _msc()
cnWex = rng.standard_normal((xe, xc, 1, 1)).astype(np.float32); cnbex = rng.standard_normal((xe,)).astype(np.float32)
cnWpr = rng.standard_normal((xc, xe, 1, 1)).astype(np.float32); cnbpr = rng.standard_normal((xc,)).astype(np.float32)
cngls = rng.standard_normal((1, xc, xh, xw)).astype(np.float32)
cndO  = rng.standard_normal((1, xc, xh, xw)).astype(np.float32)
cn_args = [cnx, cnWdw, cngln, cnbln, cnWex, cnbex, cnWpr, cnbpr, cngls]
outs, nb = compile_run("/tmp/convnext_fwd.mlir", "convnext_fwd", cn_args)
e = max_err(outs, [convnext_fwd_ref(*cn_args, EPS)]); ok &= e < TOL
print(f"convnext_fwd    ({nb}B): max_err={e:.2e}  {'PASS' if e < TOL else 'FAIL'}  (dw7×7→LN→expand→gelu→project→layerScale + skip)")
outs, nb = compile_run("/tmp/convnext_back.mlir", "convnext_back", cn_args + [cndO])
e = max_err(outs, [convnext_back_ref(*cn_args, EPS, cndO)]); ok &= e < TOL
print(f"convnext_back   ({nb}B): max_err={e:.2e}  {'PASS' if e < TOL else 'FAIL'}  (dx: layerScale + gelu-back + dw/conv input-VJP)")
...
try:
    if "hip" in rt.query_available_drivers() and (arch := gpu_arch()):
        ...
        outs, _ = compile_run("/tmp/invres_back.mlir", "invres_back", iv_args + [mbdO],
                              backend="rocm", config="hip", extra=extra)
        eg = max_err(outs, [invres_back_ref(*iv_args, EPS, mbdO)]); ok &= eg < TOL
        print(f"invres_back     (hip/{arch}): max_err={eg:.2e}  {'PASS' if eg < TOL else 'FAIL'}  (MobileNetV2 inverted residual)")
        outs, _ = compile_run("/tmp/convnext_back.mlir", "convnext_back", cn_args + [cndO],
                              backend="rocm", config="hip", extra=extra)
        eg = max_err(outs, [convnext_back_ref(*cn_args, EPS, cndO)]); ok &= eg < TOL
        print(f"convnext_back   (hip/{arch}): max_err={eg:.2e}  {'PASS' if eg < TOL else 'FAIL'}  (ConvNeXt block)")
    else:
