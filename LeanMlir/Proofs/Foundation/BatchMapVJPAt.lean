...
theorem batchMapAux_eq_batchMapHasVJPAt {N a b : Nat} (f : Vec a → Vec b)
    (g : Vec a → Vec b → Vec a) (v : Vec (N * a))
    (hf : ∀ r : Fin N, HasVJPAt f (Mat.unflatten v r))
    (hf_diff : ∀ r : Fin N, DifferentiableAt ℝ f (Mat.unflatten v r))
    (hg : ∀ r : Fin N, g (Mat.unflatten v r) = (hf r).backward) :
    StableHLO.batchMapAux N g v = (batchMapHasVJPAt f v hf hf_diff).backward := by
  funext dy idx
  exact congrFun (congrFun (hg _) _) _

/-- The linear-backward form of `batchMapAux_eq_batchMapHasVJPAt`: a backward that ignores the
    saved input lifts by `batchMap`. -/
theorem batchMap_eq_batchMapHasVJPAt {N a b : Nat} (f : Vec a → Vec b) (g : Vec b → Vec a)
    (v : Vec (N * a)) (hf : ∀ r : Fin N, HasVJPAt f (Mat.unflatten v r))
    (hf_diff : ∀ r : Fin N, DifferentiableAt ℝ f (Mat.unflatten v r))
    (hg : ∀ r : Fin N, g = (hf r).backward) :
    StableHLO.batchMap N g = (batchMapHasVJPAt f v hf hf_diff).backward := by
  funext dy idx
  exact congrFun (congrFun (hg _) _) _
...
/-- **`batchMap N f` VJP — block-diagonal (the genuinely-new lemma).** A batch-separable op's VJP
    ...
    `[N, ·]` and runs `f.backward` row-wise. Reuses `rowwiseHasVJPMat` + `HasVJPMat.toHasVJP`. This
    is `seBlockFullHasVJP` / the conv-depthwise-dense VJPs "lifted by batchMap" to the whole batch. -/
...
/-- **Reindex VJP at `reindexCLM`** — `reindexVJP`, with the backward spelled as the masked
    ...
    `rfl`. -/
noncomputable def reindexHasVJP {a b : Nat} (σ : Fin b → Fin a) :
