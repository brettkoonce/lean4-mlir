...
`SHlo.allReduceMeanF` node (`DataParallel.Node`). The train-step ties — `r34_net_tiedB`,
...
* `dpSyncGrad_eq_globalBatchGrad` — the positive twin of `dpMeanGrad_ne_globalBatchGrad`: when
  ...
  `DataParallel.Sync` shows the sync-BN ops have that form.
* `dpIterate_lockstep` / `dpIterate_eq_meanLossTrain` — the lockstep induction. Identical
  ...
  training on the mean loss. That is the property `Verified.Train` relies on when it
  checkpoints from replica 0.

## What is NOT claimed

**Nothing here is about the emitted `all_reduce`.** The `SHlo` collective and its `den`
(`den (allReduceMeanF R g) = (1/R) Σ_r den (g r)`) are `DataParallel.Node`.
...
The sync-BN renders all-reduce the statistics (`DataParallel.Sync`), and
...
replica, and that `replica_groups` names all `R` devices are the DRIVER's** (`Verified.Train`,
...
/-- The contiguous shard — replica `r` owns examples `[N·r, N·r + N)` — as an instance. This is
    the split `Verified.Train`'s DP path cuts (`elems / replicas`). -/
...
/-- **The positive twin of `dpMeanGrad_ne_globalBatchGrad`: with SYNCHRONISED statistics
    ...
    own gradient. What sync-BN buys (P1–P3, `PerChannelBN.lean` / `DataParallel.Sync`) is
...
/-- The contiguous shard — replica `r` owns examples `[N·r, N·r + N)` — as an instance, the
    split `Verified.Train`'s DP path cuts. -/
...
/-- **The lockstep induction.** Identical initial states and an identical (all-reduced)
    ...
    `n` steps of the single-device one. This is the property `Verified.Train` relies on
