...
`ResNet34FaithfulPoC.lean` makes every parameter output of the per-example SGD train step
...
never Adam." So the eight lemmas below are `EfficientNetFaithfulPoC.lean`'s proofs with the
...
* The cotangents are free variables `cot` — each lemma is `∀ cot`, so it holds at the actual
  ...
  subgraph is the §1a tie, and is `ResNet34TiePoCB.lean`.
-/
