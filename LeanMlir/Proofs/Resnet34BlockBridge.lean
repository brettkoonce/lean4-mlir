...
/-- **BN → relu block step closeness.** With the BN input `vt` (float) within `e1`
    of `va` (real) per coordinate (both magnitude `≤ A`), the float per-example BN
    mean/istd within `emean`/`eistd` of the real ones at `vt`, and the usual BN
    magnitude bounds, the rounded `relu(bnForwardF vt)` is within `bnReluBudget` of
    `relu(bnForward va)`. The composition split: rounding (`bnForward_close_of`)
    + input-shift (`bnForward_input_close`), through `relu_close`. -/
theorem bnRelu_close {n : Nat} {ε γ β emean eistd D S G Bbnd A e1 fμ fistdv : ℝ}
  ...
  -- pre-relu error, uniform over coordinates
  have hpre : ∀ j, |M.bnForwardF γ β fμ fistdv vt j - bnForward n ε γ β va j| ≤
      bnReluBudget M.u D S G Bbnd emean eistd A e1 ε := by
    intro j
    have hround := M.bnForward_close_of (ε := ε) vt j hmean histd (hD j) hSabs hγ hβ
    have hshift0 := bnForward_input_close (γ := γ) (β := β) (A := A) (ε := ε)
      vt va hn hε hAvt hAva j
    have hinner :
        (|vt j - va j| + (∑ k, |vt k - va k|) / (n:ℝ)) * (1 / Real.sqrt ε)
          + 2 * A * ((8 * A * ((∑ k, |vt k - va k|) / (n:ℝ))) / (2 * ε * Real.sqrt ε))
        ≤ (e1 + e1) * (1 / Real.sqrt ε) + 2 * A * ((8 * A * e1) / (2 * ε * Real.sqrt ε)) := by
      have hj := he1 j
      gcongr
    have hshift : |bnForward n ε γ β vt j - bnForward n ε γ β va j| ≤
    ...
    calc |M.bnForwardF γ β fμ fistdv vt j - bnForward n ε γ β va j|
        ≤ |M.bnForwardF γ β fμ fistdv vt j - bnForward n ε γ β vt j|
          + |bnForward n ε γ β vt j - bnForward n ε γ β va j| := abs_sub_le _ _ _
  ...
  exact relu_close (M.bnForwardF γ β fμ fistdv vt) (bnForward n ε γ β va)
    (bnReluBudget M.u D S G Bbnd emean eistd A e1 ε) hpre i
