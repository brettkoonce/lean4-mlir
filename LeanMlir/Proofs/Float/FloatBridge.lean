...
compositions the train-step proofs certify, and the backward-op and softmax-cotangent
bounds. The chains that assemble them for the MNIST MLP and the E4M3 linear net are
`MlpFloatBridge`.
...
* **ReLU is exact in floating point** — comparison/selection rounds nothing,
  which is why `MlpFloatBridge`'s `mlpF` interleaves bare `relu` with no `rnd` and the bridge
...
* **Error shapes are `denseErr`**, one def reused at every layer: the rounded
  ...
  capstone (`MlpFloatBridge`'s `mlp_float_close`) threads it three times; its `e₀`/`e₁`
...
theorem smRho_nonneg {eexp : ℝ} {n : ℕ} (heexp : 0 ≤ eexp) :
    0 ≤ smRho M.u eexp n :=
  add_nonneg
