...
-- ════════════════════════════════════════════════════════════════
-- § The same fold with the float map AND the budget named (`FloatBridgesTo`)
-- ════════════════════════════════════════════════════════════════

/-- **The float skeleton of the 8-conv CIFAR forward** — `cifarCnn8Forward` with every conv
    and dense replaced by the model's rounded peer (`M.flatConvF` / `M.dense`); `relu` and
    the max-pools are exact in float and unchanged. The map `floatBridgesTo_cifar8` certifies,
    and `Cifar8FloatBudget.lean` bounds numerically. -/
noncomputable def cifarCnn8ForwardF
    {ic c1 c2 c3 c4 h w d1 nClasses kH kW : Nat} (M : FloatModel)
    (W₁ : Kernel4 c1 ic kH kW) (b₁ : Vec c1)
    (W₂ : Kernel4 c1 c1 kH kW) (b₂ : Vec c1)
    (W₃ : Kernel4 c2 c1 kH kW) (b₃ : Vec c2)
  ...
  M.dense Wb bb
  ∘ (relu d1 ∘ M.dense Wa ba)
  ∘ (relu d1 ∘ M.dense W₉ b₉)
  ∘ maxPoolFlat c4 h w
  ∘ (relu (c4 * (2*h) * (2*w)) ∘ M.flatConvF (h := 2*h) (w := 2*w) W₈ b₈)
  ∘ (relu (c4 * (2*h) * (2*w)) ∘ M.flatConvF (h := 2*h) (w := 2*w) W₇ b₇)
  ∘ maxPoolFlat c3 (2*h) (2*w)
  ∘ (relu (c3 * (2*(2*h)) * (2*(2*w))) ∘ M.flatConvF (h := 2*(2*h)) (w := 2*(2*w)) W₆ b₆)
  ∘ (relu (c3 * (2*(2*h)) * (2*(2*w))) ∘ M.flatConvF (h := 2*(2*h)) (w := 2*(2*w)) W₅ b₅)
  ∘ maxPoolFlat c2 (2*(2*h)) (2*(2*w))
  ∘ (relu (c2 * (2*(2*(2*h))) * (2*(2*(2*w)))) ∘ M.flatConvF (h := 2*(2*(2*h))) (w := 2*(2*(2*w))) W₄ b₄)
  ∘ (relu (c2 * (2*(2*(2*h))) * (2*(2*(2*w)))) ∘ M.flatConvF (h := 2*(2*(2*h))) (w := 2*(2*(2*w))) W₃ b₃)
  ∘ maxPoolFlat c1 (2*(2*(2*h))) (2*(2*(2*w)))
  ∘ (relu (c1 * (2*(2*(2*(2*h)))) * (2*(2*(2*(2*w))))) ∘ M.flatConvF (h := 2*(2*(2*(2*h)))) (w := 2*(2*(2*(2*w)))) W₂ b₂)
  ∘ (relu (c1 * (2*(2*(2*(2*h)))) * (2*(2*(2*(2*w))))) ∘ M.flatConvF (h := 2*(2*(2*(2*h)))) (w := 2*(2*(2*(2*w)))) W₁ b₁)

/-- **The deeper 8-conv CIFAR CNN float-bridges TO its float skeleton, budget visible.** The
    same `.comp` chain as `cifar8_floatBridges` over the `FloatBridgesTo` leaves, so the
    conclusion names the float net AND carries `mag`/`mod` as data — the whole-net budget is a
    closed term over the eleven `layerBudget`s, which `Cifar8FloatBudget.lean` evaluates. -/
noncomputable def floatBridgesTo_cifar8
    {ic c1 c2 c3 c4 h w d1 nClasses kH kW : Nat} (M : FloatModel)
    (W₁ : Kernel4 c1 ic kH kW) (b₁ : Vec c1)
    (W₂ : Kernel4 c1 c1 kH kW) (b₂ : Vec c1)
    (W₃ : Kernel4 c2 c1 kH kW) (b₃ : Vec c2)
    ...
    FloatBridgesTo
      ...
      (cifarCnn8ForwardF M W₁ b₁ W₂ b₂ W₃ b₃ W₄ b₄ W₅ b₅ W₆ b₆ W₇ b₇ W₈ b₈ W₉ b₉ Wa ba Wb bb) :=
  (((((((((((((((((((((((
    (floatBridgesTo_flatConv M W₁ b₁ hw₁ hβ₁ (by positivity) hW₁ hb₁)
    |>.comp floatBridgesTo_relu)
    |>.comp (floatBridgesTo_flatConv M W₂ b₂ hw₂ hβ₂ (by positivity) hW₂ hb₂))
    |>.comp floatBridgesTo_relu)
    |>.comp floatBridgesTo_maxPool)
    |>.comp (floatBridgesTo_flatConv M W₃ b₃ hw₃ hβ₃ (by positivity) hW₃ hb₃))
    |>.comp floatBridgesTo_relu)
    |>.comp (floatBridgesTo_flatConv M W₄ b₄ hw₄ hβ₄ (by positivity) hW₄ hb₄))
    |>.comp floatBridgesTo_relu)
    ...
    |>.comp floatBridgesTo_relu)
    |>.comp floatBridgesTo_maxPool)
    |>.comp (floatBridgesTo_dense M W₉ b₉ hw₉ hβ₉ (by positivity) hW₉ hb₉))
    |>.comp floatBridgesTo_relu)
    |>.comp (floatBridgesTo_dense M Wa ba hwa hβa hd1 hWa hba))
    |>.comp floatBridgesTo_relu)
    |>.comp (floatBridgesTo_dense M Wb bb hwb hβb hd1 hWb hbb)
