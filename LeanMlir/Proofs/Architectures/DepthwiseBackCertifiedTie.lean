...
* `depthwiseConv2d_dwReverse_eq_input_grad_formula` — the gate: the emitted reversed-kernel forward
  ...
  `depthwiseConv2dInputGradFormula W`, for arbitrary dims with odd kernels. The exact depthwise
...
* `depthwiseFlatBack_eq_vjp_backward` — the stride-1 leaf tie: the backward map `depthwiseFlatBack W`
  ...
  `(depthwiseFlatHasVJP W b).backward x` (depthwise conv is linear ⇒ the saved activation `x` is
...
* `depthwiseStride2FlatXlaBack_eq_vjp_backward` — the XLA-`SAME` strided leaf tie:
  ...
  `(depthwiseStride2FlatXlaHasVJP W b).backward x`, for odd kernels.
-/
...
theorem depthwiseConv2d_dwReverse_eq_input_grad_formula {c h w kH kW : Nat}
    ...
    depthwiseConv2d (dwReverse W) (fun _ => 0) dy = depthwiseConv2dInputGradFormula W dy := by
  funext ch hi wi
  simp only [depthwiseConv2d, dwReverse, zero_add, depthwiseConv2dInputGradFormula]
...
/-- **Depthwise conv input-VJP leaf tie.** The backward map `depthwiseFlatBack W` (= reversed-kernel
    forward depthwise conv) IS the certified depthwise input-VJP `(depthwiseFlatHasVJP W b).backward
...
theorem depthwiseFlatBack_eq_vjp_backward {c h w kH kW : Nat}
  ...
  funext dy
  simp only [depthwiseFlatBack, depthwiseFlat, depthwiseFlatHasVJP, HasVJP3.toHasVJP,
    depthwiseHasVJP3]
...
/-- **XLA-`SAME` strided depthwise input-VJP leaf tie.** `depthwiseStride2FlatXlaBack W`
    ...
    `(depthwiseStride2FlatXlaHasVJP W b).backward x`, for odd kernels. MobileNetV2's four
...
theorem depthwiseStride2FlatXlaBack_eq_vjp_backward {c h w kH kW : Nat}
    ...
    depthwiseStride2FlatXlaBack (h := h) (w := w) W
      = (depthwiseStride2FlatXlaHasVJP W b).backward x := by
  funext dy
