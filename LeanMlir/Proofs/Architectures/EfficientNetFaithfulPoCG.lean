...
`EfficientNetFaithfulPoC.lean` makes every parameter output of the SGD-inline
...
`EfficientNetFaithfulPoC.lean`'s proofs with the `congr 1` / `congrArg (lr * ·)` wrapper peeling
dropped, exactly as `ResNet34FaithfulPoCB.lean` did for r34.

## ⭐ Five of the eight op kinds were ALREADY proven, in r34's file

`ResNet34FaithfulPoCB.lean` states its eight folds at op kinds, not at r34 — and five of them are
...
* Every lemma is `∀ cot`, so it holds at the actual backward-chain cotangent without naming it.
  Pinning each to the emitted backward subgraph is the §1a tie (`EfficientNetTiePoC.lean`), and
