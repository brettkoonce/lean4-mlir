...
The tanh-form GELU's naive input-sensitivity is magnitude-polynomial: split
`gelu(a) − gelu(b)` and bound `tanh`'s shift by its 1-Lipschitz constant and the cubic
inner term by `3A²`, and out comes `(1 + √(2/π)/2·A·(1+3·0.044715·A²))·|a−b|` — **cubic in
the window** (`geluScalar_lipschitz_abs`, `ViTFloatBridge.lean`). At ConvNeXt-T's operating
magnitudes that factor is in the hundreds, and folded through 18 GELU sites it is the
difference between a whole-net budget `norm_num` can evaluate and one it cannot.

The TRUE constant is ≈ 1.13, and the reason the polynomial bound misses it is that it throws
away the *decay* of `sech²`: past the small-`|x|` region `gelu′`'s `1 − tanh²u` factor falls
like `e^{−2√(2/π)|x|}` and beats the cubic growth. This file proves the clean global bound
`|gelu′| ≤ 3/2` from

- `sech²u ≤ 4e^{−2|u|}` (`cosh ≥ e^{|u|}/2`),
- `|u(x)| ≥ √(2/π)·|x|` (the cubic inner is same-signed),
- the cubic Taylor lower bound on `exp`, and
- two exact facts: `(cs − ½)² ≥ 0` and `π ≤ 4` (which gives `6·0.044715 ≤ 4/(3π)`, the
  cubic-coefficient domination).

⚠ **Why it lives under `Architectures/` and not next to its consumers.** It is pure real
analysis about `geluScalar`, needing only `LayerNorm.lean`'s `geluScalarDeriv_eq` /
