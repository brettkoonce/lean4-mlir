...
-- ════════════════════════════════════════════════════════════════
-- § §2m — the SAME architecture with ConvNeXt's REAL channel LayerNorm
-- ════════════════════════════════════════════════════════════════

/-! Everything above normalises with `layerNormForward`: one mean and one variance over the whole
`c·h·w` map, scalar γ/β. ConvNeXt specifies `channel_layer_norm` — `h·w` statistics per example,
each over the `c` channels at one spatial position, per-channel `[c]` affine. See
`ConvNeXtChannelLN.lean` for the primitive and for why Route A needs no new op and no new VJP.

Built as a PARALLEL chain rather than by re-instantiating the scalar one, for
`MobileNetV2RenderB`'s reason (§2f): re-instantiating changes what the committed
`convNextForwardTC` denotes, and that function backs the retired render's tie, the
`ConvNeXtChainClose` bridges and `ConvNeXtWholeFloatBridge`. The scalar chain above is untouched.

Two further deviations §2m found by the parameter count NOT matching, both fixed here and both
invisible to a VJP argument: the reference's 22 LN sites are **1 stem + 18 block + 3 downsample**,
where the committed net had **18 + 3 + 1 head**. So this forward gains a stem LN and loses the
head LN. They nearly cancel (+2·768 − 2·96 = +1,344 of 28.6M), which is exactly why a matching
parameter count is a decomposition test and not an architecture check. -/
...
/-- One channel-LN ConvNeXt block's 10 parameters. The ONLY difference from `CnxBlockParams`:
    `γn`/`βn` are `Vec c`, not two scalars — 2c floats per site instead of 2. -/
...
/-- `cnxBlockW`'s channel-LN peer: `residual` of the shared body at `chanLNTensor3`. -/
...
/-- `CnxDownParams`' peer: the LN affine is `Vec cin`, over the PRE-downsample channel width. -/
...
/-- All channel-LN ConvNeXt-T parameters. Against `CnxTWeights`: every LN affine is a `Vec`, the
    stem LN is PRESENT (`sγ`/`sβ : Vec 96`), and the head LN is GONE — the reference's `forward`
    is `patchify → channel_layer_norm → stages → GAP → dense`. -/
...
/-- The channel-LN block graph — `cnxBlockGraphW` with `chanLNGraph` where the `.bnF` was. -/
...
/-- The **channel-LN ConvNeXt-T forward graph** (3×224² → 10): patchify stem → **stem
    ...
    GAP → dense. Note what moved against `convNextFwdGraphTC`: the stem LN is back and the head
