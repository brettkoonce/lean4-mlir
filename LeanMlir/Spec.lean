...
/-- **The one writer of a trainable layer's parameter layout** — the tensors in signature order,
    ...
    The MLIR signatures (`MlirCodegen.fwdSigParts`, `emitTrainStepSig`) render it too. -/
...
def NetSpec.archStr (s : NetSpec) : String :=
  " → ".intercalate (s.layers.map fun l =>
    ...
    | .transformerEncoder dim h _ n cm ks _ _ =>
...
def Layer.outChannels : Layer → Nat
  ...
  | .transformerEncoder dim _ _ _ _ _ _ _ => dim
...
def Layer.inChannels : Layer → Nat
  ...
  | .tokenPositionEmbed v t _ ids gather _ =>
      -- idsInput/gather: [B, T] f32 token ids; else flat [B, V*T] one-hot.
      if ids || gather then t else v * t
  | .lmHead d _ _                   => d
...
def NetSpec.validate (s : NetSpec) : Option String := Id.run do
  ...
  for l in s.layers do
    ...
    | .transformerEncoder _dim _ _ _ causalMask keepSeq _ _ =>
    ...
    | .tokenPositionEmbed _ _ d _ _ _ =>
        prevOc := d
