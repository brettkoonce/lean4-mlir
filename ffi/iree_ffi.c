...
// ============================================================
// DDPM variant. `y_ddpm` is f32 [batch, C, H, W] — the noise ε the
// model is trained to predict. Loss is per-pixel MSE in the codegen
// produced with `useDdpm := true`.
// ============================================================
int iree_ffi_train_step_adam_ddpm(
  ...
  iree_status_t s = iree_runtime_call_initialize_by_name(
      sess->session, iree_make_cstring_view(fn_name), &call);
  if (!iree_status_is_ok(s)) { print_status("adam_train_ddpm_init", s); return 1; }
  ...
  if (iree_status_is_ok(s))
    s = push_input(&call, sess->device,
                   IREE_HAL_ELEMENT_TYPE_FLOAT_32, 4,
                   x_rank, x_dims, x);
  int64_t d_y[4] = {batch, outC, outH, outW};
  if (iree_status_is_ok(s))
    s = push_input(&call, sess->device,
                   IREE_HAL_ELEMENT_TYPE_FLOAT_32, 4,
                   4, d_y, y_ddpm);
  ...
  if (!iree_status_is_ok(s)) { print_status("adam_train_ddpm_push", s);
    iree_runtime_call_deinitialize(&call); return 2; }

  s = iree_runtime_call_invoke(&call, 0);
  if (!iree_status_is_ok(s)) { print_status("adam_train_ddpm_invoke", s);
    iree_runtime_call_deinitialize(&call); return 3; }
  ...
  iree_runtime_call_deinitialize(&call);
  if (!iree_status_is_ok(s)) { print_status("adam_train_ddpm_pop", s); return 4; }
  return 0;
}
...
// codegen produced with `useSeg := true`.
// ============================================================
int iree_ffi_train_step_adam_seg(
  ...
  iree_status_t s = iree_runtime_call_initialize_by_name(
      sess->session, iree_make_cstring_view(fn_name), &call);
  if (!iree_status_is_ok(s)) { print_status("adam_train_seg_init", s); return 1; }
  ...
  if (!iree_status_is_ok(s)) { print_status("adam_train_seg_push", s);
    iree_runtime_call_deinitialize(&call); return 2; }

  s = iree_runtime_call_invoke(&call, 0);
  if (!iree_status_is_ok(s)) { print_status("adam_train_seg_invoke", s);
    iree_runtime_call_deinitialize(&call); return 3; }
  ...
  iree_runtime_call_deinitialize(&call);
  if (!iree_status_is_ok(s)) { print_status("adam_train_seg_pop", s); return 4; }
  return 0;
}
