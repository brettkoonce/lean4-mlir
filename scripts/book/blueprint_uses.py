...
missing. This script makes the line a generated artifact instead.
    ...
    python3 scripts/book/blueprint_uses.py --check     # CI: exit 1 on any drift
    python3 scripts/book/blueprint_uses.py --fix       # rewrite the \\uses lines
    ...
    if a.check:
        ...
        if drift:
            print(f"\n{len(drift)} of {len(blocks)} blocks drifted from the Lean dependencies "
                  f"(- listed but unused, + used but unlisted); run scripts/book/blueprint_uses.py --fix")
            return 1
