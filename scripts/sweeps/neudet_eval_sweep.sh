...
#
# Usage: scripts/sweeps/neudet_eval_sweep.sh fpn|grid <tag> <gpu> [split=val] [epochs="2 4 … 30"]
#   scripts/sweeps/neudet_eval_sweep.sh fpn  run1 3            # val, every saved epoch
#   scripts/sweeps/neudet_eval_sweep.sh grid run1 3 test "30"  # the table's row
...
for EP in $EPOCHS; do
  ...
  python3 scripts/demos/yolo_map_visdrone.py "$L" "$GT" $SCORE_ARGS $ML_ARGS > "$OUT/score_${SPLIT}_e${EP}.log" 2>&1
  python3 scripts/demos/yolo_map_visdrone.py "$L" "$GT" $SCORE_ARGS          > "$OUT/score_argmax_${SPLIT}_e${EP}.log" 2>&1
