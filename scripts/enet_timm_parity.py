...
Usage:
    scripts/enet_timm_parity.py              # batch 4, seed 0, tol 1e-3 (relative to max |logit|)
    scripts/enet_timm_parity.py --tol 1e-4
    scripts/enet_timm_parity.py --controls   # also RED on symmetric padding and on ε 1e-5
    scripts/enet_timm_parity.py --pad tf     # the distance to TF's all-SAME net (expected above tol)
"""
...
def timm_dump(tmp, classes, batch, seed, eps, pad="hybrid"):
    ...
    r = subprocess.run([TIMM_PY, os.path.join(ROOT, "scripts", "_enet_timm_dump.py"), out,
