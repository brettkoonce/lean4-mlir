...
#       scripts/parity/score_timm.sh <net> [dataDir]
#
...
# scripts/parity/timm_eval_protocols.py — and are handed to score-checkpoint as LEAN_MLIR_EVAL_SIZE /
...
set -euo pipefail
cd "$(dirname "$0")/../.."
net="${1:?usage: LEAN_MLIR_VARIANT=<v> scripts/parity/score_timm.sh <net> [dataDir]}"
data="${2:-data}"
