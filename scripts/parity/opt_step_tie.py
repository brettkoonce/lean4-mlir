...
BY NOBODY'S CONSTRUCTION.* `tests/vjp_oracle` and `scripts/parity/grad_tie.py` diff the two at the
...
`scripts/parity/grad_tie.py`'s standing rule — *`ref_py` must be the GENERATED reference, not a
...
`scripts/probes/misplace_drop_sites.py`'s rule applied here: *an all-ones-mask gate is structurally blind
...
Usage:
    ...
    .venv/bin/python scripts/parity/opt_step_tie.py
...
def run_render(slug, arrays, n_out):
    ...
    A pure rename; no other text changes. Same approach as `scripts/parity/grad_tie.py:run_xla`.
    """
    import jax
