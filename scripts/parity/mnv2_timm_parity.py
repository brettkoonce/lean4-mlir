...
Usage:
    scripts/parity/mnv2_timm_parity.py              # batch 4, seed 0, tol 1e-3 (relative to max |logit|)
    scripts/parity/mnv2_timm_parity.py --tol 1e-4
    scripts/parity/mnv2_timm_parity.py --controls   # also show the gate RED on symmetric padding and on ε 1e-5
"""
...
def timm_dump(tmp, classes, batch, seed, eps, pad="same"):
    ...
    r = subprocess.run([TIMM_PY, os.path.join(ROOT, "scripts", "parity", "_mnv2_timm_dump.py"), out,
