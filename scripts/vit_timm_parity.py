...
Usage:
    .venv/bin/python scripts/vit_timm_parity.py              # batch 4, seed 0, tol 1e-5 of max |logit|
    .venv/bin/python scripts/vit_timm_parity.py --controls   # also RED on k/v swapped and on erf GELU
    .venv/bin/python scripts/vit_timm_parity.py --deit       # distance to DeiT's erf / 1e-6
"""
...
def timm_dump(tmp, classes, batch, seed, *flags):
    ...
    r = subprocess.run([TIMM_PY, os.path.join(ROOT, "scripts", "_vit_timm_dump.py"), out,
