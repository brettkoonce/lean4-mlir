...
Usage:  python3 scripts/gates/gen_mlir_manifest.py            # write verified_mlir/MANIFEST.md
        python3 scripts/gates/gen_mlir_manifest.py --check    # verify only, non-zero if stale
        python3 scripts/gates/gen_mlir_manifest.py --selftest # replay the marker collisions
"""
...
def build() -> str:
    ...
    for p in sorted(MLIR.glob("*.mlir")):
        ...
        # scripts/parity/score_timm.sh) — a resolution, not a variant or a batch.
    ...
    L = [
        ...
        "",
        "**Generated by `scripts/gates/gen_mlir_manifest.py`. Do not hand-edit.**",
        "",
...
def main() -> int:
    ...
    if args.check:
        ...
        if cur != text:
            print("⛔ verified_mlir/MANIFEST.md is stale — re-run scripts/gates/gen_mlir_manifest.py")
            return 1
