...
"""Every committed artifact must PARSE as StableHLO.
    ...
    .venv/bin/python scripts/gates/parse_verified_mlir.py [verified_mlir/*.mlir]
...
Every other gate in the repo passed on them:
  ...
  * `scripts/gates/check_render_coverage.py` — reachability from the Proofs and Certs roots, and a
...
# ran and passed, which is the failure mode `scripts/probes/misplace_drop_sites.py` already refuses.
...
try:
    ...
    sys.exit(f"parse_verified_mlir needs jaxlib.mlir ({e}).\n"
             "  locally: .venv/bin/python scripts/gates/parse_verified_mlir.py\n"
