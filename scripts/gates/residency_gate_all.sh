...
#
#   scripts/gates/residency_gate_all.sh [<net>...]      # default: all of them
#   scripts/gates/residency_gate_all.sh mnist-mlp r34   # a subset, by short name
#
...
# The binaries this table drives, declared for scripts/gates/check_target_names.sh — the
...
#     scripts/gates/residency_gate.sh vit-verified-adam vit 10

DET=${DET_SHIM:-/tmp/residency_detshim}
OUT=${GATE_OUT:-$(mktemp -d)}
mkdir -p "$OUT"

[ -f scripts/gates/residency_gate.sh ] || { echo "run from the repo root"; exit 2; }
...
for row in "${NETS[@]}"; do
  ...
  # worse than no gate. `scripts/gates/check_target_names.sh` now lints this file too.
  ...
  env $extra LD_LIBRARY_PATH="$DET" \
      ...
      scripts/gates/residency_gate.sh "$bin" "$slug" "$steps" > "$OUT/$name.log" 2>&1
  rc=$?
