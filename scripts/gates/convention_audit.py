...
that is what `scripts/*_forward_tie.py` and `scripts/parity/grad_tie.py` are for. It means the two sides
spell padding, activation and BN the same way, which is the precondition those ties kept failing.

Usage:
    JAX_PLATFORMS=cpu python3 scripts/gates/convention_audit.py            # every matched pair
    JAX_PLATFORMS=cpu python3 scripts/gates/convention_audit.py --net r34
    JAX_PLATFORMS=cpu python3 scripts/gates/convention_audit.py --selftest # must reproduce the ledger
"""
...
def main():
    ...
    if args.baseline:
        ...
        if args.update_baseline:
            with open(args.baseline, "w") as fh:
                fh.write("# Render/reference convention divergences this repo currently carries.\n"
                         "# Regenerate with:\n"
                         "#   python3 scripts/gates/convention_audit.py --baseline FILE --update-baseline\n"
        ...
        if gone:
            ...
            print(f"    python3 scripts/gates/convention_audit.py --baseline {args.baseline} --update-baseline")
        if new:
            ratchet_fail = True
