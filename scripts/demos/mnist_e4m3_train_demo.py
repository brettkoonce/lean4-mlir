...
Usage: scripts/demos/mnist_e4m3_train_demo.py [mnist-idx-data-dir]
...
sys.path.insert(0, str(pathlib.Path(__file__).resolve().parents[1] / "lib"))  # the shared helpers
from _mnist_io import images as load_images, labels as load_labels  # noqa: E402

DATA = sys.argv[1] if len(sys.argv) > 1 else \
    str(pathlib.Path(__file__).resolve().parents[2] / "data")
