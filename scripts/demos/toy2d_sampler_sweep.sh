...
#
# Usage: scripts/demos/toy2d_sampler_sweep.sh            # 8-gaussians, 512 samples
#        NGEN=2048 NFES="20 50" scripts/demos/toy2d_sampler_sweep.sh
set -u
...
for arm in $ARMS; do
  ...
  for n in $NFES; do
    ...
    m=$(python3 scripts/demos/toy2d_metrics.py --target="$TARGET" 2>&1)
