...
demo (the Lean sampler emits raw ids since BPE detok lives here).
  ...
  python3 scripts/demos/tinystories_decode.py encode "Once upon a time, there was a"
  .lake/build/bin/tinystories sample 200 80 40 95 1 > /tmp/gen.txt
  python3 scripts/demos/tinystories_decode.py decode "Once upon a time, there was a" < /tmp/gen.txt
"""
import sys
