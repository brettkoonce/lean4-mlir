...
-- Survey variant of tests/DocstringCheckRefs.lean (2026-09-19): checks EVERY backticked name, not
-- only marker-carrying ones, and resolves four more forms — private declarations by their user
-- name, `File.decl` when `decl` is declared in `File`, bare module names, and namespaces. Writes
-- every miss as `file<TAB>name` to $ALLREFS_OUT. Imports the lib roots listed in $ALLREFS_ROOTS
-- (every lib but CertsHeavy, minus apps/demos) instead of loading the Lake workspace, which
-- crashes the interpreter. Run: lake env lean --run scripts/audit_census/AllRefs.lean
...
3. its PREFIX resolves — `fooHasVJP.backward` is a field access on a real declaration,
...
/-- The ways a citation counts as live: exact, under a project namespace, as a dotted SUFFIX
    ...
    `fooHasVJP.backward` is not itself a declaration, but a rename of `fooHasVJP` breaks
    it and that is what we are catching), or baselined. -/
def resolves (env : Environment) (idx : Std.HashMap String (Array Name))
    (nsSet : Std.HashSet Name) (baseline : Array String) (s : String) : Bool :=
  let n := s.toName
  if env.contains n then true
    ...
    let modHit := parts.length ≥ 2 && (idx.getD (parts.getLast!) #[]).any fun d =>
      match env.getModuleIdxFor? d with
      | some i =>
        let m := env.header.moduleNames[i.toNat]!
        let mc := m.components.map lastComponent
        let pre := parts.dropLast
        pre.length ≤ mc.length && mc.drop (mc.length - pre.length) == pre
      | none => false
    let isMod := env.header.moduleNames.any fun m =>
      let mc := m.components.map lastComponent
...
unsafe def main (args : List String) : IO UInt32 := do
  ...
  let rootNames := (← IO.FS.lines ((← IO.getEnv "ALLREFS_ROOTS").getD "/tmp/audit_census/roots.txt")).filter (· ≠ "")
  let imports : Array Import := rootNames.map fun r => { module := r.toName }
  ...
  for f in files do
    ...
    for body in docBodies src do
      for ref in backtickRefs body do
        ...
        checked := checked + 1
        unless resolves env idx nsSet baseline ref do
  ...
  let hh ← IO.FS.Handle.mk ((← IO.getEnv "ALLREFS_OUT").getD "/tmp/audit_census/allmisses.tsv") .write
