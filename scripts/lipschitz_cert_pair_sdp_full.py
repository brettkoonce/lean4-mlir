...
S = 2 diag(T) - vv^T - (1/rho) T G1 T, discharged in Lean via the
DETERMINISTIC `lipsdp_slack_of_cert` route (entrywise h1/h2 identities,
norm_num per entry -- no linarith search; the pooled files' linarith-with-
hints does not scale to h=16). Everything pair-level is 16x16 (Schur:
...
def emit_net(tag, W1q, W2q, out_path):
    ...
    A("entrywise by `norm_num` and discharged DETERMINISTICALLY via")
    A("`lipsdp_slack_of_cert` (the pooled files' linarith-with-hints search does")
    A("not scale to width 16). The 784-term work — Gram wrappers `G1*_eq`, the")
    A("per-image `hpre*_eval` — is reused from the kernel-dotZ scorecard files.")
    A("")
    ...
    for (a, b) in unordered:
        ...
        A("")
        A(f"noncomputable def dP{nm} : Fin {H} → ℝ := {fvec(c['d'])}")
        A("")
        A(f"noncomputable def lP{nm} : Fin {H} → Fin {H} → ℝ :=")
        A("  " + fmat(c["L"]))
        A("")
        A(f"noncomputable def smP{nm} : Fin {H} → Fin {H} → ℝ :=")
        A("  " + fmat(c["S"]))
        A("")
        ...
        A(f"theorem h1P{nm} : ∀ a b, smP{nm} a b")
        A(f"    = 2 * (if a = b then tP{nm} a else 0) - vP{nm} a * vP{nm} b")
        A(f"      - (1/{frac(c['rho'])}) * (tP{nm} a * (G1{tag} a b * tP{nm} b)) := by")
        A("  intro a b")
        A("  fin_cases a <;> fin_cases b <;>")
        A(f"    · simp [smP{nm}, tP{nm}, vP{nm}, G1{tag}]")
        A("      try norm_num")
        A("")
        A("set_option maxHeartbeats 12800000 in")
        A(f"theorem h2P{nm} : ∀ a b, smP{nm} a b = ∑ i, lP{nm} a i * (dP{nm} i * lP{nm} b i) := by")
        A("  intro a b")
        A("  fin_cases a <;> fin_cases b <;>")
        A(f"    · simp [smP{nm}, lP{nm}, dP{nm}, Fin.sum_univ_succ]")
        A("      try norm_num")
        A("")
        A(f"theorem hS{nm} : ∀ z : Fin {H} → ℝ,")
        A(f"    (∑ k, vP{nm} k * z k) ^ 2")
        A(f"      + (1/{frac(c['rho'])}) * (∑ a, ∑ b, (tP{nm} a * z a) * (G1{tag} a b * (tP{nm} b * z b)))")
        A(f"      ≤ 2 * ∑ k, tP{nm} k * z k ^ 2 :=")
        A(f"  lipsdp_slack_of_cert G1{tag} smP{nm} lP{nm} vP{nm} tP{nm} dP{nm}")
        A(f"    dP{nm}_nonneg h1P{nm} h2P{nm}")
        A("")
