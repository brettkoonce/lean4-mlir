...
#
#   scripts/check_target_names.sh          # lint; exit 1 on a dead name
#
...
# Measured 2026-09-20: `scripts/residency_gate_all.sh` had carried
...
#
# ▶ WHAT IT SCANS. Executable drivers only — scripts/*.sh, tests/*.sh, run.sh and
...
SELF=scripts/check_target_names.sh
...
BAD=0; CHECKED=0
FILES=$(ls scripts/*.sh tests/*.sh run.sh .github/workflows/*.yml 2>/dev/null | grep -v "^$SELF\$")
