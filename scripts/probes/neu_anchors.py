...
from scripts/probes/visdrone_anchors.py (imported, not copied), the box source and
...
Usage:
  python3 scripts/probes/neu_anchors.py [neu_dir=data/neu_det] [--save DIR] [--num A]
                                 [--seed N]
"""
import sys
    ...
    for s, nm in enumerate(SCALE_NAMES):
        ...
        with open(p, "w") as f:
            ...
            f.write(f"# NEU-DET FPN {nm} anchors (grid {g}, stride {INPUT_PX // g}), "
                    f"{how}, w_rel h_rel — scripts/probes/neu_anchors.py\n")
            for a in anchors[s]:
