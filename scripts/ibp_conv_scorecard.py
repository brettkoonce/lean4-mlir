...
def main():
    ...
    *_, lg = forward(xte, Wc9q, bcq, Wdq, bdq)
    ...
    # ───── Lean ─────
    # Emitted as FOUR modules, not one. A single 8-image file peaked at 14.68 GB
    # of elaboration (measured, LEAN_NUM_THREADS=1, olean deleted) — over the
    # ~12 GB a 16 GB CI runner can actually carry. Lean does not reclaim between
    # declarations within a process, and each image costs a 256-goal `cx*_eval`
    # plus per-radius box/pool scans, so the peak tracks the number of images IN
    # THE MODULE. Splitting them across their own modules bounds it to roughly
    # one chunk — the same fix SmoothingDecChunk1..6 uses for the same reason.
    NS = "namespace Proofs\nnamespace IBP\nnamespace ConvNet\n"
...
carries. -/\n''')
    ...
    half = (n_emit + 1) // 2
    part_of = {rec["idx"]: ("A" if i < half else "B")
    ...
    LG = []
    A = LG.append
    A(f"import {MOD}ImgsA")
    A(f"import {MOD}ImgsB\n")
...
the theorem-vs-measurement split. Generated by `scripts/ibp_conv_scorecard.py`. -/\n""")
    ...
    for path, body in ((OUT.with_name("IbpConvScorecardNet.lean"), L),
                       (OUT.with_name("IbpConvScorecardImgsA.lean"), LI["A"]),
                       (OUT.with_name("IbpConvScorecardImgsB.lean"), LI["B"]),
                       (OUT, LG)):
        path.write_text("\n".join(body) + "\n")
