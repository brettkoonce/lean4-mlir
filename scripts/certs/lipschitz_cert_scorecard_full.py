...
def cache_key(*parts) -> str:
    """Short hash of everything a cached value depends on. The /tmp caches used to be keyed by
    hyperparameters alone, so an edited `train` or `pgd_robust` silently reused the old result and
    fed it into committed Lean; now the producing function's SOURCE is part of the key."""
    h = hashlib.sha256()
    for p in parts:
...
for tag, (W1q, W2q) in nets.items():
    ...
    PGD_CACHE = Path("/tmp") / (f"lipschitz_full_pgd_{tag}_"
        f"{cache_key(inspect.getsource(pgd_robust), W1q.tobytes(), W2q.tobytes(), EPS10, EPS30)}.npz")
    if PGD_CACHE.exists():
...
# ═══ Lean emission helpers: scripts/certs/_leanlit.py ═══
...
def emit_net(A, tag, W1q, W2q, B1, B2, L):
    ...
    A(f"def W2{tag}Q : Fin {K} → Fin {H} → ℚ :=")
    A("  " + qmat(W2q, DEN))
    A("")
    A(f"noncomputable def W2{tag} : Fin {K} → Fin {H} → ℝ := castM W2{tag}Q")
    A("")
    ...
    A(f"def G1{tag}Q : Fin {H} → Fin {H} → ℚ :=")
    A("  " + qmat(G1, den2))
    A("")
    A(f"noncomputable def G1{tag} : Fin {H} → Fin {H} → ℝ := castM G1{tag}Q")
    A("")
    for a in range(H):
        for b in range(H):
            ...
            A(f"  rw [show G1{tag} {a} {b} = (({int(G1[a][b])} : ℝ)/{den2}) by")
            A(f"    show ((G1{tag}Q {a} {b} : ℚ) : ℝ) = _")
            A(f"    rw [show G1{tag}Q {a} {b} = (({int(G1[a][b])} : ℚ)/{den2}) from rfl]; norm_num, {rows}]")
    ...
    A(f"def H1{tag}Q : Fin {H} → Fin {H} → ℚ :=")
    A("  " + qmat(H1, den4))
    A("")
    A(f"noncomputable def H1{tag} : Fin {H} → Fin {H} → ℝ := castM H1{tag}Q")
    A("")
    ...
    A(f"  simp [H1{tag}, H1{tag}Q, castM, Fin.sum_univ_succ]")
    A("  norm_num")
    A("")
    # layer 2: small (10×16 over 16) — the pooled file's recipe verbatim
    A(f"def G2{tag}Q : Fin {K} → Fin {K} → ℚ :=")
    A("  " + qmat(G2, den2))
    A("")
    A(f"noncomputable def G2{tag} : Fin {K} → Fin {K} → ℝ := castM G2{tag}Q")
    A("")
    A(f"def H2{tag}Q : Fin {K} → Fin {K} → ℚ :=")
    A("  " + qmat(H2, den4))
    A("")
    A(f"noncomputable def H2{tag} : Fin {K} → Fin {K} → ℝ := castM H2{tag}Q")
    A("")
    A(f"theorem G2{tag}_eq : ∀ a b, G2{tag} a b = ∑ j, W2{tag} a j * W2{tag} b j :=")
    A(f"  gram_eq_of_check G2{tag}Q W2{tag}Q (by decide +kernel)")
    A("")
...
def emit_image(A, i, tags_for_i):
    ...
    for tag in tags_for_i:
        ...
        A(f"      ∑ k : Fin {H}, W2{tag} jj k * max (hpre{tag}{i} k) 0 :=")
        ...
        A(f"    | · rw [hout, hout]")
        A(f"        simp [W2{tag}, W2{tag}Q, castM, hpre{tag}{i}, Fin.sum_univ_succ, max_def]")
        A("        norm_num")
        A("")

# ═══ images that carry theorems (computed at import: the IBP / pair-SDP generators read `need`) ═══
need = {}
for i in range(N_IMG):
    tags = [t for t in ("SF", "TF") if i in info[t]["cert10"]]
    if tags:
        need[i] = tags
# cap: the first N_EMIT certifying images PER (net, radius) column, unioned.
# Per-column rather than global so that every column with a nonzero measured
# count also has proved witnesses — otherwise a sparse column (TF at ε=0.3,
# measured 2/100) would emit an empty aggregate and say nothing at that radius.
EMITCOL = {}
for _t in ("SF", "TF"):
    for _e in ("10", "30"):
        EMITCOL[(_t, _e)] = sorted(info[_t]["cert" + _e])[:N_EMIT]
EMIT = sorted({i for c in EMITCOL.values() for i in c})
need = {i: need[i] for i in EMIT if i in need}


def write_files():
    """Write the four Lean files. Only `__main__` calls this: importing this module (as
    `lipschitz_cert_scorecard_ibp.py` / `lipschitz_cert_pair_sdp_full.py` do, for `nets`,
    `info`, `need` and the emit helpers) writes nothing."""
    # ═══ file 1: nets ═══
    L1 = []
    A = L1.append
    ...
    A("`scripts/certs/lipschitz_cert_scorecard_full.py`; weights are DATA. -/")
    A("")
    A(HEADER_OPTS)
    for tag in ("SF", "TF"):
        W1q, W2q = nets[tag]
    ...
    for part, rng_ in (("A", range(0, 50)), ("B", range(50, 100))):
        ...
        A("ε = 1/10 by either net. Generated by `scripts/certs/lipschitz_cert_scorecard_full.py`. -/")
        A("")
        A(HEADER_OPTS)
        for i in rng_:
            if i in need:
    ...
    A("")
    A(f"The pooled 49-dim scorecard, lifted to FULL 784-dim input: pixel-L2")
    A(f"ε = 1/10 and 3/10, two 784→16→10 nets. Counts below are")
    A(f"MEASURED over the first {N_IMG} MNIST test images by exact rational arithmetic:")
    A("")
    ...
    A("`scorecardFull` below states ONLY those proved counts. The images witness that")
    A("the certificate is non-vacuous at real trained weights; they are not what makes")
    A("it sound.")
    A("")
    A("Aggregates are lower bounds only — an upper-bound L cannot prove an image")
    A("UNcertifiable. Generated by `scripts/certs/lipschitz_cert_scorecard_full.py`. -/")
    A("")
    A(HEADER_OPTS)
    names = {}
    emitted = {}
    for tag in ("SF", "TF"):
        for epsname, eps, certs in (("10", EPS10, info[tag]["cert10"]),
            ...
            certs = EMITCOL[(tag, epsname)]              # emitted subset (see N_EMIT)
            for i in certs:
                y = int(yte[i])
                m = info[tag]["mp"][i][0]
            ...
            A("")
    A("/-- **The proved core of the full-input scorecard** — counts tied to the")
    A("    per-image `CertifiedAt` proofs, both nets, both radii. These are the emitted")
    A("    witnesses, NOT the measured dataset counts in the header (which are")
    A("    exact-rational measurements rather than theorems). Lower bounds only. -/")
    A("theorem scorecardFull :")
    A(f"    ({names[('SF','10')]}.length = {len(emitted[('SF','10')])} ∧")
    A(f"      ∀ p ∈ {names[('SF','10')]}, CertifiedAt mlpSF {frac(EPS10)} p.2.1 p.2.2) ∧")
    A(f"    ({names[('SF','30')]}.length = {len(emitted[('SF','30')])} ∧")
    A(f"      ∀ p ∈ {names[('SF','30')]}, CertifiedAt mlpSF {frac(EPS30)} p.2.1 p.2.2) ∧")
    A(f"    ({names[('TF','10')]}.length = {len(emitted[('TF','10')])} ∧")
    A(f"      ∀ p ∈ {names[('TF','10')]}, CertifiedAt mlpTF {frac(EPS10)} p.2.1 p.2.2) ∧")
    A(f"    ({names[('TF','30')]}.length = {len(emitted[('TF','30')])} ∧")
