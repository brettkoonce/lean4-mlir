...
A("`LipschitzCert.Instance` (test acc 89.8%) instantiates the conditional VJP")
...
A("")
A("* level 1 — `trainedMlpHasVJPAt` (+ `.correct`): the whole-net backward exists")
...
A("    `preact_ne`). The 2-layer analogue of `mlpHasVJPAt`, at trained weights")
A("    and a real input. -/")
A("noncomputable def trainedMlpHasVJPAt : HasVJPAt fwd xtV := by")
A("  unfold fwd")
A("  have step1 : HasVJPAt (relu 8 ∘ dense W1V b8) xtV :=")
A("    vjpCompAt (dense W1V b8) (relu 8) xtV")
A("      ((dense_differentiable W1V b8) xtV)")
A("      (relu_differentiableAt_of_smooth 8 _ preact_ne)")
A("      ((denseHasVJP W1V b8).toHasVJPAt xtV)")
A("      (reluHasVJPAt 8 _ preact_ne)")
A("  have step1_diff : DifferentiableAt ℝ (relu 8 ∘ dense W1V b8) xtV :=")
A("    (relu_differentiableAt_of_smooth 8 _ preact_ne).comp xtV")
A("      ((dense_differentiable W1V b8) xtV)")
A("  exact vjpCompAt (relu 8 ∘ dense W1V b8) (dense W2V b10) xtV")
A("    step1_diff")
A("    ((dense_differentiable W2V b10) _)")
A("    step1")
A("    ((denseHasVJP W2V b10).toHasVJPAt _)")
A("")
A("/-- The witness's contract, exposed: backward = the `pdiv`-contracted Jacobian. -/")
A("theorem trainedMlpHasVJP_correct (dy : Vec 10) (i : Fin 49) :")
A("    trainedMlpHasVJPAt.backward dy i = ∑ j, pdiv fwd xtV i j * dy j :=")
A("  trainedMlpHasVJPAt.correct dy i")
A("")
...
A(f"    trainedMlpHasVJPAt.backward (basisVec {c0}) {j0} ≠ 0 :=")
A("  trainedMlpHasVJPAt.backward_ne_zero_of_pdiv_ne pdiv_fwd_ne")
A("")
...
A("end TrainedMlp")
A("end Proofs")
open(os.path.join(ROOT, "LeanMlir/Proofs/Training/Trained/MlpWitness.lean"), "w").write("\n".join(W) + "\n")
print("wrote both")
