...
is the engine's, proved once -- see planning/archive/scorecard_trim.md). That cap also
...
def emit_net(tag, W1q, W2q, out_path):
    ...
    A("LDLᵀ PSD witness, which is where this tier's elaboration cost lives.")
    A("")
    A("Generated by `scripts/certs/lipschitz_cert_pair_sdp_full.py`; weights/images/")
    ...
    for (a, b) in unordered:
        ...
        A("  intro t")
        A("  fin_cases t <;>")
        A(f"    · simp [vP{nm}, W2{tag}, W2{tag}Q, castM]")
        A("      try norm_num")
        A("")
        ...
        A("  intro z")
        A(f"  simp [vP{nm}, tP{nm}, G1{tag}, G1{tag}Q, castM, Fin.sum_univ_succ]")
        A("  linarith [" + ",\n    ".join(hints) + "]")
        A("")
        ...
        A(f"  pair_sq_bound_mlp W1{tag} W2{tag} G1{tag} G1{tag}_eq {a} {b} vP{nm} tP{nm} vP{nm}_eq")
    ...
    for (a, b) in ordered:
        ...
        A(f"    ((mlp{tag} u {a} - mlp{tag} u {b}) - (mlp{tag} u' {a} - mlp{tag} u' {b})) ^ 2")
        A(f"      ≤ {frac(c['rho'])} * ‖u - u'‖ ^ 2 :=")
        A(f"  pair_sq_symm pairSq{tag}_{lo}_{hi}")
        A("")
    A("-- ════════════════════════════════════════════════════════════")
    ...
    for i in sorted(set(cert10) | set(cert30)):
        ...
        A("  rw [hout jj]")
        A("  fin_cases jj <;>")
        A(f"    · simp [W2{tag}, W2{tag}Q, castM, hpre{tag}{i}, {lg}, Fin.sum_univ_succ, max_def]")
        A("      try norm_num")
        A("")
