...
scorecard_trim.md). Run from repo root: python3 scripts/certs/lipschitz_cert_pair_sdp.py
...
def image_facts(W1z, W2z, den):
    ...
    for i in range(N_IMG):
        ...
        out.append((y, ms, pre, [int(v) for v in logit]))
    return out
...
def emit_net(tag, W1z, W2z, G1q, den, facts, w1name, w2name, g1name, g1eq,
    ...
    def img_name(i):
        return f"img{i}" if i in imgs_already else f"{img_prefix}{i}"
    certified = []
    for i, (y, ms, _pre, _logit) in enumerate(facts):
        if min(ms.values()) <= 0:
            continue
    ...
    A("`linarith` itself), turned into the squared gap bound by `pair_sq_bound`;")
    ...
    A("")
    A("Generated by `scripts/certs/lipschitz_cert_pair_sdp.py`; weights/images/")
    ...
    for (a, b) in unordered:
        ...
        A("  intro t")
        A("  fin_cases t <;>")
        A(f"    · simp [vP{nm}, {w2name}, {w2name}Q, castM]")
        A("      try norm_num")
        A("")
        ...
        A("      try norm_num")
        A("")
        # the PSD slack: expand the sums once, then linarith with the 8 LDLᵀ
        # column squares as hints (it finds the d ≥ 0 combination itself)
        hints = []
        for col in range(H):
            terms = " + ".join(
        ...
        A("  intro z")
        A(f"  simp [vP{nm}, tP{nm}, {g1name}, {g1name}Q, castM, Fin.sum_univ_succ]")
        A("  linarith [" + ",\n    ".join(hints) + "]")
        A("")
        A(f"theorem {sq_pfx}_{a}_{b} : ∀ u u' : EuclideanSpace ℝ (Fin 49),")
        A(f"    (({mlpname} u {a} - {mlpname} u {b}) - ({mlpname} u' {a} - {mlpname} u' {b})) ^ 2")
        A(f"      ≤ {frac(c['rho'])} * ‖u - u'‖ ^ 2 :=")
        A(f"  pair_sq_bound_mlp {w1name} {w2name} {g1name} {g1eq} {a} {b} vP{nm} tP{nm} vP{nm}_eq")
        A(f"    tP{nm}_nonneg (by norm_num) hS{nm}")
        A("")
    ...
    for (a, b) in ordered_pairs:
        ...
        A(f"    (({mlpname} u {a} - {mlpname} u {b}) - ({mlpname} u' {a} - {mlpname} u' {b})) ^ 2")
        A(f"      ≤ {frac(c['rho'])} * ‖u - u'‖ ^ 2 :=")
        A(f"  pair_sq_symm {sq_pfx}_{lo}_{hi}")
        A("")
    A("-- ════════════════════════════════════════════════════════════")
    ...
    for i in certified:
        ...
        if i not in imgs_already:
            ...
            A(f"noncomputable def {img} : EuclideanSpace ℝ (Fin 49) :=")
        ...
        if i in existing_hpre:
            ...
            A("  " + row_int(pre, den_h))
            A("")
            A(f"theorem {hpre}_eval : ∀ k : Fin 8, denseE {w1name} {img} k = {hpre} k := by")
            A("  intro k")
            A("  fin_cases k <;>")
            A(f"    · simp [denseE_apply, {w1name}, {w1name}Q, castM, {img}, {hpre}, Fin.sum_univ_succ]")
            A("      norm_num")
            A("")
        # exact logit table: evaluate the net ONCE, margins become numeric
        lg = f"logit{tag}{i}"
        ...
        A(f"    ∀ j, j ≠ {y} → {mlpname} ({img} + δ) j < {mlpname} ({img} + δ) {y} := by")
        A("  intro j hj")
        A("  fin_cases j")
        for j in range(K):
            if j == y:
    ...
    trips = ", ".join(f"({i}, {img_name(i)}, {facts[i][0]})" for i in certified)
    A(f"  [{trips}]")
    A("")
...
# The two files are import-INDEPENDENT (uncon duplicates its extra images as
# imgU<i>) so lake builds them in parallel — halves the CI wall time.
...
cert_u = emit_net("U", W1q, W2q, G1uq, DEN_U, facts_u,
                  ...
                  EXISTING_HPRE_U, "SU", OUT_U, EXISTING_IMGS,
                  img_prefix="imgU")
