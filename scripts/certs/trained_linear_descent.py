...
OUT = os.path.join(ROOT, "LeanMlir/Proofs/Training/Trained/LinearDescent.lean")
K, DIM = 10, 49
LR = Fraction(1, 8192)
...
A_("")
A_("`binary32_linear_sgd_descends_concrete` (Float/Binary32Instance.lean) holds only at")
A_("the degenerate `W = 0` net — a satisfiability witness. This file gives a trained")
A_("instance: **one SGD step with the FloatModel binary32 gradient on a TRAINED,")
A_(f"/128-rationalized bias-free 49→10 pooled-MNIST linear classifier (test acc")
A_(f"{accq:.3f}) decreases the real cross-entropy loss of one example**, MNIST test")
A_(f"image #{IDX}, with learning rate {LR}. The update is taken in ℝ and the")
A_("`exp` is exact (`fexp := Real.exp`, `eexp := 0`); the rounding model is the")
A_("CONSTRUCTED `rndP 23` grid (`binary32`), not an assumed one.")
A_("")
...
A_(f"≈ {float(LR * B / 8):.2e}. Generated by `scripts/certs/trained_linear_descent.py`;")
...
A_(f"/-- **Descent at TRAINED weights.** One SGD step with the FloatModel binary32 gradient (lr = {FR_LR}) on")
...
A_("    `linear_float_sgd_descends` discharged at exact `exp` (`fexp := Real.exp`,")
A_("    `eexp := 0`), the update taken in ℝ, the rounding model constructed")
A_("    (`rndP 23`). A trained-weight peer of `binary32_linear_sgd_descends_concrete`,")
A_("    which holds only at `W = 0`. -/")
