...
Three modules: LipschitzCertScorecardIBPData.lean holds what other scorecards
...
def emit_data(results, out_path):
    ...
    A("scorecards (`LipschitzCertScorecardIBP`, `…IBPUncon`) and the CROWN ones")
    A("(`LipschitzCertScorecardCrown`, `…CrownUncon`) import this module and not each")
...
def emit_net(tag, r, out_path):
    ...
    Lb = []
    A = Lb.append
    A("import LeanMlir.Proofs.Certificates.LipschitzCertScorecardIBPData")
    A("")
...
emit_data(results, OUTDIR / "LipschitzCertScorecardIBPData.lean")
emit_net("SF", results[0][1], OUTDIR / "LipschitzCertScorecardIBP.lean")
emit_net("TF", results[1][1], OUTDIR / "LipschitzCertScorecardIBPUncon.lean")
