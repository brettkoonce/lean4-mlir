...
Produces LeanMlir/Proofs/Certificates/LipschitzCert/ScorecardIBP.lean (capped sigma<=2 net)
and LipschitzCert/ScorecardIBPUncon.lean: per-image certificates in the
...
Three modules: LipschitzCert/ScorecardIBPData.lean holds what other scorecards
reuse — the per-row l1 facts (`absr*`, `absrow*`) for both nets and the fallback
...
def analyze(tag, W1q, W2q):
    """Exact IBP counts, the PGD bracket and the L2 comparison; which images get theorems."""
    A1 = [int(v) for v in np.abs(W1q).sum(1)]
    ...
    all_certified = sorted({i for _, en in EPS_GRID for i in emit[en]})
    return dict(A1=A1, certs=emit, measured=measured, pgd=pgd, l2impl=l2impl,
                base_hpre=base_hpre, base_imgs=base_imgs, all_certified=all_certified)


def emit_data(results, out_path):
    ...
    A("")
    A("/-! # IBP / CROWN L∞ scorecards — the shared data")
    A("")
    A("The facts more than one pixel-L∞ scorecard reads: each net's per-row ℓ1 norms")
    A("(`absr*`, one kernel `absSumZ` fact per row, lifted to `∑ j, |W1 t j|` by")
    A("`absrow*`), and the test images the capped base scorecard doesn't carry. The IBP")
    A("scorecards (`LipschitzCert/ScorecardIBP`, `…IBPUncon`) and the CROWN ones")
    A("(`LipschitzCert/ScorecardCrown`, `…CrownUncon`) import this module and not each")
    A("other, so they build in parallel. Generated by")
    A("`scripts/certs/lipschitz_cert_scorecard_ibp.py`; weights/images are DATA. -/")
    A("")
    A(base.HEADER_OPTS)
    for tag, r in results:
        A1 = r["A1"]
        A(f"-- ════════ net {tag}: per-row ℓ1 norms, one kernel `absSumZ` fact each ════════")
        A("")
        for k in range(H):
        ...
        A(f"    [ {leaves} ]")
        A("")
    A("-- ════════ fallback images (not in the capped base scorecard) ════════")
    A("")
    emitted = set()
    for tag, r in results:
        for i in r["all_certified"]:
...
def emit_net(tag, r, out_path):
    certs, measured, pgd, l2impl = r["certs"], r["measured"], r["pgd"], r["l2impl"]
    base_hpre, all_certified = r["base_hpre"], r["all_certified"]
    W1q = base.nets[tag][0]

    Lb = []
    A = Lb.append
    A("import LeanMlir.Proofs.Certificates.LipschitzCert.ScorecardIBPData")
    A("")
    ...
    A(f"({ENGINE_LINK}), proved once — kernel-checking the 57th")
    ...
    for i in all_certified:
        y = int(yte[i])
        # hpre fallback for images the base scorecard didn't certify (their pixels
        # are in the data module)
        if i in base_hpre:
            hpre = f"hpre{tag}{i}"
        else:
            hpre = f"hpreI{tag}{i}"
...
results = [(tag, analyze(tag, *base.nets[tag])) for tag in ("SF", "TF")]
emit_data(results, OUTDIR / "LipschitzCert/ScorecardIBPData.lean")
emit_net("SF", results[0][1], OUTDIR / "LipschitzCert/ScorecardIBP.lean")
emit_net("TF", results[1][1], OUTDIR / "LipschitzCert/ScorecardIBPUncon.lean")
