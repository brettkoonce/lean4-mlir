...
The CP scorecard (Smoothing/CPScorecard.lean) certifies per image the largest
...
whole corpus via the ONE-kernel-evaluation prefix scan:
  ...
  * recompute phiScanRev (1/1000) 3300 — the descending list
    ...
    Smoothing/PhiBounds.lean; any drift fails the emitted `decide +kernel`
...
Output: LeanMlir/Proofs/Certificates/Smoothing/DecScorecard.lean
"""
...
CERTS = Path(__file__).resolve().parents[2] / "LeanMlir/Proofs/Certificates"
CHUNK_OUT = str(CERTS / "Smoothing/DecChunk{c}.lean")
OUT = CERTS / "Smoothing/DecScorecard.lean"


# ── exact mirror of the Smoothing/PhiBounds.lean kernel functions ──
...
def main() -> None:
    ...
    L = []
    L.append(f"import LeanMlir.Proofs.Certificates.Smoothing.DecChunk{len(CHUNKS)}\n")
    L.append("/-! # The smoothing DECIMAL-radius scorecard — Φ⁻¹ leaves symbolic-land, corpus-wide\n")
    L.append("GENERATED by scripts/certs/smooth_dec_scorecard_gen.py from the same fixed-protocol")
    L.append("driver runs as Smoothing/CPScorecard.lean (first-100 test images, σ = 0.5,")
    ...
    L.append("")
    L.append("Scope: the net-semantics hypotheses (C = a net's argmax + `hp`")
    L.append("interiority) are discharged for a concrete trained net in")
    L.append("Smoothing/NetWitness.lean; the scorecard's own 784-dim driver checkpoints")
    ...
    for c in range(1, len(bounds)):
        ...
        C = []
        if c == 1:
            C.append("import LeanMlir.Proofs.Certificates.Smoothing.PhiBounds\n")
        else:
            C.append(f"import LeanMlir.Proofs.Certificates.Smoothing.DecChunk{c-1}\n")
        C.append(f"/-! # Decimal-radius scorecard, scan chunk {c}/{len(CHUNKS)} "
                 f"(panels {k}…{kc})\n")
        C.append("GENERATED by scripts/certs/smooth_dec_scorecard_gen.py — see")
        C.append("Smoothing/DecScorecard.lean for the corpus story. Each chunk lives in its")
