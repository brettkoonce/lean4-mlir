...
FloatBridge composition exists for this net, unlike the dense `LipschitzCertFloat.lean`):
...
[`IbpConvScorecardImgs{PARTS[0]}`–`{PARTS[-1]}.lean`](https://github.com/brettkoonce/lean4-mlir/tree/main/LeanMlir/Proofs/Certificates) over the net of `IbpConvScorecardNet.lean`.
...
the theorem-vs-measurement split. Generated by `scripts/certs/ibp_conv_scorecard.py`. -/\n""")
    ...
    for path, body in ((OUT.with_name("IbpConvScorecardNet.lean"), L),
                       *((OUT.with_name(f"IbpConvScorecardImgs{part}.lean"), LI[part])
                         for part in PARTS),
                       (OUT, LG)):
        path.write_text("\n".join(body) + "\n")
