...
The existing IBP tier (`LipschitzCert.ScorecardIBP*`) certifies a 784->16->10
...
what gets evaluated, certified, and emitted: the certificate is about the exact
real-number semantics of the k/256 weights. The float-evaluated forward is not
covered (no FloatBridge composition exists for this net).

Emits LeanMlir/Proofs/Certificates/IbpConvScorecard/Basic.lean.
...
OUT = ROOT / "LeanMlir" / "Proofs" / "Certificates" / "IbpConvScorecard/Basic.lean"
...
N_CHUNKS = 4               # image modules the N_EMIT certificates are split across
PARTS = "ABCDEFGH"[:N_CHUNKS]
SEED = 0
...
def main():
    ...
    *_, lg = forward(xte, Wc9q, bcq, Wdq, bdq)
    ...
    NS = "namespace Proofs\nnamespace IBP\nnamespace ConvNet\n"
    END = "end ConvNet\nend IBP\nend Proofs"
    MOD = "LeanMlir.Proofs.Certificates.IbpConvScorecard.Basic"
...
A certificate over a convolution, a max-pool, and more than two layers.
Generated by `scripts/certs/ibp_conv_scorecard.py`; the engine
...
Net (trained, then rationalized to `k/{DEN_W}`). The QUANTIZED weights are what is
certified, under exact-ℝ semantics; the float-evaluated forward is not covered (no
FloatBridge composition exists for this net, unlike the dense `LipschitzCert.Float`):
...
4/255, 8/255 — the same grid as the dense `LipschitzCert.ScorecardIBP`, so
...
`scorecard_ibp_conv` (in `IbpConvScorecard.Basic`) states ONLY the proved counts;
...
`IbpConvScorecardImgs{PARTS[0]}`–`{PARTS[-1]}` and the counts in `IbpConvScorecard`. -/\n''')
    ...
    # import-INDEPENDENT — disjoint names, both importing only the net — so lake
    # can build them in parallel; certs-heavy.yml drives one module per process,
    # which is what keeps the PEAK at one chunk rather than their sum.
    per = -(-n_emit // N_CHUNKS)
    part_of = {rec["idx"]: PARTS[i // per] for i, rec in enumerate(chosen)}
    LI = {}
    for part in PARTS:
        LI[part] = []
        sel = [r for r in chosen if part_of[r["idx"]] == part]
        LI[part].append(f"import {MOD}Net\n")
        LI[part].append(
    ...
    # the aggregate: cheap (list literals + tuples of already-proved theorems)
    LG = []
    A = LG.append
    for part in PARTS:
        A(f"import {MOD}Imgs{part}" + ("\n" if part == PARTS[-1] else ""))
    A(f"""/-! # IBP `L∞` conv scorecard — the aggregate

The counts, tied to the per-image `CertifiedAtLinf3` theorems of
[`IbpConvScorecardImgs{PARTS[0]}`–`{PARTS[-1]}.lean`](https://github.com/brettkoonce/lean4-mlir/tree/main/LeanMlir/Proofs/Certificates) over the net of `IbpConvScorecard.Net`.
See the net module's header for the certificate, the perturbation model, and
the theorem-vs-measurement split. Generated by `scripts/certs/ibp_conv_scorecard.py`. -/\n""")
    A(NS)
    A("-- ═══════════════════════ the scorecard ═══════════════════════\n")
    for n in EPSN:
    ...
    for path, body in ((OUT.with_name("IbpConvScorecard/Net.lean"), L),
                       *((OUT.with_name(f"Imgs{part}.lean"), LI[part])
                         for part in PARTS),
                       (OUT, LG)):
        path.write_text("\n".join(body) + "\n")
        print(f"[emit] {path.name}  ({len(open(path).readlines())} lines)")
