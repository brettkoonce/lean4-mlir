...
## ⚠⚠ The second finding, and it is a plain bug

`TestR50GradCheck.lean`'s control check carries this comment:
...
MINIMUM over the 21 control sites, i.e. exactly the order statistic the comment rules out. That
minimum is the least stable number in the whole report, and it is the one the verdict rests on.

## What this script does NOT do

It does not change any verdict, tolerance or pass criterion. It runs the gate as committed, N times,
and reports the spread. Changing what the gate ACCEPTS is a decision for a human with this data in
hand — see `a3_paper_fidelity.md` §3.1 for the options.
...
PATTERNS = {
    ...
    "control":   re.compile(r"weakest violation (\S+) at (.+?);\s+median (\S+);"),
...
def one_run(variant, env_extra):
    ...
    for k, rx in PATTERNS.items():
        ...
        if k == "loss":
            ...
            r["ctl_min"], r["ctl_at"], r["ctl_med"] = float(m.group(1)), m.group(2), float(m.group(3))
        else:
            r[k] = float(m.group(1))
            r[k + "_at"] = m.group(2)
    return r
...
def main():
    ...
    for v in a.variants:
        ...
        print(f"    ⟂ weakest      {spread(ctl_mins)}   <- the MINIMUM over 21 sites; the verdict "
              f"rests on it")
        ...
        seps = [r["ctl_min"] / r["tierB"] for r in runs
                if r.get("ctl_min") and r.get("tierB")]
        meds = [r["ctl_med"] / r["tierB"] for r in runs
                if r.get("ctl_med") and r.get("tierB")]
        if seps:
            ok = sum(1 for s in seps if s > 5.0)
            print(f"    separation (weakest/passing, gate wants > 5×):  "
