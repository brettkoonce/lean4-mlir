...
ε = 0.1 (pooled-feature L2). The counts below are exact-rational
MEASUREMENTS; the witness images (the first 8 certifying per net, test-set
order) each additionally carry an in-kernel exact-rational margin lemma and a
`∀ δ, ‖δ‖ < ε → argmax fixed` theorem, and `scorecard` states only those — see
"Theorem vs. measurement" below before quoting a number. Zero sorrys, standard
3-axiom closure. Counts are honest lower bounds (an
...
stated subset; on top of that, the first few certifying images at each radius carry a
per-image **theorem** (8 per net in the L2 tables; 8/7/6/3 at ε = 1/2/4/8 per 255
in the IBP conv row) (`CertifiedAtLinf`/`CertifiedAtLinf3`, i.e.
