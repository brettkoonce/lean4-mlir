...
/-! # Challenge file for `leanprover/comparator` — the architecture theorems

**This file imports `LeanMlir.Proofs.*`, and that is deliberate.** Every
theorem below is a statement *about a specific network* — "ResNet-34's
rendered backward equals its Fréchet derivative" — and such a statement
cannot be made without the network in scope. The alternative, copying each
architecture into the challenge file, would create a second definition of
every net that must never drift from the first; that is a weaker guarantee
than an import, not a stronger one.

The architecture-free half of the suite lives in `Challenge.lean`, which
imports Mathlib and nothing else, and which carries `chk_pdiv_is_fderiv` —
the theorem pinning this project's calculus vocabulary to Mathlib's
`fderiv`. Once that pin and the structural rules are checked over Mathlib
alone, the theorems here are applications of them; what a reviewer must
additionally trust is the forward functions, and nothing else.
-/
...
theorem chk_denseWeightGrad_correct {m n : Nat} (W : Mat m n) (b : Vec n)
...
theorem chk_denseBiasGrad_correct {m n : Nat} (W : Mat m n) (b : Vec n)
...
theorem chk_sdpaBackQ_correct (n d : Nat) (Q K V dOut : Mat n d)
    (i : Fin n) (j : Fin d) :
...
theorem chk_sdpaBackK_correct (n d : Nat) (Q K V dOut : Mat n d)
    (i : Fin n) (j : Fin d) :
...
theorem chk_sdpaBackV_correct (n d : Nat) (Q K V dOut : Mat n d)
    (i : Fin n) (j : Fin d) :
    sdpaBackV n d Q K V dOut i j =
    ∑ k : Fin n, ∑ l : Fin d,
      pdivMat (fun V' => sdpa n d Q K V') V i j k l * dOut k l := by sorry

/-- **`reluHasVJP` contract**: canonical witness's backward equals the
`pdiv`-contracted Jacobian by definition. -/
theorem chk_reluHasVJP_correct (n : Nat) (x : Vec n) (dy : Vec n) (i : Fin n) :
    (reluHasVJP n).backward x dy i =
    ∑ j : Fin n, pdiv (relu n) x i j * dy j := by sorry

/-- **`mlpHasVJP` contract**: same pattern for the three-layer MLP forward. -/
theorem chk_mlpHasVJP_correct {d₀ d₁ d₂ d₃ : Nat}
    (W₀ : Mat d₀ d₁) (b₀ : Vec d₁)
    (W₁ : Mat d₁ d₂) (b₁ : Vec d₂)
    (W₂ : Mat d₂ d₃) (b₂ : Vec d₃)
...
/-- **`maxPool2HasVJP3` contract**: canonical witness equals the
`pdiv3`-contracted Jacobian. Codegen substitutes argmax-routing at
non-smooth tiebreaks. -/
theorem chk_maxPool2HasVJP3_correct {c h w : Nat}
...
/-- **`depthwiseHasVJP3` contract**: proved input-VJP equals the
`pdiv3`-contracted Jacobian. -/
theorem chk_depthwiseHasVJP3_correct {c h w kH kW : Nat}
...
/-- **`residualHasVJP` contract**: skip-connection backward equals
the `pdiv`-contracted Jacobian of `f + id`. -/
theorem chk_residualHasVJP_correct {n : Nat}
...
/-- **`residualProjHasVJP` contract**: projected variant where the
skip isn't identity. -/
theorem chk_residualProjHasVJP_correct {m n : Nat}
    (proj f : Vec m → Vec n)
...
/-- **`seBlockHasVJP` contract**: SE-block backward (input × gate
Jacobian via product rule) equals the `pdiv`-contracted Jacobian. -/
theorem chk_seBlockHasVJP_correct {n : Nat}
...
/-- **`transformerBlockHasVJPMat` contract**: full transformer block
...
/-- **`vitFullHasVJP` contract**: the apex of the proof chain. The full
ViT's backward (patchEmbed → vitBody → classifierFlat composition) equals
...
/-- **`reluHasVJPAt` contract**: the pointwise (smooth-input)
variant — backward equals the `pdiv`-contracted Jacobian. Unlike the
global `chk_reluHasVJP_correct`, this instance's underlying
`.correct` is a real proof (`pdiv_relu` + sum-collapse), not `rfl`. -/
theorem chk_reluHasVJPAt_correct (n : Nat) (x : Vec n)
    (h_smooth : ∀ k, x k ≠ 0) (dy : Vec n) (i : Fin n) :
    (reluHasVJPAt n x h_smooth).backward dy i =
    ∑ j : Fin n, pdiv (relu n) x i j * dy j := by sorry

/-- **`mlpHasVJPAt` contract**: pointwise MLP backward via
`vjpCompAt` through `dense → relu_at → … → dense`. No `rfl` escape
at the ReLU kinks; smoothness required on every intermediate
pre-activation. -/
theorem chk_mlpHasVJPAt_correct {d₀ d₁ d₂ d₃ : Nat}
    (W₀ : Mat d₀ d₁) (b₀ : Vec d₁)
    (W₁ : Mat d₁ d₂) (b₁ : Vec d₂)
    (W₂ : Mat d₂ d₃) (b₂ : Vec d₃)
    (x : Vec d₀)
...
/-- **`cnnHasVJPAt` contract**: the pointwise (smooth-input) variant
for the full ResNet-style CNN — backward equals the `pdiv`-contracted
Jacobian. CNN analogue of `vitFullHasVJP_correct`; the underlying
`.correct` chains `vjpCompAt` through stem → maxpool → residual
blocks → global-avg-pool → dense under the smoothness hypotheses. -/
theorem chk_cnnHasVJPAt_correct
...
/-- **`convnextHasVJPAt` contract**: the pointwise (smooth-input)
variant for the full ConvNeXt — backward equals the `pdiv`-contracted
Jacobian. Chains `vjpCompAt` through stem → two ConvNeXt blocks
...
/-- **`convnextHasVJP_correct` contract (global)**: ConvNeXt is all-smooth
...
/-- **`efficientnetHasVJP_correct` contract (global)**: EfficientNet is
all-smooth (swish + sigmoid SE gate + conv/bn, no kinks), so its
whole-network VJP holds at *every* input — the unconditional analogue of
`vitFullHasVJP_correct`. -/
theorem chk_efficientnetHasVJP_correct
    {ic c cmid₁ cout cmid₂ h w kHs kWs kHe₁ kWe₁ kHd₁ kWd₁ kHp₁ kWp₁
      kHe₂ kWe₂ kHd₂ kWd₂ kHp₂ kWp₂ r₁ r₂ nClasses : Nat}
    (Ws : Kernel4 c ic kHs kWs) (bs : Vec c) (εs γs βs : ℝ) (hεs : 0 < εs)
    (We₁ : Kernel4 cmid₁ c kHe₁ kWe₁) (be₁ : Vec cmid₁) (εe₁ γe₁ βe₁ : ℝ) (hεe₁ : 0 < εe₁)
...
theorem chk_conv2dHasVJP3_correct {ic oc h w kH kW : Nat}
...
theorem chk_globalAvgPoolFlatHasVJP_correct (c h w : Nat)
...
/-- **`mnistLinearHasVJP_correct` contract**: whole-model VJP for the
...
theorem chk_mnistLinearHasVJP_correct {m n : Nat} (W : Mat m n) (b : Vec n)
    (x : Vec m) (dy : Vec n) (i : Fin m) :
