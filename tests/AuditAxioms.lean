...
-- SmoothingDecScorecard.lean, GENERATED by scripts/smooth_dec_scorecard_gen.py):
-- phiScanRev = the descending grid scan whose ONE kernel evaluation
-- (phiScanLit_eq, 3300 panels at h=1/1000, ~2 min) prices the whole grid, after
-- which each image's check is an O(index) list lookup (phiScanRev_getD +
-- le_stdNormalQuantile_of_scan + the sigma=1/2 protocol form smooth_radius_dec).
-- All 279 CP-scorecard images get PROVED decimal radii m/2000 <= sigma*Phi^-1(q0)
-- (grid-optimal m; 0.003-0.036 below the driver's float printout). The
-- aggregates below transitively audit all 279 per-image lemmas.
