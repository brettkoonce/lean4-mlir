...
convolution differentiates wrongly"
  ...
  if bC <= 5.0 * max wA wB then
    throw <| IO.userError s!"CONTROL DEAD: the two populations overlap — {atC}, where the \
homogeneity does NOT hold, sits at {bC} against a homogeneous worst of {max wA wB}. The cosine is \
~0 for reasons unrelated to the identity and tier 1 proves nothing"
...
violation {bC} ({bC / max (max wA wB) 1e-30}× the worst passing site), median {ctlMed} \
({ctlMed / max (max wA wB) 1e-30}×), with the tolerance {tolExact} sitting between the two \
populations."
