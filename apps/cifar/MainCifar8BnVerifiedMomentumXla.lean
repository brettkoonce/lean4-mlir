import apps.cifar.Cifar8BnMomCommon

/-! # `cifar8-bn-momentum-xla` — the CIFAR-8 + per-channel BN + Nesterov-momentum SGD trainer on **XLA/PJRT**

The XLA peer of `cifar8-bn-momentum`: same body (`apps/cifar/Cifar8BnMomCommon.lean`), same
...
Run: `.lake/build/bin/cifar8-bn-momentum-xla data`  — no `IREE_BACKEND`; the backend is
