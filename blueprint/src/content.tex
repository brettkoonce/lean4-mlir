...
because each is a statement about a specific network --- ``ResNet-34's
...
\end{verbatim}
    ...
    that command puts it; \texttt{\$PJRT\_PLUGIN} points elsewhere. The plugin
    is version-matched to your CUDA/ROCm install: a 500\,MB download, not a
    build.
...
Every verified trainer is one executable carrying both trusted lowerers,
picking between them at run time:
...
Those are XLA wall-clocks on one 7900 XTX, and they are why the default moved:
the same three tiers on IREE take $\sim$5~minutes, $\sim$34~minutes and
$\sim$42~hours. The Imagenette figure covers the five nets with recorded
timings; MobileNetV4 and ResNet-50 joined the tier more recently and are not in
that total, so budget somewhat more.

Each has an \texttt{-iree} twin --- \texttt{lake run mnist-iree} and so on ---
which runs the same binaries with the IREE lowerer selected. The tiers bundle
these targets, and any one builds on its own:
...
is absent rather than falling back to another net's --- an earlier fallback
silently gave every net ResNet-34's augmentation.
...
Data parallelism needs both replica knobs --- \texttt{PJRT\_REPLICAS}
...
\begin{itemize}
  ...
  \item \textbf{IREE} generates every kernel itself, which is why it runs where
    no vendor library exists --- Vulkan, Metal, bare CPU, custom accelerators
    --- and why its runtime is 392$\times$ smaller (1.3\,MB against the XLA
    plugin's 505\,MB, which bundles MIOpen, rocBLAS and RCCL).
\end{itemize}

That size difference is also a trust difference, counted in
Appendix~\ref{app:verification}: the fast path asks you to trust far more
unverified code.
...
\begin{itemize}
  ...
  \item \textbf{\texttt{HAL device not found}} on an IREE run --- IREE has two
    knobs and they must agree: \texttt{IREE\_BACKEND} is the
