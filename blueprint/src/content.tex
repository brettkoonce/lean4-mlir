...
momentum ($\mu$ 0.9)  & \emph{diverged} & \textbf{76.0} {\footnotesize (1.1)} & \emph{diverged} \\
\hline
\end{tabular}
\end{center}
...
\medskip\noindent\textbf{The diverged cells are Lever~1 again, not a precision
effect.} The momentum row loses a run to \texttt{NaN} in fp32 \emph{and} in fp8,
one of five each, collapsing to 10\% --- chance on ten classes. That is the same
un-normalized instability the empty cell in Lever~2 records, reproduced here
independently in two arithmetics, so the honest reading is that the network is
what is fragile, not the number format. The bf16 column happens to lose none;
with one failure in five as the base rate that is not evidence bf16 stabilizes
anything, and we do not claim it does.
...
\textbf{What has not changed is the reason not to train on it}:
MobileNetV4's collectives have no \texttt{shard-check} peer, so nothing
ties them. Cost a schedule from this row; do not quote an accuracy off it
until they are tied.}
