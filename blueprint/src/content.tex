...
Box & ms/step & min/epoch & 300 epochs & Val top-1 \\
\hline
4$\times$ 4060 Ti (CUDA)  & $224 \to 153$ & $9.3 \to 6.4$ & $\sim$50 $\to$ $\sim$35 h \;($2.1 \to 1.5$ d) & TBD \\
4$\times$ 3060 (CUDA)     & ---           & ---           & ---                                          & TBD \\
\hline
\end{tabular}
\end{center}

\noindent The columns are the two arms of \texttt{adamdp128x4wxclipdrop} at
$128$ per device, so four cards give DeiT's own global $512$ and $2{,}502$ steps
per epoch --- the reference's own figure. fp32 is the default; the run will be
bf16, so read the right-hand column, and it will also carry EMA, which
\texttt{vit-\allowbreak default-\allowbreak emabf16-\allowbreak 4gpu} names and
which measures $151$\,ms --- one under the arm above, so the shadow is free.
...
recipe. Score-SDE (Song et al.\ 2021) reads the same object as a stochastic
...
between them that DDIM's $\eta$ interpolates.
