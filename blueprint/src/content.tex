...
proven derivative. The shared machinery underneath them --- the denoted
...
\begin{itemize}
  \item \textbf{ReLU is a smooth-point bridge.} Where a pre-activation is
    ...
    measure-zero set.
  \item \textbf{Representative scale.} The listing is shown at a small
    size; the trained network is $784{\to}512{\to}512{\to}10$. The proof
    is dimension-parameterized and the printer emits the same graph at
    every width.
...
\begin{itemize}
  ...
  \item \textbf{Representative scale} --- $1{\to}2$ channels at $4\times4$
    rather than the production $1{\to}32$ net at $28\times28$; the per-op
    proofs are dimension-parameterized.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Needs $\varepsilon > 0$.} The inverse-stddev term carries an
    ...
    max-pool).
  \item \textbf{Representative scale} ($n = 4$); the proof is
    dimension-parameterized and the printer emits the same graph at every
    width.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Representative scale} --- a constant-width two-channel block
    at $4\times4$ rather than the production stem-to-512 net; the same
    \texttt{\%dadd} fan-in is repeated for each of the sixteen blocks.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Representative scale} (two channels at $4\times4$); the proof
    is dimension-parameterized.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Representative scale} (four mid-channels, reduced to two in
    the squeeze); the proof is dimension-parameterized.
\end{itemize}
...
\noindent\textbf{What is already proven.} Layer scale is a
per-channel learnable diagonal, $\mathrm{layerScale}(\gamma, x) = \gamma
\odot x$. Its Jacobian is diagonal, so it is its own adjoint:
\texttt{layerScale\_has\_vjp} proves $\mathrm{back}(x, dy) = \gamma \odot
dy$ --- the backward multiplies by the \emph{same} $\gamma$ the forward did.
The full block is $\mathrm{residual}(\mathrm{layerScale} \circ
\mathrm{project} \circ \mathrm{gelu} \circ \mathrm{expand} \circ
\mathrm{LN} \circ \mathrm{depthwise})$, and
\texttt{convNextBlock\_has\_vjp} composes the six stages through
\texttt{vjp\_comp\_at}.

\medskip\noindent\textbf{The gap, and how we close it.} The emitted graph
denotes \texttt{layerScale\_has\_vjp}'s backward exactly. Here is the block's
tail --- the layer scale, the identity skip, and the start of the backward:
...
\noindent Read it against the Jacobian: the forward \texttt{multiply \%gls,
\%pr} and the backward \texttt{multiply \%gls, \%dOut} are the \emph{same}
operation against the \emph{same} $\gamma$ tensor \texttt{\%gls} --- a
diagonal map is its own transpose, so its VJP is itself. The rest of the
block backward (the GELU, LayerNorm, pointwise, and depthwise input-VJPs)
chains through the bridges of the previous chapters.

\medskip\noindent\textbf{Caveats.}
\begin{itemize}
  \item \textbf{No kink anywhere.} This is the first block in the book whose
    every bridge is unconditional --- GELU and LayerNorm are smooth (the
    latter given $\epsilon > 0$), and layer scale and the convolutions are
    linear; the only hypothesis anywhere is the LayerNorm's $\epsilon > 0$.
  \item \textbf{Representative scale} (two channels at $4\times4$); the proof
    is dimension-parameterized.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Representative scale} (two tokens, $d = 4$, one head); the
    proof is dimension-parameterized and the printer emits the same graph at
    every sequence length and head count.
\end{itemize}
