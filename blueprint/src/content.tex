...
So how much does each ingredient actually earn? The full recipe and seven
...
\medskip\noindent The two panels are the same eight arms in the two precisions,
...
\medskip\noindent \textbf{And the two points inside the band do not reproduce.}
Weight decay is $-0.64$ in fp32 and $-0.10$ in bf16; label smoothing is $-1.50$
and $-0.46$. Each moves by about its own size. The band predicted, from
$n = 3{,}925$ alone, exactly which rows a second condition would fail to
confirm --- so the figure does not merely assert that those two rows are
unresolved, it shows them being unresolved. Reading either as an ingredient's
contribution would be reading noise, and this is what licenses training the
ImageNet jobs of \S\ref{sec:r34_pjrt} in bf16.
Chapter~\ref{chap:bn}'s Lever~3 makes the same case on the normalized CIFAR net.
...
\begin{axis}[
  width=0.52\linewidth, height=4.6cm, at={(0.48\linewidth,0)}, anchor=south west,
...
\medskip\noindent Three readings follow.

\textbf{Augmentation is the largest single contribution} ($-7.01$ points, three
...
\textbf{Half the table is not resolvable.} At $n = 3{,}925$ the 95\% Wilson
half-width is $\pm0.92$ points, wider than the weight-decay, warmup and
label-smoothing deltas combined with room to spare. Their intervals ---
$[88.80, 90.69]$, $[88.37, 90.30]$, $[87.90, 89.86]$ --- all overlap the full
recipe's $[89.46, 91.30]$. On one run each, the honest reading is that
augmentation, the schedule and the optimizer are resolved and the three small
knobs are not; separating those would take seeds, not a bigger table.

\textbf{The contributions are \emph{not} additive}, and this is where a single
vintage changes the answer. The six leave-one-out deltas sum to $-16.97$ while
turning every one of them off at once costs $-5.99$. Removing one ingredient at
a time therefore over-counts by roughly a factor of three: the recipe's parts
overlap heavily, and each is partly insurance against the absence of the others
rather than an independent addition. A network trained with none of them still
reaches $84.43\%$, within a point and a half of the one trained with everything
except augmentation.

\medskip\noindent \textbf{One row is named for what it loads.} The $-$AdamW row
swaps the optimizer for Nesterov momentum and keeps the rest of the recipe
intact; the next section is about what that swap does and does not measure.
...
artifact is
\texttt{resnet50in160\_lambaccdp8x64wxclipbcebf16\_train\_step}: LAMB $\times$
BCE-with-logits $\times$ gradient accumulation $k=8$, four replicas at
per-replica batch 64 for an effective batch of 2048, train@160 / eval@224.
...
run the $10^{-3}$ arm. Phase 2 scores $76.80\%$ / $93.26\%$ against the paper's
$77.1$ / $93.3$, and that $-0.3$ is left unattributed rather than pinned on this.

\medskip\noindent\textbf{A second difference was found after that run and is now
fixed, so the curve above predates it.} B0 is SiLU/swish throughout, stem and
head included, and the phase-4 render always was --- $194$ \texttt{stablehlo.logistic}
and not one \texttt{stablehlo.maximum}. The phase-2 spec, though, never set
\texttt{convBnAct}, which defaults to \texttt{.relu}, so the two \texttt{.convBn}
layers that bracket the MBConv stack --- the stem and the $1\times1$ head --- trained
...
will be bf16, so read the right-hand column. Set that against the $\sim$8.6 minutes per
epoch the phase-2 350-epoch row reports on the same four cards: both are bf16 over the
same architecture, and the two agree to within $4\%$.
...
\paragraph{The working detector --- demo: R34+FPN on VisDrone.}
A ResNet-34 backbone, trained by this stack on ImageNet, feeding an FPN
top-down neck into three anchor heads at strides $8/16/32$; DIoU box
loss, focal objectness, class-weighted CE; $448$\,px input, 12 epochs,
no augmentation. Scored against VisDrone's \emph{uncapped} validation
ground truth --- all $38{,}759$ boxes, not the 56-box-truncated training
record, which drops $34.9\%$ of them and is not the benchmark's protocol.
The catalog entry below keeps the paper-exact YOLOv1 \emph{shape} for the
historical record; this is the trained detector beside it.
...
\hline
\textbf{R34+FPN, 50\,ep, +aug} & \textbf{0.1674} & \textbf{0.703} & \textbf{0.429} \\
...
\noindent \textbf{The middle two rows are the result, and they only mean
something together.} Quadrupling the schedule with augmentation off \emph{costs}
19\%: less than half the training loss, more detections, lower recall --- ordinary
overfitting on 6{,}471 images. Turning augmentation on at that same 50-epoch
schedule converts $-19\%$ into $+10\%$ over the best short run. One flag decides
whether a longer schedule pays at all, so reporting either alone would mislead.

\medskip\noindent The epoch sweep sharpens it further: \emph{both} arms peak and
then decline. Without augmentation the peak is near epoch 15 at $0.132$; with it,
near epoch 30 at $0.1731$, giving $3\%$ back by epoch 50. Augmentation therefore
delays and softens overfitting rather than removing it --- it moves the optimum
from roughly 15 epochs to roughly 30 and raises it by $31\%$.

\medskip\noindent The stack beats the hand-written PyTorch replica of
\emph{this same architecture} by $9\%$, having trailed it by $10\%$ before this,
and runs at 65\,fps on one RTX 4060 Ti.
The replica, not YOLOv8, is the yardstick: it isolates implementation
quality from architecture, which a comparison against a different model
would conflate. The two YOLOv8s rows say something more interesting than
either alone. At a matched budget it scores $0.140$ --- below this
detector, though from random weights rather than an ImageNet-pretrained
backbone --- while its ordinary-practice $0.391$ comes from $8\times$ the
epochs, higher resolution, full augmentation and COCO pretraining.
\textbf{The distance to the field is recipe, not architecture.}

\medskip\noindent VisDrone is chosen because it is the harder half:
drone-altitude imagery where a median image holds \emph{seventy} objects
and many are $2$--$5$\,px after the resize. That is the regime where a
single coarse grid structurally cannot emit the objects, and it is what
makes the multi-scale head of Definition~\ref{layer:fpnModule} necessary
rather than decorative.

\medskip\noindent The result worth reading is not the mean. Per-class AP
runs from car at $0.605$ down to bicycle at $0.014$ --- a $44\times$
spread. Augmentation lifted 8 of 10 classes and lifted them hardest where the
detector was weakest (bus $0.165 \to 0.227$), so the mean is no longer one class
in a trench coat. Detection on aerial imagery does not degrade uniformly; it
collapses on whatever is simultaneously small \emph{and} rare, and an
averaged mAP is precisely the statistic that hides it. Workings:
\texttt{runs/2026-08-28-visdrone-fpn-rebuild/}.

\medskip\noindent One failure this run had to be watched for, and it is a
...
dropping the whole time} --- so loss is not a proxy for detector quality. It
did not materialise here, and the reason is instructive: at seventy scattered
objects an image, the positional marginal carries almost no information, so
the cheap minimum is not cheap. The dataset that makes the task hard is the
same one that makes this particular collapse unavailable.

\medskip\noindent A sharper version of the same warning did bite. For a full
chapter this detector was believed to be defeated by its target assignment,
on the strength of four trained ablations and eight numerical probes. All of
them were measuring a data-loading bug that paired each image with another
image's labels, so the only learnable signal was the marginal distribution of
the targets. The tell was available throughout and was not read: no lever
moved the metric, at all, ever. \textbf{When every intervention produces the
same number, the instrument is the thing to suspect, not the model.}
