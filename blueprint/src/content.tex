...
\emph{not} comparable to a BraTS~2021 leaderboard --- but this is also a
...
\emph{brain voxels only} (BraTS volumes are skull-stripped, so
...
not by slice} (adjacent axial slices of one brain are near-duplicates, so
a slice-level split leaks memorized neighbours across the boundary).
...
\texttt{LeanMlir/Proofs/Foundation/LinearFaithfulPoC.lean} and
