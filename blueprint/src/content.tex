...
Ninety epochs at global batch 256 is what He et al.\ ran and what every
number in this section is aiming at, on both the verified path and the
JAX reference. It costs about $28$ hours on the four-card box.
...
\medskip\noindent The 90-epoch row is the paper schedule, is the driver's
default, and is a \texttt{[TODO]}. It runs with:
...
\noindent The completed 30-epoch row was its own fully-annealed schedule, not
the first third of this one, which is a distinction worth keeping straight:
\texttt{LEAN\_MLIR\_EPOCHS=30} re-shapes the cosine to finish at 30, while
\texttt{LEAN\_MLIR\_MAX\_EPOCHS=30} would stop a 90-epoch decay a third of
the way down and leave the learning rate high. The 7900 XTX row is blank rather
than pending, because a verified trainer does not currently survive ROCm (11
deaths in 11 attempts on gfx1100 against 6 clean in 6 on CUDA). The repro is in
