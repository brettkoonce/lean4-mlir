...
\section*{The affine step is dense-with-broadcasting}
...
\section*{ResNet-34 is sixteen of these blocks}
...
4$\times$ 4060 Ti & 300 & $\sim$14.9 min & $\sim$74 hr & --- & --- \\
\hline
\end{tabular}
\end{center}
...
$01$\,m) including the two 30-minute thermal rests. The 300-epoch row is
the projected wall-clock; accuracy pending.)
...
tier (recipe arg \texttt{full}, \texttt{convNeXtTinyImagenetConfigFull}),
\textbf{300 epochs} --- identical config to the $78.13\%$ run above, only
\texttt{epochs := 300}. The 80-epoch curve is still climbing at its
endpoint and never plateaus, so the heavy augmentation has not saturated:
the 300-epoch schedule is expected to extend the gain toward
ConvNeXt-T's $\sim$82\% headline rather than overfit. Projected
wall-clock $\sim$74\,hr on four GPUs (the table's second row), or
$\sim$66\,hr on six if the AER-prone cards are brought back in.
