...
are now in them.} The same ledger \S\ref{sec:r50_phase4} keeps for A3, kept here
...
\noindent \textbf{Both were structural rather than forgotten, and A3 met
neither} --- its own recipe switches EMA and stochastic depth \emph{off}, which
is why nothing here was visible from A3 succeeding.

\medskip\noindent The EMA shadow and the gradient accumulator used to be the
\emph{same} fourth region of $[\theta|m|v|\cdot]$, and the driver refused the
pairing rather than letting one win. Since accumulation is not optional at
$224^2$ on a $16$\,GB card, that made EMA unreachable rather than merely
unrendered. The blob now carries both: three regions, or four, or five, with $G$
before $E$ so that every checkpoint written under the old layout still reads at
the index it was written at. \textbf{The refusal came off last} --- while it
stood a wrong render failed loudly at load, and the moment it was gone a
five-region graph packed wrongly would train and report a number.

\medskip\noindent Stochastic depth cost less in the renderer and more in the
reference. Its site sits on the residual \emph{branch}, between the last
batch-norm and the skip add, and \textbf{no structural check can tell that from a
site on the block output}: at an all-ones mask $1 \odot (b + x)$ and
$1 \odot b + x$ are the same function bit-for-bit, so a misplaced render has the
same names, the same $18{,}186$ operations and the same arity, and passes every
endpoint gate. Moving all sixteen sites and running both at a real mask separates
them by eight orders of magnitude; at an all-ones mask the difference is exactly
zero, which is the half of that measurement worth keeping.

\medskip\noindent \textbf{The one remaining delta is the batch-norm group, and it is
larger than it looks.} Nothing in this render all-reduces activation statistics
--- all $162$ collectives are on parameter-shaped tensors --- so each replica
normalises over the $64$ images it holds. The reference reaches $2048$ as four
accumulation steps of a $512$ micro-batch, and its
\texttt{jnp.mean(x,\ axis=(0,2,3))} reduces over an axis that is
\emph{mesh-sharded}, so XLA inserts the collective and the group is the full
$512$. The per-device tensor is $128$; the group is $512$. Those are different
numbers and only the second one is the batch-norm statistic.

\medskip\noindent Half of that is a re-render rather than a feature: $k=4$ at
$128$ per device is the reference's own factorisation of $2048$, and it takes the
group from $64$ to $128$. \textbf{But it only fits in bf16}, and that is measured
rather than argued --- against the $11.68$\,GiB the allocator offers on a
$16$\,GB card, the $8\times64$ render peaks at $6.37$\,GiB and the $4\times128$
one at $11.13$, which is $95\%$ and leaves nothing for the input pipeline. In
bf16 the same render peaks at $8.09$\,GiB. So the artifact ships with no fp32
twin, and the missing twin is the finding rather than an oversight. The last
$4\times$ needs the statistics themselves all-reduced --- synchronised batch-norm,
which is a new batched operator, a changed backward, and $53$ collectives a step
against a run whose whole throughput story is that the all-reduce term is
$56$\,ms.

\medskip\noindent \textbf{And the reference was the side that was wrong.} Four of
the five block emitters in \texttt{jax/Jax/Codegen.lean} drew
\texttt{bernoulli(key,\,keep)} with no shape argument --- a scalar shared by the
whole batch, so the branch was dropped for every example or for none. timm's
\texttt{drop\_path} is per \emph{sample}, and the same file already emitted the
per-example form for the transformer family. Both draw the same
\emph{expectation}, which is why it survived: at batch $64$ and keep $0.95$ the
old form dropped all $64$ or none on $200$ steps out of $200$, and the new one
drops a mean of $3.2$. Every verified \texttt{drop} render in the tree was
already right.
