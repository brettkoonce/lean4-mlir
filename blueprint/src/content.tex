...
\texttt{MlpFaithfulPoC} shows each denotes the certified gradient-descent
...
[pjrt_ffi] compiled verified_mlir/cifar8w_bn_fwd.mlir
             (@cifar8w_bn_fwd, 1 outputs, 1 replica) in 234 ms
...
configs and launch like any Track-4 row; only the two ResNet-50 recipes have
none. The status column says what each one is missing.
...
between \(3\times\) and \(26\times\). These are the theorem's formula evaluated
at the \emph{measured} per-stage magnitudes and tail gains; the theorem's own
budget takes every stage at its worst-case propagated window, which at He-init
magnitudes exceeds \(10^{13}\) on CIFAR-8, so the numbers are a-posteriori
measurements rather than values the kernel has checked. The decision statement
is still a theorem: \texttt{cifar8\_chain\_argmaxSafeH}, at the committed CIFAR-8
shape, proves that whenever the exact network's margin beats twice the budget,
binary32 rounding cannot change which class it predicts.
