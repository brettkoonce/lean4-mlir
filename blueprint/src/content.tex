...
neutral. Separating those would take a verified render that all-reduces
the batch statistics, which no committed artifact does, and which is not a
patch but a new AST node, a new specification and a re-proof of the tie
chain: the only node in the graph language that sees across replicas is
the gradient collective, and every BatchNorm node consumes a single
expression. Every BatchNorm net in this book carries the same asymmetry
for the same reason.

\medskip\noindent\textbf{[TODO: global BN.]}
...
$0.99$, 350 epochs. All of it is carried, in both phases, including the two
...
\texttt{\_bn(..., eps=1e-5)} in the emitted phase-2 code, and $147$ splat
constants of \texttt{1.0e-5} in the phase-4 render --- so the two phases agree
with each other and both differ from the paper. It is unquantified: nobody has
run the $10^{-3}$ arm. Nor is there a deficit for it to explain: phase 2 scores
...
\medskip\noindent\textbf{The activation agrees on both sides.} Phase 4 is swish
throughout --- $194$ \texttt{stablehlo.logistic} and not one
\texttt{stablehlo.maximum} --- and phase 2 sets \texttt{convBnAct} to swish on
the stem and the $1\times1$ head. The two phases describe the same network.
...
replica's $64$ against the reference's global $256$, for the structural
reason given there --- no committed render can all-reduce the batch
statistics. The two pairs that carry the same fourfold difference landed
