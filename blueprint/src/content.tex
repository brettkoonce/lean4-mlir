...
\textbf{[TODO: wire RMSProp]} and original
...
\texttt{tf.data} pipeline, each knob gated on a config flag, RandAugment as
a color-only ``lite'' variant) but left off at this validation tier.
...
RMSProp, AutoAugment (the trainer substitutes RandAugment),
stochastic depth, and EMA. Of those, EMA (decay
0.9999) and stochastic depth (drop-connect 0.2) are on here, costing
${<}5\%$ per epoch; RMSProp, the geometric half of RandAugment
\textbf{[TODO: wire RMSProp + geometric RandAugment]}, and
...
trainer (each knob gated on a config flag, RandAugment as a color-only
``lite'' variant \textbf{[TODO: wire geometric RandAugment]}) but left off
here, so an 80-epoch run lands under the paper's $82.1\%$; the 80 here is
...
\textbf{[TODO: wire geometric RandAugment]} and
...
\begin{tabular}{l l r r r r}
\hline
GPU & Precision & Per epoch & 80 epochs & Val top-1 & Val top-5 \\
\hline
4$\times$ 4060 Ti (CUDA) & fp32 & $\sim$11.1 min & $\sim$15 hr & --- & --- \\
4$\times$ 4060 Ti (CUDA) & bf16 & $\sim$7.6 min & $\sim$11 hr & $\mathbf{65.64\%}$ & $\mathbf{87.06\%}$ \\
\hline
\end{tabular}
\end{center}
...
only the bf16 configuration was trained to completion. bf16 does
not cost final accuracy here.)
