...
A2 (\texttt{a2-accum})     & 300 & $224$ & rendered, EMA + sd & $\sim$171 hr (est) & $79.8\%$ (paper) \\
A1 (\texttt{a1})           & 600 & $224$ & rendered, EMA + sd & $\sim$342 hr (est) & $80.4\%$ (paper) \\
\hline
\end{tabular}
\end{center}
...
is a re-render, kept on its own path so one line among $18{,}000$ cannot be
lost.
...
\medskip\noindent \textbf{And what the phase-2 peers cost}, on the same four
cards with A3 as the control:
...
constants. \textbf{The verified column is measured, not scaled from theirs}: on
the same four cards A3's graph runs $191.6$\,ms/step and A2's $341.1$, and A3's
completed run fixes the trainer at $1.20\times$ the bare device step --- $34.2$
verified minutes an epoch, so $171$ and $342$ hours. In bf16 at $4\times128$ it is
$18.9$, which puts A2 under four days and A1 under eight.
