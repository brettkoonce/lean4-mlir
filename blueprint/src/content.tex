...
[pjrt_ffi] compiled verified_mlir/mlp_train_step.mlir
             (@mlp_train_step, 6 outputs, 1 replica) in 2398 ms
[pjrt_ffi] compiled verified_mlir/mlp_fwd.mlir
             (@mlp_fwd, 1 outputs, 1 replica) in 122 ms
MNIST-MLP via the VERIFIED renderer (784→512→512→10) → XLA/PJRT → GPU
  ...
  epoch 1: test_acc = 9199/10000 = 91.990000% (880ms)
  epoch 2: test_acc = 9525/10000 = 95.250000% (883ms)
  epoch 3: test_acc = 9635/10000 = 96.350000% (903ms)
  epoch 4: test_acc = 9690/10000 = 96.900000% (879ms)
  epoch 5: test_acc = 9714/10000 = 97.140000% (827ms)
  epoch 6: test_acc = 9733/10000 = 97.330000% (803ms)
  epoch 7: test_acc = 9749/10000 = 97.490000% (843ms)
  epoch 8: test_acc = 9765/10000 = 97.650000% (857ms)
  epoch 9: test_acc = 9764/10000 = 97.640000% (864ms)
  epoch 10: test_acc = 9775/10000 = 97.750000% (846ms)
  epoch 11: test_acc = 9774/10000 = 97.740000% (846ms)
  epoch 12: test_acc = 9783/10000 = 97.830000% (823ms)
done (trained MNIST-MLP via the proof-rendered StableHLO).
\end{verbatim}

Twelve epochs at about 850\,ms each, and 97.83\% on the full
...
that file is \texttt{pretty} of a proven graph node, and
\texttt{MlpFaithfulPoC} proves that each of the six outputs denotes the
certified gradient-descent step derived from the Mathlib \(\fderiv\)
math. The two hidden layers, their ReLUs, the softmax cross-entropy
cotangent, the six parameter gradients, and the SGD update are all
proof-backed operations. Every theorem in the rest of this chapter is
...
\subsection*{The earlier unverified path}

The rest of this section is the older route. It is kept because it is
what the numbers below were measured on, and because it reports
per-batch loss, which the verified trainer does not. It uses the
unverified \texttt{NetSpec} and the ablation runner rather than the
rendered graph above.
...
TensorFlow book used in its Chapter 1: SGD at learning rate 0.1, 12
epochs, no regularization tricks. In our repo this config is
...
implemented in the \texttt{LeanMlir/} directory of the repo, and it
...
entire model definition, with no class, no forward-pass function, and
no \texttt{@differentiable} attribute. The forward pass is derivable
...
TensorFlow book shipped exactly this config as its reference example,
and it makes a useful starting point because every later chapter's
tweak then has a clean baseline to measure against, both the
training-recipe ones (Adam, cosine, warmup, weight decay,
augmentation, label smoothing) and the architectural ones (BN,
residuals). The repo's
...
inside \texttt{.train} is already in the framework: data loading,
mini-batching, the rendered training step, the gradient-descent loop,
the evaluation loop, and logging. Chapter~\ref{chap:tensor}'s ``What's
inside \texttt{.train}?'' walks the verified version of that driver line
by line. The user-facing program is just ``specify the network,
...
noise) down below $10^{-3}$ by the end. Final test accuracy lands at \textbf{98.57\%}, which is the going rate
for a three-layer MLP on MNIST with plain SGD.
...
loss falls $\sim$200$\times$ from the first epoch to below $10^{-3}$, and
final test accuracy is 98.57\%.
...
\begin{itemize}
  ...
  \item \textbf{Trusted surface.} The printer's faithfulness, the
    lowerer's translation, and \texttt{float32} $\approx \mathbb{R}$ are
    tested rather than proved. Appendix~\ref{sec:verified_codegen} has the
    full accounting.
\end{itemize}
