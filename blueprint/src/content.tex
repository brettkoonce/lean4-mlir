...
\end{axis}
% ── bf16 ───────────────────────────────────────────────────────────────────
\begin{axis}[ablationaxis, title={bf16}, yticklabels={{},{},{},{},{},{},{}},
  at={(0.47\linewidth,0)}, anchor=south west]
  \path[fill,gray!16] ({axis cs:-0.92,full}|-{rel axis cs:0,0})
       rectangle ({axis cs:0.92,full}|-{rel axis cs:0,1});
  \draw[black!70]        ({axis cs:0,full}|-{rel axis cs:0,0})
                      -- ({axis cs:0,full}|-{rel axis cs:0,1});
  \addplot[only marks, mark=*, mark size=2.0pt, orange!80!black] coordinates {
    (0,full) (-0.10,nowd) (-1.15,nowarm) (-0.46,nols)
    (-2.27,noadam) (-4.94,nocos) (-7.11,noaug)};
\end{axis}
\end{tikzpicture}
\end{center}

\noindent Each point is one 80-epoch run, plotted as its \emph{difference} from
the full recipe rather than as an accuracy, so the line at zero is the baseline
and distance from it is what the ingredient contributes. The shaded band is the
95\% Wilson half-width at $n = 3{,}925$, $\pm0.92$ points: a point inside it is
not resolved by a single run, however suggestive its position. Rows are sorted
by fp32 effect.
...
move}. Every point outside the band reproduces: augmentation $-7.01$ against
$-7.11$, cosine $-4.48$ against $-4.94$, the optimizer $-2.29$ against
$-2.27$, warmup $-1.04$ against $-1.15$. The largest disagreement among them is
$0.46$ and two of the four are within $0.10$ ---
this on baselines that themselves differ by $0.28$, which is the argument for
plotting differences rather than accuracies.
...
\medskip\noindent \textbf{Augmentation is the largest single contribution} ($-7.01$ points, three
times the next-largest knob). Imagenette has 9{,}469 training images for a
