...
Box & ms/step & min/epoch & 350 epochs & Val top-1 \\
\hline
4$\times$ 4060 Ti (CUDA)  & 280 & 23.4 & $\sim$139.9 h (5.8 d) & TBD \\
4$\times$ 3060 (CUDA)     & --- & --- & --- & TBD \\
\hline
\end{tabular}
\end{center}

\noindent That row is \texttt{emarmsdp64dropdo} in bf16 --- EMA, stochastic
depth and classifier dropout --- which is the artifact
\texttt{enet-\allowbreak default-\allowbreak 4gpu} names and therefore the graph
this run would actually train. Set it against the $\sim$8.6 minutes per epoch the
phase-2 350-epoch row reports on the same four cards: both are bf16 over the same
architecture, so the ${\sim}2.7\times$ per epoch that remains is the lowerer, not
...
\hline
4$\times$ 4060 Ti (CUDA)  & 131 & 21.9 & $\sim$112.4 h (4.7 d) & TBD \\
4$\times$ 3060 (CUDA)     & 161 & 26.9 & --- & TBD \\
\hline
\end{tabular}
\end{center}
