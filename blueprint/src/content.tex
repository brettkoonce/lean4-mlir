...
hypothesis is $\varepsilon > 0$ at the LayerNorms. Unlike
Chapter~\ref{chap:depthwise}, which folds a representative
stem-plus-two-blocks because relu6's kink makes a whole-network
statement false, this one is a single global \texttt{HasVJP} over the
entire network, and it can be because softmax, GELU and LayerNorm are
all smooth. So ViT gets the strong sentence: every operation in the
...
\medskip\noindent\textbf{A note on the number, because it moved.} An
earlier edition of this chapter reported $71.70\%$ here, measured on the
retired IREE path against a 3,904-image validation subset rather than
the full 3,925. That subset is worth at most half a point, so it does
not account for the difference. Chapter~\ref{chap:cnn}'s width sweep is
where this book first had to take run-to-run spread seriously, so this
number was measured three times before being printed.

\medskip\noindent All three passes came back \emph{bit-identical}, on
three different cards: the same $68.738854\%$ and $90.420382\%$ at epoch
80, and the same 160 epoch lines the whole way down. That makes ViT the
first network since the dense-only graphs of Chapter~\ref{chap:mlp} to
reproduce exactly, and for the same reason. XLA selects convolution
algorithms per process, which is what moves the convnet chapters'
numbers by a point or two between runs. This graph holds 435
\texttt{dot\_general}s and \emph{two} convolutions, so there is almost
nothing for that selection to vary. What that buys is a sharper reading
of the gap rather than an explanation of it: the difference from the
retired figure is not run-to-run spread, because on this path there is
no spread. It is a difference between two measurement setups that
differ in lowerer, in vendor, and in the vintage of the artifact, and
this chapter does not separate those.
...
capable of, and the remaining gap is the one recipe knob that run did
not have (repeated augmentation) plus two minor deviations (strict
Mixup/CutMix alternation instead of a random switch, and Random-Erase
to zero rather than to noise). The headline is a \emph{300-epoch} DeiT
...
Box & ms/step & min/epoch & 300 epochs & Val top-1 \\
\hline
4$\times$ 4060 Ti (CUDA)  & 287 & 12.0 & $\sim$60 h (2.5 d) & TBD \\
\hline
\end{tabular}
\end{center}

\noindent At global batch 512 that is 2,502 steps per epoch, which is
the reference's own figure. \textbf{One thing differs between this row
and the phase-2 rows above, and it is not the network}: the verified
path is fp32 throughout, with no bf16 artifact anywhere in
\texttt{verified\_mlir/}, while every phase-2 row in this section was
measured in bf16. The batch does \emph{not} differ, unlike
Chapter~\ref{chap:layernorm}'s phase-4 row, where the render's smaller
per-device batch confounded the comparison a second way. So 12.0
minutes per epoch against phase 2's 7.6 is close to a clean read of what
dropping to fp32 costs on this box. At 12.0 minutes it is also the
fastest phase-4 epoch in Part 1, ahead of MobileNetV2's 15.7 and
ResNet-34's 18.6, though the 300-epoch schedule still makes it a longer
...
architecture-specific bundled layers. That list runs UNet, DETR,
...
\begin{itemize}
  ...
  \item \textbf{IREE} generates every kernel itself, which is why it runs where
    no vendor library exists, including Vulkan, Metal, bare CPU and custom
    accelerators. That is also why its runtime is 392$\times$ smaller: 1.3\,MB
    against the XLA plugin's 505\,MB, which bundles MIOpen, rocBLAS and RCCL.
\end{itemize}

\noindent\textbf{The size gap is not a trust gap.} It is tempting to read
1.3\,MB against 505\,MB as one lowerer asking you to trust less code, and the
reading does not survive contact with what the two actually do. Most of the
XLA plugin is vendor kernel libraries it \emph{calls}, where IREE
\emph{generates} those kernels instead. Both bodies of code are unverified. Choosing the small
runtime relocates the trusted base rather than shrinking it, so bytes on disk
are measuring packaging, not risk.

\medskip\noindent\textbf{You do not need IREE to train.} XLA is the default and
runs every network in this book. What IREE is needed for is
\emph{reproducing the verification}: the differential oracles of
Appendix~\ref{app:verification} run the Lean-side pipeline through IREE and
compare against a JAX reference running through XLA, and it is the fact that
those are two independent compilers that makes the comparison worth anything.
Some oracle binaries are IREE-only for that reason and will refuse to start on
the XLA backend. So a training run wants one lowerer, and a full verification
run wants both.
...
the GPU transcendentals, and \texttt{float32} rounding beyond its theorem
layer (\S~\ref{sec:float32}). Each kind catches what the others structurally
...
\begin{itemize}
  ...
  \item \emph{Trusted} (validated numerically, not proven): that the
    ...
    \emph{semantics}, which does not yet exist; that the lowerer compiles the
    text correctly; and that \texttt{float32} approximates $\mathbb{R}$, the last
...
\texttt{float32} budgets are statements about the graph rather than about any
