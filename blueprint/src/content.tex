...
single \texttt{ByteArray}, and ImageNet doesn't). Until that lands,
the phase-3 dispatcher panics with a ``use phase 2'' note when
handed \texttt{.imagenet}. What's worth noting is that everything
else is identical across both paths, including the architecture, the
\texttt{.train} invocation, and the recipe knobs. Only the
...
a 10-second val pass, and a 30-epoch run finishes in roughly 15
...
90, and the MI300X row is a single-GPU batch-1024 run on gfx942). The A100
...
because the artifact above bakes four replicas, so holding effective batch 2048
