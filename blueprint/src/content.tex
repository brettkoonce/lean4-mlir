...
\hline
4$\times$ 4060 Ti (bf16)  & RSB-A3 & Ghost-BN (512)  & $\sim$9.1 min & 100 & $\sim$17 hr & $\mathbf{77.22\%}$ & $\mathbf{93.34\%}$ \\
1$\times$ A100 (bf16)     & RSB-A3 & full (2048)     & $\sim$6.3 min & 100 & $\sim$10.5 hr & $\mathbf{77.00\%}$ & $\mathbf{93.15\%}$ \\
1$\times$ MI300X (bf16)   & RSB-A3 & full (2048)     & ---           & 100 & $\sim$7.2 hr\textsuperscript{\dag} & --- & --- \\
4$\times$ 4060 Ti (bf16)  & 2018   & global (256)    & ---           & 90  & ---          & --- & --- \\
\hline
\end{tabular}
\end{center}
...
\medskip\noindent The last row is the \textbf{2018 recipe} on the same
network, a \texttt{[TODO]} on the JAX side exactly as its verified peer in
\S\ref{sec:r50_phase4} is. It is what makes the recipe comparison symmetric:
both paths get both recipes, so a difference between two rows is attributable to
the one thing that changed between them.
...
\hline
4$\times$ 4060 Ti (fp32)  & RSB-A3 (100 ep) & Ghost-BN (64) & $\sim$20.6 min & $\sim$34.1 hr & $\mathbf{77.43\%}$ & $\mathbf{93.60\%}$ \\
4$\times$ 4060 Ti (fp32)  & 2018 (90 ep)    & Ghost-BN (64) & ---            & ---           & --- & --- \\
\hline
\end{tabular}
\end{center}

\noindent The second row is the same network on the \emph{2018} recipe, a
\texttt{[TODO]}, and it is the row that makes the comparison below a controlled
