...
\textbf{Part of it is the resize, and the part is now measured.} Both
rows here are scored through this repo's \texttt{tf.image.resize}
preprocessing, whereas the timm reference reports $78.1\%$ through PIL's
bicubic resize, which antialiases where ours does not. Scoring the
$77.22\%$ checkpoint through the full PIL protocol instead gives
$\mathbf{76.25\%}$: the resampler is worth about \textbf{nine tenths of
a point} on this network. Two things follow, and the second is the more
useful one.

First, this is a \emph{mismatch} penalty rather than a verdict on either
resampler. Each network partly fits the aliasing of the one it trained
under, so moving only the evaluation side is expected to lose accuracy;
the intermediate arm, antialiasing the evaluation crop alone, sits
between the two at $76.70\%$. The matched-antialiased arm --- both sides
moved, and retrained --- has not been run, so whether timm's convention
is \emph{better} or merely \emph{different} is genuinely open.

Second, ResNet-50 is about $3.6\times$ more sensitive to this than the
other networks in the book, where the same three-way comparison spans
$0.25$ points (\S\ref{chap:layernorm}, \S\ref{chap:attention}). The plausible
mechanism is the recipe itself: A3's centre-crop ratio of $0.95$ keeps
$8\%$ more of the frame than the $0.875$ used elsewhere, so more of each
image survives to be resampled, on a recipe whose whole trick is
test-time object scale.

\noindent So the honest reading is that the two sit within about a point
of each other, and that roughly nine tenths of that point is the
evaluation pipeline rather than the recipe --- but not that the ranking
inverts, which is a stronger claim and one nothing here has tested.

\textbf{[TODO, the reverse direction.]} What remains unmeasured is
carrying the reference \emph{into} this pipeline: port the
\texttt{a3\_in1k} weights to the layout of
\texttt{init\_params\_from\_file} and score them both ways. If they lose
about the same nine tenths going the other direction, the two
conventions are equivalent and matching timm buys comparability rather
than accuracy; if they lose much less, its pipeline is genuinely the
better one and the matched-antialiased retrain is worth its cost. The
control that makes the answer believable is scoring the ported weights
through timm's own \texttt{validate.py} first and reproducing its
published number --- without it, ``they fall to $\sim$74\%'' and ``the
port is wrong'' look identical.
...
\noindent One axis is deliberately \emph{not} matched, and it is
recorded here rather than left for a reader to discover.
\texttt{timm} resamples through PIL, which antialiases when
downscaling; we resample through \texttt{tf.image.resize} with
\texttt{antialias=False}, on both the training crop and the evaluation
crop. That is self-consistent --- a network is trained and scored under
one resampler --- but it is not \texttt{timm}'s. Measured on the RSB-A3
ResNet-50 of \S\ref{sec:r50_phase4}, scoring the same weights through
the full PIL protocol instead of ours costs $0.90$ points; on
ConvNeXt-T and ViT-Ti it costs $0.25$. \emph{That figure is a mismatch
penalty and not a verdict on either resampler:} the model has partly
fitted the aliasing of the one it trained under, so changing only the
evaluation side is expected to lose accuracy, and a matched-antialiased
arm --- both sides moved, and retrained --- has not been run, so its
sign is unknown. Until it has, quoting our numbers against
\texttt{timm}'s published ones carries this term, and the honest
statement is that the two sit within about a point of each other on a
ruler that is shared in every respect but the resampler.
