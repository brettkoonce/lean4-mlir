...
\medskip\noindent\textbf{[TODO: prove the fold at full depth for this
network.]} Chapter~\ref{chap:se} does have it, so the gap is worth
closing and the shape of the answer is known:
\texttt{efficientnetForwardB\_full\_has\_vjp} chains a stem, all sixteen
MBConv blocks and a head through \texttt{vjp\_comp}. \emph{But that proof
does not transfer by copying}, and the reason is this network's
activation. That theorem is a \emph{global} statement, which it
can be because swish is smooth everywhere. MobileNetV2 uses relu6, which
has a kink, and a global VJP through a kink is false. So the full-depth
version here has to keep the pointwise \texttt{\_at} form that
\texttt{mobilenetv2\_has\_vjp\_at} already has. The axis to move is the
depth, from two blocks to seventeen, and not the pointwise-to-global one.
\texttt{MobileNetV2FullPaper.lean} already carries the four block shapes
and the full seventeen-block forward chain. What it lacks are the
per-block differentiability and VJP lemmas that EfficientNet's eight
\texttt{mb*W\_has\_vjp} / \texttt{mb*W\_differentiable} peers supply.
...
\medskip\noindent\textbf{And here this chapter can say something
Chapter~\ref{chap:depthwise} could not.} There, the theorem folding the
per-operation rules into a single statement about the whole network was
proved for a representative stem-plus-two-blocks, not for all seventeen.
EfficientNet's fold is proved at full depth.
...
batched over \(N\), with a positive-\(\varepsilon\) hypothesis for each
BatchNorm and nothing else assumed. So for this network the accurate
sentence is the strong one: every operation in the graph that just
trained carries a proved backward, and their composition into the whole
network carries one too.

What buys that is the activation. Swish is smooth everywhere, so the
statement can be global. MobileNetV2's relu6 has a kink, which is why
Chapter~\ref{chap:depthwise}'s fold is pointwise and why closing it there
is a different job rather than the same one at a different size.
...
dataset, same recipe, same eighty epochs, and now all three measured on
the verified XLA path:
...
\noindent \textbf{Provenance.} All three rows are verified-XLA
measurements on one RTX 4060 Ti, from \S\ref{sec:r34_runit},
\S\ref{sec:mnv2_runit} and \S\ref{sec:enet_runit}. The render sizes are
much larger than the phase-3 numbers this table used to carry, because
they are now \texttt{pretty(provenGraph)} rather than the older
hand-written emitter's output. Step time is the epoch wall-clock divided
by the 295 batches in it, so it includes each epoch's validation pass.
...
Box & ms/step & min/epoch & 350 epochs & Val top-1 \\
\hline
4$\times$ 4060 Ti (CUDA)  & 371 & 30.9 & $\sim$180.5 h (7.5 d) & TBD \\
\hline
\end{tabular}
\end{center}

\noindent Set that against the $\sim$8.6 minutes per epoch the phase-2
350-epoch row reports on the same four cards, and the gap is not a
property of either network. The verified path is fp32 throughout, with no
bf16 artifact anywhere in \texttt{verified\_mlir/}, while every phase-2
row in this section was measured in bf16 with the MBConv convolutions
cast. The two columns measure the same architecture at different
precisions, so the ratio between them is the cost of the missing cast and
nothing else.
...
and 1{,}316\,KB (EfficientNet-B0), and the lowerer compiles all of
them the same way.
