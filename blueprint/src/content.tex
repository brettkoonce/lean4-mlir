...
% ── fp32 ───────────────────────────────────────────────────────────────────
\begin{axis}[ablationaxis, title={fp32},
  yticklabels={full,$-$weight decay,$-$warmup,$-$label smoothing,
               $-$AdamW,$-$cosine,$-$augmentation}]
  \path[fill,gray!16] ({axis cs:-0.92,full}|-{rel axis cs:0,0})
       rectangle ({axis cs:0.92,full}|-{rel axis cs:0,1});
  \draw[black!70]        ({axis cs:0,full}|-{rel axis cs:0,0})
                      -- ({axis cs:0,full}|-{rel axis cs:0,1});
  \addplot[only marks, mark=*, mark size=2.0pt, blue!70!black] coordinates {
    (0,full) (-0.64,nowd) (-1.04,nowarm) (-1.50,nols)
    (-2.29,noadam) (-4.48,nocos) (-7.01,noaug)};
\end{axis}
% ── bf16 ───────────────────────────────────────────────────────────────────
...
from it is what the ingredient contributes. The two panels do not carry the same
evidence. On the left each point is \emph{one} 80-epoch run, and the shaded band
is the 95\% Wilson half-width at $n = 3{,}925$, $\pm0.92$ points: a point inside
it is not resolved by a single run, however suggestive its position. On the
right each arm was trained five times from five seeds; the point is the mean
difference from the full recipe and the bar is the 95\% interval on that
difference, paired by seed (arms sharing a seed share their initialization and
data order). The full recipe itself lands at $89.98 \pm 0.15$ over its five
seeds. Rows are sorted by fp32 effect.
...
move}. The four effects the single runs resolve reproduce inside the seeded
bars: augmentation $-7.01$ against $-7.11 \pm 0.38$, the optimizer $-2.29$
against $-2.21 \pm 0.33$, warmup $-1.04$ against $-0.82 \pm 0.62$, and cosine
$-4.48$ against $-4.78 \pm 2.74$. The two rows inside the left panel's band,
weight decay and label smoothing, are exactly the two whose fp32 run falls
outside the bf16 interval ($-0.64$ against $+0.14 \pm 0.45$, $-1.50$ against
$-0.50 \pm 0.62$) --- which says only that one run does not place them, not
which side they fall on. The cosine bar is the widest by far because the
constant-rate arm finishes wherever its noisy trajectory happens to be at
epoch~80: five seeds spread $82.6$--$87.9$ (sd $2.18$), so its single-run
figure was one draw from a wide distribution and its contribution is ``large
and imprecise'', not $-4.5$.
...
was given a rate that suits it. Heavy-ball at $\mu = 0.9$ multiplies the
effective step by $1/(1-\mu) = 10$, so the comparison that isolates momentum is
against plain SGD at ten times the rate --- $0.1$, not $0.01$ --- where the two
take the same size step and differ only in whether a velocity carries it.
...
\begin{axis}[
  ...
  tick label style={font=\small}, label style={font=\small},
  title style={font=\small}, title={endpoints},
  grid=major, grid style={gray!14}]
  \path[fill,gray!16] ({axis cs:-0.92,full}|-{rel axis cs:0,0})
      rectangle ({axis cs:0.92,full}|-{rel axis cs:0,1});
  \draw[black!70] ({axis cs:0,full}|-{rel axis cs:0,0})
  ...
  \addplot[only marks,mark=*,mark size=2.2pt,blue!70!black]
    coordinates {(0,full) (-2.29,mom) (-2.60,sgd10)};
\end{axis}
...
\begin{axis}[
  ...
  tick label style={font=\small}, label style={font=\small},
  title style={font=\small}, title={trajectories},
...
\noindent At a matched step, plain SGD lands $0.31$ points from momentum ---
inside the band, indistinguishable on one run each --- and the trajectories are
the stronger statement: the two are inseparable for the whole eighty epochs, not
merely at the end. Momentum is not doing something a velocity-free step cannot;
it is taking a bigger one. So the $-2.29$ charged to ``removing AdamW'' is
mostly the $10\times$ that momentum supplies for free, and only a fraction of it
survives once that is held fixed.

\medskip\noindent That is this ablation's own limit, stated plainly. Every
non-adaptive row's difference is partly a claim about how well its learning rate
happens to suit it, and an adaptive optimizer's insensitivity to that rate is
exactly what hides the dependence. The figure above measures the recipe
\emph{as tuned}, not its ingredients in isolation.

\medskip\noindent \textbf{Augmentation is the largest single contribution} ($-7.01$ points on
the fp32 run, $-7.11 \pm 0.38$ over five bf16 seeds; three times the
next-largest knob). Imagenette has 9{,}469 training images for a
21.3M-parameter network, so without the random crop and flip the model overfits
hard --- training loss falls while validation stalls. Most of what the rest of
the recipe gets credit for is augmentation.

\textbf{[TODO: more fp32 samples.]}
...
\hline
4$\times$ 4060 Ti (CUDA)  & $166 \to 140$ & $13.8 \to 11.7$ & $\sim$84 $\to$ $\sim$72 h \;($3.5 \to 3.0$ d) & TBD \\
4$\times$ 3060 (CUDA)     & ---           & ---             & ---                                          & TBD \\
\hline
\end{tabular}
\end{center}

\noindent The columns are the two arms of \texttt{rmsdp64} --- RMSProp with exponential
decay, the recipe \texttt{mnv2-\allowbreak default-\allowbreak 4gpu} names. fp32 is the
default; the run will be bf16, so read the right-hand column. Exactly half of that
$140$\,ms step is no longer the graph, which times as $69.6$\,ms on its own, so this net
is now as much data pipeline as compute --- and that is also why the fp32 arm is the
least repeatable number in this book, spanning
$159$--$196$\,ms across three consecutive runs on an idle box where the bf16 arm held
$140 \pm 1$. A step that is half producer inherits the producer's variance.
