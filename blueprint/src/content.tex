...
the \emph{whole} network, \texttt{mobilenetv2\_has\_vjp\_at}, is proved
for a representative net of the stem plus two inverted-residual blocks
plus the head, not for all seventeen. What ties the full 17-block spec to
the paper's architecture is a separate denotational result about the
forward, \texttt{mobilenetv2Verified\_denote\_eq}, with
\texttt{mobilenetv2Verified\_fwd\_faithful} above it. So the accurate
sentence is that every operation in the graph that just trained carries a
proved backward, and the fold of all of them is witnessed at
representative scale. That is the same ``representative scale'' caveat
the MLIR sections of this book carry throughout, stated here where the
network is first deep enough for the gap to be worth naming.
...
\texttt{mobilenetv2\_has\_vjp\_at} chains the whole network through
\texttt{vjp\_comp\_at}.
...
The curve says where the budget went. Two thirds of the final accuracy
is in place by epoch $50$, the run is still climbing at $150$, and it is
flat from about epoch $250$ onward, so the last $100$ epochs are worth
almost nothing and the schedule is genuinely exhausted rather than cut
short. Whatever the remaining gap to the paper is, it is not length.

That residual $0.56$ is the honest part of the result. It is small
enough to sit inside the known recipe deviations rather than needing a
new explanation. Two of those are documented and shared across every net
in this book. The resize is not antialiased on either the train or eval
path, where timm's is, worth $\sim0.2$ points. The running-BN decay is
fixed at $0.99$ where PyTorch's default works out to $0.9$. Neither is
worth a retrain of the sweep to chase.
...
\hline
4$\times$ 4060 Ti (CUDA)  & 188 & 15.7 & $\sim$91.5 h (3.8 d) & TBD \\
...
$350$ and $16$ wall-clock hours against $45$. It also clears
...
writing, and the first thing to say about it is what it is \emph{not}.

\textbf{It is Conv-S, and the numbers above are Conv-M.} The block table
this book renders and proves against is
MobileNetV4-Conv-Small, 4.1M parameters, the one
\texttt{mobilenetv4Verified} trains on Imagenette. Conv-M is a different
network at $\sim$9.7M, and it lives in \texttt{jax/} with no verified
peer. The ImageNet spec is therefore Conv-S with a 1000-class head,
built the way \S\ref{sec:mnv2_phase4} built MobileNetV2's and the way
Chapter~\ref{chap:residual} built ResNet-50's: the trunk is copied
unchanged from the Imagenette spec, a \texttt{\#guard} pins every
parameter shape but the head against it, and the head moves
$1280{\times}10 \to 1280{\times}1000$, taking the count from
$4{,}124{,}426$ to $\mathbf{5{,}392{,}616}$. \textbf{So the $75.48\%$
above is not this network's target and must not be read as one.} Conv-S
has no published ImageNet number here at all, which is why the row below
has no gap-to-reference column.
...
built as \texttt{lake build mnv4-imagenet-verified}. The renders come
...
\begin{tabular}{l r r r r}
\hline
Box & ms/step & min/epoch & 100 epochs & Val top-1 \\
\hline
4$\times$ 4060 Ti (CUDA), global 256 & $\sim$147\,* & $\sim$12.3\,* & $\sim$20 h (0.9 d)\,* & TBD \\
\hline
\end{tabular}
\end{center}

\noindent {\small *\,Estimated, not measured: scaled from this network's
measured single-card throughput by MobileNetV2's four-card efficiency,
since no data-parallel variant is rendered for MobileNetV4 yet.}

\medskip\noindent\textbf{[TODO: run \texttt{mnv4-imagenet-verified}.]}
...
\noindent \textbf{Provenance.} Every row above is a phase-3 measurement,
taken on IREE before the XLA port and kept together so the comparison
stays like-for-like. Two of the rows have since been re-measured on the
verified XLA path and moved: ResNet-34 in \S\ref{sec:r34_runit} and
MobileNet V2 in \S\ref{sec:mnv2_runit}, where the render sizes in
particular are much larger, because they are now
\texttt{pretty(provenGraph)} rather than the older hand-written
emitter's output. The remaining rows move when their own chapters are
re-measured.
...
\noindent As in Chapter~\ref{chap:se}, every row is a phase-3 IREE
measurement. The ResNet-34 and MobileNet V2 rows have been re-measured
on the verified XLA path in \S\ref{sec:r34_runit} and
\S\ref{sec:mnv2_runit}.
