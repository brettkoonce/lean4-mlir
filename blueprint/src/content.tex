...
\hline
4$\times$ 4060 Ti (CUDA)  & $188 \to 107$ & $15.7 \to 8.9$  & $\sim$95 $\to$ $\sim$56 h \;($4.0 \to 2.3$ d)  & TBD \\
4$\times$ 3060 (CUDA)     & ---           & ---             & ---                                           & TBD \\
\hline
\end{tabular}
\end{center}

\noindent The columns are the two arms of \texttt{emarmsdp64dropdo} --- EMA,
stochastic depth and classifier dropout --- the recipe
\texttt{enet-\allowbreak default-\allowbreak 4gpu} names. fp32 is the default; the run
will be bf16, so read the right-hand column. Set the 3060 row against the $8.0$
minutes per epoch phase 2 measures on that same box: both are bf16 over the same
architecture, so $8.0$ is the figure phase 4 has to reach there.
