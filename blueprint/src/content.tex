...
\textbf{Compute budget, for honest comparison.} The original paper
trained on 8 GPUs for $\sim$5 days. On the single 7900 XTX
(gfx1100) used for the Imagenette runs above, a straight port runs
into the memory ceiling at batch 256 --- ResNet-34's forward
activations plus gradients plus Adam moments exceed 24~GB --- so
you'd drop to batch 64 and 4$\times$ more steps per epoch. Call it
a week of wall-clock, and that's before allowing for the eval pass
on the 50K-image val set every epoch. The practical move for
anyone wanting ImageNet-scale numbers from this codebase is to
rent cluster time; reproducing the paper on a single GPU is
possible but not how the result was originally produced.
