...
\subsection*{What A2 and A1 would cost}
\label{sec:r50_a2_a1_cost}

A3 is the cheap tier. The other two exist as recipes --- \texttt{a2-accum}
and \texttt{a1}, selected positionally (\texttt{resnet50-imagenet a2-accum
data}) --- smoke-tested rather than trained. What stops them is the budget:

\begin{center}
\begin{tabular}{@{}l c c l l@{}}
\hline
tier & epochs & train res & verified wall clock & top-1 \\
\hline
A3 (\texttt{rsb-faithful}) & 100 & $160$ & $\sim$32.1 hr & $\mathbf{77.91\%}$ \\
A2 (\texttt{a2-accum})     & 300 & $224$ & \texttt{[todo: bf16 renderer + benchmarks]} & --- \\
A1 (\texttt{a1})           & 600 & $224$ & \texttt{[todo: bf16 renderer + benchmarks]} & --- \\
\hline
\end{tabular}
\end{center}

\noindent Four 4060\,Ti, effective batch 2048 as $512\times4$. Only the A3
row has been run on the verified path --- it is \S\ref{sec:r50_phase4}'s
fp32 result. A2 and A1 render and step but have no benchmark, and the reason
is the same one \S\ref{sec:convnext_sb} and \S\ref{sec:vit_sb} hit: the only
bf16 R50 render is \texttt{resnet50in\_momdp64bf16}, the 2018 recipe's, so the
LAMB\,+\,BCE tiers have no bf16 twin to time. What is known without running
them: accumulation is per-epoch free and holds peak at the bs512 figure, so
both tiers fit a 12\,GB card where the single-forward \texttt{a2-true-2048}
would want $\sim$160\,GB, and running A2 at bs512 with a scaled LR instead is not a cheap
A2 but the starved-LAMB regime that gave A3 $40.8\%$ in place of $78.1\%$. The
papers quote $79.8\%$ and $80.4\%$.
...
Two changes, one to the block and one to the schedule.
...
traffic dominates. That is the same trade Chapter~\ref{chap:depthwise}
made in the other direction, and it is worth noticing that the answer
flips with the feature-map size rather than being a property of the
operator. \emph{Progressive learning} then raises the image size across
training and raises the regularization with it, on the argument that a
fixed augmentation strength is wrong for both ends of a resolution ramp;
the pair trains up to $4\times$ faster than V1 at $6.8\times$ fewer
parameters, with V2-S at $83.9\%$ top-1. The fused block is already in
...
Scaling ConvNeXt-T is two moves. \textbf{S} deepens stage 3 from 9 blocks to
27 and touches nothing else. \textbf{B} does that \emph{and} widens every
stage, $96/192/384/768 \to 128/256/512/1024$. On the JAX side both are pure
parameter edits. On the verified side they are not the same job at all, and
the difference is the useful finding:

\begin{center}
\begin{tabular}{@{}l c c l@{}}
\hline
model & params & renderer work & verified cost \\
\hline
ConvNeXt-T & $28.6$\,M         & ---                     & $167$\,ms/step (fp32) \\
ConvNeXt-S & $50{,}222{,}152$  & \textbf{none}           & $280$\,ms/step (fp32) \\
ConvNeXt-B & $88{,}589{,}416$  & ${\sim}27$ literals     & \texttt{[todo: bf16 renderer + benchmarks]} \\
\hline
\end{tabular}
\end{center}

\noindent Both param counts are \texttt{\#guard}ed against the reference's
$50.22$\,M and $88.59$\,M. The two step figures are the verified renders
measured on one 4060\,Ti, fp32, batch 32, \texttt{adamwxclipdrop}, with T run
as the control in the same session; B has no benchmark yet.

\medskip\noindent S costs \emph{nothing} in the renderer because depth is
already a loop --- 27 blocks is the same code as 9. B costs about twenty-seven
threaded dimension literals across two renderers, because those hardcode
widths where the JAX emitter derives them. So the work is proportional to
\emph{how many dimensions move}, not to how many parameters: B has $1.8\times$
S's parameters and nearly all of the cost, while S has $1.8\times$ T's and
none of it. That is worth stating plainly because it is the boundary of the
``one spec, any size'' claim this chapter makes everywhere else --- the spec
really is free, and the renderer is where a new shape gets paid for.

\medskip\noindent \textbf{The bf16 renders are half-done.}
\texttt{convnextbin} carries its bf16 twins; \texttt{convnextsin} does not, an
accident of B landing after S rather than anything about the shape. Closing it
is a per-net emit and not new operator work --- every bf16 twin these nets need
already exists --- and T's bf16 render runs $1.29\times$ its fp32 peer, so it is
worth about a fifth of the wall clock. S costs $1.68\times$ T per step on the
verified path, which is the deepening and nothing else.

\medskip\noindent \textbf{Status: rendered, not trained.} Artifacts emit,
shapes tie to \texttt{VLayer.toSpecs}, param counts are \texttt{\#guard}ed
and a few steps run under \texttt{convnexts-imagenet-verified} and
\texttt{convnextb-imagenet-verified}. No accuracy, and the numeric ties
against the JAX peers have not been run. On four cards B needs gradient
accumulation even in bf16.
...
\begin{tabular}{@{}l c c l@{}}
\hline
model & $D$ / heads & params & verified cost \\
\hline
ViT-Ti & $192$ / 3  & $5.7$\,M         & \S\ref{sec:vit_imagenet} \\
ViT-S  & $384$ / 6  & $22{,}050{,}664$ & \texttt{[todo: bf16 renderer + benchmarks]} \\
ViT-B  & $768$ / 12 & $86{,}567{,}656$ & \texttt{[todo: bf16 renderer + benchmarks]} \\
\hline
\end{tabular}
\end{center}

\noindent The param counts \texttt{\#guard} against DeiT-S's $22.05$\,M and
DeiT-B's $86.57$\,M. Neither variant has been benchmarked on the verified path,
and \texttt{vitbin} renders at global batch 128 only --- the one place in
Part~I where a model's size, not its recipe, dictates how it must be run.

\medskip\noindent Where \S\ref{sec:convnext_sb} found a split between free and
costly, ViT is uniformly the costly kind: every variant moves $D$, so every
variant moves dimension literals. The batched renderer is parameterised and
carries all three; \texttt{ViTRender.lean}, the per-example peer, is still
pinned at Tiny by roughly $154$ of them, which is why there is no ViT-S or
ViT-B Imagenette demo to sit beside \S\ref{sec:vit_runit}'s.

\medskip\noindent \textbf{Neither has a bf16 render yet.} ViT-Tiny's bf16
twin runs $1.46\times$ its fp32 peer --- the largest bf16 payoff of any net in
Part~I, and most of it from one line, the dots' result type
(\S\ref{sec:precision}) --- and neither S nor B has one. So a third of the
verified wall clock for both is sitting behind a per-net emit that needs no
new proved operator, and until it lands there is nothing honest to put in the
cost column. Of the four variants in this chapter and
\S\ref{sec:convnext_sb}, only ConvNeXt-B is ported.

\medskip\noindent \textbf{Status: rendered, not trained}, the same as ConvNeXt's
pair --- \texttt{vits-imagenet-verified} and \texttt{vitb-imagenet-verified}
emit, tie and step. The only accuracy calibration that exists is this repo's
ViT-Ti at $70.28\%$ against DeiT-Ti's $72.2\%$, a $1.9$-point recipe gap
(\S\ref{sec:vit_imagenet}). DeiT quotes $79.8\%$ for S and $81.8\%$ for B; every
item missing from that $1.9$ is \emph{regularization} and S has four times
Tiny's capacity, so the gap plausibly widens rather than holds.
...
which runs the same binaries with the IREE lowerer selected ---
\texttt{imagenette-iree} covers the five nets with recorded IREE timings, not
all seven. The tiers bundle these targets, and any one builds on its own:
...
The scale tier. Seven nets train on full ImageNet-1k off the same certified
renderer as their Imagenette counterparts, in the chapter order
\S\ref{sec:getting_started_track3} uses:

\begin{center}
\begin{tabular}{@{}ll@{}}
\textbf{Target} & \textbf{Job config} \\[2pt]
\hline \\[-6pt]
\texttt{resnet34-imagenet-verified} (2018) & \texttt{r34-imagenet-4gpu} \\
\texttt{resnet50-imagenet-verified}     & \texttt{r50-imagenet-4gpu} (a3) \\
\texttt{resnet50-imagenet-verified}     & \texttt{r50-2018-4gpu} \\
\texttt{mobilenetv2-imagenet-verified}  & \texttt{mnv2-imagenet-4gpu} \\
\texttt{mnv4-imagenet-verified}         & \withheld \\
