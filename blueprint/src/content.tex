...
same proven-backward bottleneck codegen it reached $\mathbf{76.66\%}$
top-1 / $\mathbf{93.03\%}$ top-5 at 100 epochs --- within $1.4$ points
of the paper's $78.1\%$.

\textbf{[TODO --- run w/ config.]} \texttt{resnet50-imagenet
rsb-faithful}, \textbf{100 epochs} (the run above), or
\texttt{resnet50-imagenet true-2048} for a real single-forward batch
2048 on an $\sim$80\,GB card (no gradient accumulation). \emph{Changed
since the $76.66\%$ above:} RandAugment is now timm-literal
(\texttt{rand-m6-mstd0.5-inc1}). \emph{Still open:} the residual
$\sim$1.4 points is the gradient-accumulation Ghost-BN regime (each
micro-step normalizes over 512, not the full 2048) plus LAMB
implementation details and bf16 --- the \texttt{true-2048} recipe
isolates the BN piece.
