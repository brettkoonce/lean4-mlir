...
The entire training program in Lean is about twenty-five lines. Put
it in a file called \texttt{MainMlpTrainF32.lean} and wire it up in
\texttt{lakefile.lean}:
...
\texttt{train} method. Everything downstream in this chapter and the
next is a specialization of this framework.
...
\textbf{3. The training hyperparameters.} Learning rate, batch size,
epochs, optimizer choice (Adam instead of plain SGD), weight decay,
learning-rate schedule (cosine decay with a one-epoch linear
warmup). These are the standard knobs from modern ML. No surprises;
the values are conventional for MLP-scale MNIST.
...
loop, logging --- is already formalized in the framework. The user-
facing program is just "specify the network, specify the knobs,
run."

That's the whole thing. The parts people usually have to write by
hand every time (the training loop, the evaluation loop, the
gradient-computation machinery) are hidden inside the framework
because \emph{they were the same every time}. The correctness of
what's inside \texttt{.train} is what the proof chapters prove.

\subsection*{Results}

Building and running the trainer is two commands from the repo root:

\begin{verbatim}
$ lake build mnist-mlp-train-f32
Building LeanMlir.Train
Building MainMlpTrainF32
Linking mnist-mlp-train-f32
$ IREE_BACKEND=rocm IREE_CHIP=gfx1100 \
    ./.lake/build/bin/mnist-mlp-train-f32 data/
...
Compiling vmfbs...
  ...
  train step compiled
[epoch  1/12] train_loss=0.4523  val_acc=0.9522
[epoch  2/12] train_loss=0.1691  val_acc=0.9694
[epoch  3/12] train_loss=0.1135  val_acc=0.9743
...
[epoch 12/12] train_loss=0.0142  val_acc=0.9791
\end{verbatim}

On an AMD 7900 XTX (ROCm), 12 epochs take about 90 seconds. On a
laptop CPU via the Docker image in Appendix~\ref{app:getting_started}
it's closer to 5 minutes. Either way the final test accuracy lands
around 97.9\%.
