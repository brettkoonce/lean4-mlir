...
equals the true Jacobian, would not be provable.
...
Section~\ref{sec:mlp_inside_train} made a strong claim about the inner call:
...
graph construction. This is where we cash it. The ``MLIR: Linear'' section
...
\medskip\noindent\textbf{Where this stands.} The scheme above now covers more
...
\begin{itemize}
  ...
  \item \textbf{Chapter 3: MNIST: 1D MLP} --- ReLU and hidden layers on
    top of Chapter 2's dense layer: the first \emph{multi-layer}
    backward pass.
...
the cotangent). Across Chapters 2--10 there are \textbf{40 contracts}
...
Chapter~2's toolkit is used straight away by a proven model --- the
...
\textbf{Chapter 3 (MNIST: 1D)} adds depth and nonlinearity on top of
Chapter~2's dense layer --- \textbf{2 VJP contracts} plus the chapter's
...
\textbf{Chapter 4 (MNIST: 2D)} contributes 3 VJP contracts (plus four
...
\textbf{Chapter 5 (CIFAR: BN)} contributes 4 VJP contracts (plus a concrete witness): the
...
\textbf{Chapter 6 (ResNet-34)} contributes 9 VJP contracts (plus a concrete witness) and
...
built from Chapter~2's kit.

\textbf{Chapter 7 (MobileNetV2)} contributes 3 VJP contracts (plus a concrete witness): the
depthwise input VJP at stride 1 (same recipe as Ch~4 with one fewer
...
\textbf{Chapter 8 (EfficientNet)} contributes 5 VJP contracts:
...
\textbf{Chapter 9 (ConvNeXt)} contributes 7 VJP contracts: GELU
...
Chapter~8, where EfficientNet uses it; ConvNeXt's activation is GELU.)

\textbf{Chapter 10 (ViT)} contributes 6 VJP contracts (MHSA, the
...
the softmax cross-entropy gradient now lives in Chapter~2 with the
linear classifier), resting on the
largest \emph{supporting}-theorem budget in the book and the only
chapter that needs the matrix-level extensions of Chapter~2's
...
\hline
2 (Foundation)   & 1 & --- & \texttt{mnistLinear}: the whole classifier \\
3 (MNIST: 1D)    & 2 & 1   & ReLU, MLP \\
4 (MNIST: 2D)    & 3 & 4   & conv2d, maxPool2, whole-net MNIST CNN \\
5 (CIFAR: BN)    & 4 & 1   & per-channel BN (flat $+$ NCHW), CIFAR whole-nets $\pm$BN \\
6 (ResNet-34)    & 9 & 1   & residual ($+$proj), GAP, stride-2 conv, block/stage, whole-net \texttt{cnn} \\
7 (MobileNetV2)  & 3 & 1   & depthwise (stride 1$+$2), whole-net \texttt{mobilenetv2} \\
8 (EfficientNet) & 5 & --- & swish, SE, whole-net $\times$2, \textbf{full B0} (16 MBConv blocks) \\
9 (ConvNeXt)     & 7 & --- & GELU, LayerNorm, layer-scale, whole-net $\times$2, \textbf{full ConvNeXt-T} \\
10 (ViT)         & 6 & --- & MHSA, block, depth-2 ($\times$2 LN forms), \textbf{every depth $k$}, \texttt{vit\_full} \\
\hline
...
that BN's lead all but vanishes. The machine-checked VJP is what makes
the curve trustworthy: every one of those faster steps computes the
right gradient.
...
\begin{itemize}
  \item Ch 1--2: framework setup (mandatory context).
  \item Ch 3--4: MLP and CNN worked examples --- the simplest cases
...
\textbf{[TODO: paper-length run.]} The remaining $\sim$3 points to the
...
hundred epochs, where the 90-epoch cosine here is a deliberately short
...
\textbf{[TODO: paper-faithful B0.]} The faithful headline run is
therefore RMSProp at peak $\sim$0.01 $+$ AutoAugment $+$ stochastic
depth over $\sim$350 epochs --- AutoAugment, like the schedule,
under-trains at 80 epochs and only pays off long. Until then the SGD
80-epoch $72.31\%$ above stands as the sweep's B0 number; the
RMSProp$+$AA$+$350-epoch pass ($\sim$40\,hr on the six-GPU box) is
future work.
...
architecture confirmation with Ch 10's ViT-Tiny ablation (CutMix
...
epoch. One canonical deviation remains, deliberate so the port reuses
the validated layer types: the patchify stem is \texttt{.convBn}
(BN $+$ a ReLU) rather than conv $+$ LayerNorm --- param-equivalent,
the ReLU a minor variant. The augmentation pack above --- Mixup, CutMix,
RandAugment, Random Erasing --- is wired into the phase-2 Lean$\to$JAX
trainer (each knob gated on a config flag, RandAugment now a full
color+geometric variant \textbf{[TODO: run w/ geo ra]}) but left off for
this 80-epoch run, which lands under the paper's $82.1\%$; the 80 here is
the validation tier of an 80$\to$300 ladder.
...
gap to ConvNeXt-T's $\sim$82\% headline is geometric RandAugment
\textbf{[TODO: run w/ geo ra]} (now wired) and
the 300-epoch schedule. The validation curve --- note the slow start,
...
\textbf{[TODO: paper-faithful ConvNeXt-T.]} Mixup and CutMix are
long-schedule regularizers --- at 80 epochs they under-train, so the
full pack lands at best flat against the $75.93\%$ RandAugment-only run
above. The faithful headline run is the full pack over 300 epochs
($\sim$63\,hr on the six-GPU box, the projected row in the table),
where the heavier augmentation finally pays off toward the
$\sim$82\,\% paper number. Future work.
...
\noindent (The fp32 row is throughput-only --- steady-state
ms/step measured over a few hundred steps, projected to 80 epochs;
only the 80-epoch bf16 configuration was trained to completion. The
300-epoch row is a linear projection of that wall-clock, not yet
run. bf16 does not cost final accuracy here.)

\noindent\textbf{[TODO: 300 epoch run]}

\noindent\textbf{[TODO: geo ra (wired, needs to be hooked up),
stochastic depth (needs to be scoped), repeated augmentation (needs
to be scoped)]}

The 80-epoch run reached $\mathbf{65.64\%}$ top-1 /
$\mathbf{87.06\%}$ top-5 on the full 50{,}000-image validation
split. That is below the ${\sim}72\%$ ViT-Tiny is capable of --- but
that headline is a \emph{300-epoch} DeiT result with the full
distillation-free recipe; at 80 epochs, ${\sim}66\%$ is the
expected band, and the point here is the pipeline (proven attention
backward $\to$ DeiT recipe $\to$ stable bf16 training to a sensible
ImageNet number), not a SOTA chase. The validation curve shows the
familiar shape --- fast climb through warmup, a long middle, a
...
\noindent ViT-Tiny / ImageNet-1k validation accuracy per epoch
(bf16, 4$\times$ 4060 Ti; sampled across the run, which was resumed
from per-epoch checkpoints five times when the host's PCIe links
threw transient errors under load --- the checkpoint/resume harness
absorbed each without a lost epoch).
...
\node[hdr] at (5.6,0.25)  {PART 1 --- Verified spine\\\footnotesize MLP $\to$ ViT (ch 2--10)};
\node[hdr] at (12.35,0.25) {PART 2 --- Bestiary primitives\\\footnotesize ch 11 (bundled blocks)};
...
\fill[dotg] (3.2,-12.85) circle (0.082); \draw[black!45,line width=0.25pt] (3.2,-12.85) circle (0.082);
\node[font=\sffamily\scriptsize, anchor=west] at (3.37,-12.85) {ResNet-34 (ch 2--6)};
\fill[doty] (6.5,-12.85) circle (0.082); \draw[black!45,line width=0.25pt] (6.5,-12.85) circle (0.082);
\node[font=\sffamily\scriptsize, anchor=west] at (6.67,-12.85) {MobileNetV2 $\to$ EfficientNet (ch 7--8)};
\fill[dotr] (11.95,-12.85) circle (0.082); \draw[black!45,line width=0.25pt] (11.95,-12.85) circle (0.082);
\node[font=\sffamily\scriptsize, anchor=west] at (12.12,-12.85) {ConvNeXt $\to$ ViT (ch 9--10)};
\end{tikzpicture}%
}
\end{center}
...
\paragraph{YOLO on VOC persons, in passing.}
\textbf{[TODO: actually get this working --- the objectness loss still
needs the focal rebalance described in the third bullet below before the
rendered boxes are clean. The trajectory and numbers in this paragraph are
the \emph{target}, not a measured result yet; see
\texttt{planning/yolo\_v5.md} for the current state.]}
The \texttt{demos/MainYolov1VocTrainBootstrap.lean} trainer is a
single-class \emph{person} detector: a ResNet-34 backbone (bootstrapped
from the repo's ImageNet ResNet-34 checkpoint, \texttt{jax\_r34\_imagenet.bin},
$\sim$21M backbone params) with a \emph{convolutional} detection head,
trained on Pascal VOC~2007 filtered to the \texttt{person} class
($224 \times 224$ RGB $\to$ a $7 \times 7$ prediction grid). After training,
it draws boxes that track people on held-out validation images (see
\texttt{demos/README.md} for the rendered grid). Three things change
relative to the image \emph{classifiers} of Part~2 and the segmentation
nets above:

\begin{itemize}
  \item \textbf{The label is a variable-length set of boxes, baked onto a
    grid.} A classifier compares one logit vector to one class; a
    segmentation net compares per-pixel logits to a per-pixel mask.
    Detection has a \emph{variable} number of objects per image, each a
    (class, box). YOLO fixes the shape by tiling the image into a
    $7 \times 7$ grid and making the cell containing an object's
    \emph{center} responsible for it: the target tensor carries, per cell,
    the box center/size, an objectness $=1$, and a class slot --- zero
    everywhere else. The loss is multi-part (coordinate regression +
    objectness + class), \emph{masked} to the object cells, riding the
    \texttt{lossKind := .yolov1Masked} path in \texttt{TrainConfig} ---
    no new VJP machinery, just the Part~2 squared-error and cross-entropy
    backwards fired on slices of the grid.
  \item \textbf{The head is convolutional, and that is load-bearing.} The
    paper flattens the final feature map into two fully-connected layers
    (shown in the catalog entry below). This demo uses a $1 \times 1$
    convolution as the head instead ($512 \to$ per-cell channels over the
    $7 \times 7$ map). The reason is both pedagogical and practical: an FC
    head must \emph{learn} the spatial correspondence ``these features
    $\to$ a box \emph{there}'' through a dense layer, and on a small
    dataset it collapses to predicting the average box in the image
    center; the conv head predicts each cell from its \emph{own} feature
    column with shared weights, so localization is structural rather than
    learned. (This is precisely the v1$\to$v2 change.)
  \item \textbf{Class \emph{and} foreground/background imbalance shape the
    loss.} Two imbalances bite. The first is why this demo is single-class:
    on full 20-class VOC the network collapses to predicting the dominant
    ``person'' label everywhere, so we narrow to person-only and let the
    box head carry the demo. The second is intrinsic --- most of the $49$
    cells are empty, so a naive objectness term drives every prediction to
    ``no object''; a \emph{focal}-weighted objectness loss down-weights the
    easy background cells so the few foreground cells survive. Inference
    then adds a post-processing step the classifiers never needed:
    \emph{decode} each cell's box (cell-relative $\to$ absolute), score it
    (objectness $\times$ class), threshold, and run \emph{non-max
    suppression} to collapse overlapping boxes (\texttt{scripts/yolo\_render.py}).
\end{itemize}
...
demo deliberately narrows to the simplest thing that detects --- one class,
one scale, conv head --- and defers anchors and the multi-scale FPN heads
(which don't fit the linear \texttt{NetSpec}) to the limitations
discussion.
...
language --- and each pulls a dataset of its own. \textbf{Pascal
VOC~2007} backs the YOLO detector (\texttt{yolov1-voc-train-bootstrap},
trained on the \texttt{person} class); \textbf{Oxford-IIIT Pets} backs
the UNet segmenter (\texttt{unet-pets-train}); and \textbf{tiny
...
Dataset & Abbrev. & Reference & Homepage & Path in repo \\
\hline
Pascal VOC 2007 & VOC07 & Everingham et al., 2010 &
  \href{http://host.robots.ox.ac.uk/pascal/VOC/voc2007/}{\nolinkurl{host.robots.ox.ac.uk/pascal/VOC/voc2007}} &
  \texttt{download\_voc.sh} $\to$ \texttt{data/voc2007/} \\[2pt]
...
and training. Pascal VOC is fetched from the
\nolinkurl{pjreddie.com} mirror with the official
\nolinkurl{host.robots.ox.ac.uk} server as fallback; the homepage above
is the official one. (The repository uses VOC~\textbf{2007}, the
20-class detection benchmark, not the later 2012 release.)
\normalsize
...
$ lake run benchmark
  ...
  ...
  6  ResNet-34            9.5h
  Full Part-1 training   37.3h
  lake run mnist 5m | cifar 34m | imagenette 36.7h
\end{verbatim}
...
network's own forward pass. \textbf{Cross-checked} (empirical): because IREE cannot itself be
...
The Verified VJP Proofs suite proves \textbf{48 VJP correctness
...
differentiability/forward/witness machinery they rest on, plus 22
architecture definitions in the bestiary, across 97 Lean proof files,
with zero project axioms. The axiom audit
(\texttt{tests/AuditAxioms.lean}) re-checks \textbf{761 theorems} in all
--- the VJP contracts plus the forward-graph faithfulness, render, and
cotangent-chain results --- every one closing under the same three core
axioms. The most recent additions are the
\(\mathbb{R}\to\texttt{float32}\) bridge and the descent results
(\S~\ref{sec:float32}): 38 theorems --- rounding budgets, the
inexact-descent lemma, the linear loss's Lipschitz constants --- all
...
\begin{itemize}
  ...
  \item \emph{Trusted} (validated numerically, not proven): that the
    StableHLO \emph{text} faithfully denotes $[\![\cdot]\!]$ --- a property
    that would need a formal semantics of StableHLO text, which does not yet
    exist; that IREE lowers the text correctly; and that \texttt{float32}
    approximates $\mathbb{R}$.
\end{itemize}
...
claim more than the kernel checked. What stays trusted is exactly what the
...
\section{Float32, and whether it still trains}
\label{sec:float32}
...
persuasive. For the Tier-1 nets it is now a theorem chain
(\texttt{LeanMlir/Proofs/FloatBridge.lean}, \texttt{SgdDescent.lean},
\texttt{SgdDescentLinear.lean}), and the way it avoids new axioms is the
...
normal range (subnormals are open); the exact-arithmetic model
...
IREE tiles and reorders reductions freely; and \(\exp\) enters as a
...
gradient step still \emph{decreases the loss}, and for the linear
classifier \texttt{linear\_sgd\_descends} discharges its smoothness
hypothesis with the explicit constant \(2a^2/(1-2aD)\) --- no Hessian:
the softmax ratio sandwich that powers the float budgets turns out to be
the Lipschitz engine too.
...
Read the gap honestly, in both directions. The worst-case bounds hold
...
that gap is the quantitative argument for a-posteriori certificates
past toy depth, now visible in a theorem rather than asserted. And the
...
\paragraph{Lower precision, by instantiation.}
The model's unit roundoff \(u\) is a field, not a literal, so every budget
is stated for abstract \(u\) and a precision is an \emph{instantiation} ---
up to the point where the relative model \(|\mathrm{rnd}(x) - x| \le u\,|x|\)
itself breaks. The deployed kernels are not fp32 but \emph{bf16-mixed}: bf16
leaf compute, fp32 accumulate. The two-roundoff budget
\texttt{dot\_close\_mixed} splits the single \(u\) into a leaf roundoff
\(u_{\mathrm{leaf}}\) (the matmul inputs) and an accumulate \(u_{\mathrm{acc}}\)
(the reduction), and proves the leaf contributes only a \emph{flat} term
\((2u_{\mathrm{leaf}} + u_{\mathrm{leaf}}^2)\sum_i |x_i y_i|\) while the fan-in
Higham factor rides entirely on \(u_{\mathrm{acc}}\). The \(1/u\) fan-in wall
therefore sits at \(u_{\mathrm{acc}} = 2^{-24}\), not at the leaf --- which is
precisely why bf16-mixed (the shipped \texttt{r34\_imagenet\_bf16.bin}) is
non-vacuous where pure bf16, whose \(\gamma_k\) goes vacuous at fan-in
\(256\), is not.

\paragraph{fp8, and where accuracy gives way to faithfulness.}
At fp8 (E4M3, \(u_{\mathrm{leaf}} = 2^{-4}\)) the worst-case leaf term is
\(\approx 12.5\%\), so over depth it compounds to nothing. The exception is
depth one: MNIST-linear is a single \(784 \to 10\) matmul, where the
per-matmul leaf bound \emph{is} the end-to-end bound, and an honest fp8
accuracy statement survives. It takes the conditional, margin form the kink
theorems already taught: \texttt{argmax\_preserved} proves a \(B\)-accurate
matmul cannot change the prediction on any input whose top-1 logit margin
exceeds \(2B\). At the trained magnitudes the worst-case budget is
\(B \le 61\) (\texttt{linear\_e4m3\_logit\_budget}), so a margin past \(122\)
freezes the label --- a threshold that, like the \(5100\) logit bound above,
is vacuous against a mean measured margin of \(\approx 4.25\). The same
theorem with the \emph{measured} \(B = 0.38\)
(\texttt{scripts/mnist\_e4m3\_demo.py}, where fp32 \(92.25\%\) becomes E4M3
\(92.30\%\)) covers \(92.89\%\) of the test set, every one of which provably
keeps its prediction. Below this --- deeper, or at fp4 --- the relative model
gives way to block-scaled quantization, and the verifiable claim migrates from
\emph{numerical accuracy} to \emph{structural faithfulness}:
\texttt{e4m3\_render\_faithful} proves the emitted block-scaled int-matmul
graph denotes the intended algorithm (the per-output dequant scale factors out
of the fp32 accumulate, \texttt{dequant\_factors}), so the bytes implement
block-scaled-E4M3 matmul with fp32 accumulate --- a
correctness-of-implementation theorem that makes no accuracy claim and does not
pretend to one.
...
\begin{itemize}
  ...
  \item \textbf{The IREE-versus-NumPy oracle} fails if the compiled graph
    ...
    \texttt{float32}, whose rounding now can be, conditionally
    (\S~\ref{sec:float32}).
