...
scale its job is to validate the framework's logic end to end. Phase-3
verified-IREE codegen led to phase 4 --- PJRT, which these demos will be
ported to. Phase-2 is how we establish these baselines in the meantime.}
...
\prosesection{Phase 4: the PJRT bridge}

Every ImageNet recipe in this book opens with the same caveat --- the
numbers come from the phase-2 (Lean$\to$JAX) trainer, and whether the
verified codegen path can reach this scale is an open question. Phase 4
is a second trusted lowerer, \texttt{ffi/pjrt\_ffi.c}, implementing the
same C surface as the IREE (phase 3) shim and handing the graph to XLA
...
it linked --- and there is no Python at run time. What the trainer
executes is \texttt{pretty(provenGraph)}: the same rendered artifact the
proofs reason about, not a hand-written emitter.

At ImageNet scale, on the same four-card box as the bf16 run above:
...
GPU & Precision & Per epoch & Epochs & Total & Val top-1 & Val top-5 \\
\hline
2$\times$ 7900 XTX (ROCm) & fp32 & ---           & 30 & $\sim$20 hr  & --- & --- \\
4$\times$ 4060 Ti (CUDA)  & fp32 & $\sim$30.9 min & 30 & $\sim$15.5 hr & $\mathbf{70.74\%}$ & $\mathbf{90.14\%}$ \\
\hline
\end{tabular}
\end{center}

\noindent ResNet-34 / ImageNet-1k on the phase-4 path, on the two rigs
this book measures on.
...
\subsection*{Phase 4: RSB-A3 on the verified path}

The same recipe on the \emph{verified} codegen path --- not a
...
\texttt{resnet50in160\_lambaccdp8x64bce\_train\_step}: LAMB $\times$
...
\hline
GPU & BN group & Per epoch & Total & Val top-1 & Val top-5 \\
\hline
4$\times$ 4060 Ti (fp32)  & Ghost-BN (64) & $\sim$20.6 min & $\sim$34.1 hr & $\mathbf{77.43\%}$ & $\mathbf{93.60\%}$ \\
2$\times$ 7900 XTX (fp32) & Ghost-BN (64) & ---            & ---           & ---                & ---                \\
\hline
\end{tabular}
\end{center}

\noindent 100 epochs each. The 4060~Ti run logged zero thermal
duty-cycle cooldowns and zero PCIe/AER interruptions. The 7900~XTX row
is the ROCm rerun and is pending; note it needs its \emph{own} render ---
the artifact above bakes four replicas, so holding effective batch 2048
on two cards means a 2-replica graph at $k = 16$ rather than a runtime
flag.

\textbf{[TODO --- missing for a full A3 implementation.]} This finished
ahead of the phase-2 reference above ($77.22\%$ / $93.34\%$) while
lacking the following, so quote it as \emph{the A3 recipe on the verified
path, with these deltas} --- not as ``RSB-A3 reproduced''. In rough order
of expected effect:

\begin{itemize}
  \item \texttt{wdExcludeNormBias} absent --- BN $\gamma/\beta$ and biases
    are decayed at $\mathrm{wd} = 0.02$ where timm skips them. Largest
    single delta; the ConvNeXt and ViT renders already carry the
    mechanism, so this is a port.
  \item Ghost-BN group is 64 against the reference's 512 --- no collective
    touches the batch statistics. Needs a cross-replica reduction or a
    larger per-replica batch (which is baked into the signature).
  \item BN running-statistic momentum is not compensated for accumulation
    ($0.99$ per micro-batch at $k = 8$, against the reference's
    $0.99^{1/k}$). Evaluation-only, plausibly \emph{understates} top-1;
    one-line fix.
  \item fp32, not bf16 --- most of the $2\times$ wall-clock gap.
  \item $0.08\%$ of each epoch is dropped (5000 of 5004 micro-batches,
    since $8 \nmid 5004$).
  \item Mixup $\lambda$ is drawn host-side from NumPy --- agreement with
    the reference is distributional, never per-step.
  \item \emph{Verification, not recipe:} the gradient check's tier-1B
    identity does not survive CE$\to$BCE ($\|g\|$ falls $90\times$ and the
    check measures a cancellation residual). Tier-1A and tier-2 do pass.
\end{itemize}
...
file's own \texttt{categories} list --- COCO's ids are not contiguous, and
the obvious \texttt{id\,-\,1} remap mislabels 69 of the 80 classes (id 13 is
`stop sign'; \texttt{id\,-\,1} calls it `parking meter') without ever raising
an error --- and skips \texttt{iscrowd} regions, as COCO's own evaluation
does. \emph{References:} Lin et al., \emph{Microsoft COCO:
Common Objects in Context}, ECCV (2014); Zhu et al., \emph{Detection and
Tracking Meet Drones Challenge}, IEEE TPAMI~43 (2021).

\chapter{Getting started}
\label{app:getting_started}

If you want to just see it run --- train a real neural network, end to end,
with the Lean $\to$ MLIR $\to$ GPU pipeline from Part 1 --- this appendix is
the smallest path from zero to a trained model.

The thing worth knowing before you start is that \emph{what you have to
install depends entirely on how far up the scale you want to go}, and the
...
See the pipeline run at all    & Docker.                          & 2 \\
MNIST / CIFAR / Imagenette     & Lean + one plugin file.          & 3, 4 \\
Full ImageNet-1k               & the above, plus Python + tfds.   & 5 \\
\end{tabular}
\end{center}

Everything in Part 1 of this book is the third row. It needs \emph{no Python
at run time and no compiler}: you download one shared library and run a Lean
binary. Only the ImageNet tier (Track 5) needs an interpreter, and it needs
one because of how its data is streamed, not because of how it computes.

\section*{Track 1: Proofs only (no GPU needed)}

Read, build, and check the proofs without running any training. This track
depends on neither lowerer --- the theorems are about the emitted graph, not
about anything that executes it --- and it comes at three levels of
independence, which are worth distinguishing because they ask you to trust
different amounts.
...
else; Appendix~\ref{app:verification} is the accounting.
...
\texttt{tests/comparator/} re-runs Lean's kernel typechecker \emph{independently}
over 52 theorems, sandboxed, verifying each one's axiom closure statically:

\begin{verbatim}
tests/comparator/run.sh
\end{verbatim}

The part worth knowing about is how it splits. \texttt{Challenge.lean}
\textbf{imports Mathlib and nothing else} --- the 13 architecture-free
theorems (Chapter~\ref{chap:tensor}'s \texttt{pdiv} calculus rules and
Chapter~\ref{chap:attention}'s matrix-level ones), with the handful of
definitions they need copied inline, and \texttt{chk\_pdiv\_is\_fderiv} pinning
\texttt{pdiv} to Mathlib's \texttt{fderiv} by \texttt{rfl}. \emph{A reviewer can
read that one file and know exactly what is claimed without reading a line of
this project.} Copying the definitions is safe here in a way copying an
...
The other 39 live in \texttt{ChallengeArch.lean} and \emph{do} import the
project, because each is a statement about a specific network --- ``ResNet-34's
rendered backward equals its Fréchet derivative'' cannot be phrased without
ResNet-34 in scope. That division is the point: once the \texttt{fderiv} pin
and the structural rules are checked over Mathlib alone, the architecture
theorems are applications of them, and what you must additionally trust is the
forward functions and nothing else.

\section*{Track 2: No-GPU Docker demo}

The repository ships a \texttt{Dockerfile} that builds a CPU-only MNIST demo
in a multi-stage image. Once built, the final image is $\sim$300\,MB and
doesn't require a GPU or Python.
...
(784 $\to$ 512 $\to$ 512 $\to$ 10) trains for 12 epochs on real MNIST and
converges to $\sim$97.9\% test accuracy --- 92.0\% after the first epoch,
97.2\% by the fifth.

Two things to know about this track. It builds the \textbf{IREE} CPU runtime
from source, which is what makes the first build take $\sim$10 minutes ---
and that is the right choice here rather than a leftover: XLA's GPU plugins
ship prebuilt, but there is no equivalent CPU plugin exposing the PJRT C API,
and CPU-only portable execution is exactly what IREE is designed for. This
track is the small-runtime path in miniature.

It trains \texttt{mnist-mlp-verified} --- the same proof-carrying trainer
Chapter~\ref{chap:mlp} walks through, reading the same
\texttt{verified\_mlir/mlp\_*.mlir} the theorems are about. On CPU that costs
about 99~seconds an epoch, so the twelve epochs take $\sim$20 minutes rather
than the couple of minutes a GPU needs. That is the price of the demo being
the real thing rather than a stand-in.

\section*{Track 3: Native install (XLA/PJRT)}
...
\end{verbatim}
  ...
  \item \textbf{Get an XLA PJRT plugin.} This is one prebuilt shared library.
    It ships inside the JAX wheels because that is the only place XLA's GPU
    plugins are packaged --- \emph{nothing imports JAX and no interpreter runs
    at training time}; the shim opens the \texttt{.so} directly through the
    PJRT C API.
\begin{verbatim}
...
\end{verbatim}
    The shim looks for the plugin under \texttt{.venv/lib/python3.*/
    site-packages/jax\_plugins/}, so installing into the repo's own
    \texttt{.venv} is enough for it to be found. Point at one elsewhere with
    \texttt{\$PJRT\_PLUGIN}. The ROCm plugin is version-matched to your ROCm
    install, so it is \texttt{pip install} \emph{with AMD's index}, not a bare
    one --- but it is still a download, not a build.
...
\prosesection{Running the same binary on IREE}

Every verified trainer is one executable that carries \emph{both} trusted
lowerers and picks between them at run time. There are no \texttt{-xla}
targets to build and no separate binaries to keep in step:

\begin{verbatim}
.lake/build/bin/resnet34-verified-adam data                   # XLA (default)
LEAN_MLIR_LOWERER=iree ... resnet34-verified-adam data        # IREE
\end{verbatim}

The IREE path additionally needs \texttt{iree-compile} on \texttt{\$PATH}
(\texttt{pip install iree-base-compiler}), the IREE runtime built per
\texttt{IREE\_BUILD.md}, and \texttt{ffi/libiree\_ffi.so}. It also wants
\texttt{IREE\_BACKEND} and \texttt{IREE\_CHIP} --- e.g.
\texttt{IREE\_BACKEND=rocm IREE\_CHIP=gfx1100}, or \texttt{cuda} with
\texttt{sm\_86} (Ampere), \texttt{sm\_89} (Ada), \texttt{sm\_90} (Hopper).
That asymmetry is the point of the section below.

\section*{Track 4: One-command demo tiers}

With Track 3 in place, a single command builds and runs a curated,
time-budgeted group of \emph{verified} trainers --- no environment variables
or trainer list to assemble by hand.

\begin{verbatim}
lake run mnist        # verified MNIST: linear, MLP, CNN             (~1 min)
lake run cifar        # ch.4 cifar8: SGD/momentum/Adam x bn/no-bn   (~15 min)
lake run imagenette   # the Part-I nets at 224^2, 80-epoch AdamW      (~7 h)
\end{verbatim}

Those are XLA wall-clocks on one 7900 XTX, and they are the reason the default
moved. The same three tiers on IREE take $\sim$5~minutes, $\sim$34~minutes and
$\sim$42~hours --- the whole MNIST tier now finishes in under a minute, and
Imagenette drops from most of two days to an afternoon. The Imagenette figure
covers the five nets with recorded timings; MobileNetV4 and ResNet-50 joined
the tier more recently and are not in that total, so budget somewhat more.

Each has an \texttt{-iree} twin --- \texttt{lake run mnist-iree} and so on ---
which runs \emph{the same binaries} with the IREE lowerer selected. The tiers
bundle these targets, and any one builds on its own:

\begin{itemize}
  \item \texttt{mnist}: \texttt{mnist-linear-verified},
    \texttt{mnist-mlp-verified}, \texttt{mnist-cnn-verified}
  \item \texttt{cifar}: \texttt{cifar8-verified-sgdsched} and
    \texttt{cifar8-bn-verified-sgdsched}, plus each in
    \texttt{-momentum} / \texttt{-adam} (six in all)
  \item \texttt{imagenette}: \texttt{resnet34-verified-adam},
    \texttt{mobilenetv2-verified-adam}, \texttt{efficientnet-verified-adam},
    \texttt{convnext-verified-adam}, \texttt{vit-verified-adam},
    \texttt{mobilenetv4-verified-adam}, \texttt{resnet50-verified-adam}
\end{itemize}

\section*{Track 5: ImageNet-1k runners}

The scale tier. Six nets train on full ImageNet-1k off the same certified
renderer as their Imagenette counterparts:

\begin{center}
\begin{tabular}{@{}ll@{}}
\textbf{Target} & \textbf{Job config} \\[2pt]
\hline \\[-6pt]
\texttt{resnet34-imagenet-verified}     & \texttt{r34-imagenet-4gpu} \\
\texttt{resnet50-imagenet-verified}     & \texttt{r50-imagenet-4gpu} \\
\texttt{mobilenetv2-imagenet-verified}  & \texttt{mnv2-imagenet-4gpu} \\
\texttt{efficientnet-imagenet-verified} & \texttt{enet-imagenet-4gpu} \\
\texttt{convnext-imagenet-verified}     & \texttt{cnx-imagenet-4gpu} \\
\texttt{vit-imagenet-verified}          & \texttt{vit-imagenet-4gpu} \\
\end{tabular}
\end{center}

\prosesection{This tier needs Python, and only this tier does}

ImageNet data does not sit in a directory the way MNIST and Imagenette do. It
is streamed from tfds through a \emph{generated} per-net shim --- a Python
...
Each net names its own shim, and the driver \emph{refuses to start} if the
named file is absent rather than falling back to another net's --- an earlier
...
\texttt{LEAN\_MLIR\_BATCH} \emph{must match the batch its variant was rendered
at} --- the batch is baked into the graph, not a runtime dimension, so a
mismatch is a shape error at the first invoke.
...
\texttt{String.toFloat?}; the default 0.001 is an \emph{AdamW} rate and will
...
DRY_RUN=1 scripts/supervise.sh r34-imagenet-4gpu   # print the plan, run nothing
\end{verbatim}

Data parallelism needs \emph{both} replica knobs --- \texttt{PJRT\_REPLICAS}
configures the shim's client and compile options, \texttt{LEAN\_MLIR\_REPLICAS}
makes the Lean driver call the data-parallel invoke --- and setting only one
fails with a confusing ``compiled for N replicas'' error. The job configs set
both. Collectives exist only on the PJRT path, so the data-parallel runs are
XLA-only; the IREE shim refuses a DP entry point outright rather than silently
running single-device.

\section*{Choosing a lowerer}

Both lowerers consume the same proven StableHLO and are the same trusted tier
--- neither is verified, and swapping them changes nothing about what is
proved. They differ in what they are good at:

\begin{itemize}
  \item \textbf{XLA/PJRT} is faster here, by 2.3$\times$ on the MNIST linear
    net, 4.3$\times$ on the MLP and 4.9$\times$ on the CNN, because it
    dispatches convolutions and matmuls to the vendor's hand-tuned kernel
    libraries. The gap tracks how much work those kernels get.
  \item \textbf{IREE} generates every kernel itself, which is why it runs where
    no vendor library exists at all --- Vulkan, Metal, bare CPU, custom
    accelerators --- and why its runtime is \emph{392$\times$ smaller}
    (1.3\,MB against the XLA plugin's 505\,MB, which bundles MIOpen, rocBLAS
    and RCCL).
\end{itemize}

That size difference is also a trust difference, and Appendix~\ref{app:verification}
is where it is counted: the fast path asks you to trust a great deal more
unverified code than the small one does.

\section*{Common troubleshooting}

\begin{itemize}
  \item \textbf{\texttt{no lowerer shim could be loaded}} --- neither
    \texttt{ffi/libpjrt\_ffi.so} nor \texttt{ffi/libiree\_ffi.so} was found.
    Build the PJRT one with the \texttt{gcc} line in Track 3; it needs nothing
    but libc.
  \item \textbf{\texttt{\$PJRT\_PLUGIN is set but did not load}}, or no plugin
    ...
    version than the one installed. Installing JAX into the repo's own
    \texttt{.venv} puts it where the shim looks by default.
  \item \textbf{\texttt{HAL device not found}} on an IREE run --- IREE has
    \emph{two} knobs and they must agree: \texttt{IREE\_BACKEND} is the
    \emph{compile} target and \texttt{IREE\_DEVICE} is the \emph{runtime}
    device. Setting only \texttt{IREE\_BACKEND=llvm-cpu} compiles for CPU and
    then looks for a GPU to run it on, which reports as this error naming a
    \texttt{local} target. For CPU you want both:
    \texttt{IREE\_BACKEND=llvm-cpu IREE\_DEVICE=local-task}. (Failing that, the
    runtime genuinely lacks that HAL driver and needs rebuilding.)
  \item \textbf{\texttt{lake build} fails with Mathlib cache errors} --- run
    ...
    first build compiles all of Mathlib from source ($\sim$45 min); with it,
    oleans download in $\sim$30 seconds.
  \item \textbf{\texttt{iree-compile failed} on ResNet-50} --- expected.
    R50's rendered maxpool backward is a
    \texttt{stablehlo.select\_and\_scatter} that IREE does not compile at this
    size. R50 is an XLA-only net; drop \texttt{LEAN\_MLIR\_LOWERER=iree}.
  \item \textbf{First IREE compile appears stuck} --- it isn't. ResNet-34's
    full training-step compile takes 10--15 minutes. Watch RAM; if it is
    climbing the compile is progressing. The XLA path compiles the same graph
    in seconds, in process.
  \item \textbf{A run refuses to start over a missing shim} (ImageNet tier)
    --- run \texttt{scripts/gen\_shims.sh}. The refusal is deliberate.
\end{itemize}
