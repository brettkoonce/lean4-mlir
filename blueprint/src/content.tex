...
theorem layer (Appendix~\ref{sec:float32}) --- is watched by independent
...
(Appendix~\ref{sec:float32}) --- every one closing under the same three
...
\begin{itemize}
  ...
  \item \emph{Trusted} (validated numerically, not proven): that the
    ...
    (Appendix~\ref{sec:float32}) rather than a bare assumption.
\end{itemize}
...
mask, no logit drift past \(\delta\) (Appendix~\ref{sec:float32}). It is
...
\chapter{Float32: closeness, composition, and whether it still trains}
\label{sec:float32}
