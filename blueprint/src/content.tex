...
Those are XLA wall-clocks on one RTX 4060 Ti. The Imagenette figure is the
five chapter nets; ResNet-50 (about 75 minutes) and MobileNetV4 (about 35) ride
in the same tier and add to it. Each trainer's transcript is teed to
\texttt{runs/<date>-<trainer>/}. Each tier also has an \texttt{-iree} twin ---
see \emph{The second lowerer} at the end. The tiers bundle these targets, and
any one builds on its own:
...
\texttt{convnext-s-imagenet-verified}  & \withheld         & \texttt{cnxs-default-4gpu}, untrained & \S\ref{sec:convnext_sb} \\
\texttt{convnext-b-imagenet-verified}  & \withheld         & \texttt{cnxb-default-4gpu}, untrained & \S\ref{sec:convnext_sb} \\
\texttt{vit-s-imagenet-verified}       & \withheld         & global $512$; \texttt{vits-default-g512-4gpu}, untrained & \S\ref{sec:vit_sb} \\
\texttt{vit-b-imagenet-verified}       & \withheld         & global $512$; \texttt{vitb-default-g512-4gpu}, untrained & \S\ref{sec:vit_sb} \\
\end{tabular}
\end{center}
...
\texttt{\$PATH}, the runtime built per \texttt{historical/IREE\_BUILD.md}, and
