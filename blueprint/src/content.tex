...
GPU & Precision & Per epoch & 90 epochs & Val top-1 & Val top-5 \\
\hline
4$\times$ 4060 Ti (CUDA) & bf16 & $\sim$9.1 min & $\sim$14 hr & TODO & TODO \\
\hline
\end{tabular}
\end{center}

\noindent (Throughput is measured from steady-state ms/step; the run
is not yet trained to completion, so accuracy is pending. The open
question is whether the SGD peak LR of $0.1$ takes off cleanly on a
batch-normalized depthwise net --- the validation tier answers it,
and the config carries a fallback to $\sim$0.05 if the early epochs
stall.)
...
\noindent Where the words are clumsy or wrong, blame not the kernel,
which checked what it could, but the author, who is answerable for the
rest.
