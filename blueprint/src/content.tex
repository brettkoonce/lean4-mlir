...
rounding (ConvNeXt: 179/180 tensors bit-identical). The one open gap: the
...
the cotangent). Across Chapters 2--10 there are \textbf{48 VJP
correctness theorems}, one citable contract per layer, operator, and
whole-network architecture --- through the full-architecture closes of
all five Part~I networks. The foundation calculus rules,
differentiability lemmas, and concrete forward/witness definitions
...
\textbf{Chapter 5 (CIFAR: BN)} contributes 4 VJP contracts: the
...
\textbf{Chapter 6 (ResNet-34)} contributes 9 VJP contracts and
...
\textbf{Chapter 7 (MobileNetV2)} contributes 3 VJP contracts: the
...
The witness column counts each chapter's concrete end-to-end instances
(a specific net at specific weights carried through the whole pipeline);
they are \(\texttt{\_correct}\) theorems too, but of a different kind ---
sanity instances, not citable operator contracts --- so the table keeps
them apart. The shift from earlier drafts: every architecture ends in a
...
exact signatures. \textbf{Cross-checked} (empirical): because IREE and
\texttt{float32} cannot themselves be proven, three independent oracles ---
...
architecture definitions in the bestiary, across 51 Lean proof files,
with zero project axioms. The axiom audit
(\texttt{tests/AuditAxioms.lean}) re-checks \textbf{383 theorems} in all
...
axioms. Earlier drafts shipped 30
...
\begin{itemize}
  ...
  \item \textbf{The IREE-versus-NumPy oracle} fails if the compiled graph
    ...
    past the proofs to IREE's lowering and \texttt{float32}, neither of which
    can be proven.
\end{itemize}
...
structural, its oracle empirical.
