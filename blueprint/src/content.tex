...
\begin{enumerate}
  ...
  \item \textbf{One-command demo tiers.} With the native setup
    (Track 2) in place, a single \texttt{lake run mnist-iree},
    \texttt{cifar-iree}, or \texttt{imagenette-iree} builds and runs a
    curated, time-budgeted group of \emph{verified} trainers --- no env
    vars or trainer list to assemble by hand. (Drop the \texttt{-iree}
    suffix for the XLA/PJRT peers, which are the default tiers.)
\end{enumerate}
...
\end{verbatim}
    ...
    \begin{itemize}
      \item \texttt{lake run mnist-iree}: \texttt{mnist-linear-verified},
        \texttt{mnist-mlp-verified}, \texttt{mnist-cnn-verified}
      \item \texttt{lake run cifar-iree}: \texttt{cifar8-verified} and
        \texttt{cifar8-bn-verified}, each in plain-SGD /
        \texttt{-momentum} / \texttt{-adam} (six in all)
      \item \texttt{lake run imagenette-iree}: \texttt{resnet34-verified-adam},
...
\begin{verbatim}
lake run mnist-iree        # verified MNIST: linear, MLP, CNN            (~30 min)
lake run cifar-iree        # ch.5 cifar8: SGD/momentum/Adam x bn/no-bn   (~1 hr)
lake run imagenette-iree   # the 5 Part-I nets at 224^2, 80-epoch AdamW  (~37 h)
\end{verbatim}
...
The tiers escalate by time budget. \texttt{lake run mnist-iree}
($\sim$30 min) is the quickest end-to-end confirmation that the GPU
path works; \texttt{lake run cifar-iree} ($\sim$1 hr) is the
...
curves and table; and \texttt{lake run imagenette-iree} ($\sim$37 h on a
...
$ lake run benchmark
  ...
  lake run mnist-iree 5m | cifar-iree 34m | imagenette-iree 42.2h
\end{verbatim}
