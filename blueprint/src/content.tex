...
% the phase-2 (Lean->JAX) trainer to validate the framework's logic; whether
% phase-3 verified-IREE codegen can reach this scale is an open question.
\newcommand{\imagenetphasenote}{\emph{Why the phase-2 trainer?} The
ImageNet runs in this book use the phase-2 (Lean$\to$JAX) trainer: at this
scale its job is to validate the framework's logic end to end. Whether the
phase-3 verified-IREE codegen can reach ImageNet under its own codegen rules
is an open question --- phase-2 is how we establish these baselines in the
meantime.}
...
\href{https://arxiv.org/abs/2509.04922}{Gou\"ezel} (\texttt{arXiv:2509.04922})
over 2019--2025.
...
equals the true Jacobian, would not be feasible to prove.
...
\begin{itemize}
  \item \textbf{Chapter 1: MNIST: linear classifier} --- Chain rule, sum rule, product
    rule for partial derivatives; every backward in the book is a VJP
    (vector-Jacobian product) composed from these. The book's hardest
    chapter --- it builds the whole pipeline (foundation calculus
    \(\to\) VJP framework \(\to\) forward/loss/backward/SGD \(\to\) GPU),
    used by the MNIST linear classifier, proven and trained.
    Everything after is just adding a layer.
  \item \textbf{Chapter 2: MNIST: 1D MLP} --- ReLU and hidden layers on
    top of Chapter 1's dense layer: the first \emph{multi-layer}
    backward pass.
  \item \textbf{Chapter 3: MNIST: 2D CNN} --- Convolution, max pooling. The
  ...
  \item \textbf{Chapter 5: ResNet-34} --- Residual connections. Gradients from
  ...
  \item \textbf{Chapter 8: ConvNeXt} --- LayerNorm, Batchnorm in
...
If you're planning how much of the book to read, here's what each
chapter \emph{proves about gradients}: its VJP correctness theorems
--- the \(\texttt{*\_has\_vjp*\_correct}\) results, each asserting
\(\texttt{backward}\,x\,dy\,i = \sum_j \pdiv f\,x\,i\,j \cdot dy_j\)
(a layer's backward equals the \(\pdiv\)-Jacobian contracted against
the cotangent). Across Chapters 1--9 there are \textbf{40 contracts}
--- one citable per layer, operator, and whole-network architecture,
through the full-architecture closes of all five Part~I networks ---
plus \textbf{8 witnesses}: concrete end-to-end instances (a specific
net at specific weights), \(\texttt{\_correct}\) theorems too but
sanity checks rather than citable contracts, so the table below keeps
them in their own column. The foundation calculus rules,
differentiability lemmas, and concrete forward definitions
that support them are additional machinery, not counted here. Even
Chapter~1's toolkit is used straight away by a proven model --- the
MNIST linear classifier --- so no chapter is pure scaffolding.

\textbf{Chapter 1 (MNIST: linear classifier)} does the structural heavy lifting:
...
where attention first uses them. The chapter then puts the kit to work
in its first proven model: \textbf{1 VJP contract},
\(\texttt{mnistLinear\_has\_vjp\_correct}\) --- the whole model, a
single dense layer --- with the dense weight- and bias-gradient
theorems and the softmax cross-entropy gradient certified alongside
(parameter-gradient contracts, counted with the supporting machinery).
It is the degenerate-simplest whole-model VJP, and the ops it needs are
introduced right here, where the demo lives.

\textbf{Chapter 2 (MNIST: 1D)} adds depth and nonlinearity on top of
Chapter~1's dense layer --- \textbf{2 VJP contracts} plus the chapter's
concrete end-to-end witness. The guarded ReLU
Jacobian \(\pdiv\_\texttt{relu}\) is proved via local-diagonal-CLM
transport; \(\texttt{relu\_has\_vjp}\) and \(\texttt{mlp\_has\_vjp}\)
are \texttt{noncomputable def}s over the canonical pdiv-derived witness
(\texttt{HasVJP.correct} by \texttt{rfl}), with the smooth-point
\(\texttt{\_at}\) variants carrying the real chain-rule proof.

\textbf{Chapter 3 (MNIST: 2D)} contributes 3 VJP contracts (plus four
concrete witnesses at increasing structure).
\(\texttt{conv2d}\) and \(\texttt{maxPool2}\) are concrete forward
defs; the conv2d input-side VJP, maxPool2's VJP --- plus its
smooth-point \(\texttt{\_at}\) variant, whose \texttt{correct} field is
a real proof rather than \texttt{rfl} --- and the whole-net MNIST-CNN
contract are proved from foundation, with the conv weight- and
bias-gradient theorems alongside.

\textbf{Chapter 4 (CIFAR: BN)} contributes 4 VJP contracts (plus a concrete witness): the
per-channel BatchNorm VJP in both layouts (flat and NCHW tensor) and
the CIFAR whole-nets with and without BN --- all assembled from the
\(\texttt{bn\_input\_grad}\) chain of supporting Jacobians (the
inverse-stddev broadcast Jacobian and its smoothness lemma under
\(\varepsilon > 0\), plus the BN affine and centering Jacobians, all
proved from foundation).

\textbf{Chapter 5 (ResNet-34)} contributes 9 VJP contracts (plus a concrete witness) and
no new calculus: the residual additive-fan-in VJP (plus its
projected variant), the global-average-pool VJP, the stride-2
convolution's input- and weight-VJPs (= decimation \(\circ\) the
stride-1 conv, the downsample op), the strided block and stage folds,
and \(\texttt{cnn\_has\_vjp\_at}\), the \emph{whole-network} fold
composing stem, residual blocks, pool, and dense head end to end.
The scaling claim in miniature: an entire architecture's backward,
built from Chapter~1's kit.

\textbf{Chapter 6 (MobileNetV2)} contributes 3 VJP contracts (plus a concrete witness): the
depthwise input VJP at stride 1 (same recipe as Ch~3 with one fewer
summation level --- no cross-channel mixing) and stride 2, and the
whole-network \(\texttt{mobilenetv2\_has\_vjp\_at}\) over
inverted-residual blocks, which introduces the two-sided-kink
\texttt{relu6} as a new smooth-point activation.

\textbf{Chapter 7 (EfficientNet)} contributes 5 VJP contracts:
\texttt{swish}/SiLU (the MBConv activation), the squeeze-and-excitation
block (\(\texttt{seBlock\_has\_vjp}\) --- global-avg-pool plus two
1\(\times\)1 convs plus the elementwise product, every piece already
proved elsewhere), the whole-network
\(\texttt{efficientnet\_has\_vjp\_at}\) over MBConv blocks with a
full-spatial SE gate plus its unconditional global sibling, and ---
at full architecture --- \(\texttt{efficientnetForwardB\_full}\): all
16 MBConv blocks, batched, under true batch-norm.

\textbf{Chapter 8 (ConvNeXt)} contributes 7 VJP contracts: GELU
(Jacobian via \(\fderiv\) and \texttt{Real.differentiable\_tanh}),
LayerNorm (reusing the BN proof template), the elementwise
layer-scale, the whole-network \(\texttt{convnext\_has\_vjp\_at}\)
plus its unconditional global sibling --- the all-smooth architecture,
discharged with no kink hypotheses, only \(\varepsilon > 0\)
positivity --- and, at full architecture,
\(\texttt{convNextForwardT}\) (all 18 blocks, 4\(\times\)4/s4 patchify
stem) with its committed-config variant. (Swish/SiLU lives in
Chapter~7, where EfficientNet uses it; ConvNeXt's activation is GELU.)

\textbf{Chapter 9 (ViT)} contributes 6 VJP contracts (MHSA, the
transformer block, the depth-2 whole-network in scalar- and vector-LN
forms, the general-depth \(\texttt{vitForwardKV}\) --- every depth
\(k\) at once --- and the
whole-network \(\texttt{vit\_full\_has\_vjp\_correct}\) wrapper;
the softmax cross-entropy gradient now lives in Chapter~1 with the
linear classifier), resting on the
largest \emph{supporting}-theorem budget in the book and the only
chapter that needs the matrix-level extensions of Chapter~1's
foundation rules. The chapter opens with
...
The shift from earlier drafts: every architecture ends in a
\emph{whole-network} VJP contract, and for the smooth half the contract
now holds at \emph{full architecture} ---
\(\texttt{efficientnetForwardB\_full\_has\_vjp\_correct}\) (all 16 MBConv
blocks, true batch-norm),
\(\texttt{convNextForwardT\_has\_vjp\_correct}\) (all 18 blocks; plus the
committed-config variant), and
\(\texttt{vitForwardKV\_has\_vjp\_correct}\) (every depth \(k\)). The
kinked nets keep their whole-network contracts at representative depth
--- \(\texttt{cnn\_has\_vjp\_at\_correct}\),
\(\texttt{mobilenetv2\_has\_vjp\_at\_correct}\); the ReLU/ReLU6 kinks
force at-a-point statements --- while their \emph{full} architectures are
closed on the forward side (graph faithfulness at all 34 layers, all 17
bottlenecks) with parameter gradients certified family-by-family through
the chain closes. Those per-family certificates are in turn composed end to
end: every committed training step is \emph{tied} --- each parameter update
proved to consume the cotangent its own backward pass delivers, threaded
through the real forward --- in one whole-network theorem per net
(\(\texttt{<net>\_net\_tied\_certified}\), \S~\ref{sec:verified_codegen}), across
all twelve verified trainers. (The underlying forward and witness \texttt{def}s, the
foundation \(\pdiv\) rules, and the differentiability lemmas are the
supporting machinery these 48 contracts rest on; they are not counted in
the table.)
...
doing the proofs in Lean. Chapter~1 itself is short enough that
...
chapters to read, and whether you need Chapter~9's matrix-machinery
section.
...
\item \textbf{Training recipe upgraded.} The first book's codegen
  ...
  data augmentation are how this book's
  Imagenette results were achieved. That full recipe is assembled and
  ablated in Chapter~\ref{chap:residual} (ResNet) and ran against each
  architecture after as a common baseline. Later chapters add full
  ImageNet recipes, trained through a Lean 4 \(\to\) JAX bridge, with
  modern data augmentations.
...
\(\fderiv\), and those structural rules are theorems proved
...
from nothing to a complete trained model --- the \(\pdiv\) calculus,
the VJP framework, a forward pass, a loss, a backward pass, an SGD
step, all the way down to the GPU --- is the entire machinery, built in a single
chapter. We do it on the smallest possible network (one matrix multiply: the
...
In our repo this is the standalone trainer
\texttt{mnist-linear-train} (\texttt{MainMnistLinearTrain.lean}),
matching the per-chapter trainer pattern every later chapter uses.
...
cotangent $dy = \mathrm{softmax}(\mathrm{logits}) - \text{onehot}$, the input
gradient is $dx = dy\,W^{\!\top}$, a single matrix multiply. Here is exactly
what the code generator emits for it (at a small representative size; nothing
hand-written, one \texttt{stablehlo} op):
...
\noindent That \texttt{dot\_general}, contracting the output axis of $dy$
against the output axis of $W$, \emph{is} multiplication by $W^{\!\top}$ ---
which is, by a machine-checked theorem, the linear layer's exact reverse-mode
derivative. Everything else in this book scales that one move up. Each
architecture chapter closes with an ``MLIR: \emph{operator}'' section that
takes the chapter's new operator --- convolution, BatchNorm, the residual
fan-in, depthwise convolution, squeeze-excitation, layer scale, attention ---
and shows that \emph{its} emitted backward is likewise the rendering of a
proven derivative. Each of those listings is shown at a small representative
size --- a few channels, tokens, or units of width. The per-operator proof
beneath is dimension-parameterized and the printer emits the identical graph
at production scale, so each section just states its toy shape and moves on.
The shared machinery underneath them --- the denoted
intermediate representation, the bridge theorems that tie it to the proofs,
the printer that turns it into the text above, and the execution oracle that
checks the result on the GPU --- is the subject of
...
\(g \leftarrow \nabla_\theta L\) --- that's exactly what Ch~1's
...
infrastructure code. And it all runs on your hardware:
Appendix~\ref{app:getting_started} walks you through the toolchain setup,
and if you're impatient, there's a Docker image that gets you from zero
to training MNIST in one command.

\section{MLIR: Training Step}
\label{sec:mlir_training_step}

Section~\ref{sec:mlp_inside_train} made a claim about the inner call:
\emph{one} dispatched \texttt{stablehlo} vmfb does the forward pass, the loss,
the backward pass, \emph{and} the optimizer update --- no tape, no per-step
graph construction. The ``MLIR: Linear'' section
above rendered a single operator's backward in isolation; here is the
\emph{whole} thing the linear classifier compiles to. It is short enough to
show essentially in full --- only the default \texttt{precision} attributes are
...
four bands. The first (\texttt{\%v0}--\texttt{\%v8}) fuses the forward pass and
the loss cotangent --- \texttt{dot\_general} for the logits, the
\texttt{exp}/\texttt{reduce}/\texttt{divide} softmax, and the \texttt{subtract
\%onehot} that yields $dy = \partial\mathrm{CE}/\partial\text{logits}$; that
whole band is the rendering of a single verified graph, \texttt{lossCotGraph},
certified equal to the softmax--cross-entropy gradient by
\texttt{lossCotGraph\_isCEgrad}. The second band is the backward pass --- the
\texttt{dot\_general \%x, \%v8} is exactly the move from the ``MLIR: Linear''
section, $x^{\!\top} dy$, now serving as the weight gradient, and the
\texttt{reduce} sums $dy$ over the batch for the bias gradient. The third band
is the optimizer: scale each gradient by the learning rate and subtract. There
is no fourth band --- no copy-back, no separate update kernel. Forward, loss,
...
\medskip\noindent Two honest notes. \textbf{This is SGD, not Adam.} The
\texttt{mnistLinear} example runs the \texttt{s4tfBaseline} configuration ---
plain $\theta' = \theta - \alpha\nabla$ at a constant learning rate --- so the
update band is three ops per tensor. Turning on \texttt{cfg.useAdam} grows that
band (the first- and second-moment buffers enter and leave the signature and an
\texttt{rsqrt} appears) but changes nothing above it; the deeper chapters'
steps carry that Adam tail, the early ones do not. \textbf{The learning rate is
folded in.} The weight-gradient \texttt{dot\_general} contracts the batch axis,
so $dW_0$ is a batch \emph{sum}, not a mean; the constant \texttt{0.00078125}
is $\alpha/N = 0.1/128$, the per-sample rate that turns that sum back into the
mean update. Everything else in the listing is, line for line, the rendering of
...
loop shuffles, schedules the learning rate, and counts epochs --- it never
computes a gradient or touches a tensor. The lone \texttt{train\_step\_vmfb}
call is the function above, and it is the only place arithmetic happens.
...
CIFAR$\pm$BN) are closed both ways, and all five Part~I architectures ---
ResNet-34, MobileNetV2, EfficientNet-B0, ConvNeXt-T, ViT --- are proved at
full architecture: machine-checked forward graphs throughout; whole-network
backward at full depth, with the non-triviality seal (that the proven backward
is nonzero at a witness) at full depth for all five; three classical axioms.
...
threaded through the real forward, and each agrees on GPU to float rounding. The
one open gap: the
theorems are over $\mathbb{R}$, the GPU runs \texttt{float32}. Part~II's
bestiary is deliberately lighter --- decomposed into the same proved
primitives, not carried to whole-step closes. The book's official claim is
exactly this, no more and no less: \emph{every architecture in Part~I
trained end-to-end under one machine-checked scheme}.
...
inside \texttt{.train} (walked through in Chapter~\ref{chap:tensor},
``What's inside \texttt{.train}?'') --- data loading, mini-batching,
forward-pass MLIR generation, IREE compilation, gradient-descent
loop, evaluation loop, logging --- is already formalized in the
framework. The user-facing program is just ``specify the network,
specify the knobs, run.''
...
log-scale loss (\texttt{logs/ablation\_mlp-sgd.log}). Per-epoch training
loss falls $\sim$200$\times$ from the first epoch to below $10^{-3}$; final
test accuracy 98.57\%.

Chapter 1 (\texttt{MNIST 2D CNN}) runs the same \texttt{s4tfBaseline}
...
the verified renderer is parametric in the layer dimensions --- the same
\texttt{mlp\_has\_vjp} theorem covers every width, being polymorphic in
$d_0,d_1,d_2,d_3$ --- we can sweep the hidden width and train each point on
...
\addplot[blue, mark=*, mark options={fill=blue}] coordinates {
(8,91.42) (16,94.47) (32,96.18) (64,97.19) (128,97.48) (256,97.72) (512,97.82) (1024,97.89) (2048,97.95) (4096,98.03)
};
\addplot[only marks, mark=o, mark size=4pt, red, line width=1pt, forget plot] coordinates {(64,97.19)};
\node[anchor=west, font=\footnotesize, red!70!black] at (axis cs:74,95.5)
  {knee $\approx 64$: 97.2\% at 55K params};
\end{axis}
\end{tikzpicture}
\end{center}
...
(\texttt{runs/mlp\_grid\_results.tsv}). The curve flattens hard after
$\sim$64 neurons: widening $8{\to}64$ buys $+5.8$ points, but $64{\to}4096$
--- $64\times$ the width, $364\times$ the parameters --- buys only $+0.8$.
A $32{\to}32$ MLP already reaches 96.2\% at 26K parameters, $760\times$
smaller than the $4096{\to}4096$ net (98.0\%). The canonical
$512{\to}512$ sits comfortably on the plateau; anything past it is paying
for the third decimal place. Off the diagonal there is a cliff worth
noting: a $784{\to}8{\to}4096{\to}10$ net collapses to chance (9.8\%) ---
a narrow layer feeding a wide one starves the wide layer of signal and it
never trains under fixed-learning-rate SGD.

\section{MLIR: Dense}

\noindent\textbf{What is already proven.} \texttt{mlpForward} is the
three-dense-layer network; \texttt{mlp\_has\_vjp\_at} proves its
...
Here is exactly what the printer emits for that graph --- nothing
...
\texttt{selectPos}\,$p$ node --- the ReLU subgradient. The bridge
...
\begin{itemize}
  ...
  \item \textbf{Trusted surface.} The printer's faithfulness, IREE's
    lowering, and \texttt{float32} $\approx \mathbb{R}$ are tested, not
    proved; the full accounting is in Appendix~\ref{sec:verified_codegen}.
\end{itemize}
...
\section*{The rest of the network is Chapter 2}
...
\begin{theorem}[Conv2d weight VJP]
  ...
  \leanok
  \uses{ax:conv2d, ax:pdiv_add, ax:pdiv_mul, thm:pdiv_finset_sum, ax:pdiv_reindex}
...
(1,0.196587) (2,0.043851) (3,0.025374) (4,0.014667) (5,0.009847) (6,0.008109) (7,0.008859) (8,0.005455) (9,0.008056) (10,0.005157) (11,0.004191) (12,0.001768)
};
\addlegendentry{CNN}
\end{axis}
\end{tikzpicture}
...
\noindent MNIST, same SGD~0.1 / 12-epoch recipe, log-scale training loss
(\texttt{logs/ablation\_\{mlp,cnn-nobn\}-sgd.log}). The CNN converges
faster early (half the MLP's loss by epoch~2); both bottom out near $10^{-3}$
train loss, but the CNN generalizes better --- 98.98\% vs 98.57\% test.
...
(8,96.16) (16,98.30) (32,98.42) (64,98.86) (128,98.60) (256,98.87) (512,98.95) (1024,99.04) (2048,99.04) (4096,98.96)
};
...
Chapter~\ref{chap:mlp} (\texttt{runs/cnn\_grid\_results.tsv}). The whole CNN
curve sits about a point above the MLP's, and its knee arrives roughly
$4\times$ earlier: a 16-wide head already reaches 98.3\% at 110K
parameters, beating the widest 4096-wide MLP. So the 3.2M-parameter
$6272{\to}512$ layer is not buying accuracy --- shrinking the head to
$\ldots{\to}16{\to}16{\to}10$ keeps essentially all of it. The head is fat
because the flatten hands it a 6272-vector and nobody trimmed it, not
because the task needs the width; the only real drop is at $d=8$ (96.2\%),
where the $6272{\to}8$ bottleneck finally throws away features the
convolutions found. (On this GPU the wall-clock is convolution-bound, so
the shrink is a parameter win, not a speed win --- which is exactly why the
post-2015 fix was \texttt{globalAvgPool}, replacing the flatten rather than
narrowing it.)
...
network; \texttt{cnn\_has\_vjp\_at} proves its reverse-mode derivative by
chaining the per-layer VJPs --- the dense backward
...
(\texttt{conv2d\_has\_vjp3}, \S~\ref{ax:conv2d_has_vjp3}) --- through the
...
reversed-kernel identity \texttt{cnn\_has\_vjp\_at} only \emph{asserts},
discharged here by expansion at the concrete shape (and again at the
...
in-channel axes; \texttt{\%Wr} is the \texttt{reverse dims [2,3]} that
...
relu nodes of the full backward are bridged the same way --- the max-pool
...
to earn its keep, and the choice of optimizer starts to matter} --- and
the depth itself is what ResNet then scales to thirty-four layers. So
...
\item BN's payoff is \emph{speed}: a deeper network reaches the same
  accuracy in far fewer epochs with it than without. The example in
  \S\ref{sec:bn_cifar_example} measures that directly.
\end{itemize}
...
\section*{The inverse-stddev term}
...
\section*{The three-term backward}
...
the same conv/ReLU/max-pool machinery, but --- CIFAR-10 being a
genuinely harder problem --- stacks the convolutions four stages deep:
eight $3 \times 3$ convolutions in all. \textbf{Same head, four times
...
lets us ask a sharper question than ``does it train?'' --- \emph{what
governs how it trains?}

Two levers govern the answer --- \textbf{normalization} and the
\textbf{optimizer} --- and a third knob, the width of the head, turns
out not to. We move each against an \emph{identical, machine-checked
gradient}, holding everything else fixed (same architecture, same data
pipeline --- per-epoch shuffle, random horizontal flip, cosine
learning-rate schedule with warmup --- same 40 epochs). The findings,
stated up front so the graphs are no surprise:

\begin{itemize}
\item \textbf{Normalization buys speed.} BatchNorm (dropped between each
  conv and its ReLU) reaches a given accuracy in markedly fewer epochs;
  at convergence it lands level-to-slightly-above the un-normalized net.
  An accelerator first of all.
\item \textbf{The optimizer moves the ceiling most.} Trading plain SGD
  for SGD-with-momentum is worth three to four points of final accuracy
  --- more than anything else here. Adam is roughly a wash with plain
  SGD; its per-coordinate adaptivity earns little it can keep.
...
\noindent Every run below shares one proof-rendered backward pass; only
the BN layers or the rendered optimizer tail change. The numbers are
...
The diff: eight \texttt{.conv2d $\cdots$ .relu} layers become eight
\texttt{.convBn $\cdots$} layers. The dense head, the max-pools, and
the training config are all identical. Both nets are verified end to
end --- forward, gradient, and the rendered StableHLO training step
are machine-checked (\texttt{Proofs/CifarCNN.lean},
\texttt{Proofs/Cifar8Close.lean}) --- and both run through the same
IREE FFI path on the GPU.

\subsection*{Lever 1: normalization}

Fix the optimizer at plain SGD and toggle BN. Both nets train for 40
epochs on the shared pipeline; per-epoch test accuracy, from the
verified runs
(\texttt{runs/ablation\_cifar8/\{nobn,bn\}\_sgdsched.log}):
...
\addplot[blue, mark=*, mark options={fill=blue}] coordinates {
(1,36.51) (2,44.34) (3,50.14) (4,54.30) (5,60.84) (6,63.31) (7,62.39) (8,66.26) (9,66.06) (10,67.92) (11,69.88) (12,70.40) (13,71.39) (14,68.93) (15,71.61) (16,72.27) (17,72.36) (18,71.83) (19,73.11) (20,73.41) (21,73.02) (22,73.87) (23,73.72) (24,72.99) (25,73.69) (26,73.23) (27,73.66) (28,73.87) (29,74.08) (30,73.91) (31,73.85) (32,74.14) (33,74.22) (34,74.20) (35,74.24) (36,74.12) (37,74.28) (38,74.24) (39,74.15) (40,74.23)
};
\addlegendentry{with BN}
\addplot[orange, mark=*, mark options={fill=orange}] coordinates {
(1,33.90) (2,28.36) (3,34.98) (4,47.56) (5,51.55) (6,51.41) (7,57.98) (8,58.01) (9,58.40) (10,62.19) (11,62.64) (12,64.23) (13,65.03) (14,64.50) (15,64.94) (16,67.52) (17,68.46) (18,68.73) (19,68.52) (20,70.13) (21,70.42) (22,69.69) (23,70.82) (24,70.15) (25,70.32) (26,70.59) (27,71.44) (28,71.06) (29,71.97) (30,71.99) (31,71.61) (32,72.16) (33,72.26) (34,71.96) (35,72.34) (36,72.24) (37,72.40) (38,72.42) (39,72.39) (40,72.39)
};
...
through the verified renderer
(\texttt{runs/ablation\_cifar8w/\{nobn,bn\}.log}, the SGD phase).

\noindent BN leads from the second epoch --- 44\% to the bare net's
28\% --- and keeps the lead the whole way, finishing about two points
up (74\% vs 72\%); the un-normalized net traces the same arc roughly a
dozen epochs behind. Most of that is \emph{speed} (the curves are the
same shape, shifted left), but at this depth on plain SGD it is also a
small, real convergence margin --- the conditioning the three-term
...
ResNet's thirty-four layers. That margin is largest precisely for plain
SGD; under momentum and Adam (next) both nets already train well enough
that BN's lead all but vanishes.
...
pipeline and the same 40 epochs (final \% / best \%):
...
\hline
no BN  & 72.4 / 72.4 & \textbf{76.7 / 76.9} & 73.6 / 73.7 \\
BN     & 74.2 / 74.3 & \textbf{77.1 / 77.1} & 73.3 / 73.4 \\
\hline
\end{tabular}
\end{center}

\noindent Momentum wins outright --- three to four points over plain
SGD in both nets, and the best result on the board (BN${+}$momentum,
77\%). Plain SGD and Adam are a rough tie: Adam's per-coordinate
second-moment scaling earns nothing it can keep here, even with the wide
head's extra parameters to exploit. And reading \emph{down} the columns
recovers Lever~1 --- BN helps most under plain SGD ($\approx{+}1.8$) and
its margin shrinks to nothing under momentum and Adam, which already
condition the gradient well. Normalization, in other words, mostly
compensates for a weak optimizer; give it a good one and the depth
alone carries the net.

\medskip\noindent The Lever-1 graph drew this for the SGD column ---
fixed optimizer, BN toggled. The same per-epoch picture for the other
two columns, on the same axes, is what reading \emph{down} the table
looks like drawn out:
...
\addplot[blue, mark=*, mark options={fill=blue}] coordinates {
(1,43.86) (2,55.08) (3,60.85) (4,64.31) (5,67.17) (6,70.11) (7,71.45) (8,72.60) (9,72.77) (10,73.21) (11,74.21) (12,73.47) (13,74.56) (14,74.60) (15,75.14) (16,75.62) (17,75.57) (18,75.62) (19,76.66) (20,76.63) (21,76.23) (22,76.19) (23,75.88) (24,76.01) (25,76.41) (26,75.99) (27,76.85) (28,76.68) (29,76.70) (30,76.63) (31,76.65) (32,76.79) (33,76.80) (34,76.99) (35,77.04) (36,76.92) (37,76.99) (38,76.93) (39,77.01) (40,77.08)
};
\addlegendentry{with BN}
\addplot[orange, mark=*, mark options={fill=orange}] coordinates {
(1,39.65) (2,49.78) (3,54.17) (4,59.18) (5,64.09) (6,65.48) (7,67.74) (8,68.40) (9,67.12) (10,69.62) (11,70.27) (12,72.04) (13,71.82) (14,71.47) (15,71.36) (16,72.83) (17,73.02) (18,73.77) (19,73.16) (20,74.77) (21,75.10) (22,74.88) (23,75.34) (24,75.44) (25,76.24) (26,75.53) (27,76.10) (28,76.68) (29,76.68) (30,76.76) (31,76.85) (32,76.60) (33,76.69) (34,76.60) (35,76.78) (36,76.61) (37,76.55) (38,76.63) (39,76.77) (40,76.70)
};
\addlegendentry{no BN}
\end{axis}
\end{tikzpicture}
...
\addplot[blue, mark=*, mark options={fill=blue}] coordinates {
(1,33.63) (2,44.98) (3,51.91) (4,57.84) (5,60.84) (6,63.50) (7,65.32) (8,65.34) (9,67.08) (10,68.37) (11,68.88) (12,69.65) (13,70.75) (14,70.49) (15,72.12) (16,72.41) (17,71.72) (18,72.01) (19,71.86) (20,72.54) (21,72.78) (22,73.29) (23,72.56) (24,73.19) (25,72.32) (26,73.04) (27,73.30) (28,73.20) (29,73.15) (30,73.21) (31,73.36) (32,73.30) (33,73.42) (34,73.37) (35,73.38) (36,73.29) (37,73.40) (38,73.41) (39,73.36) (40,73.35)
};
\addlegendentry{with BN}
\addplot[orange, mark=*, mark options={fill=orange}] coordinates {
(1,40.87) (2,47.06) (3,53.37) (4,58.26) (5,59.70) (6,61.16) (7,63.41) (8,65.70) (9,65.20) (10,68.51) (11,68.28) (12,67.93) (13,69.26) (14,70.59) (15,70.54) (16,71.65) (17,72.09) (18,71.44) (19,71.59) (20,73.10) (21,73.06) (22,73.27) (23,73.46) (24,73.00) (25,72.68) (26,73.11) (27,72.95) (28,73.45) (29,73.58) (30,73.21) (31,73.44) (32,73.51) (33,73.58) (34,73.70) (35,73.45) (36,73.66) (37,73.52) (38,73.52) (39,73.57) (40,73.57)
};
\addlegendentry{no BN}
\end{axis}
\end{tikzpicture}
...
\noindent Same wide 8-conv net, same shared pipeline, same 40 epochs ---
only the optimizer changes from the SGD panel above
(\texttt{runs/ablation\_cifar8w/\{nobn,bn\}.log}, the momentum and AdamW
phases). Under \textbf{momentum} BN still takes the early lead --- 44\% to
40\% at epoch~1, and a visible margin through the first dozen epochs --- but
the un-normalized net pulls level around epoch~28 and both finish at 77\%,
BN ahead by only a couple of tenths. Under \textbf{AdamW} even that early
gap is gone: the two curves are tangled from the first epoch and the no-BN
net actually ends a hair in front ($73.6$ vs $73.4$), because Adam's
per-coordinate second-moment scaling already conditions the step that BN
would otherwise smooth. Read top-to-bottom --- SGD, then momentum, then
Adam --- the blue--orange gap collapses panel by panel: the same
``BN helps most under a weak optimizer'' story the table tells in two
columns of numbers, now drawn epoch by epoch.
...
time we ran this, the optimizers looked far more separated --- momentum
and Adam beat SGD by some ten points. That gap was mostly an artifact:
...
Holding the pipeline genuinely fixed --- the table above --- collapsed
...
\emph{not} catch: the math was correct in every run; the experiment
...
params, whole net down to $53{,}000$) and the same six runs land within
a point either way --- best-of-board $77.1\%$ wide vs $77.2\%$ narrow,
with the optimizer ordering unchanged. Seven times the parameters buys
no accuracy, and only about $1.4\times$ the wall-clock per epoch
(${\approx}14$ vs ${\approx}10$ seconds on a gfx1100): the eight
convolutions over the $32 \times 32$ maps dominate the compute, while
the head --- big as it is --- is cheap matmul. That is the lesson the
...
budget on \emph{more convolution} --- thirty-four layers of it --- not a
bigger head.
...
essentially flat --- everything from $d{=}16$ to $d{=}4096$ lands inside a
...
points below the 40-epoch board above --- this is a 25-epoch single-optimizer
sweep --- but the \emph{shape} is the point.)

\subsection*{Why the levers work}

Both levers do the same underlying thing --- they make each step's
gradient a more reliable guide to the next --- by attacking different
sources of noise. Each layer is tuned for the distribution of its
...
and reliable progress compounds across epochs --- precisely what the
curve and the table measure. (On larger networks, and with true
batch-statistic normalization, BN's effect is stronger still --- it
opens up learning rates that diverge without it --- but at this depth,
what we can honestly measure and verify is the speed, so that is the
claim we make here.) That is why every image architecture since 2015
bakes a normalization layer in by default, and why the optimizer is the
first knob any practitioner reaches for: between them they decide not
whether the net can learn this data, but how many passes it takes.
...
$\varepsilon > 0$ to stay differentiable --- the single place in the book
where the math reaches past chain-sum-product into real analysis.

\medskip\noindent\textbf{The gap, and how we close it.} The three-term
backward is not elementwise --- the two $\sum_k$ reductions couple every
coordinate to every other. The emitted backward graph is given a denotation
in the proofs' own vector type, and
...
printer emits (the forward-statistic recompute --- mean, variance,
\texttt{rsqrt}, normalize --- elided to its one comment line; $n=4$):
...
backward $\gamma\,dy$; each \texttt{reduce} along \texttt{dimensions = [1]}
...
$\mathrm{istd}/n$ and not $\mathrm{istd}\,\gamma/n$ --- the same formula,
$\gamma$ pulled inside the parenthesis. The bridge theorem is precisely the
claim that this text computes \texttt{bn\_has\_vjp}'s backward.
...
the normalization sitting inside the residual, depthwise, and ConvNeXt blocks
...
post-add, giving \texttt{\%dadd}), and then --- this is the fan-in --- sends
...
\medskip\noindent\textbf{Caveats.}
\begin{itemize}
  \item \textbf{The fan-in \texttt{add} is unconditional} --- addition is
  ...
  \item \textbf{Representative scale} --- a two-channel block at $4\times4$;
    the same \texttt{\%dadd} fan-in repeats for all sixteen blocks.
\end{itemize}
...
epochs. The headline ResNet result --- 21.8M parameters reaching
74.4\% top-1 on full 1000-class ImageNet --- comes from a different
recipe: 1.28M training images, 90 epochs, paper-recipe SGD with
momentum. The architecture is unchanged; only the dataset, the
output head width, and the training schedule differ. Here's what
the loop looks like:
...
Today, the phase-2 JAX path (\texttt{jax/MainResnetImagenet.lean},
runs via \texttt{lake build resnet34-imagenet}) wires
\texttt{.imagenet} through tfds streaming. The phase-3 IREE path
shown in the snippet above still needs a C-side streaming reader
for the 1.28M-image training set (Imagenette's 9.5K fits in a
single \texttt{ByteArray}; ImageNet doesn't) --- until that lands,
the phase-3 dispatcher panics with a ``use phase 2'' note when
handed \texttt{.imagenet}. What's worth noting is that everything
else --- the architecture, the \texttt{.train} invocation, the
recipe knobs --- is identical across both paths; only the
data-loading layer differs. The framework isn't doing anything
fundamentally different at ImageNet scale; it's just the data
that's bigger.

\textbf{Compute budget.} The phase-2 (Lean$\to$JAX) path trains
\texttt{resnet34Imagenet} on two AMD Radeon RX 7900 XTX GPUs
(consumer hardware: gfx1100, ROCm 7.2, JAX 0.10.0, multi-GPU
sharding via \texttt{Mesh + NamedSharding(P('batch'))} with RCCL
preloaded). At batch 256 (128 per device), one epoch over the
1.28M-image training set runs in about 24 minutes of compute plus
a 10-second val pass; a 30-epoch run finishes in roughly 15
wall-clock hours including first-epoch JIT compile. Final result
on the standard 50K-image validation split is $\mathbf{69.26\%}$
top-1 (val\_loss $1.232$). The parameter checkpoint
(\texttt{jax\_r34\_imagenet.bin}, 87\,MB, 37 param groups) is
written via the same \texttt{params\_to\_file} emit that phase-3
IREE consumes --- so the JAX-trained weights drop into a phase-3
\texttt{TrainConfig.bootstrapBackbone} field byte-for-byte without
a separate conversion step.

The recipe trims the paper's 90 epochs to 30 (SGD with momentum
$0.9$, batch 256, cosine LR with a 5-epoch warmup from peak $0.1$,
label smoothing $0.1$, Inception-style random-resized-crop
(sampling $8$--$100\%$ of the image area at a $\frac{3}{4}$--$\frac{4}{3}$
aspect ratio, then resizing to $224\times224$) plus a horizontal
flip, $\ell_2$ weight decay $10^{-4}$) --- a deliberate trade for
roughly $95\%$ of the paper's $\sim$73\% headline at $\sim$1/3 the
compute.

\textbf{bf16 and the full 90-epoch run.} Bf16 mixed
precision roughly halves the wait. On a second box (four usable
NVIDIA RTX 4060 Ti, CUDA/cuDNN), the same \texttt{resnet34Imagenet}
spec with \texttt{bf16 := true} casts both the matmuls and the
convolutions to bfloat16 while keeping fp32 master weights. NVIDIA's
tensor cores run bf16 convolution about $1.6\times$ faster than
fp32 --- the opposite of AMD's MIOpen, where bf16 conv is a wash,
which is why conv precision is gated behind its own
\texttt{bf16Conv} flag (matmul-only bf16 helps transformers on
either vendor; bf16 conv only pays off on cuDNN). The wall-clock
difference over a full 90-epoch run, batch 256 across the four
GPUs:
...
\hline
2$\times$ 7900 XTX (ROCm) & fp32 & $\sim$24.2 min & 30 & $\sim$15 hr & $\mathbf{69.26\%}$ & --- \\
4$\times$ 4060 Ti (CUDA)  & fp32 & $\sim$16.8 min & 90 & $\sim$25 hr & --- & --- \\
4$\times$ 4060 Ti (CUDA)  & bf16 & $\sim$10.5 min & 90 & $\sim$16 hr & $\mathbf{72.02\%}$ & $\mathbf{90.62\%}$ \\
\hline
\end{tabular}
\end{center}

\noindent ResNet-34 / ImageNet-1k: per-epoch throughput and final
validation across the two GPU sets (ROCm at 30 epochs, CUDA at the
full 90).

The bf16 run finished all 90 epochs in about 16 wall-clock hours
and reached $\mathbf{72.02\%}$ top-1 / $\mathbf{90.62\%}$ top-5 on
the full 50{,}000-image validation split --- squarely on the
paper's ResNet-34 mark, in bf16, on consumer GPUs. (fp32 was
measured at the matched $1.6\times$-slower throughput but not run
to completion; bf16 does not cost final accuracy here.) The
$1.6\times$ figure tracks a microbenchmark of the bare conv
operator at this network's stage-1 shape almost exactly, so the
speedup is the convolution itself going through tensor cores, not
a scheduling artifact.

The validation curve over the run tells the usual story: a fast
climb through the warmup and high-LR phase, a long shallow middle,
and a final lift as the cosine schedule anneals the learning rate
to zero.
...
\noindent ResNet-34 / ImageNet-1k validation accuracy per epoch
(bf16, 4$\times$ 4060 Ti). The kink near epoch~5 is the end of
warmup; the steady late climb is the cosine anneal.

\section*{A reasonable stopping point}
...
\begin{itemize}
  ...
  \item Skip Ch~\ref{chap:bn} (BatchNorm). It's structurally the
...
Reading through Ch 5 is enough to know the framework works. For a
reinforcement-learning detour, read the AlphaGo Zero paper
(\hyperref[bestiary:alphagozero]{bestiary entry}), which combines
reinforcement learning with a large residual network of $3\times 3$
convolutions.

\paragraph{Or, for breadth.} If you want to cover most of modern
deep learning instead of stopping short, the fuller path is to
keep reading through Ch~\ref{chap:attention} (ViT) for the full
image-classification arc and then run the trainers in
\texttt{demos/}:
\begin{verbatim}
lake exe yolov1-pets-train-bootstrap # detection (YOLO on Pets, cat/dog)
...
\end{verbatim}
The arc from ViT to TinyGPT in particular reuses the attention
proofs from Ch~\ref{chap:attention} verbatim --- image-token
attention and character-token attention are the same operator,
just at different token alphabets. Image classification + detection
+ segmentation + diffusion + NLP, all on the same Lean$\to$MLIR$\to$IREE
pipeline, covers most of what people mean by ``modern deep learning.''

\section*{Side quest: ResNet-50, RSB-A3 (100 epochs)}
...
same proven-backward bottleneck codegen it reached $\mathbf{77.22\%}$
top-1 / $\mathbf{93.34\%}$ top-5 at 100 epochs --- within $0.9$ points
of the paper's $78.1\%$, reproducing (and slightly beating) an earlier
$76.66\%$ / $93.03\%$ run to within run-to-run noise.

\textbf{Compute + the BN-sharding axis.} The run trained on the same
four usable NVIDIA RTX 4060 Ti (CUDA, bf16 matmul and convolution) as
the ResNet-34 bf16 run, at effective batch 2048. Four 16\,GB cards
cannot hold that batch on one device, so it is split $4\times512$ across
the GPUs and accumulated $4\times$ to reach 2048 --- which means
BatchNorm normalizes over each $512$-image micro-batch (``Ghost-BN''),
not the full $2048$. A single $\sim$80\,GB A100 holds the whole batch,
so the \texttt{true-2048} recipe (no gradient accumulation) normalizes
BatchNorm over all $2048$ images at once. Same network, same effective
batch --- the only difference is the BN statistic group, which is the
axis this comparison isolates:

\begin{center}
\begin{tabular}{l l r r r r r}
\hline
GPU & BN regime & Per epoch & Epochs & Total & Val top-1 & Val top-5 \\
\hline
4$\times$ 4060 Ti (CUDA, bf16)   & Ghost-BN (512)   & $\sim$9.1 min & 100 & $\sim$17 hr & $\mathbf{77.22\%}$ & $\mathbf{93.34\%}$ \\
1$\times$ A100 (bf16, true-2048) & full-batch (2048) & $\sim$5.4 min\textsuperscript{\dag} & 100 & $\sim$9 hr\textsuperscript{\dag} & TBD & TBD \\
\hline
\end{tabular}
\end{center}

\noindent ResNet-50 / RSB-A3, 100 epochs: the 4-GPU run splits the
batch and normalizes BatchNorm per $512$-image shard; the A100
\texttt{true-2048} row (to be run) normalizes over the full $2048$. The
$\sim$17\,hr wall-clock includes three 30-minute thermal duty-cycle
cooldowns (the 4060 Ti box is duty-cycled on long runs); the run itself
logged zero PCIe/AER interruptions across all 100 epochs.
\textsuperscript{\dag}\,A100 timing is an estimate ($\sim$9\,hr,
single-device, no gradient accumulation); its accuracies are pending
the run.

\textbf{What the A100 row tests.} The residual $\sim$0.9 points to the
paper's $78.1\%$ is expected to come mostly from the
gradient-accumulation Ghost-BN regime (each micro-step normalizes over
$512$, not the full $2048$) plus LAMB implementation details and bf16.
The \texttt{true-2048} recipe (\texttt{resnet50-imagenet true-2048}, a
real single-forward batch 2048 on an $\sim$80\,GB card) isolates the BN
piece: a single-device run over the full batch removes the Ghost-BN
approximation entirely, so the top-1 gap between the two rows is a
direct read-out of what BatchNorm sharding costs. \emph{Changed since
the $76.66\%$ first run:} RandAugment is now timm-literal
(\texttt{rand-m6-mstd0.5-inc1}).

\subsection*{What A3 changes from the 2018 recipe}

The ResNet-34 trainer above is a \emph{2018-era} recipe: the original
paper's SGD-with-momentum plus the ``bag of tricks'' polish (cosine
schedule, warmup, label smoothing, random-resized-crop). RSB-A3 is the
\emph{2021} ``ResNet Strikes Back'' recipe (Wightman et al.). The
architecture aside (basic block vs.\ bottleneck), every training knob
that A3 sets differently is one lever of the three-year jump in how
these networks are trained. The backbone code and the \texttt{.train}
entry point are identical; only the \texttt{TrainConfig} differs. Here
is the full diff:

\begin{center}
\begin{tabular}{@{}l l l p{5.7cm}@{}}
\hline
Knob & ResNet-34 (2018) & RSB-A3 (2021) & What the A3 choice buys \\
\hline
Backbone      & basic block, 21.8\,M   & bottleneck, 25.6\,M          & deeper net at similar cost; $1\times1$--$3\times3$--$1\times1$ bottleneck \\
...
BN statistics & global over batch      & Ghost-BN over 512             & side-effect of grad-accum (each micro-step normalizes its own 512); the A100 \texttt{true-2048} row removes it \\
\hline
\end{tabular}
\end{center}

\noindent What carries over unchanged: cosine LR decay, the 5-epoch
warmup, random-resized-crop + horizontal flip as the base augmentation,
bf16 matmul and convolution, and running-BN evaluation. The through-line
...
\begin{theorem}[Depthwise weight VJP]
  ...
  \leanok
  \uses{ax:depthwiseConv2d, ax:pdiv_add, ax:pdiv_mul, thm:pdiv_finset_sum, ax:pdiv_reindex}
...
\texttt{.invertedResidual} blocks (17 in all, widths $16$ to $320$,
expansion $t{=}6$ except the first), a $1 \times 1$ lift to 1280
channels, global average pool, and a dense head --- and \emph{no}
...
magnitude reduction in parameter count.
...
pointwise convs, BatchNorm, and relu6; \texttt{mobilenetv2\_has\_vjp\_at}
chains the whole network through \texttt{vjp\_comp\_at}.
...
and out-channel axes before reversing; this one has \emph{no transpose}. The
\texttt{reshape} only adds the singleton input-channel axis the grouped form
expects ($[c,k,k] \to [c,1,k,k]$), the \texttt{reverse} flips the two
spatial axes, and \texttt{feature\_group\_count = 2} --- equal to the channel
count --- tells the convolution to run each channel through its own filter,
no mixing. That single attribute \emph{is} the depthwise structure; the
bridge theorem is the claim that this graph computes
\texttt{depthwise\_has\_vjp3}'s backward.

\medskip\noindent\textbf{Caveats.}
\begin{itemize}
  \item \textbf{The depthwise convolution bridge is unconditional} --- it is
    linear.
...
schedule grow. The spec mirrors
\texttt{jax/MainMobilenetV2Imagenet.lean}:
...
decay also drops to $4\times10^{-5}$ --- MobileNet's depthwise
filters are tiny ($9$ weights per channel), and the $10^{-4}$ that
suits ResNet over-regularizes them.

\textbf{Compute budget.} On six RTX 4060 Ti (CUDA, bf16, batch
$252$ across the six GPUs), steady-state throughput is
$\sim$89\,ms per step, about $7.5$ minutes per epoch --- so the full
90-epoch schedule completed in roughly 11 wall-clock hours. Even at
this throughput the wall-clock win over the $9.5\times$-larger ResNet-34 is
modest: depthwise convolution is memory-bound and
low-arithmetic-intensity, so MobileNet's order-of-magnitude FLOP
saving does not translate into a proportional wall-clock advantage on
this hardware.
...
\noindent The full 90-epoch run --- now on MobileNetV2's native
RMSProp ($\rho=0.9$, $\mu=0.9$, $\varepsilon=1.0$) with crop/flip
augmentation, the paper recipe --- completed at $\mathbf{68.77\%}$
top-1 / $\mathbf{88.53\%}$ top-5 on the full 50{,}000-image
validation split (EMA weights). That is only $+0.44$ over the earlier
SGD run at the same schedule ($68.33\%$ / $88.17\%$): swapping in the
paper's optimizer barely moved the needle, so the remaining few points
to the $\sim$71\% paper number are the \emph{schedule} --- the paper
trains far longer with RMSProp's exponential-decay LR and a heavier
augmentation stack --- not the optimizer, precision, or pipeline. The
RMSProp peak LR of $0.045$ took off cleanly with no collapse: the
large $\varepsilon=1.0$ keeps the adaptive step well-damped (in sharp
contrast to EfficientNet's $\varepsilon=10^{-3}$ in
Chapter~\ref{chap:se}, which is far more LR-sensitive), so
the $\sim$0.02 fallback was never needed. The validation curve has the
usual shape --- a fast warmup climb, a long middle, and a final lift
as the cosine schedule anneals to zero:
...
\addplot[blue, mark=*, mark options={fill=blue}] coordinates {
(5,34.43) (6,39.02) (7,42.76) (8,45.35) (13,52.59) (14,53.58) (15,53.83) (16,54.63) (21,56.73) (22,57.33) (23,57.39) (24,57.80) (29,58.78) (30,59.21) (31,59.71) (32,60.04) (37,60.77) (38,61.47) (39,61.20) (40,61.67) (45,61.99) (46,62.73) (47,62.80) (48,62.88) (53,63.73) (54,64.23) (55,64.22) (56,64.65) (61,65.26) (62,65.51) (63,65.44) (64,65.87) (69,66.64) (70,66.76) (71,67.16) (72,67.37) (75,67.60) (84,68.61) (85,68.75) (86,68.73) (87,68.84) (88,68.86) (89,68.84) (90,68.83)
};
\addlegendentry{top-1}
\addplot[orange, mark=*, mark options={fill=orange}] coordinates {
(5,60.54) (6,65.37) (7,68.80) (8,71.29) (13,77.44) (14,78.10) (15,78.59) (16,78.85) (21,80.74) (22,80.87) (23,81.23) (24,81.70) (29,82.27) (30,82.46) (31,82.71) (32,83.10) (37,83.59) (38,83.99) (39,83.70) (40,84.19) (45,84.49) (46,84.83) (47,84.94) (48,84.98) (53,85.58) (54,85.64) (55,85.64) (56,86.07) (61,86.45) (62,86.52) (63,86.65) (64,86.85) (69,87.27) (70,87.47) (71,87.56) (72,87.67) (75,87.84) (84,88.51) (85,88.53) (86,88.62) (87,88.60) (88,88.64) (89,88.63) (90,88.63)
};
\addlegendentry{top-5}
\end{axis}
\end{tikzpicture}
...
\noindent MobileNetV2 / ImageNet-1k validation accuracy per epoch
(bf16, 6$\times$ 4060 Ti, RMSProp).

\textbf{[TODO --- run w/ config.]} \texttt{mobilenet-v2-imagenet} full
tier (recipe arg \texttt{full}), \textbf{350 epochs}. \emph{Changed
since the $68.77\%$ above:} label smoothing $0.1\to0.0$ (the MobileNetV2
paper used none; the number above predates the fix), and the schedule
goes $90\to350$. The remaining $\sim$3 points to the
paper's $72.0\%$ are schedule, not recipe: MobileNetV2 was trained with
RMSProp under an exponential LR decay ($\sim$0.98 per epoch) for several
hundred epochs, where the 90-epoch tier here is a deliberately short
validation tier. The same holds across the family --- EfficientNet-B0
($\sim$77\%) and ConvNeXt-T ($\sim$82\%) both assume $\sim$300--350-epoch
schedules where their heavier augmentation (AutoAugment;
Mixup/CutMix/RandErase) finally pays off rather than under-training. A
dedicated paper-faithful pass --- each net at its full schedule on the
six-GPU box ($\sim$35\,hr apiece) --- is left as future work to close
these gaps.
...
\texttt{.mbConv} blocks (kernel sizes mixing $3 \times 3$ and
...
terms --- each piece of the gate carrying the swish, sigmoid, and dense
bridges of the previous chapters. Here is the fan-in, with the excite
...
\noindent Read it against the product rule: \texttt{\%gdleft} is
$g(x)\odot dy$, the term a naive generator would stop at; \texttt{\%gdgate}
reduces $x \odot dy$ over the spatial axes --- the adjoint of the gate's
broadcast, i.e.\ the cotangent on the per-channel gate values --- which then
flows back through the excite MLP and the global-average-pool to
\texttt{\%gdgate\_sp}; and \texttt{\%gdds = \%gdleft + \%gdgate\_sp} adds the
two. That final \texttt{add} \emph{is} the product rule: the gradient that
...
\begin{itemize}
  \item \textbf{The SE fan-in carries no kink condition} --- swish and
    sigmoid are smooth everywhere; the only smooth-point caveats in an
...
MBConv stack stays put, the head widens to 1000, the schedule grows.
...
Second --- a correctness note worth surfacing --- the SE bottleneck
is sized off the block's \emph{input} channels ($c/4$), not the
$6\times$-larger expanded width. An earlier version of the codegen
sized it off the expanded width and inflated B0 from its canonical
$5.3$M parameters to $8.4$M; sizing the squeeze correctly restores
the faithful $\mathbf{5.29}$M-parameter B0.

\textbf{Augmentation.} Like MobileNet-V2, the run uses base aug only:
Inception-style random-resized-crop (sampling 8--100\% of the image
area at a 3/4--4/3 aspect ratio, resized to $224\times224$) plus a
random horizontal flip, with label smoothing $0.1$ in the loss;
validation center-crops. The heavier pack EfficientNet's headline
recipe leans on --- Mixup, CutMix, RandAugment, Random Erasing --- is
wired into the phase-2 Lean$\to$JAX trainer (emitted into the generated
\texttt{tf.data} pipeline, each knob gated on a config flag, RandAugment in
a color-only or full geometric variant, plus AutoAugment) but left off at
this validation tier.

\textbf{Compute budget.} This was the first run on all \emph{six} RTX
4060 Ti (CUDA, bf16, batch 252 = 6$\times$42), steady-state
$\sim$90\,ms per step, about $8.0$ minutes per epoch --- so the
80-epoch run took $\sim$10.5 wall-clock hours. Six GPUs only buy
$\sim$1.17$\times$ over four here (the same B0 ran $\sim$102\,ms/step
on four): with the model held at batch 256, the bottleneck moves off
the GPUs and onto the host data pipeline, which cannot decode and
augment fast enough to keep six cards fed (they sit at ${\sim}65\%$
utilization). It is not an interconnect limit --- the gradient
all-reduce is a couple milliseconds of a $\sim$90\,ms step.
...
\hline
6$\times$ 4060 Ti & 80 & $\sim$8.0 min & $\sim$10.5 hr & $\mathbf{72.31\%}$ & $\mathbf{90.31\%}$ \\
6$\times$ 4060 Ti & 300 & $\sim$8.0 min & $\sim$40 hr & --- & --- \\
\hline
\end{tabular}
\end{center}

\noindent (CUDA, bf16. The 300-epoch row is the projected wall-clock
for the full-recipe run; accuracy pending.) The 80-epoch run reached
$\mathbf{72.31\%}$ top-1 / $\mathbf{90.31\%}$ top-5 on the full
50{,}000-image validation split --- the strongest result in this
book's sweep, edging ResNet-34's $72.0\%$ at roughly a quarter of the
parameters ($5.3$M vs $21.8$M). It still sits under EfficientNet's
headline ${\sim}77\%$, which comes from a far longer schedule with
RMSProp, AutoAugment, stochastic depth, and EMA. Of those, EMA (decay
0.9999) and stochastic depth (drop-connect 0.2) are on here, costing
${<}5\%$ per epoch; RMSProp, AutoAugment (both now wired), and the
300-epoch length remain the gap to the paper number --- though the
RMSProp attempt proved unexpectedly delicate, discussed below. The
validation curve:
...
\textbf{RMSProp re-run --- a cautionary result.} EfficientNet's native
optimizer is RMSProp ($\rho=0.9$, $\mu=0.9$, $\varepsilon=10^{-3}$),
and re-running B0 with it (crop/flip held fixed, to isolate the
optimizer) exposed a learning-rate sensitivity absent from
MobileNet-V2's RMSProp. At the MobileNet-style peak
$\mathrm{lr}=0.045$ the run \emph{diverged}: once warmup reached the
peak near epoch 5, the loss climbed and top-1 collapsed to
$\sim$0.5\% (random) by epoch 6. Dropping to $\mathrm{lr}=0.016$ ---
the paper's linear-scaled value ($0.256$ at batch $4096 \to 0.016$ at
batch $256$) --- did not diverge but \emph{eroded}: top-1 peaked at
$\sim$31\% near epoch 4 (where $\mathrm{lr}\approx0.013$) then decayed
to $\sim$19\% and stalled, the slow cosine holding the rate near its
$0.016$ peak for $\sim$10 epochs while the model could not recover.
Training stayed clean only up to $\mathrm{lr}\approx0.013$, so the
usable peak is $\sim$0.01 or below. The culprit is the small
$\varepsilon=10^{-3}$: where MobileNet-V2's $\varepsilon=1.0$ damps the
adaptive step (the contrast flagged in the MobileNet-V2 recipe), B0's
tiny denominator lets the same nominal LR explode.

\textbf{[TODO --- run w/ config.]} \texttt{efficientnet-b0-imagenet}
full tier (recipe arg \texttt{full}), \textbf{350 epochs}, RMSProp
peak $0.01$ $+$ AutoAugment $+$ stochastic depth. \emph{Changed:} the
shipped \emph{default} LR is now the paper base $0.016$ (was an arbitrary
divergent $0.045$); the full tier keeps the stable $0.01$ because our
$\varepsilon{=}10^{-3}$ stack erodes at $0.016$ (above). AutoAugment,
like the schedule, under-trains at 80 epochs and only pays off long.
Until this pass runs ($\sim$40\,hr on the six-GPU box), the SGD 80-epoch
$72.31\%$ above stands as the sweep's B0 number; the faithful headline is
$\sim$77\%.
...
depths $3, 3, 9, 3$), glued by \texttt{.convNextDownsample} transitions
...
$\mathrm{layerScale}(\gamma, x) = \gamma \odot x$ --- the simplest operator in
this series. Its Jacobian is diagonal, so it is its own adjoint:
...
nothing to bridge past that identity --- the emitted graph is two
...
and, uniquely in this book, every one is unconditional (the only hypothesis
...
\begin{itemize}
  ...
  \item \textbf{Random Erasing and RandAugment at M=9 are in the
    same tier.} $+0.7\%$ and $+0.5\%$ respectively; both at the
    edge of seed noise. M=9 is too aggressive for our 9.5K-image
    scale (the paper trained on 1.2M images), and erasing 25\% of
    pixels is in the same noise band.
  \item \textbf{Mixup actively hurts at this scale.} $-1.5\%$
    below bare. The blended-label gradient signal is too aggressive
    at $\sim$475 images per class; the model can't extract a clean
    target from a Beta(0.8, 0.8) mix of two images when each class
    has so few exemplars. A Mixup ablation on full ImageNet
    (1.28M images, $\sim$1280 per class) typically lifts $+0.5$ to
    $+1.0$; here we're 100$\times$ below that data scale.
\end{itemize}

One seed per cell. The $+0.5\%$ and $+0.7\%$ deltas on RandAugment
and Random Erasing are within noise; the $+2.9\%$ delta from CutMix
and the $-1.5\%$ delta from Mixup are well above. The cross-
architecture confirmation with Ch 9's ViT-Tiny ablation (CutMix
$\gg$ RandAug at the same data scale) suggests the ranking is
data-regime-driven, not architecture-driven.

\section{ImageNet recipe}

\imagenetphasenote

ConvNeXt's architecture first reached this book through the phase-3
IREE backend --- the Imagenette example earlier in this chapter. The
phase-2 port wires the two ConvNeXt-specific layers
(\texttt{.convNextStage},
...
had to be written --- the backward comes for free. The spec mirrors
...
The two regularizers core to ConvNeXt's 300-epoch recipe ---
\textbf{stochastic depth} (drop-path 0.1, the paper value) and
\textbf{EMA} (weight averaging, decay 0.9999) --- are now wired and on
(\texttt{dropPath} and \texttt{useEMA} above), and cost ${<}5\%$ per
epoch. The patchify stem is now faithful (\texttt{.convNextStem}: $4\times4$
stride-4 conv $\to$ channel-LayerNorm, no BN/ReLU), so no architectural
deviation from the paper remains. The augmentation pack --- Mixup, CutMix,
Random Erasing, and a full color$+$geometric RandAugment --- is wired into
the phase-2 Lean$\to$JAX trainer and now \emph{on} in the config; the
$75.93\%$ run below predates it (base RandAugment only), and its 80 epochs
are the validation tier of an 80$\to$300 ladder.

\textbf{Compute budget.} ConvNeXt-T is the heaviest of the three
($28.6$M parameters --- the canonical count, verified by an
init/forward/backward/save/reload round-trip). Run on all \emph{six}
RTX 4060 Ti (CUDA, bf16, batch 252 = 6$\times$42), steady-state
throughput is $\sim$143\,ms per step, about $12.6$ minutes per epoch
--- the most expensive per-epoch of any net in this book, driven by
the $7\times7$ depthwise and the channel LayerNorms. Unlike the
lighter EfficientNet, ConvNeXt gets a healthy $\sim$1.27$\times$ from
four to six GPUs (it ran $\sim$185\,ms/step on four): being
compute-bound, it keeps the cards near $100\%$ utilization rather than
starving on the host data pipeline, so more of the added GPUs
actually turns.
...
GPU & Epochs & Per epoch & Wall-clock & Val top-1 & Val top-5 \\
\hline
6$\times$ 4060 Ti & 80 & $\sim$12.6 min & $\sim$15.5 hr & $\mathbf{75.93\%}$ & $\mathbf{92.27\%}$ \\
6$\times$ 4060 Ti & 300 & $\sim$12.6 min & $\sim$63 hr & --- & --- \\
\hline
\end{tabular}
\end{center}

\noindent (CUDA, bf16. The 300-epoch row is the projected wall-clock;
accuracy pending.) The 80-epoch run reached $\mathbf{75.93\%}$ top-1 /
$\mathbf{92.27\%}$ top-5 on the full 50{,}000-image validation split
--- the strongest result in this book's sweep, ahead of EfficientNet-B0
($72.3\%$) and ResNet-34 ($72.0\%$). The modern recipe earns it:
AdamW, LayerScale, stochastic depth, and EMA are all in; the remaining
gap to ConvNeXt-T's $\sim$82\% headline is the 300-epoch schedule plus
the full augmentation pack (now wired, see below), not the architecture.
The validation curve --- note the slow start,
an artifact of the EMA shadow (decay $0.9999$) lagging the live
weights for the first few epochs, then catching up:
...
\noindent ConvNeXt-T / ImageNet-1k validation accuracy per epoch
(bf16, 6$\times$ 4060 Ti).

\textbf{Full-augmentation smoke test --- config validated.} The run
above used base RandAugment only; turning on the rest of the headline
pack (Mixup $\alpha{=}0.8$, CutMix $\alpha{=}1.0$, Random Erasing
$p{=}0.25$) was a chance to check whether ConvNeXt's AdamW recipe is as
learning-rate-robust as EfficientNet's RMSProp
(Chapter~\ref{chap:se}) proved fragile. It is: at
$\mathrm{lr}=4\mathrm{e}{-}4$ with global-norm gradient clipping at
$1.0$, top-1 climbed cleanly through the warmup peak ---
$0.3\%\to2.8\%$ over epochs 1--3 (the heavy pack under-trains early),
reaching $\sim$40\% by epoch 9 --- with no erosion or divergence, the
opposite of B0's RMSProp blow-up. AdamW's bounded, decoupled step plus
the grad-clip makes the $4\mathrm{e}{-}4$ peak a non-event. (The smoke
test was cut short at epoch 9 by a wall-power outage, not anything in
the run itself.)

\textbf{[TODO --- run w/ config.]} \texttt{convnext-tiny-imagenet} full
tier (recipe arg \texttt{full}), \textbf{300 epochs}. \emph{Changed
since the $75.93\%$ above:} the full augmentation pack (Mixup
$\alpha{=}0.8$, CutMix $\alpha{=}1.0$, Random Erasing $p{=}0.25$) $+$
geometric RandAugment (timm-literal \texttt{rand-m9-mstd0.5-inc1}) now on;
weight decay now excludes norm/bias/LayerScale (\texttt{wdExcludeNormBias});
LR $4\mathrm{e}{-}4\to2.5\mathrm{e}{-}4$ (linear-scaled). Mixup and CutMix
are long-schedule regularizers --- at 80 epochs they under-train, so the
full pack only pays off over the 300-epoch run ($\sim$63\,hr on the
six-GPU box, the projected row in the table), toward the $\sim$82\% paper
number.
...
\section*{Attention in one equation}
...
--- two residual sub-blocks, attention then MLP, each pre-normed.
...
Three layers. The entire model body is
...
need gentler optimization schedules --- the attention
...
head; the SDPA backward that produces $dQ,dK,dV$ elided to its comment):
...
and so on, contracting the model axis); the two \texttt{add}s sum the three.
This is the residual fan-in of Chapter~\ref{chap:residual} with three
branches instead of one --- attention reads the input thrice, so the gradient
returns thrice and adds. The softmax-weighted $dQ,dK,dV$ that feed the
...
every operator in the modern vision stack --- dense, convolution, BatchNorm,
residual, depthwise, squeeze-excite, layer scale, and attention --- each
emitted as the rendering of a machine-checked derivative, from the same three
pieces: denoted IR, bridge theorem, printer.
...
full 1000-class run is the same architecture --- patch16, embed
192, 12 transformer blocks, 3 heads, $\sim$5.7M parameters --- with
a 1000-class head, the \texttt{.imagenet} dataset, and the
DeiT-flavored training recipe. Here's what the loop looks like:
...
As with the ResNet-34 ImageNet run (Ch~\ref{chap:residual}), the
phase-2 JAX path (\texttt{jax/MainVitImagenet.lean}, via
\texttt{lake build vit-tiny-imagenet}) wires \texttt{.imagenet}
through tfds streaming; the phase-3 IREE path awaits the same
C-side streaming reader for the 1.28M-image set. Only the
data-loading layer differs --- the spec, the recipe knobs, and the
\texttt{.train} invocation are identical.
...
moment warmup ramps the LR past ${\sim}1.6\times 10^{-4}$ --- train
loss pins at $\ln(1000)\approx 6.9$ and never recovers. This is the
...
\textbf{bf16 and the run.} ViT is matmul-bound --- patch embed,
attention QKV/scores/output, the MLP, the head are all dense
matrix multiplies, with no convolution to speak of. So unlike the
ResNet case, the relevant precision flag is \texttt{bf16} alone
(matmul compute in bfloat16, fp32 master weights, fp32
LayerNorm/softmax/GELU); there is no \texttt{bf16Conv} to set.
...
see no such lift on gfx1100 --- MIOpen's bf16 convolution is no
...
--- up from the 80-epoch ancestor's $65.64\%$ / $87.06\%$. That lands
within ${\sim}2$ points of the ${\sim}72\%$ ViT-Tiny is capable of; the
remaining gap is the one recipe knob still left on the table (repeated
augmentation) plus two minor deviations (strict Mixup/CutMix alternation
instead of a random switch, and Random-Erase to zero rather than to
noise). The headline is a \emph{300-epoch} DeiT result with the full
distillation-free recipe, and the point here is the pipeline (proven
attention backward $\to$ DeiT recipe $\to$ stable bf16 training to a
sensible ImageNet number), not a SOTA chase. The validation curve shows
the familiar shape --- fast climb through warmup, a long middle, a
final lift as the cosine schedule anneals to zero:
...
augmentation} --- DeiT samples each image in a batch three times
under different augmentations, which disproportionately helps the
data-hungry tiny models; our pipeline does not, and it is the
obvious next lever. The rest is augmentation-fidelity drift ---
RandomResizedCrop and Random-Erasing are a from-scratch
\texttt{tf.data} reimplementation rather than timm's exact code (our
RandAugment now matches timm's \texttt{rand-m9-mstd0.5-inc1} op-set
literally) --- plus a few small knobs (batch $512$ vs.\ $1024$, the
gradient clipping we add for stability, Random-Erase to zero rather
than to noise). None of it is a bug: the loss curve is textbook
DeiT. The further $+2.3$ points to DeiT-Ti's $74.5\%$ headline are a
different thing entirely --- the \emph{distillation} variant (a
second ``distillation token'' trained against a strong CNN teacher),
which we do not implement.

\textbf{[TODO --- run w/ config.]} \texttt{vit-tiny-imagenet},
\textbf{300 epochs} (a re-run of the $70.28\%$ above). \emph{Changed
since that number:} RandAugment is now timm-literal
(\texttt{rand-m9-mstd0.5-inc1}: SolarizeAdd$+$Invert op-set, per-op
apply prob $0.5$, bilinear geometric interpolation, relative
translate). \emph{Still open:} repeated augmentation ($3\times$, the
largest remaining lever) stays deferred. Re-running with the
timm-faithful RandAugment --- and, once built, repeated augmentation ---
...
\paragraph{YOLO on Oxford-IIIT Pets.}
The \texttt{demos/MainYolov1PetsTrainBootstrap.lean} trainer is a cat-vs-dog
\emph{head} detector: a ResNet-34 backbone (bootstrapped from the repo's
ImageNet ResNet-34 checkpoint, \texttt{jax\_r34\_imagenet.bin}, $\sim$21M
backbone params) with a convolutional detection head, trained on
Oxford-IIIT Pets head boxes ($224 \times 224$ RGB $\to$ a $7 \times 7$
prediction grid). After training it draws boxes that land on cat and dog
faces and labels them, on held-out images (see \texttt{demos/README.md} for
the rendered grid). Three things change relative to the image
\emph{classifiers} of Part~2 and the segmentation nets above --- and the
third is the whole lesson:

\begin{itemize}
  \item \textbf{The label is a variable-length set of boxes on a grid.} A
    classifier compares one logit vector to one class; a segmentation net
    compares per-pixel logits to a per-pixel mask. Detection has a
    \emph{variable} number of objects per image, each a (class, box). YOLO
    fixes the shape by tiling the image into a $7 \times 7$ grid and making
    the cell containing an object's \emph{center} responsible for it: the
    target tensor carries, per cell, the box center/size, an objectness
    $=1$, and a class slot --- zero everywhere else. The loss is multi-part
    (coordinate regression + objectness + class), \emph{masked} to the
    object cells, riding the \texttt{lossKind := .yolov1Masked} path in
    \texttt{TrainConfig} --- no new VJP machinery, just the Part~2
    squared-error and cross-entropy backwards fired on slices of the grid.
  \item \textbf{The head is convolutional} --- a $1 \times 1$ (here, a
    $3 \times 3$ then $1 \times 1$) convolution over the $7 \times 7$ map
    instead of the paper's two fully-connected layers, so each cell
    predicts from its \emph{own} feature column with shared weights. This
    makes localization \emph{structural} rather than routed through a dense
    layer --- necessary, but, as the next point shows, not sufficient.
  \item \textbf{Detection is hard here because of the data distribution,
    not the architecture --- and that is the demo's real content.} On a
    coarse $7 \times 7$ grid, when objects \emph{cluster} (centered people
    in VOC, centered pet heads in Pets) the loss has a cheap minimum:
    predict \emph{where objects usually are} (the marginal) instead of
    localizing each one. The objectness map collapses to a fixed,
    image-independent center-prior that scores well on average --- and the
    training loss keeps dropping the whole time, so loss is \emph{not} a
    proxy for detector quality. The fix is to make the marginal useless:
    train on a $2 \times 2$ \emph{mosaic} of four pet images, which
    scatters the heads across the grid (and keeps backgrounds natural ---
    a flat synthetic canvas pushes the ImageNet backbone out of
    distribution and regresses). With heads decentered, localization goes
    from a degenerate $0$ to all heads found. A \emph{second}, independent
    imbalance is the class label --- Pets is $\sim 1{:}2$ cat:dog, so the
    class head defaults to always-``dog'' until the mosaic sampler is
    balanced. Inference then adds a post-processing step the classifiers
    never needed: \emph{decode} each cell's box (cell-relative $\to$
    absolute), score it (objectness $\times$ class), threshold, and run
    \emph{non-max suppression} to collapse overlapping boxes
    (\texttt{scripts/yolo\_render.py}).
\end{itemize}

The catalog entry below keeps the paper-exact YOLOv1 \emph{shape} ($448$
input, FC head, all 20 classes) for the historical record; the working
demo deliberately narrows to the simplest thing that detects --- two
classes, one scale, conv head, mosaic training --- and defers anchors and
the multi-scale FPN heads (which don't fit the linear \texttt{NetSpec}) to
...
datasets span the book's whole range: \textbf{MNIST} and
\textbf{CIFAR-10} drive the verified-codegen chapters of Part~1
(\S\ref{chap:cnn}, \S\ref{chap:bn}); \textbf{Imagenette} --- a ten-class
subset of ImageNet at $224\times224$ --- is the ResNet-scale recipe
(\S\ref{sec:residual_r34_example}); and full \textbf{ImageNet-1k} backs
the JAX-bridge runs of the larger architectures (ResNet-34, MobileNetV2,
EfficientNet-B0, ConvNeXt-T, ViT-Ti).
...
mirror (\nolinkurl{storage.googleapis.com/cvdf-datasets/mnist}); the
homepage above is the canonical reference.
\textsuperscript{*}ImageNet-1k is license-gated and not
redistributable, so the repository ships no fetch script: download it from
\nolinkurl{image-net.org} under its terms, prepare it through TensorFlow
Datasets (the standard \texttt{tfds} \texttt{imagenet2012} route), and load
it via the JAX dataset handlers that drive the bridge's ImageNet runs
(\texttt{jax/runs/*\_imagenet\_*}).
\normalsize

\medskip\noindent\textbf{Demo datasets.} The \texttt{demos/} trainers step past image classification into the
three other task families in the book --- detection, segmentation, and
language --- and each pulls a dataset of its own. \textbf{Oxford-IIIT
Pets} backs both the UNet segmenter (\texttt{unet-pets-train}, via
trimaps) and the YOLO detector (\texttt{yolov1-pets-train-bootstrap}, via
head boxes tiled into $2 \times 2$ mosaics); and \textbf{tiny
Shakespeare} --- a single 1\,MB character stream --- is what the TinyGPT
char-level transformer (\texttt{tinygpt-shakespeare}) trains on. Same
fetch-and-unpack story as the table above, with one extra step: each is
tokenized or tiled into the trainer's input format by the matching
\texttt{preprocess\_*.py} script before the first \texttt{lake exe} run.
...
Dataset & Abbrev. & Reference & Homepage & Path in repo \\
\hline
Oxford-IIIT Pets & Pets & Parkhi et al., 2012 &
  \href{https://www.robots.ox.ac.uk/~vgg/data/pets/}{\nolinkurl{robots.ox.ac.uk/~vgg/data/pets}} &
  \texttt{download\_pets.sh} $\to$ \texttt{data/pets/} \\[2pt]
tiny Shakespeare & TShk & Karpathy, 2015 &
  \href{https://github.com/karpathy/char-rnn}{\nolinkurl{github.com/karpathy/char-rnn}} &
  \texttt{download\_shakespeare.sh} $\to$ \texttt{data/shakespeare/} \\
\hline
\end{tabular}
\end{center}
...
and training. For detection, \texttt{preprocess\_pets\_mosaic.py} tiles
the Pets head boxes into class-balanced $2 \times 2$ mosaics
(\texttt{data/pets\_mosaic\_bal/}); the same \texttt{download\_pets.sh}
download feeds both the segmenter and the detector.
\normalsize
...
appendix is the smallest path from zero to a trained model. Four
tracks are included:
...
\begin{enumerate}
  ...
  \item \textbf{One-command demo tiers.} With the native setup
    (Track 2) in place, a single \texttt{lake run mnist},
    \texttt{cifar}, or \texttt{imagenette} builds and runs a curated,
    time-budgeted group of \emph{verified} trainers --- no env vars or
    trainer list to assemble by hand.
\end{enumerate}
...
\end{verbatim}
  ...
  \item \textbf{Fetch data} for the tier(s) you want to run:
\begin{verbatim}
./download_mnist.sh        # MNIST       (mnist tier)
./download_cifar.sh        # CIFAR-10    (cifar tier)
./download_imagenette.sh   # Imagenette 320px, ~300 MB (imagenette tier)
\end{verbatim}
...
\end{verbatim}
    ...
    The targets are the verified trainers the three demo tiers
    (Track 4) bundle --- build any one on its own:
    \begin{itemize}
      \item \texttt{lake run mnist}: \texttt{mnist-linear-verified},
        \texttt{mnist-mlp-verified}, \texttt{mnist-cnn-verified}
      \item \texttt{lake run cifar}: \texttt{cifar8-verified} and
        \texttt{cifar8-bn-verified}, each in plain-SGD /
        \texttt{-momentum} / \texttt{-adam} (six in all)
      \item \texttt{lake run imagenette}: \texttt{resnet34-verified-adam},
        \texttt{mobilenetv2-verified-adam},
        \texttt{efficientnet-verified-adam},
        \texttt{convnext-verified-adam}, \texttt{vit-verified-adam}
    \end{itemize}
    (Unverified \texttt{<arch>-train} targets --- \texttt{vgg-train},
    \texttt{resnet50-train}, \texttt{mobilenet-v3-train},
    \texttt{efficientnet-v2-train}, etc. --- also build, for
    architectures outside the verified set.)
...
CUDA_VISIBLE_DEVICES=0 IREE_BACKEND=cuda IREE_CHIP=sm_86 \
  .lake/build/bin/resnet34-verified-adam
\end{verbatim}
...
lake build ProofsMinimal       # the smallest end-to-end story (~seconds)
lake build Proofs              # type-checks the ENTIRE VJP proof suite
\end{verbatim}

\textbf{Start with \texttt{ProofsMinimal}.} The full \texttt{Proofs}
build type-checks every chapter at once --- the real verification, but a
lot to meet head-on. \texttt{lake build ProofsMinimal} instead compiles
just the suite's ``hello world'': the linear softmax classifier ---
the book's first end-to-end-verified example --- in both halves of its
claim.
\begin{itemize}
  \item \texttt{LinearFaithfulPoC} ($\sim$145 lines) --- the
    \emph{faithfulness} half: the StableHLO training step the codegen
    emits is, line for line, the certified softmax--cross-entropy
    gradient.
  \item \texttt{SgdDescentLinear} ($\sim$255 lines) --- the
    \emph{descent} half: one SGD step on that certified gradient
    provably decreases the loss.
\end{itemize}
Their transitive closure \emph{is} the minimum working set: those two
files plus the shared StableHLO / tensor / float-bridge foundation, and
nothing per-network beyond the linear case. \textbf{This is the
jumping-off point for the code.} Open
\texttt{LeanMlir/Proofs/LinearFaithfulPoC.lean} and
\texttt{SgdDescentLinear.lean} and read them top to bottom: you will
have seen the entire shape of the argument the rest of the suite scales
up --- every later chapter is these same two halves at a larger network.
The \textbf{Start here} section of \texttt{LeanMlir/Proofs/README.md}
lays out that read order and flags which large files to skip on a first
pass. Once \texttt{ProofsMinimal} builds green, graduate to the whole
suite:

The \texttt{Proofs} target's roots are the apex VJP modules; their
transitive imports cover every chapter's proof file, so this single
build type-checks all of it against Lean's kernel --- that is the
entire verification, no module list to keep current. Because the proof
suite imports only Mathlib --- never the codegen --- \texttt{Proofs}
skips the IREE/StableHLO layer entirely; it is also the default target,
so a bare \texttt{lake build} runs the same check. (\texttt{lake build
LeanMlir} builds the whole repo instead --- codegen, trainers, and
proofs.) Every VJP formula is proved against Mathlib's \(\fderiv\) (see
Appendix~\ref{app:verification}).
...
\section*{Track 4: One-command demo tiers}

Track 2 is the full manual path --- pick a trainer, set the backend
env vars, run. Once that setup works, you rarely want to assemble a
trainer list and environment by hand every time. Three Lake
\texttt{script}s bundle the common cases into one command that builds
and runs a curated, time-budgeted group of \emph{verified} trainers
back to back:

\begin{verbatim}
lake run mnist        # verified MNIST: linear, MLP, CNN            (~30 min)
lake run cifar        # ch.5 cifar8: SGD/momentum/Adam x bn/no-bn   (~1 hr)
lake run imagenette   # the 5 Part-I nets at 224^2, 80-epoch AdamW  (~37 h)
\end{verbatim}

Each script streams every trainer live and tees it to
\texttt{<name>.log}. Two conveniences make it genuinely one-click on
top of the Track 2 install:

\begin{itemize}
  \item \textbf{Backend auto-detects.} \texttt{cuda} when an NVIDIA GPU
    is visible (\texttt{nvidia-smi -L} succeeds), otherwise
    \texttt{rocm}. Set \texttt{IREE\_BACKEND} to override and
    \texttt{LEAN\_DEMO\_GPU} to pick the card (default \texttt{0}).
  \item \textbf{The venv is found for you.} The trainers shell out to
    \texttt{iree-compile}; the script prepends the project's
    \texttt{.venv/bin} to \texttt{PATH} for the run, so it resolves
    without activating the venv first --- the usual one-click footgun.
\end{itemize}

The tiers escalate by time budget. \texttt{lake run mnist}
($\sim$30 min) is the quickest end-to-end confirmation that the GPU
path works; \texttt{lake run cifar} ($\sim$1 hr) is the
Chapter~\ref{chap:bn} optimizer ablation in full --- the six runs
(SGD/momentum/AdamW $\times$ BN/no-BN) behind that chapter's accuracy
curves and table; and \texttt{lake run imagenette} ($\sim$37 h on a
single 7900 XTX) trains all five Part-I architectures
(ResNet-34, MobileNetV2, EfficientNet-B0, ConvNeXt-T, ViT-Ti) at full
$224\times224$ --- a deliberate investment, not a quick look. Anything
beyond these stays a single, deliberate \texttt{run.sh} invocation
(Track 2).

\medskip\noindent\textbf{Which tier fits your GPU?} Before committing to one
--- especially the 37-hour Imagenette tier --- run \texttt{lake run
benchmark}. It briefly times a couple of the verified nets on your card and
prints how long each chapter and each tier above would take on it:
...
$ lake run benchmark
  ...
  6  ResNet-34            9.5h
  10 ViT                  7.8h
  Full Part-1 training   42.8h
  lake run mnist 5m | cifar 34m | imagenette 42.2h
\end{verbatim}

\noindent Backend auto-detects (CUDA or ROCm), so the numbers come out scaled
to whatever card you run it on.
...
this appendix relies on, and they are not the same kind. \textbf{Proven}
(deductive): in Lean, against three axioms, each operator's gradient is its
exact reverse-mode derivative --- and an independent re-checker confirms no
project axiom slipped into a proof, so ``proven'' means proven-clean.
\textbf{By construction} (structural): the StableHLO the GPU runs is not
written but \emph{printed} from the same datatype the proofs reason about,
with a bridge theorem pinning it to the proven formula, so the code cannot
drift from the math --- and this now covers whole training steps (forward,
backward, parameter update) for all twelve verified networks at the production
trainers' exact signatures, each emitted step tied operator-by-operator to the
network's own forward and backward pass. \textbf{Cross-checked} (empirical): because IREE cannot itself be
proven, and \texttt{float32} only conditionally --- its rounding now
carries a theorem layer whose interface constants are empirically anchored
(\S~\ref{sec:float32}) --- three independent oracles ---
a finite-difference probe, a parallel JAX pipeline, and a CPU/GPU/NumPy
comparator --- must agree on the gradient before the hardware is believed.
Each kind catches what the others structurally cannot; the five mechanisms
this appendix walks through are these three kinds of certainty, with the
empirical one split among its witnesses.
...
each asserting backward \(= \sum_j \pdiv f\,x\,i\,j \cdot dy_j\), now
including the full-architecture closes of all five Part~I networks --- on
top of the foundation \(\pdiv\) calculus and the
differentiability/forward/witness machinery they rest on, plus 41
architecture definitions in the bestiary, across 156 Lean proof files,
with zero project axioms. The axiom audit
(\texttt{tests/AuditAxioms.lean}) re-checks \textbf{1{,}153 theorems} in all
--- the VJP contracts plus the forward-graph faithfulness, render,
cotangent-chain, and backward certified-VJP tie results --- every one
closing under the same three core axioms. The most recent additions are
the \(\mathbb{R}\to\texttt{float32}\) bridge and the descent results
(\S~\ref{sec:float32}): roughly 600 theorems --- the rounding budgets,
now architecture-complete across every operator of all the convolutional
and transformer nets; the per-layer float-fused descent guarantees for the
shallow nets; and the subnormal and eval-mode-BN lemmas --- all
\emph{hypothesis-style}, so the audit still closes under the same three
axioms with zero project axioms.

What landed in the final rounds to retire the last of the project axioms
earlier drafts had shipped:
...
The 51-theorem coverage is illustrative rather than exhaustive ---
...
\section{Verified code generation}
\label{sec:verified_codegen}

The four layers above answer whether the \emph{mathematics} is right --- the
...
runs a block of emitted \texttt{stablehlo} that IREE compiles once per
...
section in each chapter closes that gap for one operator; this section
...
\begin{itemize}
  \item \textbf{Denoted IR and a bridge theorem.} The emitted backward (and
    ...
    \texttt{relu\_back\_bridge}; each closes under the same three axioms as
    the rest of the project.
  \item \textbf{A computable printer.} A small printer walks that same IR and
    emits one \texttt{stablehlo} operation per node ---
  ...
  \item \textbf{An execution oracle.} A Python harness regenerates the
    ...
    correctness gate; the GPU run (ROCm on a Radeon RX 7900 XTX, gfx1100)
...
\begin{itemize}
  ...
  \item \emph{Trusted} (validated numerically, not proven): that the
    ...
    \emph{syntactic} half is now partly closed --- the emitted text lexes and
    parses back to the proven op-graph (\texttt{StableHLOLex.lean},
    \texttt{StableHLOParse.lean}) --- leaving a formal StableHLO
    \emph{semantics}, which does not yet exist; that IREE lowers the text
    correctly; and that \texttt{float32} approximates $\mathbb{R}$, the last
    now a conditional theorem validated on real silicon
    (\S~\ref{sec:float32}) rather than a bare assumption.
\end{itemize}
...
printer, IREE, and floating point. Where a conventional generator leaves
...
dimensions, before any code generation existed --- and the whole-network VJPs
...
per-operator bridge is the demonstration; the verified trainers run the full
construction. Each reads a single committed \texttt{.mlir} --- its entire
training step, forward through backward through the parameter update --- and that
file is the printout of one graph (\texttt{SHlo}, the StableHLO syntax tree in
\texttt{LeanMlir/Proofs/}) whose denotation $[\![\cdot]\!]$ a faithfulness
theorem proves equal to the certified loss-descent step, output by output. A
second theorem then closes the gap a per-output bridge leaves open: that the
cotangent each parameter update consumes is the one the network's \emph{own}
backward pass delivers at that site, not an arbitrary one fed in by name. These
\emph{tie} theorems --- one per network, \texttt{<net>\_net\_tied\_certified} ---
thread the real forward activations and the loss-driven backward chain through
every parameter, with the fan-in sum at each residual skip, the per-head
attention backward, and the layer-norm input-VJP rendered explicitly, so each
output becomes one composed equation rooted at the proven forward rather than a
quantifier over a free cotangent. All twelve chapter networks are tied this way,
from the MNIST linear classifier to depth-12 ViT-Tiny (200 parameters, the first
network with no rendered-gradient gaps); the axiom audit re-derives every tie
under the same three axioms, and the project's CI prints a green cell for a
network only when its tie capstone is in that closure --- so the scorecard cannot
claim more than the kernel checked. The backward direction is now tied the same
way: each deep net's rendered block-backward is pinned to that block's certified
VJP (\texttt{vitBlockBackPR\_eq\_transformerBlock\_vjp} and its peers for
ResNet-34, MobileNetV2, ConvNeXt, and EfficientNet), so the emitted backward
denotes the certified reverse-mode derivative graph-for-graph at block
granularity --- the whole-net backward fold assembled for ResNet-34 and
EfficientNet, and elsewhere gated only by a realistic-dimension whole-net VJP
that stays an honest stop, not remaining work. What stays trusted is exactly what the
per-operator story already trusted, now carried across the whole step: the one
printer, and that each emitted operation's text denotes what $[\![\cdot]\!]$ says
it does.

\medskip\noindent\textbf{The one conditional.} For the smooth operators ---
BatchNorm and LayerNorm (given $\varepsilon > 0$), GELU, swish, sigmoid,
softmax, and attention --- the bridge is unconditional, holding at every
input. For the kinked operators --- ReLU, ReLU6, and max-pool --- it holds
only at a \emph{smooth point}, where no pre-activation sits exactly on the
kink (zero for ReLU, $0$ or $6$ for ReLU6, an argmax tie for max-pool); the
equality is permitted to fail precisely on that measure-zero set, and nowhere
else. That set is the one irreducible boundary the smooth-point caveats in
the chapter sections refer to.

\section{Inside a bridge theorem}
...
feed the incoming cotangent into a single matrix multiply. Two functions are
...
type --- it says what the graph \emph{means} mathematically:
\[
...
\texttt{sumBroadcast}, \texttt{scaleConst}, \texttt{add} --- which are the
...
\texttt{stablehlo.dot\_general}; \texttt{selectPos} $\mapsto$ \texttt{compare
GT 0} plus \texttt{select}) --- that arrow is the trusted one, producing the
text in each chapter's listing. The denotation interprets the same value into
the proofs' \texttt{Vec} type, and the bridge proves \emph{that} equals the
derivative --- that arrow is machine-checked. Because both arrows start from
...
\section{Float32: closeness, composition, and whether it still trains}
\label{sec:float32}

Every theorem in this book is over exact reals; the GPU computes in
binary32. Until recently that gap lived entirely in the empirical layer
--- the oracles agree to 1--2 ULP, and you were asked to find that
persuasive. It is now a theorem layer, and it has two halves of different
reach --- keeping them apart is the honest part. The \emph{closeness} half
...
and backward, from the MNIST linear classifier to ViT-Tiny --- and (the
...
on all eight committed renders. The
...
\paragraph{The model is a hypothesis, not an axiom.}
...
\texttt{FloatSubnormalBridge.lean} --- the normalized blocks provably stay
normal, and the residual underflow floor is proven negligible,
...
postulated --- the theorems are conditional on the standard model, the
...
\(n\cdot u\); the tree-reduction bound below recovers \(\log_2 n\cdot u\)
under a named balance hypothesis); and \(\exp\) enters as a
hypothesis (\(|\widehat{\exp}(t) - e^t| \le e_{\exp}\, e^t\))
...
\mathrm{lr}\cdot(a_i c_j)\) --- entry for entry the
...
the explicit constant \(2a^2/(1-2aD)\) --- no Hessian: the softmax ratio
sandwich that powers the float budgets turns out to be the Lipschitz
engine too. With the float budget \emph{fused in} --- the step's
\(\eta\) is the \emph{proven} binary32 gradient accuracy, not an assumed
parameter --- this closes end-to-end for the linear classifier
...
\texttt{cifar8\_lastConv\_sgd\_descends} is the first non-MNIST descent ---
...
hypotheses return with a number in them: \(\mathit{ez} < |z_i|\) ---
...
the bound grows like \((\text{gain})^{\text{depth}}\) and blows up ---
\(2.7\cdot 10^{60}\) on MobileNetV2, \(\sim\!10^{364}\) on ViT-Tiny --- while
...
before anything is inherited) and \(H_i\) is its \emph{tail gain} --- how much a
perturbation at layer \(i\) is stretched by all the layers after it. The proof
...
estimate of \(H_i\) only helps. And an honest estimate is cheap --- one backward
pass reads the \(H_i\) off the network's own trajectory. They enter the theorem
...
operator --- each convolution, normalization, and dense its own link, with
...
deep: the bound lands \emph{below the logit scale} on every committed render ---
MNIST-CNN \(0.015\), CIFAR-8 \(2.6\), MobileNetV2 \(0.30\), ResNet-34 \(0.17\),
ViT-Tiny \(0.57\), EfficientNet-B0 \(0.10\), ConvNeXt-T \(0.80\), against logits
of order one to five. And it is a statement about the \emph{decision}, not just
...
measured \(0.52\); He initialization already exceeds prettier bounds
...
checkpoints --- ResNet-34, MobileNetV2, EfficientNet-B0, ViT-Tiny,
...
\section{The five layers, as three kinds of certainty}

The five mechanisms are three kinds of certainty about a single claim ---
that the GPU computed the network's exact gradient --- with the empirical
kind split among its witnesses. Each kind catches what the others
structurally cannot.

\paragraph{Proven (deductive): the math is right.}
\begin{itemize}
  \item \textbf{Formal proofs} fail if the math is wrong. Every theorem
    reduces to Mathlib's \(\fderiv\) and Lean core; the trust kernel is
    exactly \texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound},
    plus what Mathlib proves about \(\fderiv\). Type-checking can't catch the
    prose narrating a different formula than the theorem states, though.
  \item \textbf{The comparator} fails if the elaborator accepted a term the
    kernel wouldn't, or if any project axiom snuck into a proof's transitive
    closure --- the independent re-check that makes ``proven'' mean
    proven-clean, since \(\#\)\texttt{print axioms} and \(\texttt{lake
    build}\) share an elaborator and cannot audit themselves.
\end{itemize}

\paragraph{By construction (structural): the code is the math.}
\begin{itemize}
  \item \textbf{The bridge theorems} (\S~\ref{sec:verified_codegen}) fail if
    the emitted StableHLO denotes anything other than the proven derivative.
    The graph is printed from the same datatype the bridge reasons about, so
    it cannot drift: ``the code computes the proven gradient'' is a theorem
    about a value, not a hope about a comment.
\end{itemize}

\paragraph{Cross-checked (empirical): the hardware agrees.}
\begin{itemize}
  \item \textbf{FD gradient checks} catch prose-vs-theorem drift and any
    algebraic typo in a closed-form Jacobian. FD can't spot compiler bugs or
    non-smooth-point tiebreak disagreements.
  \item \textbf{The JAX-parallel pipeline} fails if either compiler has a
    codegen bug, if the runtime drifts, or if a hand-derived VJP disagrees
    with JAX autodiff.
  \item \textbf{The IREE-versus-NumPy oracle} fails if the compiled graph
    computes the wrong thing on real hardware --- the one check that reaches
    past the proofs to IREE's lowering --- which cannot be proven --- and
    \texttt{float32}, whose rounding now carries a conditional
    \emph{whole-network} certificate: a depth-linear drift bound below the
    logit scale, and with it a proof that rounding cannot flip the prediction
    (\S~\ref{sec:float32}).
  \item \textbf{The margin probe} (\texttt{scripts/margin\_probe.py})
    fails if a real training run violates the float theorems' hypotheses
    --- a flipped ReLU mask, a logit drift past \(\delta\) --- the
    check that keeps the conditional theorems honest about actual
    training rather than hypothetical nets.
\end{itemize}

Each kind fails differently --- a deductive proof is blind to a miscompile,
an empirical diff is blind to a measure-zero kink, a faithful bridge is blind
to a wrong formula it renders perfectly --- and overlaying them is the
guarantee. Verified code generation straddles two of these: its bridge is
structural, its oracle empirical. The \texttt{float32} bridge straddles
the other pair: its budgets are deductive, its interface constants
(\(u\), \(e_{\exp}\), \(\delta\)) empirical.
...
Claude --- which
served by turns as \emph{scribe}, taking dictation; as
\emph{compilator}, searching and assembling; and as
