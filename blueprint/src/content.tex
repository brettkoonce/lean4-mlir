...
``the same render as A2''}: the decay is baked, so its $0.01$ against A2's $0.02$
is a re-render, kept apart so one line among $18{,}000$ cannot be lost.
...
depth costs nothing measurable, in memory or in time. \textbf{And the two budget columns are the point.}
...
its per-device tensor is $128$, and only the group is the statistic. Half the gap
is a re-render, $k=4$ at $128$ per device, which is also the \emph{faster}
arrangement; the last $4\times$ needs synchronised batch-norm --- a new operator,
