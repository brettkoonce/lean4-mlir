...
\begin{verbatim}
lake run r34-default-4gpu
\end{verbatim}

\noindent The job config behind that name,
\texttt{scripts/jobs/r34-default-4gpu.conf}, carries what used to be a
...
\noindent Global batch 256 as $4 \times 64$ replicas, fp32 throughout,
...
precision           & bf16 (matmul + conv)     & fp32 \\
BN statistic group  & 256 (global)             & 64 (per replica) \\
per epoch           & $\sim$8.8 min            & $\sim$24.7 min \\
total               & $\sim$14.8 hr            & $\sim$37.1 hr \\
\hline
...
\hline
\textbf{val top-1}  & $\mathbf{74.16\%}$       & $\mathbf{74.14\%}$ \\
\textbf{val top-5}  & $\mathbf{91.92\%}$       & $\mathbf{91.86\%}$ \\
\hline
\end{tabular}
\end{center}

\noindent Both columns are ResNet-34 on the 2018 recipe, 90 epochs, on the
same four 4060\,Ti cards, scored over the same $50{,}000$ validation images
under the same protocol. Rows above the first rule differ; rows between the
rules are held equal by construction.

\medskip\noindent\textbf{The two paths agree to $0.02$ points}, and that is
well inside what one run can resolve. At $n = 50{,}000$ the 95\% Wilson
interval of Chapter~\ref{chap:tensor} is $\pm0.38$ points on either top-1
--- $[73.77, 74.54]$ for the reference, $[73.75, 74.52]$ for the verified
path --- so every $\Delta$ in the table is a fraction of the interval, and
...
the verified run is fp32 where the reference is bf16, and its BatchNorm
normalizes over each replica's $64$ where the reference's \texttt{@jit}
mesh reduces the statistics over the global $256$. The $0.02$ bounds the
two together. \S\ref{sec:mnv2_phase4} is the same comparison with
precision held equal and the BatchNorm group left as the only asymmetry.
...
learning rate SGD moves top-1 by up to three points between consecutive
epochs, in both directions --- so the row is read for its ends:
...
verified path (PJRT)  & 37.11 & 43.78 & 47.02 & 53.92 & 58.98 & 68.10 & 74.14 \\
$\Delta$              & $+1.26$ & $-1.16$ & $+0.01$ & $+2.62$ & $-0.49$ & $-0.61$ & $-0.02$ \\
\hline
\end{tabular}
\end{center}
...
precision difference besides, landed $0.02$ apart at convergence; the
$+0.01$ here is the same order. So the claim is that the two paths agree
...
\noindent Five of these jobs have run and are the ones behind their chapters'
numbers; the EfficientNet-B0 and ConvNeXt-T rows are configured and have not
finished, so Chapters~\ref{chap:se} and~\ref{chap:layernorm} carry the
reference only. \texttt{scripts/jobs/} also carries \emph{axis} siblings that
this table does not list --- an fp32 peer of each of the ResNet-50 2018,
RSB-A3 and ViT rows, and an earlier A3 artifact --- which differ from their
...
r34-default-4gpu once} --- the next section.
