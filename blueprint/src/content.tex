...
\hline
GPU & Precision & Per epoch & 30 epochs & Val top-1 & Val top-5 \\
\hline
2$\times$ 7900 XTX (ROCm) & fp32 & $\sim$24.2 min & $\sim$15 hr & $\mathbf{69.26\%}$ & --- \\
\hline
\end{tabular}
\end{center}

\noindent (No bf16 row here: on AMD's MIOpen, bf16 convolution is no
faster than fp32 --- see the CUDA discussion below --- so the ROCm
run stays fp32.)
...
\textbf{bf16 and the full 90-epoch run.} Running the whole schedule
is a question of patience, not capability --- and bf16 mixed
...
\hline
4$\times$ 4060 Ti (CUDA) & fp32 & $\sim$16.8 min & $\sim$25 hr & --- & --- \\
4$\times$ 4060 Ti (CUDA) & bf16 & $\sim$10.5 min & $\sim$16 hr & $\mathbf{72.02\%}$ & $\mathbf{90.62\%}$ \\
\hline
\end{tabular}
\end{center}
...
If you're skimming this book to decide whether the framework
delivers --- does Lean actually verify modern deep learning end to
end? --- Chapter 6 is the answer. ResNet-34 has 21M parameters, 36
residual blocks, batch normalization, the full production training
recipe, and it trains to 90.29\% val on Imagenette through MLIR
generated by the same Lean code that proves the gradients. Every
operator's backward pass is formalized; every chain-rule step
through the architecture is type-checked.
...
\textbf{Compute budget.} On the four RTX 4060 Ti (CUDA, bf16, batch
256 across the four GPUs), steady-state throughput is
$\sim$106\,ms per step, about $9.1$ minutes per epoch --- so the
30-epoch validation tier is a $\sim$4.5-hour run and the full
90-epoch schedule projects to roughly 14 wall-clock hours.
Curiously, that is \emph{slower} per epoch than the
$9.5\times$-larger ResNet-34 at $\sim$10.5\,min: depthwise
convolution is memory-bound and low-arithmetic-intensity, so
MobileNet's order-of-magnitude FLOP saving does not translate into
a proportional wall-clock win on this hardware.
...
GPU & Precision & Per epoch & 90 epochs & Val top-1 & Val top-5 \\
\hline
4$\times$ 4060 Ti (CUDA) & bf16 & $\sim$9.1 min & $\sim$14 hr & $\mathbf{68.33\%}$ & $\mathbf{88.17\%}$ \\
\hline
\end{tabular}
\end{center}

\noindent The full 90-epoch run completed at $\mathbf{68.33\%}$
top-1 / $\mathbf{88.17\%}$ top-5 on the full 50{,}000-image
validation split --- a few points under the $\sim$71\% paper number,
the gap attributable to the missing RMSProp optimizer
\textbf{[TODO: run w/ RMSProp]} (now wired) and the original
longer schedule, not the precision or pipeline. The SGD peak LR of
$0.1$ took off cleanly (no collapse), so the $0.05$ fallback was
never needed. The validation curve has the usual shape --- a fast
warmup climb, a long middle, and a final lift as the cosine schedule
anneals to zero:
...
${<}5\%$ per epoch; RMSProp and AutoAugment
\textbf{[TODO: run w/ rmsprop + geo aa]} (both now wired), and
the 300-epoch length remain the gap to the paper number. The
validation curve:
