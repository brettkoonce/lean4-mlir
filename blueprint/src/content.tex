...
full architecture: machine-checked forward graph plus whole-network backward,
...
in its first proven model: the \textbf{4 VJP contracts} of the MNIST
linear classifier --- \(\texttt{mnistLinear\_has\_vjp\_correct}\) (the
whole model, a single dense layer), the dense weight- and
bias-gradient theorems, and the softmax cross-entropy gradient. It is
the degenerate-simplest whole-model VJP, and the ops it needs are
introduced right here, where the demo lives.

\textbf{Chapter 3 (MNIST: 1D)} adds depth and nonlinearity on top of
Chapter~2's dense layer --- \textbf{4 VJP contracts}. The guarded ReLU
...
\textbf{Chapter 4 (MNIST: 2D)} contributes 3 VJP contracts.
\(\texttt{conv2d}\) and \(\texttt{maxPool2}\) are concrete forward
defs; the conv2d input-side VJP and maxPool2's VJP --- plus its
smooth-point \(\texttt{\_at}\) variant, whose \texttt{correct} field is
a real proof rather than \texttt{rfl} --- are proved from foundation,
with the conv weight- and bias-gradient theorems alongside.

\textbf{Chapter 5 (CIFAR: BN)} contributes 1 VJP contract
(\(\texttt{bn\_input\_grad}\)), assembled from a chain of supporting
Jacobians: the inverse-stddev broadcast Jacobian and its smoothness
lemma under \(\varepsilon > 0\), plus the BN affine and centering
Jacobians, all proved from foundation.

\textbf{Chapter 6 (ResNet-34)} contributes 6 VJP contracts and
no new calculus: the residual additive-fan-in VJP (plus its
projected and smooth-point \texttt{\_at} variants), the
global-average-pool VJP, and --- new in this revision ---
\(\texttt{cnn\_has\_vjp\_at}\), the \emph{whole-network} fold
...
\textbf{Chapter 7 (MobileNetV2)} contributes 2 VJP contracts: the
depthwise input VJP (same recipe as Ch~4 with one fewer summation
level --- no cross-channel mixing) and the whole-network
\(\texttt{mobilenetv2\_has\_vjp\_at}\) over inverted-residual blocks,
which introduces the two-sided-kink \texttt{relu6} as a new
smooth-point activation.

\textbf{Chapter 8 (EfficientNet)} contributes 4 VJP contracts:
...
proved elsewhere), the smooth \texttt{sigmoid} gate activation, and the
whole-network \(\texttt{efficientnet\_has\_vjp\_at}\) over MBConv blocks
with a full-spatial SE gate.

\textbf{Chapter 9 (ConvNeXt)} contributes 4 VJP contracts: GELU
(Jacobian via \(\fderiv\) and \texttt{Real.differentiable\_tanh}),
LayerNorm (reusing the BN proof template), the elementwise
layer-scale, and the whole-network
\(\texttt{convnext\_has\_vjp\_at}\) --- the all-smooth architecture,
discharged with no kink hypotheses, only \(\varepsilon > 0\)
positivity. (Swish/SiLU lives in Chapter~8, where EfficientNet uses
it; ConvNeXt's activation is GELU.)

\textbf{Chapter 10 (ViT)} contributes 6 VJP contracts (the three SDPA
cotangents, MHSA, the transformer block, and --- new in this revision
--- the whole-network \(\texttt{vit\_full\_has\_vjp\_correct}\) wrapper;
...
\begin{tabular}{l c l}
\hline
Chapter & VJP contracts & New primitive / result \\
\hline
2 (Foundation)         & 4 & \texttt{mnistLinear} (linear classifier), dense weight/bias-grad, softmax-CE \\
3 (MNIST: 1D)          & 4 & ReLU, MLP ($+$ smooth-point \texttt{\_at}) \\
4 (MNIST: 2D)          & 3 & Conv, MaxPool ($+$ \texttt{\_at}) \\
5 (CIFAR: BN)          & 1 & BatchNorm input-gradient \\
6 (ResNet-34)          & 6 & Residual ($+$proj, $+$\texttt{\_at}), global-avg-pool, whole-net \texttt{cnn} \\
7 (MobileNetV2)        & 2 & Depthwise, whole-net \texttt{mobilenetv2} \\
8 (EfficientNet)       & 4 & swish, SE (mult. fan-in), sigmoid, whole-net \texttt{efficientnet} \\
9 (ConvNeXt)           & 4 & GELU, LayerNorm, layer-scale, whole-net \texttt{convnext} \\
10 (ViT)               & 6 & matrix kit, Attention (SDPA/MHSA/block), whole-net \texttt{vit\_full} \\
\hline
                       & \textbf{34} & \\
\hline
\end{tabular}
...
The shift from earlier drafts: every architecture now ends in a single
\emph{whole-network} VJP contract --- \(\texttt{cnn\_has\_vjp\_at\_correct}\),
\(\texttt{mobilenetv2\_has\_vjp\_at\_correct}\),
\(\texttt{convnext\_has\_vjp\_at\_correct}\),
\(\texttt{efficientnet\_has\_vjp\_at\_correct}\),
\(\texttt{vit\_full\_has\_vjp\_correct}\) --- the entire forward pass's
backward, machine-checked end to end, not just the per-layer pieces. (The
underlying forward and witness \texttt{def}s, the foundation \(\pdiv\)
rules, and the differentiability lemmas are the supporting machinery these
33 contracts rest on; they are not counted in the table.)
