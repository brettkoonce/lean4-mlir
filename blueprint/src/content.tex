...
The previous two chapters trained MNIST classifiers and a
four-convolution net was plenty. CIFAR-10 --- color $32 \times 32$
images, a genuinely harder problem --- rewards going deeper, so this
chapter's demo \textbf{doubles the depth}: an eight-convolution network
(twice the four-conv MNIST CNN). Depth also lets us ask a sharper
question than ``does it train?'' --- namely, \emph{what governs how it
trains?}

Two levers do most of the work, and the proof suite lets us move each
one against an \emph{identical, machine-checked gradient}. The first is
\textbf{normalization} --- the BatchNorm we just proved, dropped between
each convolution and its ReLU. The second is the \textbf{optimizer} ---
the rule that turns a gradient into a weight update. We hold everything
else fixed (same architecture, same data pipeline: per-epoch shuffle,
random horizontal flip, a cosine learning-rate schedule with warmup;
same 40 epochs) and move one lever at a time. Two findings, stated up
front so the graphs below are no surprise:

\begin{itemize}
\item \textbf{Normalization buys speed, not ceiling.} BN reaches a given
  accuracy in fewer epochs, but both nets top out in the same place ---
  at this depth BN's \emph{final} number even lands a hair below the
  un-normalized net's. It is an accelerator, not a capacity boost.
\item \textbf{The optimizer moves the ceiling.} Trading plain SGD for
  SGD-with-momentum is worth two to three points of final accuracy ---
  more than normalization changes anything here. Adam, notably, is a
  wash with plain SGD on a net this small: its per-coordinate
  adaptivity earns nothing it can keep.
\end{itemize}

\noindent Every run below shares one proof-rendered backward pass; only
the BN layers or the rendered optimizer tail change between them. The
numbers are from that verified training step running on a GPU.
...
and runs through the same three-layer dense head.
...
\begin{tikzpicture}[
  ...
  \node[dense,  below=0.18cm of fl]    (d1)   {\textbf{Dense} $128\to64$, ReLU};
  \node[dense,  below=0.18cm of d1]    (d2)   {\textbf{Dense} $64\to64$, ReLU};
  \node[head,   below=0.18cm of d2]    (d3)   {\textbf{Dense} $64\to10$ \;(identity)};
...
$16, 16, 32, 32$, a max-pool after each stage), then a small dense
head. Without BN, each convolution is followed by a plain ReLU; with
...
def cifar8NoBn : NetSpec where
  ...
  layers := [
    ...
    .flatten,
    .dense 128 64 .relu, .dense 64 64 .relu, .dense 64 10 .identity
  ]
\end{verbatim}
...
\begin{axis}[
    ...
    xmin=0, xmax=41, ymin=28, ymax=76,
...
\addplot[blue, mark=*, mark options={fill=blue}] coordinates {
(1,30.80) (2,42.83) (3,47.53) (4,55.43) (5,58.87) (6,64.16) (7,62.85) (8,66.18) (9,65.74) (10,68.93) (11,69.16) (12,69.69) (13,71.15) (14,70.15) (15,70.45) (16,72.10) (17,72.15) (18,71.62) (19,73.01) (20,73.28) (21,73.70) (22,73.02) (23,73.77) (24,73.76) (25,73.58) (26,73.93) (27,73.56) (28,73.69) (29,74.13) (30,73.98) (31,73.82) (32,74.01) (33,73.94) (34,74.07) (35,73.93) (36,74.15) (37,73.86) (38,74.04) (39,74.02) (40,73.98)
};
\addlegendentry{with BN}
\addplot[orange, mark=*, mark options={fill=orange}] coordinates {
(1,29.37) (2,31.83) (3,42.01) (4,46.92) (5,51.07) (6,55.18) (7,56.00) (8,59.91) (9,59.46) (10,62.36) (11,64.89) (12,64.32) (13,66.80) (14,66.89) (15,68.00) (16,68.45) (17,69.20) (18,70.25) (19,70.22) (20,71.60) (21,71.56) (22,71.34) (23,71.73) (24,72.06) (25,72.56) (26,73.03) (27,72.31) (28,73.12) (29,73.18) (30,73.46) (31,73.51) (32,73.55) (33,73.86) (34,74.06) (35,73.96) (36,73.74) (37,73.66) (38,73.73) (39,73.73) (40,73.78)
};
...
\noindent CIFAR-10, 8-conv arch, plain SGD (lr~0.1, cosine${+}$warmup)
on the shared pipeline, 40 epochs, BN vs no-BN, through the verified
renderer (\texttt{runs/ablation\_cifar8/\{nobn,bn\}\_sgdsched.log}).

\noindent BN leads from the second epoch and is essentially flat by
epoch~16; the un-normalized net traces the same arc about a dozen epochs
behind and pulls dead level by the end. Same ceiling --- about 74\% ---
reached sooner. BN is not buying capacity here, only time: under this
optimizer the two finish in a statistical tie. The head start is largest
precisely for plain SGD, where conditioning matters most; the momentum
and Adam runs below smooth the gradient enough on their own that BN's
lead shrinks --- and there, as the next table shows, the un-normalized
net actually ends a touch ahead. The three-term backward from
\S\ref{thm:bnNormalize_has_vjp} is what makes the curve trustworthy: the
machine-checked VJP guarantees every one of those faster steps computes
the right gradient.
...
\hline
no BN  & 73.8 / 74.1 & \textbf{77.0 / 77.2} & 74.1 / 74.3 \\
BN     & 74.0 / 74.1 & \textbf{76.1 / 76.3} & 73.5 / 73.7 \\
\hline
\end{tabular}
\end{center}

\noindent Momentum wins outright --- two to three points over plain SGD
in both nets. Plain SGD and Adam, by contrast, are a statistical tie: on
a convolutional net this small, Adam's per-coordinate second-moment
scaling has nothing to grip. And reading \emph{down} the columns
recovers Lever~1 from the other side --- BN moves the final number by at
most a fraction of a point (a touch down under momentum and Adam, even
under SGD), confirming that its contribution is timing, not ceiling.
...
the difference to the two-to-three points that are really about the
...
\begin{tikzpicture}[
  ...
  \node[term]                       (x)   {$x$};
  \node[box, right=0.5cm of x]      (e)   {$1\times1$ expand\\$C\!\to\!tC$, ReLU6};
  \node[box, right=0.4cm of e]      (dw)  {$3\times3$ DW\\ReLU6};
  \node[box, right=0.4cm of dw]     (p)   {$1\times1$ project\\(linear)};
...
\begin{tikzpicture}[
  ...
  \node[term] (x){$x$};
  \node[box, right=0.45cm of x]   (e){$1\times1$ expand\\$C\!\to\!tC$, ReLU6};
  \node[box, right=0.4cm of e]    (dw){$k\times k$ DW\\ReLU6};
  \node[se, right=0.4cm of dw]    (s){SE gate\\$\times g$};
  \node[box, right=0.4cm of s]    (p){$1\times1$ project\\(linear)};
...
\begin{tikzpicture}[
  ...
  \node[op]   (s1) at (3.0,0){$+$};
  \node[op]   (s2) at (6.6,0){$+$};
  \node[term] (y)  at (8.0,0){$y$};
  \node[proc] (ln1) at (1.3,-1.25){LN};
  \node[proc, right=8mm of ln1] (mh){MHSA};
  \node[proc] (ln2) at (4.6,-1.25){LN};
  \node[proc, right=8mm of ln2] (ml){MLP};
  \draw[skip](x)--(s1);
  \draw[skip](s1)--(s2) node[midway, above, term, blue!45!violet]{$z$};
  \draw[flow](s2)--(y);
  \draw[flow](x) -- (x|-ln1) -- (ln1);
  \draw[flow](ln1)--(mh);
  \draw[flow](mh) -- (mh|-s1) -- (s1);
  \draw[flow](s1) -- (s1|-ln2) -- (ln2);
  \draw[flow](ln2)--(ml);
  \draw[flow](ml) -- (ml|-s2) -- (s2);
  \node[term, below=1.95cm of x, anchor=west, gray!55!black] at (0,0)
    {one transformer block: $z = x + \mathrm{MHSA}(\mathrm{LN}\,x)$, \ $y = z + \mathrm{MLP}(\mathrm{LN}\,z)$};
\end{tikzpicture}
\end{center}
...
\opchiprow{cBr}{\PA}{-7.1}{residual, invResidual, layerScale}
...
\opchiprow{cVi}{\PA}{-9.2}{squeeze-excite}
...
\opchiprow{cRd}{\PA}{-10.7}{MHSA, SDPA, tfEncoder}
...
\node[font=\sffamily\scriptsize\itshape, gray!55!black, anchor=west] at (0,-12.3)
  {color $=$ op-type (orange weights $\cdot$ gray activation $\cdot$ teal norm/reshape $\cdot$ brown skip $\cdot$ violet multi-branch $\cdot$ red attention) \quad\quad column $=$ which part \emph{introduces} the op};
\end{tikzpicture}%
}
\end{center}
