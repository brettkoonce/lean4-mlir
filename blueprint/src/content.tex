...
\noindent Twenty artifacts: each tier at one and four replicas, in whichever
precisions fit. Effective batch $2048$ throughout --- $512\times4$ on the
reference, $8\times64$ or $4\times128$ here. \textbf{A1 is not ``the same render
as A2'', and the difference is one line}: the decay is baked, so A1's $0.01$
against A2's $0.02$ is a re-render. The two are identical in $18{,}000$ other
lines, kept apart so that one cannot be lost.
...
switches both \emph{off}, which is why neither was visible from A3 succeeding.
The shadow and the accumulator used to be the \emph{same} fourth region of
$[\theta|m|v|\cdot]$, and accumulation is not optional at $224^2$ on a $16$\,GB
card: EMA was unreachable, not merely unrendered.
...
a real mask, and by exactly zero at an all-ones one.

\medskip\noindent \textbf{What each render costs on one card.} XLA's own
\texttt{peak\_memory\_in\_bytes}, against a $15.11$\,GiB budget --- which is a
setting and not the hardware, as the paragraph after the table explains:
...
depth costs nothing measurable.

\medskip\noindent \textbf{And the two budget columns are the point.} A PJRT client
created with no allocator options takes the plugin's default reservation of
$0.75$, which is where $11.68$ comes from --- leaving four gigabytes of a $16$\,GB
card unreachable. \texttt{XLA\_PYTHON\_CLIENT\_MEM\_FRACTION} does not change
that: the plugin never reads it. JAX's Python layer reads it and passes
\texttt{memory\_fraction} as a \emph{create option}, so the JAX trainers could
always be told to use the whole card and the verified ones could not. The shim now
passes the same option, and XLA's own log line is the measurement ---
\emph{allocating 15.11GiB} against $11.68$ unset. \textbf{The largest render goes
from not fitting to $79\%$, and nothing about the hardware moved.}

\medskip\noindent \emph{Also worth knowing before quoting a peak}: the fp32
$4\times128$ figure is $11.52$\,GiB under the default budget and $11.91$ under the
raised one. XLA rematerialises less when it has room, so a compile-time peak is
not independent of the allocator it was compiled against.
...
normalises over its own $64$; the reference's
\texttt{jnp.mean(x,\ axis=(0,2,3))} reduces over a \emph{mesh-sharded} axis, so
XLA inserts one and its group is the full $512$ --- its per-device tensor is
$128$, and only the group is the statistic. Half the gap is a re-render, $k=4$ at
$128$ per device; the last $4\times$ needs synchronised batch-norm --- a new
batched operator, a changed backward, a variance that does not average, and $53$
collectives a step.

\medskip\noindent \textbf{The reference had the same feature wrong in a different
way}: four of its five block emitters drew a scalar \texttt{bernoulli} for the
whole batch where timm's is per \emph{sample}. Same expectation, which is why it
survived --- at keep $0.95$ the old form dropped all $64$ examples or none, on
$200$ steps out of $200$.
...
constants. Compute-only, so each is a lower bound: scaling by the one tier where
both paths have a number ($19.2$ verified minutes per epoch against $7.5$) puts a
