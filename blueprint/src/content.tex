...
init/forward/backward/save/reload round-trip). On the four RTX 4060
Ti (CUDA, bf16, batch 256 across the four GPUs), steady-state
throughput is $\sim$185\,ms per step, about $15.9$ minutes per epoch
--- the most expensive per-epoch of any net in this book, driven by
the $7\times7$ depthwise and the channel LayerNorms. The 80-epoch
validation tier projects to roughly 21 wall-clock hours and the full
300-epoch recipe to about 80 hours (a bit over three days).

\begin{center}
\begin{tabular}{l l r r r r r}
\hline
GPU & Precision & Per epoch & 80 epochs & 300 epochs & Val top-1 & Val top-5 \\
\hline
4$\times$ 4060 Ti (CUDA) & bf16 & $\sim$15.9 min & $\sim$21 hr & $\sim$80 hr & TODO & TODO \\
\hline
\end{tabular}
\end{center}

\noindent (Throughput is measured from steady-state ms/step; the run
is not yet trained to completion, so accuracy is pending. With EMA $+$
stochastic depth now on, the remaining gap to the paper recipe is the
300-epoch schedule and the augmentation pack above.)
