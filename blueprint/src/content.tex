...
\subsection*{Is that MLIR verified? In what sense, and to what tolerance}

That 20\,502-character line is the entire training step --- the forward
pass through all three dense layers, the softmax cross-entropy loss, the
backward pass (the VJP of every layer), and the optimizer update for each
of the $669\,706$ parameters --- emitted as one \texttt{stablehlo} block
that IREE compiles once and then runs per mini-batch. The question this
section answers is the one that matters for a book about \emph{verified}
learning: in what precise sense is that emitted code correct, and how do
we know?

\medskip\noindent\textbf{What is already proven.} The mathematics is
settled earlier in this chapter. \texttt{mlpForward} is the
...
\texttt{softmaxCE\_grad}. Each closes under the three standard axioms
(\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}) --- no
\texttt{sorry}, no \texttt{native\_decide}. So \emph{what} the gradient
is, is not in question.

\medskip\noindent\textbf{The gap, and how we close it.} The GPU never
runs those Lean functions; it runs the emitted \texttt{stablehlo}. The
dangerous failure mode of any code generator is that the math is right
while the emitted op contracts the wrong axis, transposes backwards, or
silently drops a term --- and the only thing traditionally linking the
proof to the emitted string is a code comment. We replace that comment
with a theorem. The emitted backward is given a Lean datatype,
\texttt{Back}, with a denotation $[\![\cdot]\!]$ valued in the proofs'
own vector type. For the MLP the graph is
\[
  \texttt{emitMlpBack}
...
v \text{ else } 0)$. The payoff is the bridge theorem
\[
  \texttt{mlp\_whole\_bridge}:\quad
  [\![\texttt{emitMlpBack}]\!](dy)
  = \texttt{mlp\_has\_vjp\_at}.\mathrm{backward}(dy),
\]
proved by bridging each op (\texttt{dense\_at\_bridge},
\texttt{relu\_at\_bridge}) and composing them with the IR-level chain
rule \texttt{denote\_subst} --- the exact analogue of
\texttt{vjp\_comp\_at}. The denotation of the emitted graph \emph{is},
by machine check, the proven derivative. The forward
($[\![\texttt{emitMlpFwd}]\!] = \texttt{mlpForward}$), the loss
cotangent, and the parameter gradients are bridged the same way, so
every mathematical operation in the train step is covered. A small
computable printer then walks that same graph and emits one
\texttt{stablehlo} op per node ($\texttt{dotGeneral}\,W \mapsto
\texttt{stablehlo.dot\_general}$; $\texttt{selectPos}\,p \mapsto
\texttt{compare GT 0} + \texttt{select}$), so the emitted text \emph{is}
the printout of \texttt{emitMlpBack} by construction.
...
\medskip\noindent\textbf{Proven versus trusted.} The honest accounting:
\begin{itemize}
  \item \emph{Proven} (Lean, three axioms): the IR's denotation equals
    the proven derivative, and likewise for the forward, loss, and
    gradient bridges.
  \item \emph{By construction}: the emitted StableHLO is the rendering
    of exactly that graph.
  \item \emph{Trusted} (validated, not proven): that the StableHLO
    \emph{text} denotes $[\![\cdot]\!]$ --- a faithful printer, whose
    proof would require a formal semantics of StableHLO text that does
    not yet exist; that IREE lowers it correctly; and that
    \texttt{float32} approximates $\mathbb{R}$.
\end{itemize}
So the claim is exact and bounded: \emph{the gradient the GPU computes
is, by a machine-checked theorem, the network's exact reverse-mode
derivative over $\mathbb{R}$ --- up to a faithful printer, IREE, and
floating point.} The first half is a proof; the rest is tested, end to
end. Compiled by IREE and run on the GPU, the emitted train step updates
all six MLP parameter tensors to within $1.2\times10^{-7}$ of an
independent NumPy SGD reference. The irreducible unproven surface is one
$\sim$200-line printer plus IREE plus float --- where a conventional
generator leaves thousands of lines of string-building linked to the
proof by nothing at all.

\medskip\noindent\textbf{One honest caveat.} The ReLU bridge holds at a
\emph{smooth point} --- an input where no pre-activation is exactly
zero. At the kink ReLU has no derivative, and the code uses a
subgradient (the \texttt{compare GT 0} sends the zero case to $0$); the
equality is permitted to fail precisely on that measure-zero set, and
nowhere else. This is the smallest setting in which the technique
appears; the same three pieces --- denoted IR, bridge theorem, printer
--- carry unchanged to the CNN of Chapter~\ref{chap:cnn} and to a full
transformer block.
...
\subsection*{Is that convolution backward verified? The reversed-kernel VJP}

The CNN's emitted train step is 21\,884 characters --- the two
convolutions, the max-pool, the flatten, the three dense layers, the
softmax cross-entropy loss, the backward pass for each of them, and the
SGD update for all $3\,489\,130$ parameters, as one \texttt{stablehlo}
block IREE compiles once and runs per mini-batch. The new machinery this
chapter adds over the MLP is the \emph{convolution}, and its backward is
the part most likely to be silently wrong --- so it is the part we pin
with a theorem.
...
Each closes under the three standard axioms (\texttt{propext},
\texttt{Classical.choice}, \texttt{Quot.sound}) --- no \texttt{sorry}, no
\texttt{native\_decide}. So \emph{what} the convolution's gradient is, is
not in question.

\medskip\noindent\textbf{The gap, and how we close it.} The dangerous
failure mode of a convolution code generator is specific and easy to hit:
the backward op transposes the wrong pair of axes, reverses one spatial
dimension instead of two, or contracts input against output channels ---
each a one-token change that still type-checks and still yields a
plausibly-shaped tensor, and the only thing traditionally linking the
proof to the emitted string is a code comment. We replace that comment
with a theorem. The emitted backward convolution is given a denotation
$[\![\cdot]\!]$ valued in the proofs' own tensor type,
\[
...
\]
where $\bar k = K - 1 - k$ is the spatial flip. The payoff is the bridge
theorem
\[
...
\]
proved by expanding both sides at the concrete shape --- the
reversed-kernel identity that \texttt{cnn\_has\_vjp\_at} only \emph{asserts}
is here discharged by machine check (and again at the $2{\to}2$ shape, by
\texttt{conv\_back\_bridge\_2to2}). The denotation of the emitted graph
\emph{is}, by machine check, the proven conv input-VJP. A small computable
printer then walks that same graph and emits one \texttt{stablehlo} op per
node --- $\texttt{transpose}$ for the channel swap, $\texttt{reverse}$ for
the spatial flip, $\texttt{convolution}$ for the contraction --- so the
emitted text \emph{is} the printout of \texttt{convBackDenote}\,$W$ by
construction.
...
\medskip\noindent\textbf{Proven versus trusted.} The same honest
accounting as the MLP:
\begin{itemize}
  \item \emph{Proven} (Lean, three axioms): the emitted conv-backward
    graph's denotation equals \texttt{conv2d\_has\_vjp3}'s backward, and
    likewise for the forward, max-pool, dense, and loss bridges.
  \item \emph{By construction}: the emitted StableHLO is the rendering of
    exactly those graphs.
  \item \emph{Trusted} (validated, not proven): that the StableHLO
    \emph{text} denotes $[\![\cdot]\!]$ --- the $\sim$200-line printer,
    whose faithfulness would need a formal semantics of StableHLO text
    that does not yet exist; that IREE lowers it correctly; and that
    \texttt{float32} approximates $\mathbb{R}$.
\end{itemize}
Compiled by IREE and run on the GPU, the emitted conv backward matches an
independent NumPy convolution input-VJP to $1.9\times10^{-6}$, and the
full CNN train step updates all four parameter tensors --- the
convolution's weight and bias and the dense head's weight and bias --- to
within $1.2\times10^{-7}$ of an independent NumPy SGD reference. So the
gradient the GPU computes through the convolution is, by a machine-checked
theorem, the network's exact reverse-mode derivative over $\mathbb{R}$ ---
up to that one printer, IREE, and floating point.

\medskip\noindent\textbf{One honest caveat.} The CNN has two kinked ops,
not one. The convolution itself is linear, so its bridge is
\emph{unconditional} --- it holds at every input. But the relu bridges
hold only at a smooth point (no pre-activation exactly zero), and the
max-pool bridge holds only where every $2\times2$ window has a unique
strict argmax; each equality is permitted to fail precisely on that
measure-zero set, and the max-pool tie set is exactly the codegen trust
boundary for pooling. As in the MLP, the listing is shown at a
representative scale ($1{\to}2$ channels at $4\times4$, where the bridge is
discharged by expansion) rather than the production $1{\to}32$ net at
$28\times28$; the per-op proofs are dimension-parameterized and the
generator composes them. The same three pieces --- denoted IR, bridge
theorem, printer --- carry unchanged to the BatchNorm of
Chapter~\ref{chap:bn} and the residual net of
...
It was written with the help of a machine. The proofs in it were
checked by Lean's kernel, which does not care what hand produced
them, whether a human or a model. The prose, the code, and the path
to both were drafted in collaboration with Qwen and Claude --- which
...
\noindent Where the words are clumsy or wrong, blame not the kernel nor
