...
data}) --- smoke-tested rather than trained. What stops them is the renderer:

\begin{center}
\begin{tabular}{@{}l c c l l l@{}}
\hline
tier & epochs & train res & renderer work & verified wall clock & top-1 \\
\hline
A3 (\texttt{rsb-faithful}) & 100 & $160$ & --- & $\sim$32.1 hr & $\mathbf{77.91\%}$ \\
A2 (\texttt{a2-accum})     & 300 & $224$ & LAMB\,+\,BCE at $224$, bf16 & \texttt{[todo]} & $79.8\%$ (paper) \\
A1 (\texttt{a1})           & 600 & $224$ & same render as A2          & \texttt{[todo]} & $80.4\%$ (paper) \\
\hline
\end{tabular}
\end{center}

\noindent Effective batch 2048 as $512\times4$.
...
built as \texttt{lake build mobilenetv4-imagenet-verified}. The renders come
...
\medskip\noindent\textbf{[TODO: run \texttt{mobilenetv4-imagenet-verified}.]}
...
\hline \\[-6pt]
\texttt{resnet50-imagenet-verified}    & \texttt{a2-accum} & render owed & \S\ref{sec:r50_a2_a1_cost} \\
\texttt{resnet50-imagenet-verified}    & \texttt{a1}       & render + shim owed & \S\ref{sec:r50_a2_a1_cost} \\
\texttt{mobilenetv4-imagenet-verified} & Conv-M            & rendered; single-device only & \S\ref{sec:mnv4_side_quest} \\
...
\noindent \textbf{``Single-device only'' is a missing gate, not a missing
render.} MobileNetV4's data-parallel artifact is on disk and it is deliberately
not quotable: this book does not report a collective it has not tied against a
single-device peer, and MNv4 has no \texttt{shard-check} arm. So it trains, on
one card, at four times the wall clock a job config would cost it.
