...
Reading through Ch 5 is enough to know the framework works.
...
\paragraph{Going further.} For a reinforcement-learning detour, read
the AlphaGo Zero paper (\hyperref[bestiary:alphagozero]{bestiary
entry}), which combines reinforcement learning with a large residual
network of $3\times 3$ convolutions. And if you have the compute, run
the ResNet-50 ImageNet demo in the \texttt{jax/} folder
(\texttt{jax/MainResnet50Imagenet.lean}).
...
obvious next lever. The rest is augmentation-fidelity drift --- our
RandAugment, RandomResizedCrop, and Random-Erasing are a
from-scratch \texttt{tf.data} reimplementation rather than timm's
exact code --- plus a few small knobs (batch $512$ vs.\ $1024$, the
...
(\texttt{jax/MainResnet50Imagenet.lean}) wires that recipe.
\textbf{[TODO: run with phase-2 trainer]}
