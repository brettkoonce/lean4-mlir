...
\noindent Neither regulariser is what makes this tight: the fifth region costs
$0.19$\,GiB, and stochastic depth nothing measurable in memory or in time.
\textbf{The two budget columns are the point.}
...
\hline
4$\times$ 4060 Ti (CUDA)  & $187 \to 136$ & $15.6 \to 11.3$ & $\sim$95 $\to$ $\sim$70 h \;($3.9 \to 2.9$ d) & TBD \\
...
default; the run will be bf16, so read the right-hand column. Both are at
\texttt{SHIM\_WORKERS=8}, which is worth $216 \to 187$\,ms on the fp32 arm; $16$ is
\emph{worse} than $8$ on both, and on the bf16 arm worse than fp32 at $8$ --- half of that
$136$\,ms step is no longer the graph, so this net is now as much data pipeline as compute.
...
Box & ms/step & min/epoch & 350 epochs & Val top-1 \\
\hline
4$\times$ 4060 Ti (CUDA)  & $357 \to 280$ & $29.8 \to 23.4$ & $\sim$177 $\to$ $\sim$140 h \;($7.4 \to 5.8$ d) & TBD \\
4$\times$ 3060 (CUDA)     & ---           & ---             & ---                                           & TBD \\
\hline
\end{tabular}
\end{center}
...
same architecture, so the ${\sim}2.7\times$ per epoch that remains is the lowerer, not
the cast and not the network.
...
\medskip\noindent \textbf{What it costs to render: how many dimensions move, not
how many parameters.} Depth is already a loop, so S --- twenty-two million
parameters heavier than T --- came free, with \emph{no} renderer work at all.
B's widening cost twenty-seven threaded literals for half as much again. That is
the whole of what ``not the same job at all'' means here, and it is the opposite
of what a parameter count predicts. \S\ref{sec:vit_sb} finds no such split.

\medskip\noindent \textbf{What bf16 buys: the two large sizes convert better
than the small one}, $2.42\times$ and $2.40\times$ against $2.20\times$, and
depth versus width does not separate them --- S leads B by $0.02\times$, which is
inside what the measurement resolves. \S\ref{sec:vit_sb} finds a real split on
ViT; this net does not. Note how much of it the trainer eats: a $2.42\times$
graph is a $1.80\times$ run, because the shim feed and the all-reduce do not get
faster.

\medskip\noindent \textbf{A full schedule}, four cards, $300$ epochs, totals
including \evalckptnote:
...
\noindent All six are direct $40$-step probes against real ImageNet, through the
job configs the runs would actually use. \textbf{Take the bf16 column as the cost
of these runs} --- the configs still name the fp32 artifact, so flipping
\texttt{LEAN\_MLIR\_VARIANT} to the bf16 twin is worth $63$, $128$ and $184$
hours respectively, and every twin is already rendered.

\medskip\noindent \textbf{The step count is what costs, and a bigger batch does
not fix it.} That $10{,}009$ is double every other net in the book, and it is
what the $300$-epoch schedule really pays for. The obvious remedy fails on
measurement rather than on memory: the block interior's cost per \emph{image} is
flat from batch $8$ to $128$ ($1.03\times$ across that whole range), so doubling
the batch halves the step count and doubles the step. Memory was never the
constraint. \S\ref{sec:vit_sb} asks the same question of ViT and gets the
opposite answer, because there the per-image cost does improve.

\sqstatus{} Artifacts emit, shapes tie, counts are guarded, forty steps run
under each size in both precisions. No accuracy, and the numeric ties against
the JAX peers have not been run.

\medskip\noindent \textbf{Launching it.} Both have job configs ---
\texttt{cnxs-\allowbreak default-\allowbreak 4gpu} and
\texttt{cnxb-\allowbreak default-\allowbreak 4gpu} --- \textbf{and both refuse
to start if the allocator fraction is raised.} That setting is what lets ViT-B
run at all (\S\ref{sec:vit_sb}); here it crashes both bf16 arms where the
default leaves them fine. The smaller graph failing while the larger one
survives is the sign that whatever ran out was never the graph. The phase-2
peers, at twice the batch and so half the steps, take $71.6$, $108.8$ and
$166.5$ hours compute-only, so the ${\sim}1.5\times$ gap to the bf16 rows above
is the batch and not the lowerer.
...
\medskip\noindent \textbf{What it costs to render: uniformly costly.} Where
\S\ref{sec:convnext_sb} found a split between a free variant and a paid one, ViT
has none to find: every variant moves $D$, so every variant moves dimension
literals. \texttt{ViTRender.lean}, the per-example renderer, is still pinned at
Tiny by roughly $154$ of them, so unlike ConvNeXt there is no Imagenette demo at
either size.

\medskip\noindent \textbf{What bf16 buys: ViT-S's $1.89\times$ is the largest
payoff in the book}, ahead of ResNet-50's $1.66\times$ measured the same way.
Tiny's matmuls are narrow enough to underuse the tensor cores even in bf16; S's
$D = 384$ suits them; B pays a $346$\,MB \emph{fp32} all-reduce every step, a
fixed cost in both arms. The one op where bf16 \emph{loses} --- the stem weight
gradient, at $0.19\times$ for want of a bf16 kernel at a $209\times209$ window
--- is kept out of the emit at every size, which is why all three ratios exceed
one. \S\ref{sec:convnext_sb} finds the same middle-size peak for the same
reason.
...
ViT-B  & $512$ & $2{,}502$ & $1381 \to 837$  & $291 \to 178$\,h \;($12.1 \to 7.4$\,d) \\
\hline
\end{tabular}
\end{center}

\noindent Re-probing Tiny and S alongside B gives $241 \to 166$ and $528 \to
319$ against the committed rows, so all three sit on one basis. These are probes
and not arithmetic: scaling Tiny's trainer step by the ratio it bears to its own
graph time overestimates S by $19\%$, because the device figures are already
four-replica and a trainer adds only the data feed, an additive constant near
$65$\,ms at $128$ images per device. \S\ref{sec:convnext_sb}'s are
single-device, where a multiplier is the right shape instead.

\sqstatus{} The only accuracy calibration is this repo's ViT-Ti, and it is
now $72.31\%$ against DeiT-Ti's $72.2\%$ --- the $1.9$-point recipe gap
that used to sit here has closed (\S\ref{sec:vit_imagenet}). DeiT quotes
$79.8\%$ for S and $81.8\%$ for B. Do not read a closed gap at Tiny as a
closed gap at S and B: the largest single item behind it was a weight-init
mismatch that scales as $1/\sqrt{d}$ and is therefore \emph{worst} at Tiny
--- $3.6\times$ there against $2.6\times$ at S and $1.8\times$ at B. The
same fix is wired for all three (\texttt{vitInit}), but it has the least
room to help on the two that have not run.

\medskip\noindent \textbf{Launching it.} Both have job configs ---
\texttt{vits-\allowbreak default-\allowbreak g512-\allowbreak 4gpu} and
\texttt{vitb-\allowbreak default-\allowbreak g512-\allowbreak 4gpu} --- and they
differ in one line. \textbf{B is the one net in the book that will not fit a
default allocator}: PJRT's CUDA plugin reserves three quarters of a card, B's
fp32 graph wants more, and left alone it dies on its first step. Raised to
$0.97$ it runs at $93\%$ of what it then gets, and its driver refuses to start
without the setting, since the error it prevents reads as a card being too small
and B is the net where that reading is wrong. S's config does not raise it, and
an early version did --- it looked like free headroom for a graph that already
fits, and the same setting turns out to kill ConvNeXt's bf16 arms outright
(\S\ref{sec:convnext_sb}). Raise it for a graph that does not otherwise fit;
leave it alone for one that does. The phase-2 peers, same box at global $512$,
cost $25.8$, $60.4$ and $146.1$ hours compute-only, and \textbf{B is the only
one that is not a single forward}: it peaks at $11.41$ against that same default
arena and dies, so it runs as $4\times128$ accumulation. Whether it too fits
once the arena is raised has not been measured.
