...
\medskip\noindent\textbf{Where this stands.} As of this writing, the linear
classifier above is the one model carried all the way through this scheme ---
its loss, backward, and SGD update each tied to a machine-checked theorem,
fused into the single step shown and running on GPU. The deeper architectures
of Part~I and the bestiary are proved operator by operator and code-generated;
bringing each one's whole training step under the same end-to-end check is in
progress, one model at a time. Until that work is finished the claims here
stay scoped to what is done --- but when it is, the book earns its stronger
sentence, \emph{every architecture in this book trained end-to-end under one
machine-checked scheme}, and this note retires.
