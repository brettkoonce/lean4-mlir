...
The code is illustrative: our \texttt{DatasetKind} ships
\texttt{.mnist}, \texttt{.cifar10}, \texttt{.imagenette},
\texttt{.pets} but not yet \texttt{.imagenet}. Running it for real
would add one constructor plus a streaming ImageNet loader (the
1.28M-image training set doesn't fit in memory the way Imagenette's
9.5K does). What's worth noting is that everything else --- the
architecture, the \texttt{.train} invocation, the recipe knobs ---
is already in place and would carry over unchanged. The framework
isn't doing anything fundamentally different at ImageNet scale;
it's just the data that's bigger.
