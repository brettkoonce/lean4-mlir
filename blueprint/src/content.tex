...
Chapters~1--9 there are \textbf{44 contracts} and \textbf{16 witnesses}.
...
3 (MNIST: 2D)     & 3  & 4   & conv2d, maxPool2, whole-net MNIST CNN; three hand-built witnesses and one at trained weights \\
4 (CIFAR: BN)     & 6  & 1   & per-channel BN (flat $+$ NCHW), CIFAR whole-nets $\pm$BN at 2 and at 8 convs \\
5 (ResNet-34)     & 11 & 6   & residual ($+$proj), GAP, stride-2 conv ($+$its weight grad), strided block, stage, whole-net \texttt{cnn}; \textbf{full-depth ResNet-34 and ResNet-50} at batch statistics; live-shaped witnesses up to $224 \times 224$ \\
6 (MobileNetV2)   & 6  & 3   & depthwise (stride 1$+$2); \texttt{mobilenetv2} at representative depth and at \textbf{all 17 blocks}, per example and at batch statistics; \textbf{\mbox{MobileNetV4}} at full depth \\
7 (EfficientNet)  & 4  & --- & swish, SE, whole-net at representative depth, \textbf{full B0} (16 \mbox{MBConv} blocks) \\
8 (ConvNeXt)      & 5  & --- & GELU, LayerNorm, layer-scale, whole-net at representative depth, \textbf{full ConvNeXt-T} (18 blocks) \\
9 (ViT)           & 6  & --- & MHSA, block, \textbf{every depth $k$} (ViT-Tiny is $k = 12$), \texttt{vit\_full} \\
\hline
                  & \textbf{44} & \textbf{16} & total \textbf{60} \\
\hline
\end{tabular}
\end{center}
...
\texttt{mobilenetv2VerifiedB\_fwd\_faithful} above it. The same list at
per-example BatchNorm, \texttt{mobilenetv2Verified\_denote\_eq}, is the
forward of the retired SGD artifact.
