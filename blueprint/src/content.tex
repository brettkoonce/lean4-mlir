...
all four measured on the verified XLA path:
...
\noindent \textbf{Provenance.} All four rows are verified-XLA
measurements on one RTX 4060 Ti, from \S\ref{sec:r34_runit},
\S\ref{sec:mnv2_runit}, \S\ref{sec:enet_runit} and
\S\ref{sec:convnext_runit}. Step time is the epoch wall-clock divided by
the 295 batches in it, so it includes each epoch's validation pass.
...
over a 20-epoch warmup. No architectural or recipe deviation from the
paper remains, and the schedule is no longer short of it either. The
run below is the full 300-epoch tier.
...
Lean$\to$JAX-codegen reimplementation. The residual $\sim$1\% is the
usual EMA-eval / test-crop / augmentation-detail territory, not the
architecture or the recipe.
...
\hline
4$\times$ 4060 Ti (CUDA)  & 223 & 37.2 & $\sim$186 h (7.8 d) & TBD \\
\hline
\end{tabular}
\end{center}

\noindent That is 10,009 steps per epoch, against the reference's 5,004.
\textbf{Do not read 37.2 minutes against the phase-2 rows' 14.9 as an
architecture result}, because two things differ at once and neither is
the network. The verified path is fp32 throughout, with no bf16 artifact
anywhere in \texttt{verified\_mlir/}, while every phase-2 row in this
section was measured in bf16. And the verified render's batch is 32 per
device, so four replicas give a global 128 against phase 2's 256. The
batch is 32 because \texttt{cBS} is still a private constant in the
renderer while \texttt{nClasses} became a parameter, which is a refactor
rather than a limit.
