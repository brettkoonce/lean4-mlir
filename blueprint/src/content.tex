...
to happen eventually but not soon enough to matter for that book.
...
have changed, and the thing I wished for is sitting in a GitHub repo
today, training ResNets on a single GPU, with machine-checked proofs that
...
is open the hood, look at what \texttt{.backward()} computes layer by
layer, and check that it's mathematically correct.
...
\begin{enumerate}
  ...
  \item \textbf{Binary selection.} Max pooling routes the gradient to
    whichever input happened to be the max.
...
covers U-Net, DETR, Mamba, GAN, VAE, diffusion models, CLIP, and
...
first book, fast.ai's free courses, and plenty of good material all over
the web. Come back to this one when you've trained something and want to
...
Put the three together and one person can do the whole thing. You write
