...
backward at full depth for the smooth three, at representative depth for the
two ReLU nets (their parameter gradients certified through the chain closes);
three classical axioms. For ViT-Tiny and ConvNeXt-T the proven graphs were
re-rendered at the committed trainers' signatures and agree on GPU to float
rounding. The one open gap: the
...
\item \textbf{Training recipe upgraded.} The first book's codegen
  ...
  data augmentation --- the DAWNBench 2018 recipe, and how this book's
  Imagenette results were achieved. That full recipe is assembled and
  ablated in Chapter~\ref{chap:residual} (ResNet); the later chapters
  add full ImageNet recipes, trained through a Lean 4 \(\to\) JAX
  bridge, with modern data augmentations.
...
\begin{tikzpicture}[
  ...
  every node/.style={font=\sffamily\footnotesize},
  io/.style     = {draw=blue!55!black,   rounded corners=2pt, fill=blue!8,    align=center, inner sep=4pt, minimum height=1.25cm, minimum width=2.0cm},
  dense/.style  = {draw=orange!65!black, rounded corners=2pt, fill=orange!12,  align=center, inner sep=4pt, minimum height=1.25cm, minimum width=2.2cm},
  head/.style   = {draw=red!60!black,    rounded corners=2pt, fill=red!8,      align=center, inner sep=4pt, minimum height=1.25cm, minimum width=2.2cm},
  logits/.style = {draw=green!50!black,  rounded corners=2pt, fill=green!14, very thick, align=center, inner sep=4pt, minimum height=1.25cm, minimum width=2.0cm},
  arr/.style    = {->, thick, gray!60, shorten >=1pt, shorten <=1pt},
]
  \node[io]                    (input) {Input\\$28\times28$ flatten\\$\to 784$};
  \node[dense, right=of input] (d1)    {Dense\\$784 \to 512$\\ReLU\\401,920 params};
  \node[dense, right=of d1]    (d2)    {Dense\\$512 \to 512$\\ReLU\\262,656 params};
  \node[head,  right=of d2]    (d3)    {Dense\\$512 \to 10$\\(identity)\\5,130 params};
  \node[logits, right=of d3]   (out)   {Logits\\10 classes};
  \draw[arr] (input) -- (d1);
  \draw[arr] (d1) -- (d2);
  \draw[arr] (d2) -- (d3);
  \draw[arr] (d3) -- (out);
...
\begin{tikzpicture}[
  ...
  node distance=0.7cm,
  every node/.style={font=\sffamily\footnotesize},
  io/.style     = {draw=blue!55!black,    rounded corners=2pt, fill=blue!8,    align=center, inner sep=4pt, minimum height=1.3cm, minimum width=2.0cm},
  conv/.style   = {draw=orange!65!black,  rounded corners=2pt, fill=orange!12,  align=center, inner sep=4pt, minimum height=1.3cm, minimum width=2.2cm},
  pool/.style   = {draw=teal!60!black,    rounded corners=2pt, fill=teal!10,    align=center, inner sep=4pt, minimum height=1.3cm, minimum width=2.0cm},
  flat/.style   = {draw=purple!60!black,  rounded corners=2pt, fill=purple!8,   align=center, inner sep=4pt, minimum height=1.3cm, minimum width=2.0cm},
  dense/.style  = {draw=orange!65!black,  rounded corners=2pt, fill=orange!12,  align=center, inner sep=4pt, minimum height=1.3cm, minimum width=2.2cm},
  head/.style   = {draw=red!60!black,     rounded corners=2pt, fill=red!8,      align=center, inner sep=4pt, minimum height=1.3cm, minimum width=2.2cm},
  logits/.style = {draw=green!50!black,   rounded corners=2pt, fill=green!14, very thick, align=center, inner sep=4pt, minimum height=1.3cm, minimum width=2.0cm},
  arr/.style    = {->, thick, gray!60, shorten >=1pt, shorten <=1pt},
]
  % Row 1: input -> conv -> conv -> pool -> flatten
  \node[io]                  (input) {Input\\$28\times28\times1$};
  \node[conv, right=of input] (c1)   {Conv2D\\$1 \to 32$ ch\\$3\times3$, ReLU\\320 p};
  \node[conv, right=of c1]    (c2)   {Conv2D\\$32 \to 32$ ch\\$3\times3$, ReLU\\9,248 p};
  \node[pool, right=of c2]    (p1)   {maxPool\\$2\times2$\\$28 \to 14$};
  \node[flat, right=of p1]    (fl)   {flatten\\$14\times14\times32$\\$\to 6272$};
  % Row 2: dense -> dense -> dense -> logits (wraps below row 1)
  \node[dense, below=1.4cm of input] (d1) {Dense\\$6272 \to 512$\\ReLU\\3,211,776 p};
  \node[dense, right=of d1]   (d2)   {Dense\\$512 \to 512$\\ReLU\\262,656 p};
  \node[head,  right=of d2]   (d3)   {Dense\\$512 \to 10$\\(identity)\\5,130 p};
  \node[logits, right=of d3]  (out)  {Logits\\10 classes};
  \draw[arr] (input) -- (c1);
  \draw[arr] (c1) -- (c2);
  \draw[arr] (c2) -- (p1);
  \draw[arr] (p1) -- (fl);
  \draw[arr] (fl.south) -- ++(0,-0.55) -| (d1.north);   % wrap end-of-row-1 to start-of-row-2
  \draw[arr] (d1) -- (d2);
  \draw[arr] (d2) -- (d3);
  \draw[arr] (d3) -- (out);
\end{tikzpicture}}
...
\textbf{[TODO: these results are tied to the init parameters and need to be redone.]}
%
% WHY THIS NEEDS UPDATING (investigation 2026-06-08, GPU-validated):
% The "one learns, one doesn't" framing overstates the no-BN result. The no-BN
% freeze at SGD lr=0.1 is NOT a law of the architecture --- it is an
% init-conditioning artifact sitting on a knife-edge stability boundary:
%   * This cifar-nobn-sgd ablation cell freezes at log(10)=2.302 (10% = chance).
%   * But the *proof-verified* no-BN net (same architecture, same He init scale,
%     same effective lr=0.1) TRAINS to ~67% --- and does so across all 5 random
%     seeds tested (LEAN_MLIR_SEED). It only collapses above its ~0.25 LR ceiling.
%   * The two differ ONLY in the He-init draw: the ablation uses consecutive
%     seeds (base 42) whose xorshift streams are correlated -> worse-conditioned
%     init -> ceiling drops below 0.1; the verified net uses stride-2 seeds ->
%     ceiling ~0.25. The frozen cell also TRAINS fine at lr=0.05 (~67%), i.e. it
%     is LR-limited, not broken.
% So the honest statement is "no-BN is fragile right at lr=0.1 --- whether a given
% init lands above or below the dead-ReLU collapse edge is luck, and BN removes
% that fragility," NOT "no-BN cannot learn." The verified math was never wrong.
%
% BN SIDE: the 61.57% here is per-channel BN. The verified BN net is now also
% per-channel (commit a8b91ed) and reaches ~71% at lr=0.1; the earlier
% scalar-global verified BN actually UNDERPERFORMED no-BN (~58% < ~67%).
%
% CAVEAT to not overclaim: per-example per-channel (instance-norm) BN gives the
% accuracy lift but NOT extra LR headroom --- it collapses at lr>=0.2, same as
% no-BN. "BN lets you crank the LR" needs batch-statistic BN (train != eval).
%
...
\noindent CIFAR-10, 4-conv arch, SGD~0.1, 30 epochs (BN vs no-BN;
\texttt{logs/ablation\_cifar-\{bn,nobn\}-sgd.log}). The no-BN run sits at
$\log 10 \approx 2.30$ --- random-guess loss --- for the entire run.
...
Why does normalizing help at all? Each layer is tuned for the
distribution of its inputs --- but those inputs are the outputs of every
layer below, which are themselves changing every step. So each layer
chases a moving target, and at a high learning rate the target moves
fast: activations drift to scales where the nonlinearity saturates or
gradients blow up, and the step that helped one layer wrecks the next.
BN removes the moving part --- by pinning every layer's input to
mean-zero, unit-variance before the learnable \(\gamma, \beta\) get a
say, each layer always sees a well-conditioned input no matter what the
layers below just did. Ioffe \& Szegedy framed this as reducing
``internal covariate shift''; later analysis (Santurkar et al., 2018)
argued the sharper reason is that normalization smooths the loss
landscape --- gradients stay predictable from one step to the next ---
which is exactly what lets you take the bigger steps the rest of this
section measures. Either way: BN decouples a layer's job from the chaos
below it, and that decoupling is worth an order of magnitude of learning
rate.
...
backward, update), for ViT-Tiny and ConvNeXt-T at the production trainers'
exact signatures. \textbf{Cross-checked} (empirical): because IREE cannot itself be
...
architecture definitions in the bestiary, across 54 Lean proof files,
with zero project axioms. The axiom audit
(\texttt{tests/AuditAxioms.lean}) re-checks \textbf{421 theorems} in all
...
zero project axioms. It runs 49 theorems --- the foundation rules,
...
The 49-theorem coverage is illustrative rather than exhaustive ---
