...
with RMSProp, AutoAugment, stochastic depth, and EMA --- this SGD
validation tier, the 80 of an 80$\to$300 ladder, is a
soundness-and-throughput check and will sit well under the paper
number until those land.)
...
Two deviations from canonical ConvNeXt, both deliberate so the port
reuses the validated layer types: the patchify stem is
\texttt{.convBn} (BN $+$ a ReLU) rather than conv $+$ LayerNorm
--- param-equivalent, the ReLU a minor variant --- and stochastic
depth and EMA are not yet wired on the JAX path. Those last two are
core to ConvNeXt's 300-epoch recipe; without them, and without the
augmentation pack above, an 80-epoch run lands well under the paper's
$82.1\%$. The 80 here is the validation tier of an 80$\to$300 ladder.
...
\noindent (Throughput is measured from steady-state ms/step; the run
is not yet trained to completion, so accuracy is pending --- and a
faithful run waits on stochastic depth $+$ EMA plus the augmentation
pack above.)
