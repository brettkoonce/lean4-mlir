...
\textbf{Compute budget.} On four RTX 3060 (CUDA, bf16, batch 256, four
replicas of $64$), steady-state throughput is $6.6$ minutes per epoch ---
$374$ s of training plus $18$ s of validation, held to within a second an
epoch across all $350$ --- so the paper's schedule costs $38.4$ wall-clock
hours. The wall-clock win over the $9.5\times$-larger ResNet-34 is real but
...
GPU & Precision & Per epoch & Epochs & Total & Val top-1 & Val top-5 \\
\hline
4$\times$ 3060 (CUDA) & bf16 & $6.6$ min & 350 (exp-decay) & $38.4$ hr & $\mathbf{71.90\%}$ & $\mathbf{90.41\%}$ \\
\hline
\end{tabular}
\end{center}
...
Run end to end on four RTX 3060 it lands at $\mathbf{71.90\%}$ top-1 and
$\mathbf{90.41\%}$ top-5 over the full 50{,}000-image validation split in
$38.4$ hours, one attempt, no restarts and no thermal pauses; the best
epoch, $331$, reaches $71.99\%$. The paper's figure is $72.0\%$, and
\textbf{at $n=50{,}000$ a Wilson 95\% interval on this run is
$[71.50, 72.29]$} --- so the recipe is not distinguishable from the paper,
which is the claim here and a weaker one than closing a gap by a measured
amount:
...
exponential-decay run (bf16, 4$\times$ RTX 3060, RMSProp), 2026-08-30.
...
denominator --- both halves of that protocol
  & \textbf{\texttt{convBnAct := .relu6}} --- the stem and the $1\times1$ head trained
    ...
    2026-08-30, one day after this run started, and it is a \emph{phase-2 only} defect:
...
\medskip\noindent\textbf{And now it has run.} The phase-4 trainer
completed the full 350-epoch schedule, scoring
$\mathbf{71.912\%}$ top-1 and $\mathbf{90.520\%}$ top-5 over the
same $50{,}000$ validation images the phase-2 reference was scored on.
...
ms/step              & 97                       & 79 \\
per epoch            & 8.8 min                  & 6.6 min \\
total                & 51.2 hr                  & 38.4 hr \\
...
\hline
\textbf{val top-1}   & $\mathbf{71.912\%}$      & $\mathbf{71.90\%}$ \\
\textbf{val top-5}   & $\mathbf{90.520\%}$      & $\mathbf{90.41\%}$ \\
\hline
\end{tabular}
\end{center}

\noindent Both columns are MobileNetV2 on the paper-faithful RMSProp recipe,
350 epochs, on the same four RTX 3060 cards. Rows above the first rule differ;
rows between the rules are held equal by construction.

\medskip\noindent\textbf{On BN statistic group differences:}
MobileNetV2 has BatchNorm in every block. The
verified path runs four replicas of $64$ with no collective touching the
batch statistics, so it normalizes over groups of $64$; the JAX reference
under a \texttt{NamedSharding} mesh reduces globally, so it normalizes over
$256$. The ResNet-34 pair of \S\ref{sec:r34_pjrt}, which carries the same
fourfold difference and a precision difference besides, landed $0.02$ apart at
convergence. The $+0.012$ here is the same order. So the honest claim is that the two paths agree to
within the BatchNorm-group effect --- not that the lowerer has been shown
neutral. Separating those would take a verified render that all-reduces the
batch statistics, which no committed artifact does --- and which is not a patch
but a new AST node, a new specification and a re-proof of the tie chain, because
the only node in the graph language that sees across replicas is the gradient
collective and every BatchNorm node consumes a single expression.
...
\hline
phase-2 reference & 46.69 & 61.71 & 67.40 & 69.78 & 71.23 & 71.85 & 71.90 \\
phase-4 verified  & 51.36 & 63.82 & 67.74 & 69.97 & 70.95 & 71.84 & 71.91 \\
...
\medskip\noindent\textbf{The remaining $51.2$ against $38.4$ hours is the data
feed, not the renderer.} With the shim removed the verified step is $78$\,ms
--- a $38$-hour schedule, level with the reference --- and the fed step is
$97$. The gap is the tfds shim streaming $602$\,KB per image of fp32 through a
subprocess pipe, $154$\,MB per step at global $256$, which is host work the
JAX trainer does in-process. This is the net where that shows most plainly:
the compute floor is already at the reference's wall clock, so every hour of
