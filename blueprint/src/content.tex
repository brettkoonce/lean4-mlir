...
data}) --- smoke-tested rather than trained. Both now render on the
verified path:
...
A2 (\texttt{a2-accum})     & 300 & $224$ & rendered, fp32 + bf16 & \texttt{[todo]} & $79.8\%$ (paper) \\
A1 (\texttt{a1})           & 600 & $224$ & rendered --- its own decay & \texttt{[todo]} & $80.4\%$ (paper) \\
\hline
\end{tabular}
\end{center}

\noindent Effective batch 2048 as $512\times4$ on the reference; as
$8\times64$ across four replicas on the verified path.

\medskip\noindent \textbf{A1 is not the same render as A2, and the difference
is one line.} This table said it was. The weight decay is a baked
\texttt{stablehlo.constant} rather than a runtime operand, so A1's $0.01$
against A2's $0.02$ is a re-render and not a flag --- the two artifacts
differ in exactly one line of $18{,}000$, and they are kept on separate paths
(\texttt{\ldots wxclipbce} against \texttt{\ldots wxclipbcewd001}) precisely so
that one line cannot be lost. A1's other two deltas are $600$ epochs, which is
a driver knob, and Mixup $\alpha\,0.2$ against $0.1$, which is data-side and
rides its own generated shim.

\medskip\noindent These renders are \emph{not} faithful A2 and A1, and the
ledger below says how --- the same accounting \S\ref{sec:r50_phase4} keeps for
A3, kept here before anything is run rather than after.

\medskip\noindent\emph{In.} What these artifacts carry that a bare A2/A1
transcription would not:

\begin{itemize}
  \item \texttt{wdExcludeNormBias} and LAMB's gradient clip, both inherited
    from the A3 work --- the clip after the all-reduce \emph{and} after the
    accumulation, at threshold $kC$.
  \item BCE-with-logits over Mixup/CutMix soft labels, which is what makes the
    recipe's learning rate the rate it is.
  \item Gradient accumulation $k=8$ to LAMB's design batch of $2048$. This is
    not a convenience: $2048$ at $224^2$ has no other route on a $16$\,GB card,
    and a bs512 LAMB is the regime that gave A3 $40.8\%$ instead of $78.1\%$.
  \item Repeated augmentation $3\times$, which rides the shim.
  \item Both precisions, fp32 and bf16, for both tiers and at one and four
    replicas --- so neither tier waits on a precision peer.
  \item \emph{A1's own everything}: its baked decay $0.01$ on its own artifact
    path, and its own generated shim at Mixup $\alpha\,0.2$. The driver refuses
    to start if that shim is absent rather than falling back to A2's.
  \item \emph{Verification:} the optimizer is tied to the reference at the
    update, six variants to ${\sim}10^{-7}$ --- including A1's decay against an
    emitted A1 trainer that bakes $0.01$, so ``the constant reaches the graph''
...
\begin{itemize}
  \item \textbf{Model EMA} ($\text{decay}=0.9999$), and it is not merely
    unrendered but \emph{unrepresentable}: the EMA shadow and the gradient
    accumulator are the same fourth region of
    $[\theta\,|\,m\,|\,v\,|\,\cdot\,]$, and the driver refuses the combination
    outright rather than letting one win. Since accumulation is not optional
    here, EMA cannot be had at all. Lifting it is a fifth region in the
    pack/unpack and in every optimizer's return list --- a change to the blob
    layout, not a flag.
  \item \textbf{Stochastic depth} ($0.05$). The module exists and EfficientNet
    and ConvNeXt render variants off it; the residual family does not import
    it, and the variant grammar has no marker to ask for it with.
  \item Ghost-BN group $64$ against the reference's $128$ --- immaterial to a
    wall clock, a different regime for an accuracy claim.
  \item Repeated augmentation is a stream-level approximation rather than
    per-batch, which the generated shim says of itself.
  \item Nothing has been trained. The renders emit and the optimizer ties; no
    epoch of either tier has run on this path.
\end{itemize}

\noindent \textbf{A3 met neither of the first two}, because A3's own recipe
switches EMA and stochastic depth \emph{off} --- which is why none of this was
visible from A3 succeeding. So these artifacts are A2's graph minus two
regularisers: they price the tier honestly, since neither changes the step
cost, and they are what a fifth region and a \texttt{drop} marker would extend.
A result off them would be ``the A2 recipe with these deltas'', never ``RSB-A2
reproduced''.

\medskip\noindent \textbf{What the phase-2 peers cost, measured.} The verified
column has one entry because A3 is the only tier that has been run there. The
JAX trainers for all three exist, though, and they set the floor.
\texttt{jax/scripts/step\_probe.py} on four 4060\,Ti, bf16, effective batch 2048
as $4\times512$ --- 625 steps per epoch --- with A3 as the control in the same
session:
...
\noindent \textbf{A1 and A2 carry the same step figure because they are the same
graph}: the emitted trainers differ in exactly three constants ---
\texttt{EPOCHS}, \texttt{WD} and Mixup's $\alpha$ --- so on this path A1 is A2 at
twice the schedule and nothing else, to the digit. \textbf{FixRes is also worth the
$\sim$2$\times$ \S\ref{sec:r50_phase4} claims for it}: A3's $160^2$ step runs
$1.91\times$ faster than A2's $224^2$ at the same batch, measured rather than
asserted. Peak is $4.30$\,GiB for A3 and $7.61$\,GiB for A2 against the
$11.68$\,GiB a 16\,GB card's allocator hands out, so accumulation holds both
well inside one card.

\medskip\noindent These figures are compute-only --- the probe never starts the
\texttt{tf.data} pipeline --- so each is a lower bound. Scaling by the one tier
where both paths have a number (A3 costs $19.2$ minutes per \emph{verified}
epoch against $7.5$ here, a $2.6\times$ ratio) puts a verified A2 near seven days
and a verified A1 near fifteen. That is an extrapolation from a single point and
it is not what the \texttt{[todo]} column is waiting for, but it is the right
order for a budget decision: on this box A2 is a week and A1 is a fortnight.
...
ConvNeXt-B & $88{,}589{,}416$  & ${\sim}27$ literals     & see the bf16 table below \\
\hline
\end{tabular}
\end{center}
...
\medskip\noindent \textbf{The bf16 renders were half-done, and now all three
sizes carry both.} \texttt{convnextsin} lacked its bf16 twins where T and B had
theirs --- an accident of B landing after S rather than anything about the
shape, which is the kind of gap that reads as a decision about the middle size
and is really about the calendar. Closing it was a per-net emit and not new
operator work: S is T's depth table at T's widths, so it instantiates the same
bf16 operators at the same widths, and B is the stronger precedent still,
because it instantiates them at four widths no committed artifact had used.

\medskip\noindent Measured, one session, one 4060\,Ti, batch 32,
\texttt{adamwxclipdrop} against its \texttt{bf16} twin:
...
\hline
ConvNeXt-T & $157.9$\,ms & $121.1$\,ms & $1.30\times$ \\
ConvNeXt-S & $268.4$\,ms & $192.7$\,ms & $\mathbf{1.39\times}$ \\
ConvNeXt-B & $395.7$\,ms & $294.3$\,ms & $1.34\times$ \\
\hline
\end{tabular}
\end{center}

\noindent \textbf{These are the graph's numbers, not a trainer's}, timed on the
device with nothing around them --- which is the only way to make a statement
about a \emph{render}. A trainer's step also moves with the shim feed and with
whether device-resident parameters are switched on, and both dilute the ratio:
the same two ConvNeXt renders that run $1.30\times$ here read $1.18\times$ inside
a four-card run on real data.

\medskip\noindent The reason to trust S's row is that the other two reproduce.
T and B were run as controls in the same session and came back at $157.9$ and
$395.7$\,ms against previously committed bare-device figures of $157.8$ and
$396.0$ --- so the tool and the box agree with the record, and S's $1.39\times$
is a measurement rather than the extrapolation from T that stood here before.
S is also the \emph{fastest} of the three in bf16, which is not what
``deeper costs more'' would predict and is worth not smoothing over: depth adds
blocks at widths bf16 already suits, where B's widening moves every stage.
...
and a few steps run under \texttt{convnext-s-imagenet-verified} and
\texttt{convnext-b-imagenet-verified}. No accuracy, and the numeric ties
against the JAX peers have not been run. On four cards B needs gradient
accumulation even in bf16.

\medskip\noindent \textbf{The phase-2 peers price the schedule.}
\texttt{jax/scripts/step\_probe.py} on four 4060\,Ti, bf16, global batch 256 ---
5{,}004 steps per epoch --- with T as the control:

\begin{center}
\begin{tabular}{@{}l r r r r r@{}}
\hline
phase-2 net & ms/step & device peak & min/epoch & 80 epochs & 300 epochs \\
\hline
ConvNeXt-T & $172$ & $4.22$\,GiB & $14.3$ & $19.1$\,h & $71.6$\,h (3.0\,d) \\
ConvNeXt-S & $261$ & $6.78$\,GiB & $21.8$ & $29.0$\,h & $108.8$\,h (4.5\,d) \\
ConvNeXt-B & $399$ & $9.56$\,GiB & $33.3$ & $44.4$\,h & $166.5$\,h (6.9\,d) \\
\hline
\end{tabular}
\end{center}

\noindent Compute-only --- the probe never starts the \texttt{tf.data} pipeline,
so read the totals as ``at least this long''. \textbf{The renderer's split does
not reach the clock}: each rung costs the same, $1.52\times$ then $1.53\times$,
though S cost the renderer nothing and B cost it twenty-seven literals.
...
\emph{verified} cost column. Of the four variants in this chapter and
\S\ref{sec:convnext_sb}, only ConvNeXt-B is ported.

\medskip\noindent \textbf{Status: rendered, not trained}, the same as ConvNeXt's
pair --- \texttt{vits-\allowbreak imagenet-\allowbreak verified} and
\texttt{vitb-\allowbreak imagenet-\allowbreak verified} emit, tie and step. The
only accuracy calibration that exists is this repo's
...
\medskip\noindent \textbf{What the phase-2 peers cost.} The same probe
\S\ref{sec:convnext_sb} uses, at global batch 512 --- 2{,}502 steps per epoch,
the DeiT recipe's own figure --- with Tiny as the control:
...
phase-2 net & ms/step & device peak & min/epoch & 80 epochs & 300 epochs \\
\hline
ViT-Ti & $124$ & $2.89$\,GiB & $5.2$  & $6.9$\,h  & $25.8$\,h (1.1\,d) \\
ViT-S  & $290$ & $7.72$\,GiB & $12.1$ & $16.1$\,h & $60.4$\,h (2.5\,d) \\
ViT-B  & $701$ & $5.36$\,GiB & $29.2$ & $39.0$\,h & $146.1$\,h (6.1\,d) \\
\hline
\end{tabular}
\end{center}

\noindent \textbf{B is the only row that is not a single forward}: global 512 in
one shot peaks at $11.41$ of the allocator's $11.68$\,GiB and dies, so it runs as
$4\times128$ gradient accumulation --- which is why its peak sits \emph{below}
S's. Compute-only again, and the omitted pipeline costs more here than in
Chapter~\ref{chap:layernorm}: ViT-Ti's live epoch is the $7.6$ minutes
\S\ref{sec:vit_imagenet} quotes against this probe's $5.2$.
...
them cannot be launched, and a job name is exactly what they lack. The status
column says what each one is missing.
...
\hline \\[-6pt]
\texttt{resnet50-imagenet-verified}    & \texttt{a2-accum} & rendered fp32 + bf16; no EMA/sd & \S\ref{sec:r50_a2_a1_cost} \\
\texttt{resnet50-imagenet-verified}    & \texttt{a1}       & rendered + own shim; no EMA/sd & \S\ref{sec:r50_a2_a1_cost} \\
...
\texttt{vit-s-imagenet-verified}       & \withheld         & rendered fp32; bf16 owed & \S\ref{sec:vit_sb} \\
\texttt{vit-b-imagenet-verified}       & \withheld         & rendered fp32 @ global $128$; bf16 + accum owed & \S\ref{sec:vit_sb} \\
EfficientNetV2-S                       & \withheld         & no spec & \S\ref{sec:enet_side_quests} \\
\end{tabular}
\end{center}

\noindent \textbf{MobileNetV4's row shows what a status column is for.} Its
data-parallel artifact sat on disk for two days deliberately \emph{not} quotable:
this book does not report a collective it has not tied against a single-device
peer, and MobileNetV4 had no \texttt{shard-check} arm. Nothing about the artifact
changed when the gate arrived --- the same bytes became reportable, which is the
whole point. What the row still says is \emph{untrained}: a job config and a
verified collective are not a result.
