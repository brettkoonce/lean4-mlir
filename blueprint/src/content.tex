...
lake build LeanMlir            # builds the library — all proof modules included
\end{verbatim}

The library root now imports the entire proof suite (every chapter's
VJP module, transitively), so this single build type-checks all of it
against Lean's kernel --- that is the entire verification, no module
list to keep current. Every VJP formula is proved against Mathlib's
\(\fderiv\) (see Appendix~\ref{app:verification}).
