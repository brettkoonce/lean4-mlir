...
A3 is the cheap tier. The other two exist as recipes --- \texttt{a2-accum} and
\texttt{a1}, selected positionally --- and both now render on the verified path
as well:
...
A1 (\texttt{a1})           & 600 & $224$ & rendered --- own decay & \texttt{[todo]} & $80.4\%$ (paper) \\
\hline
\end{tabular}
\end{center}

\noindent Effective batch 2048, as $512\times4$ on the reference and $8\times64$
across four replicas here. \textbf{A1 is not ``the same render as A2'', and the
difference is one line}: the weight decay is a baked constant rather than a
runtime operand, so A1's $0.01$ against A2's $0.02$ is a re-render, and the two
artifacts --- identical in $18{,}000$ other lines --- are kept on separate paths
so that line cannot be lost. A1's other deltas are $600$ epochs, a driver knob,
and Mixup $\alpha\,0.2$, which rides its own generated shim.

\medskip\noindent \textbf{These renders are not faithful A2 and A1.} The same
ledger \S\ref{sec:r50_phase4} keeps for A3, kept here before anything is run:
...
\texttt{wdExcludeNormBias}; LAMB's clip at $kC$, after both the all-reduce and the accumulation
  & \textbf{Model EMA} --- \emph{unrepresentable}. The shadow and the accumulator want the same fourth region of $[\theta|m|v|\cdot]$ and the driver refuses both; accumulation is not optional, so EMA cannot be had at all. A fifth region, not a flag \\[2pt]
BCE-with-logits over Mixup/CutMix soft labels
  & \textbf{Stochastic depth} $0.05$ --- the residual renderers neither import the module nor have a marker for it \\[2pt]
Accumulation $k=8$ to LAMB's design batch of $2048$, which at $224^2$ has no other route on a $16$\,GB card
  & Ghost-BN group $64$ against the reference's $128$ \\[2pt]
Repeated augmentation $3\times$; both precisions, both tiers, one and four replicas
...
Optimizer tied at the update, six variants to ${\sim}10^{-7}$ & \\
\hline
\end{tabular}
\end{center}

\noindent \textbf{A3 met neither of the first two}, its own recipe switching both
\emph{off} --- which is why none of this was visible from A3 succeeding. These
are A2's graph minus two regularisers: they price the tier honestly, since
neither changes the step cost, but a result off them is ``the A2 recipe with
these deltas'', never ``RSB-A2 reproduced''.
...
as $4\times512$ --- 625 steps per epoch --- with A3 as the control:
...
\noindent A1 and A2 share a step figure because the emitted trainers differ in
three constants, so A1 is A2 at twice the schedule, to the digit. FixRes is worth
its claimed ${\sim}2\times$ --- A3's $160^2$ step runs $1.91\times$ faster at the
same batch --- and peak is $4.30$ and $7.61$\,GiB against the $11.68$ a 16\,GB
card offers, so accumulation holds both inside one card.

\medskip\noindent Compute-only, so each is a lower bound. Scaling by the one tier
where both paths have a number ($19.2$ verified minutes per epoch against $7.5$
here, $2.6\times$) puts a verified A2 near seven days and A1 near fifteen --- an
extrapolation from a single point, but the right order for a budget decision.
...
published totals to within $0.5$\,h. Note how much of bf16 the trainer eats: a
