...
\begin{itemize}
  ...
  \item fp32, not bf16. The 2018 row above \emph{is} bf16, but the only bf16
    ResNet-50 render is that recipe's --- the LAMB\,+\,BCE shape at $160^2$
    has no bf16 twin to run, the same gap that blocks A2 and A1. Worth a
    measured $1.54\times$ on this network, so most of the $2.1\times$ wall
    clock against the JAX row: a throughput lever rather than a fidelity one.
...
\medskip\noindent \textbf{What these renders are not: faithful A2 and A1.} Two
of A2's regularisers have no expression on this path, and both are inherited by
A1:

\begin{itemize}
  \item \textbf{Model EMA} ($\text{decay}=0.9999$). Not merely unrendered ---
    unrepresentable, because the EMA shadow and the gradient accumulator are the
    same fourth region of $[\theta\,|\,m\,|\,v\,|\,\cdot\,]$, and the driver
    refuses the combination outright rather than letting one win. Accumulation
    is not optional here: effective batch $2048$ at $224^2$ has no other route
    on a $16$\,GB card. Lifting this is a fifth region in the pack/unpack and in
    every optimizer's return list.
  \item \textbf{Stochastic depth} ($0.05$). The module exists and EfficientNet
    and ConvNeXt render variants off it; the residual family does not import it,
    and the variant grammar has no marker to ask for it with.
\end{itemize}

\noindent A3 met neither obstacle because A3's own recipe switches both
\emph{off}. So these artifacts are A2's graph minus two regularisers: they price
the tier honestly, since neither affects the step cost, and they are what a
fifth region and a \texttt{drop} marker would extend. They are not a run at the
paper's numbers, and a result off them would be ``the A2 recipe with these
deltas'' in exactly the sense \S\ref{sec:r50_phase4} uses for A3.
