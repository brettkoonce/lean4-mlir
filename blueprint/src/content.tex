...
$256$. The architecture lever: against ResNet-34's $74.16\%$ on the same
...
\hline
A3 (\texttt{rsb-faithful}) & 100 & $160$ & shipped, and run & $22.8$ hr (bf16, measured) & $\mathbf{77.98\%}$ \\
A2 (\texttt{a2-accum})     & 300 & $224$ & rendered, EMA + sd & $\sim$95 hr (bf16) & $79.8\%$ (paper) \\
A1 (\texttt{a1})           & 600 & $224$ & rendered, EMA + sd & $\sim$190 hr (bf16) & $80.4\%$ (paper) \\
\hline
\end{tabular}
\end{center}
...
\medskip\noindent \textbf{And what the JAX-lowerer peers cost}, A3 the control:

\begin{center}
\begin{tabular}{@{}l r r r r@{}}
\hline
JAX tier & train res & ms/step & min/epoch & full schedule \\
\hline
...
\noindent \textbf{And the verified column is measured, not scaled from theirs.}
A3's graph runs $191.4$\,ms/step on four cards and the complete A2 at $4\times128$
bf16 runs $376.9$, while A3's finished run fixes what the trainer adds at
$38.8$\,ms per $256$ images --- \emph{additive}, since the device figures already
contain the all-reduce and only the host's blob patch sits outside. That is $19.0$
verified minutes an epoch, and run backwards it puts A3 itself at $23$ hours.

\medskip\noindent \textbf{And A3 has been run, which is what makes the A2 and A1
figures worth quoting.} The bf16 render finished in $22.8$ hours at $13.7$
minutes an epoch, against the $23$ predicted from step timings alone --- an
error of $0.2$ hours. The A2 and A1 rows are built the same way from the same
measurements, at a scale where a wrong estimate costs a week.
...
--- or $35.7$\,fps on a 25\,W Jetson Orin Nano under TensorRT, which is the
deployment this dataset implies. The replica, not YOLOv8, is the yardstick: it
...
(with CIFAR-10, sinusoidal-time and attention variants beside it) is a
timestep-conditioned UNet trained to predict the Gaussian noise added to an
...
\begin{itemize}
  \item \textbf{The target is the noise, and it is self-supervised.} Every model
    ...
    the same squared-error backward from Part~2
