...
GPU & Precision & Per epoch & Epochs & Total & Val top-1 & Val top-5 \\
\hline
4$\times$ 3060 (CUDA) & bf16 & $8.8$ min & 350 & $51.2$ hr & $\mathbf{71.912\%}$ & $\mathbf{90.520\%}$ \\
\hline
\end{tabular}
\end{center}
...
\hline
\textbf{val top-1}   & $\mathbf{71.90\%}$       & $\mathbf{71.912\%}$ \\
\textbf{val top-5}   & $\mathbf{90.41\%}$       & $\mathbf{90.520\%}$ \\
\hline
\end{tabular}
\end{center}
...
That driver wires \texttt{.imagenet} through tfds streaming.
\S\ref{sec:vit_phase4} is the verified path's side of it.

\textbf{Gradient clipping is load-bearing.} At the DeiT learning rate
...
LayerNorm/softmax that bf16 leaves alone. The run:
...
\hline
4$\times$ 4060 Ti (CUDA) & bf16 & $\sim$7.6 min & 300 & $\sim$41 hr & \withheld & \withheld \\
4$\times$ 3060 (CUDA) & bf16 & 6.8 min & 300 & 34.2 hr & $\mathbf{72.31\%}$ & $\mathbf{91.12\%}$ \\
\hline
\end{tabular}
\end{center}

\noindent The 4060 Ti row is a per-epoch rate carried out to the full
schedule, not a completed run, hence no accuracy. The 3060 row is the
completed run: \textbf{72.31\% top-1 / 91.12\% top-5} over all 50{,}000
validation images, 300 epochs in 34.2 hours, one attempt, no restarts,
no thermal pauses. DeiT-Ti without distillation publishes \textbf{72.2\%
/ 91.1\%}, so this is $+0.11$ and $+0.02$ --- on the paper, not past it
in any meaningful sense. Per-epoch validation:
...
4$\times$ RTX 3060, CUDA), sampled every 5 epochs across the 300-epoch
run and scored over all 50{,}000 validation images.
...
\item \textbf{Gradient clipping at global norm $1.0$ is ours.} It is
  load-bearing: without it the model collapses to chance the moment
  warmup ramps past ${\sim}1.6\times 10^{-4}$. Whether DeiT itself
  clips is \emph{not verified in this repo}. The config comment calls
  it ``the DeiT default'' and that claim has never been checked against
  DeiT's \texttt{main.py}; until someone reads the reference
  implementation, treat it as an addition of ours.
...
\item \textbf{Geometric RandAugment interpolates bilinearly}, where
  timm resolves bicubic from the model's data config;
  appendix~\ref{app:data} carries the measurement and the decision to
  leave it.
\end{enumerate}

\noindent None of these is a bug, and the loss curve is textbook DeiT.
But ``matches the paper's number'' and ``matches the paper'' are
...
def vitImagenetConfig : VerifiedConfig where
  epochs    := 300
  batchSize := 128
  vitInit   := true  -- timm/DeiT init, as the phase-2 `deit-init` recipe above
\end{verbatim}
...
here they compose.} Weight-decay exclusion,
...
of AdamW and the four-way all-reduce. EMA is a variant too, and
\texttt{vitin\_emadp128x4wxclipdropbf16} is the render that carries all
four at once --- AdamW, the four-way all-reduce, the three regularizer
axes and the EMA shadow. It is what made the pair below one-variable in
recipe, and it is the artifact the run used. Mixup, CutMix and repeated augmentation never enter the
...
ms/step              & 199                      & 161 \\
per epoch            & 9.6 min                  & 6.8 min \\
total                & 48.0 hr                  & 34.2 hr \\
\hline
precision            & bf16                     & bf16 \\
...
\hline
\textbf{val top-1}   & $\mathbf{72.350\%}$      & $\mathbf{72.31\%}$ \\
\textbf{val top-5}   & $\mathbf{91.216\%}$      & $\mathbf{91.12\%}$ \\
\hline
\end{tabular}
\end{center}

\noindent Both columns are ViT-Tiny on the DeiT-Ti recipe, 300 epochs, on the
same four RTX 3060 cards, scored over the same $50{,}000$ validation images.
Rows above the first rule differ; rows between the rules are held equal by
construction; the results below do not differ in any way that matters.
...
\hline
phase-2 reference & 14.07 & 57.36 & 63.62 & 66.99 & 69.40 & 71.57 & 72.31 \\
phase-4 verified  & 14.32 & 56.86 & 63.63 & 66.86 & 69.31 & 71.36 & 72.35 \\
...
\noindent Two independent lowerings of one recipe, sampled every 5 epochs and
scored over all $50{,}000$ validation images.
