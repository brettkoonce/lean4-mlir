...
the three-term backward this chapter proves. Every theorem in the rest
of the chapter is about the graph that just trained.
...
come from. \texttt{lake run cifar} does the narrow-head tier.
...
and \emph{parametrically in the head width}, which is why the wide head
here and the narrow one behind \texttt{lake run cifar} need no separate
proof between them. \texttt{cifarCnn8\_has\_vjp\_at\_correct}
...
done (trained ResNet-34 via the proof-rendered StableHLO).
\end{verbatim}
...
Here is the part that matters. That run did not execute a reimplementation of
...
epochs. This section is the same 21.8M-parameter network on 1.28M
images and 1000 classes, and \textbf{it reaches $\mathbf{70.74\%}$
top-1 / $\mathbf{90.14\%}$ top-5 in 30 epochs, on the verified path},
which is the number to take away from this chapter. The architecture is
unchanged. Only the dataset, the output head width, and the training
schedule differ.

Thirty epochs is a deliberate trim of the paper's ninety, bought at
roughly a point of top-1 for a third of the compute, and it is the tier
this book runs because it is the one that fits on the hardware this book
is written on. The 90-epoch schedule appears later in the section as a
reference point, measured on the JAX path.

Here is the trainer, and it is the real one:
...
numbers in \S\ref{sec:r34_phase4} came from. Three things in it are worth
...
\textbf{This now runs on the verified path.} When this chapter was
first written it did not. ImageNet needs a C-side streaming reader for
the 1.28M-image training set, because Imagenette's 9.5K fits in a single
\texttt{ByteArray} and ImageNet does not, and until that landed the
verified dispatcher refused \texttt{.imagenet} outright and sent you to
the JAX trainer. The reader exists now, so the numbers below are the
rendered graph training on full ImageNet rather than a promise that it
could. \S\ref{sec:r34_phase4} is the result.

Both paths share everything above the data loader: the architecture, the
\texttt{.train} invocation, and every recipe knob. The framework isn't
doing anything fundamentally different at ImageNet scale. It's just the
data that's bigger.

The phase-2 JAX path (\texttt{jax/MainResnetImagenet.lean}, via
\texttt{lake build resnet34-imagenet}) wires \texttt{.imagenet} through
tfds streaming, and it is still here. It is no longer the only way to
get an ImageNet number, though, so its role has changed from
\emph{the} trainer to the independent reference the verified path is
checked against, which is what the next section does with it.

\textbf{Compute budget.} Both paths run on the same box: four NVIDIA
RTX 4060 Ti, consumer hardware, CUDA/cuDNN. The verified path trains
\texttt{resnet34Imagenet} at global batch 256 as $4 \times 64$, one
epoch over the 1.28M-image training set in about 31 minutes at fp32,
so the 30-epoch schedule lands at roughly $15.5$ wall-clock hours.
The JAX reference runs the same recipe through tfds streaming with
sharding via \texttt{Mesh + NamedSharding(P('batch'))}, and in bf16 it
is about three times quicker per epoch, which is what made it the
practical trainer for as long as it was the only one.

The parameter checkpoint (\texttt{jax\_r34\_imagenet.bin}, 87\,MB, 37
param groups) is written via the same \texttt{params\_to\_file} emit
the verified path consumes, so weights move between the two byte-for-byte
with no conversion step. That shared format is what makes the epoch-by-epoch
comparison in \S\ref{sec:r34_phase4} possible at all.
...
\textbf{The JAX reference, and the longer schedule.} Everything from
here to \S\ref{sec:r34_phase4} is the phase-2 (Lean$\to$JAX) trainer. It
is no longer the path this book recommends, and the verified numbers in
\S\ref{sec:r34_phase4} are the ones to read as the result. Two things
keep it in the chapter. It is the independent implementation the verified
run is checked against, and it is the only 90-epoch ImageNet schedule
either path has actually completed, so it is where the full-schedule
number lives until a 90-epoch verified run exists (about 28 hours of
this box, on the throughput measured today).

\medskip\noindent\textbf{bf16 and the full 90-epoch run.} Bf16 mixed
...
either vendor, and bf16 conv only pays off on cuDNN). The wall-clock
...
validation for the completed runs (CUDA at the full 90, and the MI300X
row is a single-GPU batch-1024 run on gfx942). The A100
...
rather than the hardware. A matched batch-256 run for an apples-to-apples
...
to completion, and bf16 does not cost final accuracy here.) The
...
warmup, and the steady late climb is the cosine anneal.

\prosesection{Phase 4: the PJRT bridge}
\label{sec:r34_phase4}

Earlier printings of this chapter asked whether the verified codegen path
could reach ImageNet scale at all. It can, and the rest of this section is
that run. Phase 4
...
\noindent ResNet-34 / ImageNet-1k on the phase-4 path: 35{,}311 of 49{,}920
validation images, global batch 256 as $4 \times 64$, heavy-ball momentum
with coupled $\ell_2$, cosine schedule with 5-epoch warmup at peak $0.1$,
weight decay $10^{-4}$, label smoothing $0.1$
(\texttt{runs/r34in\_verified\_30ep\_2026-08-05/}).

\medskip\noindent Read the accuracy carefully, because the epoch counts
differ. This is a 30-epoch run and the phase-2 bf16 number above is a
90-epoch one, so $70.74\%$ against $72.02\%$ is not the two lowerers
disagreeing by $1.3$ points. It is a third of the training schedule
costing $1.3$ points, which is about what a third of a cosine schedule
should cost.

\medskip\noindent\textbf{The evidence is the agreement, not the endpoint.}
Comparing final accuracies across different schedules proves little. The
load-bearing check is that both lowerers were run on the same box from the
same initialization and their trajectories were diffed. At epoch~5 the two
sets of weights scored \textbf{17{,}312} and \textbf{17{,}313} correct out
of 49{,}920. One image apart, on fifty thousand. Training losses agreed to
$0.055$ nats at epochs 1, 2, 5 and 6.

\medskip\noindent That is the claim this section makes, and it is worth
being exact about its ceiling. The proof-carrying tier of this book stops
at Imagenette. What the ImageNet run inherits is provenance, because it
executes \texttt{pretty(provenGraph)} off the same renderer the theorems
are about, plus the agreement above. So the honest phrasing is \emph{one
architecture, two independent lowerings, agreeing to one image at epoch
five}, and never ``proven at ImageNet scale.''
...
batch, and the only difference is the BN statistic group, which is the
...
and the timm reference on one ruler, either a single fixed resampler or a
...
\noindent 100 epochs. The run logged zero thermal duty-cycle cooldowns and
zero PCIe/AER interruptions. There is no ROCm peer row, because the
verified trainers do not survive a run on ROCm. A command-buffer fault
kills them, reproducibly, at a rate of 11 deaths in 11 attempts on gfx1100
against 6 clean runs in 6 on CUDA at the identical JAX version. The repro
and the upstream report are in
\texttt{upstream-issues/2026-08-jax-rocm-command-buffer-launchgraph-segv/}.
Every verified number in this chapter is therefore CUDA.
...
\begin{itemize}
  \item \texttt{wdExcludeNormBias} absent, so BN $\gamma/\beta$ and biases
    are decayed at $\mathrm{wd} = 0.02$ where timm skips them. Largest
    single delta, and the ConvNeXt and ViT renders already carry the
    mechanism, so this is a port.
  \item Ghost-BN group is 64 against the reference's 512, because no collective
  ...
  \item BN running-statistic momentum is not compensated for accumulation
    ...
    $0.99^{1/k}$). Evaluation-only, plausibly \emph{understates} top-1, and a
    one-line fix.
  \item fp32, not bf16, which is most of the $2\times$ wall-clock gap.
  \item $0.08\%$ of each epoch is dropped (5000 of 5004 micro-batches,
    since $8 \nmid 5004$).
  \item Mixup $\lambda$ is drawn host-side from NumPy, so agreement with
...
entry point are identical, and only the \texttt{TrainConfig} differs. Here
is the full diff:
