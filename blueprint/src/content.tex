...
\hline \\[-6pt]
Read and check the proofs      & Lean. No GPU.                    & 3 \\
See the pipeline run at all    & Docker.                          & 1 \\
MNIST / CIFAR / Imagenette     & Lean + one plugin file.          & 2, 4 \\
...
converges to $\sim$97.9\% test accuracy.
...
It also currently trains the \emph{unverified} \texttt{mnist-mlp-train-f32}
rather than the proof-carrying \texttt{mnist-mlp-verified} the rest of the
book runs, so it demonstrates that the toolchain works end to end, not that
the proofs do. Prefer Track 2 if what you want to see is the verified path.
...
\section*{Track 3: Proofs only (no GPU needed)}
...
\begin{itemize}
  ...
  \item \textbf{\texttt{HAL device not found}} on an IREE run --- the IREE
    runtime was built without the requested backend's HAL driver. A shim built
    for ROCm will fail under \texttt{IREE\_BACKEND=llvm-cpu} with exactly this
    error, and vice versa; rebuild the runtime with the driver you want.
