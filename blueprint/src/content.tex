...
$50{,}000$ images. One attempt, no restarts.
...
\hline
\textbf{val top-1}   & $\mathbf{76.878\%}$         & $\mathbf{77.15\%}$ \\
\textbf{val top-5}   & $\mathbf{93.154\%}$         & $\mathbf{93.30\%}$ \\
\hline
\end{tabular}
\end{center}

\noindent Both columns are EfficientNet-B0 on the paper-faithful RMSProp recipe,
350 epochs, on the same four RTX 3060 cards. Rows above the first rule differ;
rows between the rules are held equal by construction. The reference's own
per-epoch evaluation reads $77.18\%$ at epoch 350; the $77.15\%$ headline is
that same checkpoint rescored over all $50{,}000$ images, and the gap between
those two figures is the scale of a single evaluation's noise.

\medskip\noindent\textbf{$-0.27$ points --- and unlike MobileNetV2's pair, this
one is not a dead heat.} The verified final carries a 95\% Wilson interval of
$[76.51, 77.25]$, which contains the reference, so the last epoch alone cannot
distinguish them. Averaging removes that ambiguity: over the last fifty epochs
the verified path scores $76.71\%$ against the reference's $77.03\%$, a
$-0.32$ gap across fifty independent evaluations rather than one. The offset is
real, it is small, and this section does not claim to know its cause.

\medskip\noindent\textbf{[TODO: revisit BN sharding]}

\medskip\noindent\textbf{The curves track, and the gap closes.} The verified path
leads through warmup, falls about half a point behind by epoch~50, and recovers
to $-0.3$ by epoch~200, where it stays:
...
epoch & 10 & 50 & 100 & 150 & 200 & 300 & 350 \\
\hline
phase-2 reference & 61.69 & 70.78 & 73.14 & 75.07 & 75.93 & 76.92 & 77.18 \\
phase-4 verified  & 61.79 & 70.22 & 72.83 & 74.41 & 75.50 & 76.54 & 76.88 \\
...
\addlegendentry{phase-2 reference (JAX)}
\addplot[red, dashed, mark=o] coordinates {
...
\addlegendentry{phase-4 verified (PJRT)}
\end{axis}
\end{tikzpicture}
\end{center}

\noindent Two independent lowerings of one recipe, sampled every 10 epochs and
scored over all $50{,}000$ validation images. The verified curve's epoch-1
$20.62\%$ against the reference's $10.09\%$ is unexplained: both sides apply the
same warmup-corrected decay $\min(\mathrm{decay}, (1+t)/(10+t))$, so it is not
that. The two are level by epoch~10 and the gap never recurs.
...
\noindent Global batch 512 as $4 \times 128$ replicas, the EMA weights
scored over the same $50{,}000$ images. One attempt, no restarts.
