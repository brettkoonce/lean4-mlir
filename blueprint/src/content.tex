...
Everything in Part 1 of this book is the middle row. It needs \emph{no Python
at run time and no compiler}: you download one shared library and run a Lean
binary. Only the ImageNet tier (Track 4) needs an interpreter, and it needs
...
\end{verbatim}
    Those packages carry the PJRT plugin and \emph{nothing else} --- not
    \texttt{jax}, not \texttt{jaxlib}, so ``no interpreter at training time'' is
    structural rather than a promise. The shim looks under
    \texttt{.venv/lib/python3.*/site-packages/jax\_plugins/}, which is exactly
    where that command puts it; \texttt{\$PJRT\_PLUGIN} points elsewhere. The
    plugin is version-matched to your CUDA/ROCm install, and it is a 500\,MB
...
With Track 2 in place, a single command builds and runs a curated,
...
\begin{itemize}
  \item \textbf{\texttt{no lowerer shim could be loaded}} --- neither
    ...
    Build the PJRT one with the \texttt{gcc} line in Track 2; it needs nothing
    but libc.
