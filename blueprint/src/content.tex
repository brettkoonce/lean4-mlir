...
\newcommand{\globalbntodo}{\medskip\noindent\textbf{[TODO: run with global BN.]}}
...
% not run as a Track-4 job, and the heading says which of the three kinds it is:
% a RECIPE (§\ref{sec:r50_a2_a1_cost}), a SIZE (§\ref{sec:convnext_sb},
...
\newcommand{\sqstatus}{\medskip\noindent \textbf{Status: rendered, not trained.}}
...
A short \emph{Foundations} chapter comes first: what a derivative is in
Mathlib, and why the book tracks vector--Jacobian products rather than
Jacobians. Everything after assumes it.
...
The chapters are also the book's runnable spine: \texttt{lake run mnist},
\texttt{lake run cifar}, \texttt{lake run imagenette} and \texttt{lake run
imagenet} train them in this order, one tier per scale, and ``Getting
started'' is where those commands live.
...
\begin{itemize}
  \item \textbf{Chapter 1: MNIST: linear classifier.} Chain rule, sum
    ...
    the book's densest chapter, because it builds the whole pipeline
...
\textbf{Part 2: The bestiary (Chapter 10).} A catalog of 45
...
than counted as a contract. A theorem that only restates a counted one at
fixed dimensions or over the committed spec, and a smooth-point form that
a later global theorem superseded, is not counted again. Across
Chapters~1--9 there are \textbf{45 contracts} and \textbf{6 witnesses}.
Even Chapter~1's toolkit is used straight away by a proven model, so no
chapter is pure scaffolding.
...
\hline
1 (MNIST: linear) & 1  & --- & dense, softmax-CE, \texttt{mnistLinear}: the whole classifier \\
2 (MNIST: 1D)     & 2  & 2   & ReLU, MLP; witnesses hand-built and at trained weights \\
3 (MNIST: 2D)     & 3  & 1   & conv2d, maxPool2, whole-net MNIST CNN; witness at trained weights \\
4 (CIFAR: BN)     & 6  & 1   & per-channel BN (flat $+$ NCHW) and BN at batch statistics; CIFAR whole-nets at 2 convs and at 8 convs $\pm$BN; a hand-built tiny witness \\
5 (ResNet-34)     & 10 & 2   & residual ($+$proj), GAP, stride-2 conv ($+$its weight grad), the $3 \times 3$/s2 stem pool, whole-net \texttt{cnn}; ResNet-34 per example, \textbf{full-depth ResNet-34 and ResNet-50} at batch statistics; witnesses: a hand-built BN CNN and a $224 \times 224$ ResNet-34 with channel-mixing downsamples \\
6 (MobileNetV2)   & 6  & --- & ReLU6, depthwise (stride 1$+$2); \texttt{mobilenetv2} at representative depth and at \textbf{all 17 blocks} at batch statistics; \textbf{\mbox{MobileNetV4}} at full depth \\
7 (EfficientNet)  & 5  & --- & swish, sigmoid, SE, whole-net at representative depth, \textbf{full B0} (16 \mbox{MBConv} blocks) \\
8 (ConvNeXt)      & 6  & --- & GELU, LayerNorm, layer-scale, the $4 \times 4$/s4 patchify conv, whole-net at representative depth, \textbf{full ConvNeXt-T} (18 blocks) \\
9 (ViT)           & 6  & --- & MHSA, block, vector LayerNorm and its block, \textbf{every depth $k$} (ViT-Tiny is $k = 12$), \texttt{vit\_full} \\
\hline
                  & \textbf{45} & \textbf{6} & total \textbf{51} \\
\hline
\end{tabular}
\end{center}
...
Lean. Chapter~1 is the one chapter every target reads in full: the
foundation rules, the dense layer and the loss. The interesting subsetting decision is
...
and the Dense Jacobians, all of them Chapter~1's.
...
bigger brother ResNet-50 is the standard benchmark (the GOAT) for
...
\item \textbf{Chapter 1 is the new material for you.} Its first
  half --- the calculus every later chapter composes --- is what the
  first book didn't have. It's where the gradients get formalized. If you only read one chapter
...
\texttt{runs/2026-09-01-linear-verified-xla-cuda/}. XLA's startup banner is
...
The theorems of this chapter all reduce to a single definition
...
question, asked about a different building block: \emph{if I jiggle
this input by} \(\varepsilon\), \emph{how does the output jiggle?}
...
\(\pdiv\) above is one entry of it at a chosen \((i, j)\).
...
gradient reads \(B(x, dy) = W\, dy\) in Theorem~\ref{thm:dense_has_vjp},
and in the MLIR it is a contraction over the output axis of both
operands. One transpose, three spellings.
...
\begin{theorem}[Dense bias gradient is identity]
  ...
  \leanok
  \uses{ax:pdiv, ax:pdiv_add, ax:pdiv_const, ax:pdiv_id}
  \(db = dy\).
  \Prove{} for all \(i\):
  \[
...
seed. From Chapter~\ref{chap:mlp} onward the He(fan-in) in code
\texttt{0} is what actually runs.
...
The trainer is 54 lines including its docstring, so here it is entire,
...
(Theorems~\ref{thm:dense_has_vjp}--\ref{thm:dense_bias_grad_correct})
...
same recipe to 97.81\%. That gap of about 5.7 points is the value of
...
\texttt{IO.getStdout} flushes, two environment-variable overrides
(\texttt{LEAN\_MLIR\_PERTURB\_R}, which displaces the initialisation for
a residency check, and \texttt{LEAN\_MLIR\_MAX\_EPOCHS}, which caps the
epoch count), and everything after the loop, which dumps the final
...
whole thing the linear classifier compiles to. This is
\texttt{verified\_mlir/linear\_train\_step.mlir} as committed, with the
default \texttt{precision} attributes dropped and the long type
signatures wrapped for the page. Nothing else is removed, including one
band that repeats:
...
is the weight half: \texttt{dot\_general \%x, \%v8} is exactly the move
from the ``MLIR: Linear'' section, $x^{\!\top} dy$, now serving as the
weight gradient, and the three operations after it scale by the learning
rate and subtract.
...
rendering costs: 27 operations where 19 would do.
...
but it costs roughly 2.3 times the graph: Chapter~\ref{chap:bn}'s CIFAR
network, rendered both ways, goes from 1,001 StableHLO operations under
SGD to 2,293 under AdamW, because the first- and second-moment buffers
...
same proven gradient. Only SGD also gets a \emph{descent} theorem, that
...
drives it. Stripped of its logging and its session setup,
\S~\ref{sec:mlp_inside_train}'s driver is five lines:
...
\noindent Five lines of host control flow around one compiled dispatch. The
...
\medskip\noindent\textbf{Where this stands.} The scheme above is the one
every architecture in Part~I trains under, and the book's official claim
is exactly this, no more and no less: \emph{every architecture in Part~I
trained end-to-end under one machine-checked scheme}. The one open gap
is that the theorems are over \(\mathbb{R}\) and the GPU runs a finite
precision; \S~\ref{sec:precision} is the budget that sits between them,
and Appendix~\ref{app:verification} carries the rest of the accounting.
...
theorem here is the same question as before --- \emph{if I jiggle this
input by} \(\varepsilon\), \emph{how does the output jiggle?} --- asked
...
On one RTX 4060 Ti (CUDA 12.9), from
\texttt{runs/2026-09-01-mlp-verified-xla-cuda/}. XLA's startup banner is
removed and two long lines are wrapped:
...
close to four times the wall clock per epoch.

Chapter~\ref{chap:tensor}'s accuracy did not move across seeds, because
that model starts at zero and has no seed to spend. This one does. Run at
...
\mathrm{relu} \circ \mathrm{dense}_0\). Chapter~\ref{chap:tensor}'s VJP
chain rule (Theorem~\ref{thm:vjp_comp}) says the backward of a
...
There is one catch, and it is ReLU's. Theorem~\ref{thm:vjp_comp} asks
for both functions to be differentiable everywhere, and ReLU is not. So
the whole-network result is stated \emph{at a point}: wherever no
pre-activation is exactly zero, the composed backward equals the
\(\pdiv\)-contracted Jacobian (Theorem~\ref{thm:mlp_has_vjp_at}). The
canonical witness \texttt{mlp\_has\_vjp}
(Definition~\ref{ax:mlp_has_vjp}) is the object later chapters name; the
...
\begin{theorem}[MLP VJP at smooth inputs]
  \label{thm:mlp_has_vjp_at}
  \lean{Proofs.mlp_has_vjp_at_correct}
  \leanok
  \uses{ax:pdiv, ax:pdiv_comp, ax:pdiv_relu, def:hasvjp, thm:dense_has_vjp}
  \Assume{}
  \begin{enumerate}
  ...
  \end{enumerate}
  \Prove{} the backward obtained by composing the five per-layer
  backwards in reverse order (\texttt{mlp\_has\_vjp\_at}, built by
  \texttt{vjp\_comp\_at}) satisfies, for all \(dy\), \(i\):
  \[
    \mathrm{backward}(dy)_i
      = \sum_j \pdiv(\mathrm{mlpForward})\, x\, i\, j \cdot dy_j .
  \]
  Off the two smooth-point conditions the composed backward is not
  defined, and \texttt{mlp\_has\_vjp} is the object that stands in.
\end{theorem}

\begin{proof}
\leanok
Chain rule at a point, five times; see
\leandocref{Proofs.mlp\_has\_vjp\_at\_correct}.
\end{proof}
...
\texttt{Proofs.StableHLO.mlpTrainStepFaithfulV} is what emits the train
...
\texttt{verified\_mlir/mlp\_train\_step.mlir} that is \emph{not}
...
depends on it. The six \emph{parameter} outputs are the proved ones ---
...
The two compile lines in \S\ref{sec:mlp_runit} are where that shows up:
the train step takes about three and a half times as long to compile as
Chapter~\ref{chap:tensor}'s, which is the cost of six parameter tensors
and two ReLU subgradients rather than one dense layer.
...
(\texttt{runs/2026-09-01-mlp-verified-xla-cuda/}). The loss falls
$14\times$ across the run, and test accuracy ends at 97.81\%.
...
\texttt{mlp\_has\_vjp} is polymorphic in $d_0,d_1,d_2,d_3$, so one theorem
...
coordinates {
(8,91.99) +- (0,0.53) (16,94.01) +- (0,0.47) (32,96.76) +- (0,0.35) (64,97.02) +- (0,0.33) (128,97.25) +- (0,0.32) (256,97.67) +- (0,0.30) (512,97.83) +- (0,0.29) (1024,97.89) +- (0,0.28) (2048,98.0) +- (0,0.27) (4096,98.01) +- (0,0.27)
};
...
(\texttt{runs/mlp\_grid\_results\_xla.tsv}). Bars are the 95\% Wilson
interval on 10,000 test images --- the same interval the transcripts
print, and about $\pm 0.3$ points once accuracy is in the nineties.
...
reaches 96.8\% at 27K parameters, $755\times$ smaller than the
...
per-layer chain rule \texttt{vjp\_comp\_at}
(Theorem~\ref{thm:mlp_has_vjp_at}). The parameter gradients are
...
Here is what \texttt{IRPrint.lean} prints for that graph, one
\texttt{stablehlo} op per node (default \texttt{precision} attributes
elided for the page). \texttt{Back} carries real-valued operands and cannot
be printed directly, so the printer walks a mirror of it, \texttt{Hlo},
built node for node from \texttt{emitMlpBack}'s shape with SSA names in
place of values; that the mirror matches is checked by reading the two
definitions side by side, not by a theorem:
...
On one RTX 4060 Ti (CUDA 12.9), from
\texttt{runs/2026-09-01-cnn-verified-xla-cuda/}. XLA's startup banner is
...
[pjrt_ffi] compiled verified_mlir/cnn_train_step.mlir
             (@cnn_train_step, 11 outputs, 1 replica) in 2566 ms
[pjrt_ffi] compiled verified_mlir/cnn_fwd.mlir
             (@cnn_fwd, 1 outputs, 1 replica) in 210 ms
MNIST-CNN via the VERIFIED renderer (conv→conv→pool→512→512→10) → XLA/PJRT → GPU
  ...
  train 60000, test 10000; bs 128, MNIST-CNN (10 params, 3489130 floats),
    mean-loss SGD lr=0.100000, He init
  epoch 1: loss = 0.352314, test_acc = 9295/10000 = 92.950000%  [95% CI 92.43–93.44] (5219ms)
  epoch 2: loss = 0.094411, test_acc = 9749/10000 = 97.490000%  [95% CI 97.16–97.78] (5072ms)
  epoch 3: loss = 0.056773, test_acc = 9800/10000 = 98.000000%  [95% CI 97.71–98.26] (4888ms)
  epoch 4: loss = 0.038316, test_acc = 9819/10000 = 98.190000%  [95% CI 97.91–98.43] (4672ms)
  epoch 5: loss = 0.026202, test_acc = 9838/10000 = 98.380000%  [95% CI 98.11–98.61] (4357ms)
  epoch 6: loss = 0.018312, test_acc = 9853/10000 = 98.530000%  [95% CI 98.27–98.75] (5069ms)
  epoch 7: loss = 0.022045, test_acc = 9857/10000 = 98.570000%  [95% CI 98.32–98.78] (5537ms)
  epoch 8: loss = 0.011130, test_acc = 9859/10000 = 98.590000%  [95% CI 98.34–98.80] (5390ms)
  epoch 9: loss = 0.006994, test_acc = 9869/10000 = 98.690000%  [95% CI 98.45–98.89] (4808ms)
  epoch 10: loss = 0.006176, test_acc = 9861/10000 = 98.610000%  [95% CI 98.36–98.82] (4351ms)
  epoch 11: loss = 0.004395, test_acc = 9867/10000 = 98.670000%  [95% CI 98.43–98.88] (4647ms)
  epoch 12: loss = 0.033374, test_acc = 9877/10000 = 98.770000%  [95% CI 98.53–98.97] (4981ms)
...
Chapter~\ref{chap:mlp}'s classifier buy about a point over the MLP's
97.81\%, and they do it with 3.5M parameters against the MLP's 670K.
...
same seed put it anywhere between 92.5 and 94.8\%, because a convolution's
...
\noindent Thirty seeds of each net, one dot per run
(\texttt{runs/2026-09-01-mnist-n30/}). The MLP's runs sit in a band; the
CNN's do too, apart from one.

\medskip\noindent Summarised as a mean and a standard deviation the CNN is
$98.73 \pm 0.27$. Summarised as a median and quartiles it is $98.78$, with
...
\item \textbf{Hierarchy.} Digits are made of strokes, and strokes of
  ...
  discards \emph{where} within a window a feature fired. That is where
...
\begin{theorem}[Conv2d input VJP]
  ...
  \leanok
  \uses{ax:conv2d, ax:pdiv, def:hasvjp3}
...
\begin{theorem}[Conv2d weight VJP]
  ...
  \leanok
  \uses{ax:conv2d, ax:pdiv, def:hasvjp}
...
\begin{theorem}[MNIST CNN VJP at smooth inputs]
  \label{thm:mnistCnnNoBn_has_vjp_at}
  \lean{Proofs.mnistCnnNoBn_has_vjp_at_correct}
  \leanok
  \uses{ax:conv2d, ax:conv2d_has_vjp3, ax:maxPool2, ax:maxPool2_has_vjp3, ax:pdiv,
      ax:pdiv_comp, ax:pdiv_relu, def:hasvjp, def:hasvjp3, thm:dense_has_vjp}
  \Assume{}
  \begin{enumerate}
  ...
  \end{enumerate}
  \Prove{} the backward obtained by composing the per-layer backwards
  in reverse order --- dense, dense+ReLU, dense+ReLU, max pool,
  conv+ReLU, conv+ReLU (\texttt{mnistCnnNoBn\_has\_vjp\_at}, built by
  \texttt{vjp\_comp\_at}) --- satisfies, for all \(dy\), \(i\):
  \[
    \mathrm{backward}(dy)_i
...
\leandocref{Proofs.mnistCnnNoBn\_has\_vjp\_at\_correct}.
\end{proof}
...
this chapter (Theorem~\ref{ax:conv2d_has_vjp3}): the input VJP is
another convolution, using the kernel reversed along both spatial
axes. The max-pool backward (Definition~\ref{ax:maxPool2_has_vjp3}) routes
the gradient to the argmax position in each $2 \times 2$ window.
The dense backward is already proved in
Chapter~\ref{chap:tensor} (Theorem~\ref{thm:dense_has_vjp}). Adding
...
\texttt{Proofs.StableHLO.cnnTrainStepFaithfulV} emits it, and
...
(\texttt{runs/2026-09-01-\{mlp,cnn\}-verified-xla-cuda/}), both nets now on
the same twelve. The CNN sits below the MLP through epoch~11 and reaches
...
accuracy is at its \emph{highest} on that same epoch, 98.77\%. Training
...
five runs and its whiskers are the observed range across those five ---
not an interval, just the highest and lowest the configuration actually
produced. Eight of the ten widths repeat to within 0.4 and their whiskers
...
\medskip\noindent Those tails are the reason this curve is a median of
five and Chapter~\ref{chap:mlp}'s is not. A symmetric error bar would have
been the wrong shape for them --- four runs cluster and one falls away, so
...
behind a rounding error. The MLP points are single runs, and
Chapter~\ref{chap:mlp} gives them half a point of give for the same reason
the CNN's needed five.
...
network, and \texttt{mnistCnnNoBn\_has\_vjp\_at}
(Theorem~\ref{thm:mnistCnnNoBn_has_vjp_at}) proves its reverse-mode
derivative by chaining the per-layer VJPs: the dense backward
(Theorem~\ref{thm:dense_has_vjp}, from Chapter~\ref{chap:tensor}), the
max-pool backward (\texttt{maxPool2\_has\_vjp3},
Definition~\ref{ax:maxPool2_has_vjp3}), and the convolution backward
(\texttt{conv2d\_has\_vjp3}, Theorem~\ref{ax:conv2d_has_vjp3}), all through the
...
(the transpose trick) and the dense grads from Chapter~\ref{chap:tensor}, and
...
reversed-kernel identity that \texttt{mnistCnnNoBn\_has\_vjp\_at} only \emph{asserts}.
...
\texttt{convolution} for the contraction. Here it is for the first
conv's backward ($1{\to}2$ channels, $3\times3$ kernel, at $4\times4$):
...
\item BN's payoff is \emph{speed} and \emph{stability}: a deeper
  ...
  without, and five runs of the same command land two to three times
  closer together. The example in
...
\texttt{runs/2026-09-01-cifar8w-bn-xla-cuda/cifar8w-bn.log}. XLA's
startup banner is removed, the per-step loss lines are dropped, and
epochs 6 through 36 are elided:
...
Forty epochs at about six seconds each, and 75.94\% on the full
10,000-image CIFAR-10 test set, 98.17\% inside the top five. CIFAR-10
...
$75.8$ and $76.7\%$, so the number above is an ordinary
...
make this term differentiable. This is the \emph{first place in
...
\begin{theorem}[BN normalize 3-term VJP]
  ...
  \leanok
  \uses{ax:bnIstdBroadcast_diff, ax:pdiv, ax:pdiv_bnCentered, ax:pdiv_bnIstdBroadcast,
...
\begin{theorem}[Full BN VJP]
  ...
  \leanok
  \uses{ax:bnIstdBroadcast_diff, def:hasvjp, thm:bnAffine_has_vjp,
      thm:bnNormalize_has_vjp, thm:vjp_comp}
  \Assume{}
  \begin{enumerate}
    \item \(\varepsilon > 0\) \hfill[\texttt{h\(\varepsilon\)}]
  \end{enumerate}
...
\item \textbf{Normalization buys stability, and speed on the way.}
  ...
  accuracy in markedly fewer epochs. It also makes the result
  repeatable: five runs of the un-normalized net spread over 2.7 to 3.3
  points under every optimizer, five of the normalized net over 0.2 to
  1.1.
\item \textbf{The optimizer moves the ceiling most.} Trading plain SGD
  ...
  more than anything else here. AdamW lands level with plain SGD once
  BN is in, and its per-coordinate adaptivity earns less than momentum
  does.
\item \textbf{The arithmetic does not move the ranking.} Recomputing the
  ...
  was, with medians moving by $+0.31$, $-0.02$ and $-0.33$ --- every one of
  them smaller than the spread between repeated runs of a single
  configuration. Drop the normalization and bf16 starts to show: the
  medians move by up to $1.4$ points and one seed collapses. The
...
of the same command at the same seed, and each
...
source, wrapped here; the second \texttt{blurb} is the banner
\S\ref{sec:bn_runit}'s transcript opens with. The diff is eight
\texttt{.bnPerChannel} entries, one inserted between
...
gradient itself is \texttt{Proofs.cifarCnnBn8\_has\_vjp\_at}, proved once
...
need no separate proof between them. \texttt{cifarCnnBn8\_has\_vjp\_at\_correct}
...
epochs on the shared pipeline at a \emph{constant} learning rate --- no
warmup, no decay --- because a schedule is a fourth thing changing and
this section is about the third. Per-epoch test accuracy, the median of
five runs each, from \texttt{runs/2026-09-01-cifar8w-6arm-constlr/}:
...
\noindent CIFAR-10, wide 8-conv net, plain SGD at a constant lr~0.1 on the
shared pipeline, 40 epochs, BN vs no-BN, through the verified renderer.
Median of $n{=}5$ runs per curve. The final points span 74.1 to 75.2 with BN
and 68.0 to 70.7 without.

\medskip\noindent BN leads from the first epoch --- 41\% against the bare net's 13\% ---
and never gives the lead back, finishing 5.8 points up
(74.5\% vs 68.8\%). The un-normalized net traces the same
arc roughly ten epochs behind, and the gap never closes: it is still
losing at epoch 40 by more than the spread of either set of runs.

\medskip\noindent That first epoch is worth a second look. Thirteen per
cent is barely above chance on ten classes, and the un-normalized net
spends four epochs getting to where the normalized one starts. The
conditioning the three-term backward
(Theorem~\ref{thm:bnNormalize_has_vjp}) buys is what that is, and it is worth
more the deeper the stack --- exactly the trend that makes BN standard
equipment by ResNet's thirty-four layers.
...
pipeline, the same 40 epochs and the same constant rate. Median final
accuracy, with the observed range across five runs:
...
\hline
no BN  & 68.8 {\footnotesize (2.7)} & 72.2 {\footnotesize (3.3)} & 72.8 {\footnotesize (2.8)} \\
BN     & 74.5 {\footnotesize (1.1)} & \textbf{76.3 {\footnotesize (1.0)}} & 74.3 {\footnotesize (0.2)} \\
\hline
\end{tabular}
\end{center}

\noindent $n=5$ per cell. Parenthesised figures are the range from lowest
run to highest.

\medskip\noindent Momentum with BN is the best result on the board at
76.3\%, about two points over plain SGD and two over AdamW,
and that gap is wider than the spread within any cell. AdamW lands
level with SGD rather than between the two: its per-coordinate
second-moment scaling is worth something on the un-normalized net and
nothing once BN is doing the conditioning.

\medskip\noindent\textbf{Reading down the columns recovers Lever~1, and
this time it recovers it everywhere.} BN is ahead in all three ---
$+5.8$ under SGD, $+4.2$ under momentum,
$+1.5$ under AdamW. The benefit shrinks as the optimizer gets
stronger, which is the familiar reading: a better update rule can
partly substitute for better conditioning, and never entirely.

\medskip\noindent\textbf{Where the spread is.} Read the parenthesised
ranges. Without BN, five runs of the same command land 2.7 to 3.3 points
apart under every optimizer; with it, 1.1 under SGD, 1.0 under momentum
and 0.2 under AdamW. The normalized net is not only ahead, it is the one
whose number means something from a single run. The un-normalized AdamW
runs also printed \texttt{loss=NaN} in late epochs in three of the five
seeds while their accuracy held, and that turned out to be the
diagnostic, not the training: the progress line's loss is
$\sum \mathrm{onehot}\cdot\log(\mathrm{softmax})$ with no log-sum-exp
guard, so once an off-class probability underflows to zero it prints
$0\cdot(-\infty)$. The gradient is $\mathrm{softmax}-\mathrm{onehot}$
and never takes the log; the next epoch's loss came back finite and the
...
cover. The proofs say the emitted graph computes the gradient this
...
a report-only op outside the proven graph, which is exactly where the
\texttt{NaN} above came from. A verified gradient is not a guarantee of
numerical stability, and the low-precision seed that collapses to chance
in the next section is what that distinction looks like when it bites.
...
\medskip\noindent It can, on the net that matters, and getting there
took one change worth a paragraph. The renderer has two convolution op
families --- the per-example one these MNIST and CIFAR chapters use, and
the batched one the ImageNet chapters use --- and the bf16 twins were
built for the batched family only. The normalized net was rendered
per-example, so until it moved across there was no bf16 arm to run on it.
Moving it needed no new arithmetic: BatchNorm itself stays
...
\medskip\noindent So: the eight-convolution net \emph{with} BatchNorm,
...
gradients (the first layer's input is the image and needs none). Six
arms, five seeds each, from
\texttt{runs/2026-09-01-bnprec-seeds/}:
...
\hline
SGD (lr 0.1)   & fp32 & 74.63 & 75.00 & 73.11 & 73.76 & 74.39 & 74.39 \\
               & bf16 & 74.84 & 74.65 & 75.30 & 73.55 & 74.70 & 74.70 \\[2pt]
momentum       & fp32 & 76.72 & 76.46 & 77.14 & 76.39 & 76.78 & 76.72 \\
               & bf16 & 77.32 & 76.91 & 76.28 & 76.29 & 76.70 & 76.70 \\[2pt]
AdamW          & fp32 & 74.75 & 75.08 & 73.90 & 73.98 & 74.48 & 74.48 \\
               & bf16 & 74.53 & 73.76 & 74.23 & 74.15 & 73.95 & 74.15 \\
\hline
\end{tabular}
\end{center}

\noindent Medians move by $+0.31$, $-0.02$ and $-0.33$. Each of those is
smaller than the spread \emph{within} the corresponding fp32 row --- $1.89$,
$0.75$ and $1.18$ --- so on this net, at this scale, the number format is
not measurable against the noise of rerunning the same configuration.
Thirty arm-runs, no \texttt{NaN}, nothing collapsed to chance. That is the
result Chapter~\ref{chap:residual} was going to assume, and it no longer
has to.

\medskip\noindent The un-normalized net answers differently, and the
difference is the interesting part. Run the same comparison there (the
\texttt{fp32b} and \texttt{bf16} arms of
\texttt{runs/2026-09-01-cifar8w-6arm-constlr/}) and the medians move by
$-1.40$, $-0.56$ and $-0.09$, and one bf16 momentum seed collapses to
chance where no fp32 seed does. Normalization is doing more
than accelerating training: it is absorbing the perturbation. A net that
was already the less repeatable one in fp32 --- the three-point ranges
of the previous section --- is the one where sixteen fewer mantissa
bits show up at all.

\medskip\noindent The same pattern holds for a lever we did not mean to
pull. Changing the op family at fixed fp32 --- per-example to batched, the
same arithmetic throughout --- moves the un-normalized net by $+2.0$ on
SGD and $+1.7$ on momentum, which is larger than anything precision does
to it. On the normalized net the identical change is worth $-0.11$,
$+0.37$ and $+0.19$. The rendering path is a lever, it is invisible to the
proofs, and normalization damps it too.
...
below, the two land within a point of each other with overlapping
intervals: $73.13\%$ narrow, $74.09\%$ wide. Seven times the parameters
...
matmul at these widths --- the sweep's 4096-wide point is where it stops
being cheap, nine times the wall clock of the 64-wide run for nothing.
...
That two-point comparison is worth drawing out in full,
...
train each point on its own proof-rendered StableHLO. Dealt across three
of the box's four cards, the whole curve is a 22-minute run:
...
\begin{axis}[
    ...
    xmode=log, log basis x=2,
    xmin=6.5, xmax=5000, ymin=69, ymax=75.5,
    xtick={8,16,32,64,128,256,512,1024,2048,4096},
    xticklabels={8,16,32,64,128,256,512,1024,2048,4096},
    ytick={69,70,71,72,73,74,75},
    grid=major, grid style={gray!18},
    tick label style={font=\small},
    label style={font=\small},
    every axis plot/.append style={line width=1pt, mark size=1.5pt},
...
\addplot[blue, mark=*, mark options={fill=blue},
  ...
  error mark options={rotate=90, mark size=2.5pt, blue!55!white, line width=0.6pt}]
coordinates {
(8,70.38) += (0,0.89) -= (0,0.90) (16,72.06) += (0,0.87) -= (0,0.89) (32,73.61) += (0,0.85) -= (0,0.87) (64,73.13) += (0,0.86) -= (0,0.88) (128,73.60) += (0,0.85) -= (0,0.87) (256,73.66) += (0,0.85) -= (0,0.87) (512,74.09) += (0,0.85) -= (0,0.87) (1024,73.34) += (0,0.86) -= (0,0.88) (2048,73.38) += (0,0.86) -= (0,0.88) (4096,73.07) += (0,0.86) -= (0,0.88)
};
\addplot[only marks, mark=o, mark size=4pt, red, line width=1pt, forget plot] coordinates {(64,73.13)};
\node[anchor=west, font=\footnotesize, red!70!black] at (axis cs:74,71.6)
  {canonical head $d{=}64$};
\end{axis}
\end{tikzpicture}
\end{center}
...
BatchNorm, AdamW, 25 epochs under a cosine schedule with a three-epoch
warmup (the grid driver's own recipe; the levers above ran at a constant
rate), conv backbone held at $[16,16,32,32]$
(\texttt{runs/2026-09-13-cifar8bn-grid-xla/}). Each point is one run, so
the whiskers are the 95\% Wilson interval for that accuracy on the full
$10{,}000$-image test set --- about $\pm 0.9$ here --- and not a spread
over repeats. Read that way the curve makes a sharper claim than
``essentially flat'': every width from $d{=}32$ to $d{=}4096$ spans only
$73.1$ to $74.1\%$, which is narrower than a single point's own interval,
so the eight plateau intervals all overlap and \emph{none} of the
differences between them is resolvable from this experiment. The
$128\times$ wider head buys nothing measurable; its $17$M-parameter
classifier fits the training set a little harder (final train loss $0.39$
against $0.52$ at $d{=}64$) and the test set does not notice. Two
differences do survive. $d{=}16$ ($72.1\%$, interval $71.2$--$72.9$) sits
under the top of the plateau, its interval stopping short of $d{=}512$'s,
and $d{=}8$ ($70.4\%$, interval $69.5$--$71.3$) is below every plateau
point, its upper bound about a point under the lowest plateau bound of
$72.2$: there the $128{\to}8$ first layer throttles the 128-dim feature
map the convolutions produce. The canonical $d{=}64$ (circled) sits
squarely on the plateau: the head width genuinely does not matter here,
which is the whole reason the net could borrow MNIST's classifier
untouched. (The $d{=}512$ point is the board's own net, and lands within a
quarter point of its 40-epoch AdamW cell on a shorter, scheduled run; the
\emph{shape} is the point.)
...
what let one low-precision momentum run collapse to chance. Normalization
is what makes the aggressive optimizer survivable. That is the standard
...
Theorem~\ref{thm:bnNormalize_has_vjp}: with $\hat{x} = (x-\mu)\,\mathrm{istd}$,
\[
...
half). The one subtlety, isolated in Theorem~\ref{ax:bnIstdBroadcast_diff}, is
...
The next chapter (Chapter~\ref{chap:residual}) adds residual connections
...
PJRT_FFI_RESIDENT=1 ./.lake/build/bin/resnet34-verified-adam data
\end{verbatim}

On one RTX 4060 Ti (CUDA 12.9), from
\texttt{runs/2026-09-12-r34-ablation-fp32-seeds/full\_s2.log}: seed 2 of the
five that \S\ref{sec:r34_ablation} averages, run by the ablation driver's
\texttt{full} arm, which is this binary's render and recipe under
\texttt{LEAN\_MLIR\_SEED=2}. XLA's startup banner and the resident-parameter
notices are removed, the network description line is wrapped, each interval is
...
[pjrt_ffi] compiled verified_mlir/resnet34_adam_train_step.mlir
             (@resnet34_adam_train_step, 405 outputs, 1 replica) in 6127 ms
[pjrt_ffi] compiled verified_mlir/resnet34_fwd.mlir
             (@resnet34_fwd, 1 outputs, 1 replica) in 596 ms
[pjrt_ffi] compiled verified_mlir/resnet34_fwd_eval.mlir
             (@resnet34_fwd_eval, 1 outputs, 1 replica) in 293 ms
Real ResNet-34 on Imagenette 224² (7×7-s2 stem→3×3-s2 overlapping max pool→
  ...
  xla/pjrt verified_mlir/resnet34_adam_train_step.mlir
  xla/pjrt verified_mlir/resnet34_fwd.mlir
  xla/pjrt verified_mlir/resnet34_fwd_eval.mlir
...
Epoch 1/80: loss=1.971035 lr=0.000333
  epoch 1: val_acc = 1239/3925 = 31.566879%  top5 = 2766/3925 = 70.471338%
    [95% CI 30.13–33.04]
Epoch 2/80: loss=1.553382 lr=0.000667
  epoch 2: val_acc = 1471/3925 = 37.477707%  top5 = 3123/3925 = 79.566879%
    [95% CI 35.98–39.00]
Epoch 3/80: loss=1.381944 lr=0.001000
  epoch 3: val_acc = 1880/3925 = 47.898089%  top5 = 3159/3925 = 80.484076%
    [95% CI 46.34–49.46]
Epoch 4/80: loss=1.311593 lr=0.001000
  epoch 4: val_acc = 1851/3925 = 47.159236%  top5 = 3547/3925 = 90.369427%
    [95% CI 45.60–48.72]
Epoch 76/80: loss=0.502131 lr=0.000007
  epoch 76: val_acc = 3536/3925 = 90.089172%  top5 = 3859/3925 = 98.318471%
    [95% CI 89.11–90.99]
Epoch 79/80: loss=0.502245 lr=0.000000
  epoch 79: val_acc = 3535/3925 = 90.063694%  top5 = 3859/3925 = 98.318471%
    [95% CI 89.09–90.96]
Epoch 80/80: loss=0.502085 lr=0.000000
  epoch 80: val_acc = 3535/3925 = 90.063694%  top5 = 3859/3925 = 98.318471%
    [95% CI 89.09–90.96]
done (trained ResNet-34 adam + cosine/warmup via packed threading).
\end{verbatim}

Eighty epochs at about 35 seconds each, 47 minutes in all, and
\textbf{90.06\%} top-1 with \textbf{98.32\%} top-5 on Imagenette's 3,925-image
validation split. This is one seed of five; the five land at
$89.99 \pm 0.32$ top-1 (\S\ref{sec:r34_ablation}), and that mean is the
number every later chapter compares against.
...
\begin{theorem}[Residual block VJP]
  ...
  \leanok
  \uses{def:hasvjp, thm:biPath_has_vjp, thm:identity_has_vjp}
  \Assume{}
  \begin{enumerate}
    \item \(B_f\) is a correct backward function for \(f\)
          (\(\HasVJP\, f\)) \hfill[\texttt{hf}]
...
def resnet34Verified : VerifiedNetSpec where
  ...
  layers   := [
    ...
    .dense 512 10 ]
  blurb := "Real ResNet-34 on Imagenette 224² (7x7-s2 stem->3x3-s2 overlapping
            max pool->[3,4,6,3] blocks w/ batch-norm, He et al. option-B 1x1
...
property of the network. Swapping AdamW for momentum is a different \emph{argument}
...
\S\ref{sec:r34_runit} has the run: 80 epochs on one RTX 4060 Ti, finishing at
\textbf{90.06\%} top-1 and \textbf{98.32\%} top-5 on Imagenette's 3,925-image
validation split, one seed of the five that average $89.99 \pm 0.32$. That is
...
\begin{itemize}
\item \textbf{Per-step time is about 120\,ms} at batch 32, 295 steps to the
  epoch, dominated by the four \texttt{.residualStage} groups where the
  $3 \times 3$ convolutions at 64, 128, 256 and 512 channels are. The first
  stage alone, at 64 channels on the $56 \times 56$ grid, is
  $3 \times 3 \times 64 \times 64$ multiply-accumulates per output location,
  about $1.2 \times 10^8$ per convolution, six convolutions in the stage,
  times the batch, and the backward costs about twice the forward again.

\item \textbf{Final loss plateaus near 0.50}, not zero. That's the
...
\item \textbf{BN layers: 36}. Every residual block contains two normalized
  convolutions, and there are 16 blocks across the four stages
  (3+4+6+3): 32. The projection shortcuts that open stages 2--4 carry
  one each, and the stem one, for 36 total. Each one proves its own VJP
  via Theorem~\ref{thm:bn_has_vjp} and composes with the rest of the
  network via Theorem~\ref{thm:vjp_comp}.
\end{itemize}
...
rule \texttt{vjp\_comp\_at} (Theorem~\ref{thm:vjp_comp}).
...
$83.20 \pm 1.57$ (momentum at a constant rate and nothing else, the
\texttt{bare} arm of \S\ref{sec:r34_ablation}) to $89.99 \pm 0.32$.
...
So how much does each ingredient actually earn? The full recipe and six
leave-one-out variants, ResNet-34 on Imagenette, 80 epochs each. Every variant
...
\emph{given everything else is present} --- not the order-dependent story an
...
\noindent Each point is plotted as its \emph{difference} from the full recipe
rather than as an accuracy, so the line at zero is the baseline and distance
from it is what the ingredient contributes. Each arm was trained five times from
five seeds; the point is the mean difference and the bar its 95\% interval,
paired by seed, since arms sharing a seed share initialization and data order. An
arm whose bar crosses zero is not placed, however suggestive its mean. The full
recipe lands at $89.99 \pm 0.32$ (fp32) and $89.98 \pm 0.15$ (bf16). Rows are
sorted by fp32 effect.

\medskip\noindent The two panels are the same seven arms in the two precisions,
and the reason to draw them side by side is that \emph{a real effect should not
...
agreeing on \emph{how} imprecise says the width belongs to the arm, not to
...
it finishes slightly ahead, by a paired $+0.403 \pm 0.162$ with all five seeds
agreeing in sign. The two lower bars overlap while that difference is resolved,
...
measures the recipe \emph{as tuned}, not its ingredients in isolation.

\medskip\noindent \textbf{Augmentation is the largest single contribution}
($-7.33 \pm 0.49$ fp32, $-7.11 \pm 0.38$ bf16; three times the next-largest
knob). Imagenette has 9{,}469 training images for a 21.3M-parameter network, so
...
This section runs it \emph{twice}. The \emph{reference} is the
...
tricks'' polish that became standard after 2018, and
...
through tfds. It runs in bf16 --- both matmuls and convolutions cast to
bfloat16 against fp32 master weights --- which is a throughput choice
...
\noindent Global batch 256 as $4 \times 64$ replicas, scored over all
...
\noindent The run, epoch by epoch. The kink near epoch~5 is the end of
...
\texttt{momdp64bf16} for the run below --- which is
...
\texttt{scripts/jobs/r34-default-bf16-4gpu.conf}, carries what used to be a
seven-variable command line --- the device list, both replica knobs, the
variant, the per-replica batch and the feed setting --- and
...
GPU & Precision & Per epoch & Epochs & Total & Val top-1 & Val top-5 \\
\hline
4$\times$ 3060 (CUDA) & bf16 & $\sim$14.6 min & 90 & $\sim$22.2 hr & $\mathbf{74.06\%}$ & $\mathbf{91.75\%}$ \\
\hline
\end{tabular}
\end{center}

\noindent Global batch 256 as $4 \times 64$ replicas, bf16 matmul and
convolution as on the reference, scored over the same $50{,}000$ images.

\medskip\noindent\textbf{The lowerer underneath is interchangeable.} Run
from the same initialization on the same box, the IREE and XLA lowerings of
this artifact scored $17{,}312$ and $17{,}313$ correct at epoch~5 --- one
image apart. The proof-carrying tier stops at Imagenette, so the claim is
\emph{one architecture, two independent lowerings, agreeing}, not ``proven
at ImageNet scale''.
...
box                 & 4$\times$ 4060\,Ti       & 4$\times$ 3060 \\
BN statistic group  & 256 (global)             & 64 (per replica) \\
per epoch           & $\sim$8.8 min            & $\sim$14.6 min \\
total               & $\sim$14.8 hr            & $\sim$22.2 hr \\
\hline
precision           & bf16 (matmul + conv)     & bf16 (matmul + conv) \\
...
\hline
\textbf{val top-1}  & $\mathbf{74.16\%}$       & $\mathbf{74.06\%}$ \\
\textbf{val top-5}  & $\mathbf{91.92\%}$       & $\mathbf{91.75\%}$ \\
\hline
\end{tabular}
\end{center}

\noindent Both columns are ResNet-34 on the 2018 recipe, 90 epochs, scored
over the same $50{,}000$ validation images under the same protocol; the
reference ran on four 4060\,Ti and the verified path on four 3060, the
arrangement \S\ref{sec:r50_pjrt}'s grid keeps. Rows above the first rule
differ; rows between the rules are held equal by construction.

\medskip\noindent\textbf{The two paths agree to $0.10$ points on top-1 and
$0.17$ on top-5}, and that is well inside what one run can resolve. At $n = 50{,}000$ the 95\% Wilson
interval of Chapter~\ref{chap:tensor} is $\pm0.38$ points on either top-1
--- $[73.77, 74.54]$ for the reference, $[73.68, 74.45]$ for the verified
path --- so the gap in the table is a fraction of the interval, and
...
\medskip\noindent The pair is not one-variable, and the table says where:
the verified run is on four 3060 where the reference is on four 4060\,Ti,
and its BatchNorm normalizes over each replica's $64$ where the reference's
\texttt{@jit} mesh reduces the statistics over the global $256$. The $0.10$
bounds the two together. \S\ref{sec:mnv2_phase4} is the same comparison on
one box, with the BatchNorm group left as the only asymmetry.
...
learning rate SGD moves top-1 by several points between consecutive
epochs, in both directions, seven at the worst on either path --- so the row
is read for its ends. Over the last fifteen epochs the two average within
$0.04$ of each other:
...
\hline
reference (JAX)       & 35.85 & 44.94 & 47.01 & 51.30 & 59.47 & 68.71 & 74.16 \\
verified path (PJRT)  & 30.06 & 44.32 & 48.70 & 53.34 & 60.72 & 68.54 & 74.06 \\
$\Delta$              & $-5.79$ & $-0.62$ & $+1.69$ & $+2.04$ & $+1.25$ & $-0.17$ & $-0.10$ \\
\hline
\end{tabular}
\end{center}
...
(1,8.70) (2,17.34) (3,25.78) (4,33.29) (5,30.06) (6,32.50) (7,40.35) (8,40.42) (9,37.95) (10,44.64) (11,43.02) (12,41.01) (13,43.50) (14,42.27) (15,44.32) (16,41.56) (17,41.19) (18,48.75) (19,45.40) (20,46.14) (21,43.96) (22,45.61) (23,47.07) (24,44.61) (25,48.23) (26,47.12) (27,47.14) (28,47.71) (29,49.06) (30,48.70) (31,50.76) (32,49.48) (33,49.92) (34,50.87) (35,45.04) (36,49.85) (37,51.51) (38,49.34) (39,52.37) (40,48.84) (41,52.12) (42,51.94) (43,52.54) (44,53.34) (45,53.34) (46,53.55) (47,53.95) (48,55.02) (49,54.30) (50,56.34) (51,55.95) (52,53.80) (53,57.11) (54,56.81) (55,58.26) (56,59.13) (57,56.06) (58,57.57) (59,59.86) (60,60.72) (61,59.12) (62,61.55) (63,61.54) (64,62.14) (65,58.11) (66,62.46) (67,63.99) (68,64.53) (69,63.40) (70,65.45) (71,66.20) (72,67.16) (73,67.61) (74,67.28) (75,68.54) (76,69.73) (77,69.57) (78,70.41) (79,70.62) (80,71.56) (81,71.61) (82,72.37) (83,72.70) (84,73.04) (85,73.42) (86,73.72) (87,73.90) (88,73.97) (89,74.06) (90,74.06)
};
...
\noindent One recipe, two paths to XLA, scored over all $50{,}000$
validation images at every epoch.
...
which is worth close to three points over ResNet-34. Then the recipe, with
the architecture held fixed: the same network on RSB-A3 (Wightman et al.\
2021, ``ResNet Strikes Back''), worth another point and a third. Both
recipes run on both paths, so the result is a grid rather than a pair, and
the table that closes the section is the 2018 column every later chapter's
recipe is diffed against.
...
row is the ResNet-34 recipe as it stands. A3 is LAMB at effective batch
2048 (accumulated as $4\times512$), BCE over Mixup/CutMix soft targets,
RandAugment, and train@160 / eval@224.

\begin{center}
\begin{tabular}{l l l r r r r r}
\hline
GPU & Recipe & BN group & Per epoch & Epochs & Total & Val top-1 & Val top-5 \\
\hline
4$\times$ 4060 Ti (bf16) & 2018   & 256 (global)   & $\sim$14.8 min & 90  & $\sim$24.0 hr & $\mathbf{76.95\%}$ & $\mathbf{93.44\%}$ \\
4$\times$ 4060 Ti (bf16) & RSB-A3 & 512 (Ghost-BN) & $\sim$7.9 min  & 100 & $\sim$15.1 hr & $\mathbf{78.26\%}$ & $\mathbf{93.79\%}$ \\
\hline
\end{tabular}
\end{center}

\noindent Both rows scored over all $50{,}000$ validation images. A3
normalizes per $512$-image micro-batch because it accumulates
...
as a side effect of accumulation rather than a choice, and the group size
is a knob of its own that \S\ref{sec:r50_a2_a1_cost} returns to. The
architecture lever: against ResNet-34's $74.16\%$ on the same
recipe, the bottleneck is worth $+2.79$ points. The recipe lever: A3 adds
$+1.31$ on the same network, in less wall clock, because it trains at
$160^2$. It also clears the paper's $78.1\%$ and the $78.05\%$ that
\texttt{timm}'s own \texttt{resnet50.a3\_in1k} weights score under this
protocol.
...
\begin{axis}[
    ...
    every axis plot/.append style={line width=1pt, mark=none},
    title style={font=\small},
    title={ResNet-50, two recipes --- reference},
]
\addplot[blue] coordinates {
...
\addlegendentry{2018 (90 ep)}
\addplot[orange] coordinates {
(1,0.32) (2,5.34) (3,6.81) (4,14.70) (5,11.62) (6,23.92) (7,27.06) (8,27.37) (9,35.02) (10,35.15) (11,37.37) (12,36.58) (13,41.56) (14,42.43) (15,41.37) (16,38.14) (17,43.49) (18,42.18) (19,39.75) (20,45.80) (21,43.14) (22,42.05) (23,46.11) (24,44.61) (25,46.88) (26,44.95) (27,47.33) (28,45.88) (29,50.31) (30,48.04) (31,36.35) (32,44.06) (33,47.24) (34,52.21) (35,40.58) (36,49.36) (37,43.32) (38,54.21) (39,49.11) (40,52.03) (41,51.79) (42,53.93) (43,48.36) (44,52.52) (45,59.70) (46,52.96) (47,54.78) (48,55.10) (49,56.45) (50,52.80) (51,59.18) (52,62.93) (53,62.82) (54,62.71) (55,61.33) (56,62.99) (57,62.87) (58,60.49) (59,60.91) (60,63.07) (61,62.78) (62,66.71) (63,64.46) (64,67.76) (65,70.47) (66,69.36) (67,69.58) (68,70.74) (69,66.93) (70,68.29) (71,71.02) (72,71.03) (73,71.12) (74,73.37) (75,72.69) (76,72.41) (77,73.87) (78,74.58) (79,75.03) (80,75.70) (81,75.25) (82,75.94) (83,75.53) (84,76.58) (85,76.17) (86,77.22) (87,77.39) (88,77.26) (89,77.84) (90,77.72) (91,77.96) (92,77.91) (93,78.11) (94,78.17) (95,78.20) (96,78.23) (97,78.19) (98,78.28) (99,78.26) (100,78.26)
};
\addlegendentry{RSB-A3 (100 ep)}
\end{axis}
\end{tikzpicture}
\end{center}

\noindent The two recipes on the reference, epoch by epoch. A3 starts far
lower --- $0.32\%$ at epoch~1 against $5.62\%$ --- because BCE over mixup
targets gives almost no signal until the logits separate; from roughly
epoch~25 the two trade the lead, and A3 holds it outright only from
epoch~86. At the matched epoch~90 it leads by $0.77$ ($77.72$ against
$76.95$), and its last ten epochs of anneal add the rest. The schedules are
different lengths, so at a shared epoch the two cosines are at different
points in their decay; the endpoints are what the $+1.31$ is measured from.
...
\texttt{resnet50in\_momdp64bf16} for 2018, and for A3
\texttt{resnet50in160\_lambaccdp8x64wxclipbcebf16} --- LAMB $\times$
BCE-with-logits $\times$ accumulation $k=8$ at four replicas of $64$, the
same effective $2048$, train@160 / eval@224. They run with:

\begin{verbatim}
lake run r50-2018-bf16-4gpu
lake run r50-a3-wxclip-bf16-4gpu
\end{verbatim}
...
GPU & Recipe & BN group & Per epoch & Epochs & Total & Val top-1 & Val top-5 \\
\hline
4$\times$ 3060 (bf16) & 2018   & 64 (per replica) & $\sim$20.5 min & 90  & $\sim$30.7 hr & $\mathbf{77.07\%}$ & $\mathbf{93.48\%}$ \\
4$\times$ 3060 (bf16) & RSB-A3 & 64 (per replica) & $\sim$13.7 min & 100 & $\sim$22.8 hr & $\mathbf{77.98\%}$ & $\mathbf{93.76\%}$ \\
\hline
\end{tabular}
\end{center}

\noindent Scored over the same $50{,}000$ images. \texttt{shimScript}
(\S\ref{sec:r34_pjrt}) is a field on the network rather than on the recipe,
so at $224^2$ the two recipes need two shims and the wrong one trains the
right optimizer on the wrong data; the job configs refuse to start with it.

\medskip\noindent The A3 render carries the recipe as
\S\ref{sec:r50_a3_vs_2018} tabulates it, including the three places a bare
...
\emph{and} the accumulation, at threshold $kC$,
\texttt{Proofs.clipFactor\_accum}), and the \texttt{no\_weight\_decay} group
left out of layer adaptation. The optimizer is tied to the reference at the
update as well as the gradient, six variants to $\sim\!10^{-7}$. What it
does not carry is the reference's BatchNorm group: no collective touches the
batch statistics, so each replica normalizes over its own $64$ against the
reference's $512$ --- an eightfold gap, and much the largest asymmetry
between the two A3 columns. The rest of the list is small: $0.08\%$ of each
epoch is dropped because $8 \nmid 5004$, and Mixup's $\lambda$ is drawn
host-side, so agreement there is distributional rather than per-step.
...
\hline
 ...
 & reference & verified & $\Delta$ & reference & verified & $\Delta$ \\
\hline
top-1        & $76.95$ & $\mathbf{77.07}$ & $+0.12$ & $78.26$ & $\mathbf{77.98}$ & $-0.28$ \\
top-5        & $93.44$ & $\mathbf{93.48}$ & $+0.04$ & $93.79$ & $\mathbf{93.76}$ & $-0.03$ \\
precision    & bf16 & bf16 & & bf16 & bf16 & \\
BN group     & 256 & 64 & & 512 & 64 & \\
box          & 4060\,Ti & 3060 & & 4060\,Ti & 3060 & \\
wall clock   & $24.0$\,h & $30.7$\,h & & $15.1$\,h & $22.8$\,h & \\
\hline
\end{tabular}
\end{center}

\noindent Four runs, one per cell, every cell bf16 and every cell scored over
the same $50{,}000$ images. The reference column is four 4060\,Ti and the
verified column four 3060 throughout, which is what lets $\Delta$ be read as
a path difference rather than a path-and-box one.

\medskip\noindent\textbf{The two paths agree within $0.28$ points on both
recipes}, and within $0.04$ on top-5 --- inside the floor \S\ref{sec:r34_pjrt}
set, $\pm0.37$ at $77\%$ on $50{,}000$ images, at one seed per cell. \textbf{And the recipe delta
replicates}: A3 beats 2018 by $+1.31$ on the reference and $+0.91$ on the
verified path. Agreement at a single configuration can be a coincidence of
that configuration; a \emph{difference} that reproduces across two
independent implementations is harder to arrange by accident, and it is
what a one-row comparison cannot show. Two things the grid does not settle.
The A3 column trains at $160$ and evaluates at $224$ while 2018 does both at
$224$, so the recipe delta confounds resolution with recipe, by A3's own
design. And the A3 pair's $-0.28$ sits on an eightfold BatchNorm-group
difference that nothing here isolates.

\globalbntodo

\medskip\noindent\textbf{The A3 curves meet; they do not travel together.}
The verified path runs ahead through most of the schedule, by as much as
seventeen points, and the two converge only in the anneal --- within a
point from epoch~80 and $0.01$ apart at epoch~90. Where that route
difference lives is not established; the BatchNorm group is the one large
known asymmetry, and it is why this pair is read at its endpoints and
nowhere else:
...
\hline
reference (JAX)       & 11.62 & 46.88 & 52.80 & 72.69 & 77.72 & 78.26 \\
verified path (PJRT)  & 24.53 & 53.98 & 65.17 & 74.83 & 77.73 & 77.98 \\
$\Delta$              & $+12.91$ & $+7.10$ & $+12.37$ & $+2.14$ & $+0.01$ & $-0.28$ \\
\hline
\end{tabular}
\end{center}
...
\addlegendentry{reference (JAX)}
\addplot[red, dashed] coordinates {
(1,2.81) (2,7.78) (3,15.89) (4,23.13) (5,24.53) (6,29.49) (7,33.10) (8,35.75) (9,40.62) (10,42.59) (11,43.50) (12,45.92) (13,47.79) (14,45.68) (15,49.27) (16,49.15) (17,50.42) (18,51.79) (19,50.85) (20,53.34) (21,47.99) (22,52.52) (23,54.02) (24,51.77) (25,53.98) (26,54.38) (27,53.02) (28,54.08) (29,52.88) (30,55.30) (31,53.70) (32,57.21) (33,55.45) (34,57.14) (35,56.24) (36,56.14) (37,60.42) (38,52.69) (39,60.44) (40,61.01) (41,58.73) (42,59.63) (43,60.16) (44,63.77) (45,61.29) (46,63.13) (47,62.03) (48,63.23) (49,60.37) (50,65.17) (51,61.85) (52,65.30) (53,66.21) (54,65.86) (55,66.96) (56,67.20) (57,67.09) (58,66.80) (59,67.37) (60,68.23) (61,70.11) (62,69.99) (63,70.08) (64,70.80) (65,71.48) (66,70.92) (67,70.83) (68,72.56) (69,72.21) (70,73.25) (71,72.99) (72,73.60) (73,74.17) (74,74.24) (75,74.83) (76,75.08) (77,75.09) (78,75.16) (79,75.40) (80,75.89) (81,76.16) (82,76.44) (83,76.32) (84,76.93) (85,76.94) (86,77.01) (87,77.20) (88,77.24) (89,77.49) (90,77.73) (91,77.60) (92,77.80) (93,77.81) (94,77.86) (95,77.95) (96,77.94) (97,77.96) (98,77.99) (99,78.02) (100,77.98)
};
\addlegendentry{verified path (PJRT)}
\end{axis}
\end{tikzpicture}
...
\noindent RSB-A3 on both paths, scored over all $50{,}000$ validation images
at every epoch.
...
\hline
A3 (\texttt{rsb-faithful}) & 100 & $160$ & shipped, and run (\S\ref{sec:r50_a3_vs_2018}) & $\mathbf{77.98\%}$ \\
A2 (\texttt{a2-accum})     & 300 & $224$ & rendered, EMA + sd & $79.8\%$ (paper) \\
A1 (\texttt{a1})           & 600 & $224$ & rendered, EMA + sd & $80.4\%$ (paper) \\
\hline
\end{tabular}
\end{center}

\noindent Eight artifacts: each tier at one and four replicas, in both precisions,
all at $4\times128$ for the reason the batch-norm paragraph gives. \textbf{A1 is
not ``the same render as A2''}: the decay is baked, so its $0.01$ against A2's
$0.02$ is a re-render, kept apart so one line among $18{,}000$ cannot be lost.

\medskip\noindent \textbf{The two regularisers these renders used to be missing
are now in them.} What is in these renders and what is still out, before anything
is run:
...
Accumulation to LAMB's design batch of $2048$, at $k=8$ or at the reference's own $k=4$
  & Neither tier has been run yet \\[2pt]
...
\emph{same} fourth region of $[\theta|m|v|\cdot]$, and accumulation is not optional
at $224^2$: EMA was unreachable, not unrendered.
...
a real mask, and by exactly zero at an all-ones one. The reference had it wrong
the other way round --- a scalar \texttt{bernoulli} for the whole batch where
timm's is per \emph{sample}, same expectation, which is why it survived.
...
\emph{mesh-sharded} axis, so XLA inserts one and its group is the full $512$ ---
its per-device tensor is $128$, and only the group is the statistic. \textbf{These renders
take the first half of that gap}: $k=4$ at $128$ per device is the reference's own
factorisation, and it is the \emph{faster} arrangement besides. The last $4\times$ needs synchronised batch-norm --- a new operator,
a changed backward, a variance that does not average, and $53$ collectives a
step.
...
and about thirty-five minutes of GPU time:

\begin{verbatim}
lake exe cache get                            # Mathlib oleans, ~30 s
./download_imagenette.sh                      # 330 MB dl, 2.4 GB unpacked
lake build mobilenetv2-verified-adam
PJRT_FFI_RESIDENT=1 ./.lake/build/bin/mobilenetv2-verified-adam data
\end{verbatim}
...
$9.5\times$ the parameters and averages $89.99 \pm 0.32$ top-1 and
$98.46\%$ top-5 over five seeds. MobileNetV2 gives up three quarters of
a point of top-1 --- less than one run's own $\pm 0.97$ interval --- picks
up a fifth of a point of top-5, and does it in thirty-five minutes
against forty-seven.
...
the \emph{whole} network, at the batch statistics this artifact runs, is
\texttt{mobilenetv2ForwardB\_full\_has\_vjp\_at\_correct}, in
\texttt{Proofs/Nets/MobileNet/MobileNetV2FullBVJP.lean}, and it covers the
stem, all seventeen bottlenecks and the head. The typed graph the artifact
is printed from denotes that forward by
\texttt{mobilenetv2FwdGraphB\_full\_faithful}, and the tie from the
\texttt{mobilenetv2Verified} layer list to that forward is
\texttt{mobilenetv2VerifiedB\_denote\_eq}, a \texttt{rfl} that stops
reducing if any \texttt{[t,c,n,s]} entry of the spec drifts, with
\texttt{mobilenetv2VerifiedB\_fwd\_faithful} above it.

The full-depth fold is \emph{pointwise}. It holds at a given input, and
it carries one side condition per activation site, at every example of
the batch, saying that the value entering relu6 is neither $0$ nor $6$.
There are thirty-five such sites per example,
...
($89.25\%$ against ResNet-34's $89.99\%$ on Imagenette, both
...
\begin{theorem}[Depthwise input VJP]
  ...
  \leanok
  \uses{ax:depthwiseConv2d, ax:pdiv, def:hasvjp3}
...
\begin{theorem}[Depthwise bias VJP]
  ...
  \leanok
  \uses{ax:depthwiseConv2d, ax:pdiv, def:hasvjp}
  \Prove{} \(\HasVJP\, \bigl(b \mapsto \mathrm{flatten}
...
is Theorem~\ref{ax:depthwise_has_vjp3} for the depthwise conv composed with
...
$89.99 \pm 0.32$ on the same data and the same recipe, that is
\textbf{0.74 points of top-1} for \textbf{9.5$\times$ fewer parameters}
...
\begin{itemize}
\item \textbf{2.24M parameters, and 1.05\,MB of MLIR} (1\,047\,327 chars)
...
\item \textbf{Per-step time is about 90\,ms} at batch 32, against
  ResNet-34's 120\,ms at the same batch, the same 295 steps to the epoch
  and the same resident parameters, so the whole run takes thirty-five
  minutes rather than forty-seven. That is a real $1.3\times$, and it is
  far short of what the parameter count promises: $9.5\times$ fewer
  weights buys $1.3\times$ the throughput, not $9.5\times$. Depthwise
  convolution is
...
\item \textbf{Three artifacts, not one.} The train step is accompanied by
...
\item \textbf{52 BN layers}, up from ResNet-34's 36, for the same reason
  the MLIR is bigger: more internal convolutions means more
  BN-after-conv pairs. Each one still proves its VJP via
  Theorem~\ref{thm:bn_has_vjp}.

\item \textbf{Final loss plateaus near 0.50} again, and for the same reason
...
reverse-mode derivative (Theorem~\ref{ax:depthwise_has_vjp3},
...
\texttt{mobilenetv2ForwardB\_full\_has\_vjp\_at} chains the whole network through
\texttt{vjp\_comp\_at}.
...
the head widens to 1000 and the dataset and schedule grow. As there, the
recipe runs on both paths --- the reference first, then the verified path,
then the two side by side.

\prosesection{The reference (JAX)}

The reference trainer is \texttt{jax/MainMobilenetV2Imagenet.lean}, and its
spec and recipe are:
...
convolutions cast to bfloat16 the instant the flag is on, but MobileNet's
compute lives inside inverted-residual blocks (a $1\times1$ expansion, a
depthwise $3\times3$, a $1\times1$ projection) whose convolutions
originally ran in fp32 regardless. Routing them through the same
\texttt{convdt} cast as the plain convolutions is what lets bf16 reach the
bulk of the network, and the whole block runs $\sim2\times$ faster on
cuDNN for it: the $1\times1$s are really matmuls and love bfloat16, and the
depthwise $3\times3$ is a wash. The weight decay also drops to
$4\times10^{-5}$, because MobileNet's depthwise filters are tiny at nine
weights per channel, and the $10^{-4}$ that suits ResNet over-regularizes
them.
...
and leaves the length of the schedule unstated. Given the rest, the length
is not free: under $\times 0.98$/epoch the learning rate after $90$ epochs
is still $0.045 \times 0.98^{85} \approx 8.1\times10^{-3}$, barely
annealed, with none of the end-of-schedule polish a cosine run has already
collected by then, and only past $300$ or so does it reach the
$10^{-5}$ range where the run stops moving ($0.045 \times 0.98^{345}
\approx 4.2\times10^{-5}$ at $350$). So $350$ is this book's choice, made
the way the decay makes it --- it is also the length Tan \& Le state for
EfficientNet's version of the same RMSProp lineage in
Chapter~\ref{chap:se} --- and the curve below is the check that it is
long enough. A shortened run of this recipe has to substitute cosine to be
worth anything, which is a different experiment rather than a cheaper
...
GPU & Precision & Per epoch & Epochs & Total & Val top-1 & Val top-5 \\
\hline
4$\times$ 3060 (CUDA) & bf16 & $6.6$ min & 350 & $38.4$ hr & $\mathbf{71.90\%}$ & $\mathbf{90.41\%}$ \\
\hline
\end{tabular}
\end{center}

\noindent Global batch 256 as $4 \times 64$ replicas, scored over all
$50{,}000$ validation images; one attempt, no restarts, and $6.6$ minutes
an epoch held to within a second across all $350$. The wall-clock win over
the $9.5\times$-larger ResNet-34 is real but sub-proportional, as
...
three on this hardware rather than nine. The RMSProp peak of $0.045$ took
...
Chapter~\ref{chap:se}. The best epoch, $331$, reaches $71.99\%$. The
paper's figure is $72.0\%$, and at $n=50{,}000$ the Wilson 95\% interval
on this run is $[71.50, 72.29]$: the paper's recipe, at a length the
paper leaves open, is not distinguishable from the paper's number, which
is the claim here and a weaker one than closing a gap by a measured
amount.
...
\begin{axis}[
    ...
    title style={font=\small},
    title={MobileNetV2 recipe, 350 epochs --- reference},
]
\addplot[blue, mark=*, mark options={fill=blue}] coordinates {
...
\noindent The run, sampled every 10 epochs. Two thirds of the final
accuracy is in place by epoch $50$, the run is still climbing at $150$, and
it is flat from about epoch $250$ onward, so the last $100$ epochs are
worth almost nothing and the schedule is genuinely exhausted rather than
cut short.

\medskip\noindent \textbf{[TODO: re-run.]} This result predates four fixes
that landed while it was training, so it is a clean run of a recipe that is no
longer the current one. The ledger, worst first:
...
\emph{In} & \emph{Still out}, worst first \\
\hline
Antialiased resize on train \emph{and} eval, and \texttt{timm}'s $50{,}000$-image
denominator --- both halves of that protocol
  & \textbf{\texttt{convBnAct := .relu6}} --- the stem and the $1\times1$ head trained
    plain \texttt{relu} while every inverted-residual interior used ReLU6. Landed
    2026-08-30, one day after this run started, and it is a reference-only defect:
    the verified render was always ReLU6 \\[2pt]
Label smoothing $0$, as Sandler et al.\ used
  & \textbf{BN running-stat decay} as a per-net knob --- still $0.99$ here where
    PyTorch's default works out to $0.9$ \\[2pt]
RMSProp $\varepsilon$ inside the root, matching TensorFlow
  & Mixup/CutMix label-smoothing fix --- inert for this recipe, which runs neither,
    but it moves the shared emitter \\[2pt]
Exponential decay $\times0.98$/epoch over the full $350$, dropout $0.2$, running-BN eval
  & Nothing re-run: the four above are known, none is measured \\[2pt]
...
\noindent The first row is the one to price. Two clamped activations are not
obviously worth much, but this net carried the fleet's largest paper gap for
months with the schedule as the only suspect, and the gap was never decomposed
against a reference that had them.
...
\texttt{lake build mobilenetv2-\allowbreak imagenet-\allowbreak verified}:
...
-- 350 because that is the schedule the reference number above was measured on.
...
\S\ref{sec:r34_pjrt} gives. The head is the only parameter shape that
...
paper's.} \texttt{rmsdp64bf16} is the artifact the run below used: RMSProp
at $\rho = 0.9$, $\mu = 0.9$, $\varepsilon = 1.0$ and coupled weight decay
$4 \times 10^{-5}$, at peak LR $0.045$ with a 5-epoch warmup and
$\times 0.98$ per epoch after it, data-parallel over four replicas in
bf16. Those constants live in \texttt{mnv2RmsSchedule} and
\texttt{mnv2RmsHyper}, shared with the Imagenette peer so the two cannot
carry different values. \texttt{adam64} is AdamW at $10^{-3}$ instead,
which is the default because it matches the other four ImageNet drivers.
It runs with:
...
GPU & Precision & Per epoch & Epochs & Total & Val top-1 & Val top-5 \\
\hline
4$\times$ 3060 (CUDA) & bf16 & $8.8$ min & 350 & $51.2$ hr & $\mathbf{71.91\%}$ & $\mathbf{90.52\%}$ \\
\hline
\end{tabular}
\end{center}

\noindent Global batch 256 as $4 \times 64$ replicas, scored over the same
$50{,}000$ images.
...
BN statistic group   & 256 (global)             & 64 (per replica) \\
ms/step              & 79                       & 97 \\
per epoch            & 6.6 min                  & 8.8 min \\
total                & 38.4 hr                  & 51.2 hr \\
\hline
precision            & bf16                     & bf16 \\
global batch         & 256 ($4\times64$)        & 256 ($4\times64$) \\
steps/epoch          & 5{,}004                  & 5{,}004 \\
optimizer            & RMSProp, $\varepsilon=1.0$ & RMSProp, $\varepsilon=1.0$ \\
\hline
\textbf{val top-1}   & $\mathbf{71.90\%}$       & $\mathbf{71.91\%}$ \\
\textbf{val top-5}   & $\mathbf{90.41\%}$       & $\mathbf{90.52\%}$ \\
\hline
\end{tabular}
\end{center}

\noindent Both columns are MobileNetV2 on the paper's RMSProp recipe at
350 epochs, on the same four RTX 3060 cards, scored over the same
$50{,}000$ validation images. Rows above the first rule differ; rows
between the rules are held equal by construction.

\medskip\noindent\textbf{The two paths agree to $0.01$ points on top-1 and
$0.11$ on top-5}, inside the floor \S\ref{sec:r34_pjrt} set ($\pm0.39$ at
$72\%$ on $50{,}000$ images), at one seed each. Precision is held equal
here where ResNet-34's pair did not, so the one asymmetry left above the
rule is the BatchNorm group, and this is the pair that isolates it.

\medskip\noindent\textbf{What the BatchNorm group is.} MobileNetV2 has
BatchNorm in every block. The verified path runs four replicas of $64$ with
no collective touching the batch statistics, so it normalizes over groups
of $64$; the reference under a \texttt{NamedSharding} mesh reduces
globally, so it normalizes over $256$. The ResNet-34 pair of
\S\ref{sec:r34_pjrt}, which carries the same fourfold difference and a
box difference besides, landed $0.10$ apart at convergence, inside the
floor; the $+0.01$ here is smaller still. So the claim is that the two paths agree
to within the BatchNorm-group effect --- not that the path has been shown
neutral. Separating those takes a verified render that all-reduces the
batch statistics, and when this pair trained none did: the only node in
the graph language that saw across replicas was the gradient collective,
and every BatchNorm node consumed a single expression. Every BatchNorm pair
in this book was trained under that asymmetry.

\globalbntodo

\medskip\noindent\textbf{The curves converge; they do not merely end together.}
The verified path runs ahead through warmup, is level by epoch~20, and from
epoch~100 every sampled point sits within a third of a point. The $+2.11$ at
epoch~50 is the reference's own slow stretch rather than a verified lead --- it
gains $1.18$ points over epochs $40$--$50$ and then $2.57$ over $50$--$60$:
...
\hline
reference (JAX)       & 46.69 & 61.71 & 67.40 & 69.78 & 71.23 & 71.85 & 71.90 \\
verified path (PJRT)  & 51.36 & 63.82 & 67.74 & 69.97 & 70.95 & 71.84 & 71.91 \\
$\Delta$              & $+4.67$ & $+2.11$ & $+0.34$ & $+0.19$ & $-0.28$ & $-0.01$ & $+0.01$ \\
\hline
\end{tabular}
\end{center}
...
(1,3.16) (10,51.36) (20,56.47) (30,60.30) (40,61.43) (50,63.82) (60,64.51) (70,65.18) (80,66.37) (90,66.85) (100,67.74) (110,68.19) (120,68.53) (130,69.40) (140,69.74) (150,69.97) (160,70.44) (170,70.40) (180,70.48) (190,70.80) (200,70.95) (210,71.33) (220,71.43) (230,71.38) (240,71.67) (250,71.70) (260,71.67) (270,71.75) (280,71.77) (290,71.89) (300,71.84) (310,71.93) (320,71.94) (330,71.87) (340,71.93) (350,71.91)
};
\addlegendentry{verified path (PJRT)}
\end{axis}
\end{tikzpicture}
...
\noindent One recipe, two paths to XLA, sampled every 10 epochs and scored
over all $50{,}000$ validation images.

\medskip\noindent\textbf{The remaining $51.2$ against $38.4$ hours is the
data feed, not the renderer.} With the shim removed the verified step is
$78$\,ms --- a $38$-hour schedule, level with the reference's $79$ --- and
the fed step is $97$; the difference is host work the reference does
in-process, and \S\ref{sec:vit_phase4} has the accounting. This is the net
where it shows most plainly: the compute floor is already at the
reference's wall clock, so every hour of the gap is pipeline.
...
whole six-year argument in two rows: $\mathbf{+3.58}$ points over
MobileNetV2's paper-faithful $71.90\%$, from $2.8\times$ the
parameters and \emph{less than half} the epochs, $100$ against
$350$ and $16$ wall-clock hours against $38.4$.
...
\hline
Box & ms/step & min/epoch & 100 ep & 500 ep & Val top-1 \\
\hline
4$\times$ 4060 Ti, global 256 & $176 \to 122$ & $14.7 \to 10.2$ & $25.5 \to 18.0$ h & $127.5 \to 90.0$ h & TBD \\
4$\times$ 3060, global 256    & ---           & ---             & ---               & ---                & TBD \\
\hline
\end{tabular}
\end{center}

\noindent {\small The columns are the two arms of \texttt{adamdp64}; fp32 is what
\texttt{mnv4-\allowbreak default-\allowbreak 4gpu} names today. $100$ epochs is the
reduced-regularization tier, $500$ the paper's.}
...
arm by default --- the left-hand column above, $176$ ms/step and
$\sim$$25.5$ hr; one line in the config switches it, and the choice was to move one
...
commands, and about forty minutes of GPU time:

\begin{verbatim}
lake exe cache get                            # Mathlib oleans, ~30 s
./download_imagenette.sh                      # 330 MB dl, 2.4 GB unpacked
lake build efficientnet-verified-adam
PJRT_FFI_RESIDENT=1 ./.lake/build/bin/efficientnet-verified-adam data
\end{verbatim}
...
$90.14\%$. That is level with ResNet-34's five-seed $89.99 \pm 0.32$ and
three quarters of a point over MobileNetV2's $89.25\%$, on the same data,
the same recipe and the same eighty epochs. EfficientNet-B0 does it with
$5.3\times$ fewer parameters than ResNet-34 and in forty-one minutes
against forty-seven.
...
weight. As in Chapter~\ref{chap:depthwise}, what ran is
\texttt{pretty(provenGraph)} ---
\texttt{verified\_mlir/efficientnet\_adam\_train\_step.mlir} --- and
every squeeze-excitation gate in it backpropagates by the product rule
Theorem~\ref{thm:seBlock_has_vjp} proves; the depthwise, BatchNorm and
residual rules are Chapters~\ref{chap:depthwise}, \ref{chap:bn}
and~\ref{chap:residual}'s.
...
The three compile lines and the \texttt{running-stats BN} line are
Chapter~\ref{chap:depthwise}'s, at 49 BatchNorms; the train step's 740
outputs are what 213 parameter tensors plus their Adam moments plus the
batch statistics plus the report-only loss come to.
...
\begin{theorem}[SE block VJP]
  ...
  \leanok
  \uses{def:hasvjp, thm:elemwiseProduct_has_vjp, thm:identity_has_vjp}
  \Assume{}
  \begin{enumerate}
...
two-layer MLP: that is exactly Theorem~\ref{thm:seBlock_has_vjp} composed
with Theorem~\ref{thm:elemwiseProduct_has_vjp} and the already-proved dense
VJPs
...
\hline
ResNet-34        & 21.29M  & 729\,KB    & 120 ms    & 47 min   & \textbf{89.99\%} & 98.46\% \\
MobileNetV2      &  2.24M  & 1{,}047\,KB &  90 ms   & 35 min   & 89.25\% & \textbf{98.68\%} \\
EfficientNet-B0  &  4.02M  & 1{,}316\,KB & 103 ms   & 41 min   & 89.96\% & 98.45\% \\
\hline
\end{tabular}
\end{center}
...
\texttt{<slug>\_adam\_train\_step.mlir} in 1000-byte units. ResNet-34's
accuracies are the five-seed means of \S\ref{sec:r34_ablation} and its
times are one of those seeds' (\S\ref{sec:r34_runit}); the other two rows
are one run each.

\begin{itemize}
\item \textbf{EfficientNet-B0 is level with ResNet-34 on top-1}, at
  $89.96\%$ against $89.99 \pm 0.32$, and three quarters of a point over
  MobileNetV2's $89.25\%$. It gets there with $5.3\times$ fewer parameters
  than ResNet-34 and in six fewer minutes. MobileNetV2 keeps top-5 by two
  tenths, which is the one place the larger model does not win.
...
\item \textbf{103\,ms per step sits between the two}, slower than
  MobileNetV2's 90\,ms because of exactly those extra ops, and quicker
  than ResNet-34's 120\,ms by less than the parameter count suggests,
  for the reason Chapter~\ref{chap:depthwise} gave: the core is a
  depthwise sandwich, and depthwise is memory-bound.
...
verified path, then the two side by side.

\prosesection{The reference (JAX)}

The reference trainer is \texttt{jax/MainEfficientNetImagenet.lean}, and
its spec and recipe are:
...
\hline
4$\times$ 3060 & 350 (RMSProp) & $\sim$8.0 min & $46.6$ hr & $\mathbf{77.15\%}$ & $\mathbf{93.30\%}$ \\
\hline
\end{tabular}
\end{center}

\noindent (CUDA, bf16, batch 256.) That is the paper recipe end to end,
and it reached $\mathbf{77.15\%}$ top-1 / $\mathbf{93.30\%}$ top-5 on the
full 50{,}000-image validation split against EfficientNet-B0's published
$77.1\%\,/\,93.3\%$ --- a match. Four details of the recipe are worth stating precisely, because in each
one the obvious implementation is not the one B0 was trained with.
...
$\sqrt{\mathrm{sq}+\varepsilon}$, with $\varepsilon$ \emph{inside} the
...
evaluation then has to pair those averaged weights with equally-lagged
...
per step, so for the first several thousand steps the \emph{evaluated}
...
\textbf{The 350-epoch run.} The full tier reached $\mathbf{77.15\%}$
top-1 / $\mathbf{93.30\%}$ top-5 against EfficientNet-B0's paper
$77.1\%\,/\,93.3\%$: level on top-1 and within $0.03$ on top-5. That tier
is the paper recipe end to end, with RMSProp at $0.016$, AutoAugment,
stochastic depth, EMA and exponential LR decay --- a faithful,
from-scratch reproduction with no hacks: real RMSProp, real LR, real EMA,
real swish. It ran $46.6$\,hr on four RTX 3060 at $8.0$ minutes per
epoch. The headline figure is the EMA weights scored over all
50{,}000 validation images; the same run's raw weights score
$76.33\%\,/\,92.90\%$, so the weight average is worth $+0.82$ points:
...
faithful 350-epoch RMSProp run (bf16, 4$\times$ 3060), scored over all
50{,}000 validation images at every epoch. The final
$77.15\%\,/\,93.30\%$ matches the paper's $77.1\%\,/\,93.3\%$. The
flattening past
epoch $\sim$300 is the exponential schedule running out of learning rate
($0.016 \to 2.0\times10^{-4}$), not saturation of the architecture.
...
\medskip\noindent\textbf{Where this differs from the paper.} The recipe is
RMSProp $\rho\,0.9$ / $\mu\,0.9$ / $\varepsilon\,10^{-3}$, lr $0.256$ at batch
$4096$ (so $0.016$ at $256$), weight decay $10^{-5}$, exponential decay
$\times 0.97$ every $2.4$ epochs after a five-epoch warmup, dropout $0.2$,
drop-connect $0.2$, AutoAugment, label smoothing $0.1$, EMA $0.9999$, BN momentum
$0.99$, 350 epochs. All of it is carried on both paths, including the two
places the TensorFlow original differs from a naive port: $\varepsilon$ goes
\emph{inside} the square root ($g/\sqrt{s+\varepsilon}$, not
$g/(\sqrt{s}+\varepsilon)$), and the mean-square accumulator starts at $1.0$
rather than $0$. Drop-connect is a linear ramp over blocks, as the official
implementation does it, and the EMA shadows the BN buffers as well as the
weights.

\medskip\noindent\textbf{One difference is known and open: batch-norm
$\varepsilon$ is $10^{-5}$ here against the TF reference's $10^{-3}$.}
\texttt{\_bn(..., eps=1e-5)} in the reference's emitted code, and $147$ splat
constants of \texttt{1.0e-5} in the verified render --- so the two paths agree
with each other and both differ from the paper. It is unquantified: nobody has
run the $10^{-3}$ arm. Nor is there a deficit for it to explain: the reference scores
$77.15\%$ / $93.30\%$ against the paper's $77.1$ / $93.3$. The difference stands
and remains unquantified; what it does not have is a symptom.

\medskip\noindent\textbf{The activation agrees on both sides.} The verified
render is swish throughout --- $194$ \texttt{stablehlo.logistic} and not one
...
BN statistic group   & 256 (global)             & 64 (per replica) \\
ms/step              & $\sim$92                 & 134 \\
per epoch            & 8.0 min                  & 11.7 min \\
total                & 46.6 hr                  & 73.4 hr \\
\hline
precision            & bf16                     & bf16 \\
global batch         & 256 ($4\times64$)        & 256 ($4\times64$) \\
steps/epoch          & 5{,}004                  & 5{,}004 \\
optimizer            & RMSProp (TF form)        & RMSProp (TF form) \\
EMA                  & $0.9999$, weights + BN buffers & $0.9999$, weights + BN buffers \\
augmentation         & AutoAugment              & AutoAugment \\
\hline
\textbf{val top-1}   & $\mathbf{77.15\%}$       & $\mathbf{76.878\%}$ \\
\textbf{val top-5}   & $\mathbf{93.30\%}$       & $\mathbf{93.154\%}$ \\
\hline
\end{tabular}
\end{center}

\noindent Both columns are EfficientNet-B0 on the paper's RMSProp recipe at
350 epochs, on the same four RTX 3060 cards, scored over the same
$50{,}000$ validation images; the verified column is the
\texttt{emarmsdp64dropdobf16} render, \texttt{lake run enet-default-4gpu}.
Rows above the first rule differ; rows between the rules are held equal by
construction. The reference's in-training evaluation reads $77.18\%$ at
epoch~350; the $77.15\%$ headline is the same checkpoint rescored from its
saved state at a different evaluation batch size, and the $0.03$ between
them is bf16 batch-shape noise. Against the in-training figure the verified
path's $76.878\%$ is $-0.30$, the $\Delta$ the curve table below carries.

\medskip\noindent\textbf{$-0.27$ points --- and unlike MobileNetV2's pair,
this one is not a dead heat.} Read against the floor \S\ref{sec:r34_pjrt}
set, one evaluation at $-0.27$ is not a result: the verified final's 95\%
Wilson interval, $[76.51, 77.25]$, contains the reference. What separates
this pair from MobileNetV2's is persistence. From epoch~51 the verified
curve sits below the reference at every one of 300 evaluations, and over the
last fifty the paired gap averages $-0.32$ with a spread of $0.07$ ---
narrower than either curve's own spread over the same fifty ($0.11$ and $0.14$).
On this validation set the two paths did not converge to one curve but to
two, three tenths of a point apart. Whether the three tenths would survive
on images neither run scored is what one validation set cannot say; that
the trajectories differ, it can. This section does not claim to know why.

\medskip\noindent\textbf{The suspect is the BatchNorm group, and it is
unmeasured.} As in \S\ref{sec:mnv2_phase4}, precision is held equal, so the
one numerical asymmetry above the rule is the BatchNorm group: each
replica's $64$ against the reference's global $256$, for the reason given
there --- the render this pair trained on normalizes each replica's shard
on its own. The two pairs that carry the same fourfold difference landed
$0.10$ and $0.01$ apart, and MobileNetV2 has as much BatchNorm as this net
($52$ layers to $49$), so the count of normalization layers does not by
itself explain a $0.3$. What this recipe adds is that evaluation scores an
EMA of the BatchNorm buffers in place of the live statistics, and whether
that turns a per-replica group into a persistent offset is not known. The
test is cheap and touches no proof: train the reference with per-shard
BatchNorm and see whether the $0.3$ appears on the JAX side. A third
difference is variance rather than bias --- the stochastic-depth and
dropout masks are host-drawn on one side and JAX-PRNG on the other --- and
does not survive a fifty-epoch average.

\globalbntodo

\medskip\noindent\textbf{The curves track; the gap narrows and then holds.}
The verified path leads through the first fifty epochs on average ($+0.17$),
runs about half a point behind from there to epoch~200, and narrows to
$-0.4$ over epochs 200--250 and $-0.3$ from 250 on, where it stays:
...
epoch & 10 & 50 & 100 & 150 & 200 & 300 & 350 \\
\hline
reference (JAX)       & 61.69 & 70.78 & 73.14 & 75.07 & 75.93 & 76.92 & 77.18 \\
verified path (PJRT)  & 61.79 & 70.22 & 72.83 & 74.41 & 75.50 & 76.54 & 76.88 \\
$\Delta$              & $+0.10$ & $-0.56$ & $-0.31$ & $-0.66$ & $-0.43$ & $-0.38$ & $-0.30$ \\
\hline
\end{tabular}
\end{center}
...
\addplot[blue, mark=*, mark options={fill=blue}] coordinates {
(1,10.09) (10,61.69) (20,66.35) (30,68.83) (40,70.02) (50,70.78) (60,71.70) (70,71.95) (80,72.31) (90,73.12) (100,73.14) (110,73.57) (120,74.15) (130,74.33) (140,74.84) (150,75.07) (160,75.30) (170,75.47) (180,75.65) (190,75.84) (200,75.93) (210,76.07) (220,76.20) (230,76.22) (240,76.35) (250,76.42) (260,76.52) (270,76.65) (280,76.67) (290,76.76) (300,76.92) (310,76.84) (320,76.95) (330,77.18) (340,77.19) (350,77.18)
};
\addlegendentry{reference (JAX)}
\addplot[red, dashed, mark=o] coordinates {
(1,20.62) (10,61.79) (20,66.06) (30,67.91) (40,69.39) (50,70.22) (60,70.91) (70,71.57) (80,72.01) (90,72.40) (100,72.83) (110,73.30) (120,73.53) (130,73.84) (140,74.12) (150,74.41) (160,74.68) (170,74.88) (180,75.04) (190,75.40) (200,75.50) (210,75.51) (220,75.81) (230,75.90) (240,75.99) (250,76.12) (260,76.22) (270,76.29) (280,76.36) (290,76.47) (300,76.54) (310,76.62) (320,76.71) (330,76.75) (340,76.79) (350,76.88)
};
\addlegendentry{verified path (PJRT)}
\end{axis}
\end{tikzpicture}
...
\noindent One recipe, two paths to XLA, sampled every 10 epochs and scored
over all $50{,}000$ validation images. The verified curve's epoch-1
$20.62\%$ against the reference's $10.09\%$ is unexplained; the two are
level by epoch~10 and the difference never recurs.
...
Optimizer     & SGD + momentum $0.9$   & RMSProp (TF form)             & $\rho{=}0.9$, $\mu{=}0.9$, $\varepsilon{=}10^{-3}$ --- and the exact variant matters (below) \\
...
The two quietest rows carry the chapter's two debugging lessons, both
told in full above. ``RMSProp (TF form)'' is doing real work:
$\varepsilon$ inside the square root and a mean-square accumulator that
starts at $1$ are the difference between training stably at the paper's
$0.016$ and eroding top-1 over the run. And the EMA row only evaluates
correctly because the BN running statistics are shadowed alongside the
weights (\texttt{ema\_bn}); averaged weights against live BN statistics
is an eval blow-up, a failure mode specific to BatchNorm nets trained
...
inside an hour and a quarter:

\begin{verbatim}
lake exe cache get                            # Mathlib oleans, ~30 s
./download_imagenette.sh                      # 330 MB dl, 2.4 GB unpacked
lake build convnext-verified-adam
PJRT_FFI_RESIDENT=1 ./.lake/build/bin/convnext-verified-adam data
\end{verbatim}

On one RTX 4060 Ti (CUDA 12.9), from
\texttt{runs/2026-09-13-convnext-imagenette-ls1e-6-resident/}. XLA's
startup banner and the resident-parameter notices are removed, the network
description line is wrapped and its trailing LayerNorm note dropped, each
...
[pjrt_ffi] compiled verified_mlir/convnext_fwd.mlir
             (@convnext_fwd, 1 outputs, 1 replica) in 588 ms
ConvNeXt-T on Imagenette 224² (patchify /4 → stem channel-LN →
  ...
  via the VERIFIED renderer → XLA/PJRT → GPU
  xla/pjrt verified_mlir/convnext_adam_train_step.mlir
  xla/pjrt verified_mlir/convnext_fwd.mlir
  train 9469, val 3925; bs 32, ConvNeXt-T adam
    (cosine+warmup 3ep, baseLR 0.001000), He init
Epoch 1/80: loss=2.321469 lr=0.000333
  epoch 1: val_acc = 1106/3925 = 28.178344%  top5 = 2873/3925 = 73.197452%
    [95% CI 26.79–29.61]
Epoch 2/80: loss=2.000190 lr=0.000667
  epoch 2: val_acc = 1566/3925 = 39.898089%  top5 = 3378/3925 = 86.063694%
    [95% CI 38.38–41.44]
Epoch 3/80: loss=1.675131 lr=0.001000
  epoch 3: val_acc = 2057/3925 = 52.407643%  top5 = 3601/3925 = 91.745223%
    [95% CI 50.84–53.97]
Epoch 4/80: loss=1.506907 lr=0.001000
  epoch 4: val_acc = 2226/3925 = 56.713376%  top5 = 3640/3925 = 92.738854%
    [95% CI 55.16–58.26]
Epoch 79/80: loss=0.500732 lr=0.000000
  epoch 79: val_acc = 3230/3925 = 82.292994%  top5 = 3789/3925 = 96.535032%
    [95% CI 81.07–83.46]
Epoch 80/80: loss=0.500793 lr=0.000000
  epoch 80: val_acc = 3229/3925 = 82.267516%  top5 = 3788/3925 = 96.509554%
    [95% CI 81.04–83.43]
done (trained ConvNeXt-T adam + cosine/warmup via packed threading).
\end{verbatim}

Eighty epochs at about 55 seconds each, one hour and fourteen minutes in
total, and \textbf{82.27\%} top-1 with \textbf{96.51\%} top-5 on
Imagenette's 3,925-image validation split. The best epoch reached
$82.32\%$. That is the \emph{lowest} top-1 of the convnets in this book's
Imagenette column, from much the largest network among them, and
...
28.2, 39.9, 52.4, 56.7, smoothly. Chapter~\ref{chap:se} had to explain a
...
Three new operations this chapter, still three rules. What ran is
\texttt{pretty(provenGraph)}, the file
\texttt{verified\_mlir/\allowbreak convnext\_adam\_train\_step.mlir}, and
every LayerNorm in it backpropagates by
Theorem~\ref{thm:layerNorm_has_vjp}, every GELU by
...
forward the spec denotes. It assumes 23 positive LayerNorm epsilons and
nothing else. Unlike Chapter~\ref{chap:depthwise}, whose full-depth fold is
pointwise because relu6's kink makes a statement over all inputs false,
this one is a single global \texttt{HasVJP} over the entire network, and it
can be because GELU is smooth everywhere. So for ConvNeXt the accurate sentence is the strong one:
...
\begin{theorem}[LayerNorm VJP]
  ...
  \leanok
  \uses{def:hasvjp, thm:bn_has_vjp}
  \Assume{}
  \begin{enumerate}
    \item \(\varepsilon > 0\) \hfill[\texttt{h\(\varepsilon\)}]
  \end{enumerate}
...
\begin{theorem}[GELU VJP]
  ...
  \leanok
  \uses{ax:geluScalar, ax:geluScalarDeriv, ax:pdiv, ax:pdiv_gelu, def:hasvjp}
...
modernization items listed at the top of the chapter. The same base recipe
as Chapters~\ref{chap:residual}--\ref{chap:se} (AdamW at $10^{-3}$, cosine
with a three-epoch warmup, weight decay $10^{-4}$, label smoothing $0.1$)
keeps the cross-architecture comparison clean.
...
\textbf{And it is worth three points here, the wrong way.} Until
2026-09-13 the verified path initialised layer scale to ones, and the same
render on the same recipe landed at $85.45\%$ against the $82.27\%$
above, resolved at $\pm 1.2$ per run
(\texttt{runs/\allowbreak 2026-08-30-\allowbreak convnext-imagenette-headln/}). The two are level through
epoch 10 and three to four points apart from epoch 20 on ($80.87$ against
$77.43$ at 20, $84.18$ against $80.03$ at 40): with $\gamma$ at $10^{-6}$
every residual branch's gradient is scaled by $\gamma$, so the branches
train late, and 9,469 images over 80 epochs never make the time up. The
recipe this init belongs to is 300 epochs on 1.28M images, where a late
start costs nothing, and \S\ref{sec:convnext_imagenet} is that regime.
The book keeps the paper's value because it is the paper's and the JAX
reference's; the ones run is the better number on this recipe and not the
...
\S\ref{sec:convnext_runit} has the run: 80 epochs at about 55 seconds
each, one hour and fourteen minutes, $\mathbf{82.27\%}$ top-1 and
$\mathbf{96.51\%}$ top-5, with the loss settling on the same
...
MobileNetV2      &  2.24M  & 1{,}047\,KB &  90 ms   & 35 min   & 89.25\% & \textbf{98.68\%} \\
EfficientNet-B0  &  4.02M  & 1{,}316\,KB & 103 ms   & 41 min   & 89.96\% & 98.45\% \\
ConvNeXt-T       & \textbf{27.83M} & 1{,}019\,KB & 188 ms & 1.2 h & 82.27\% & 96.51\% \\
\hline
\end{tabular}
\end{center}

\noindent Columns as in Chapter~\ref{chap:se}'s table; ResNet-34's row is
the five-seed mean of \S\ref{sec:r34_ablation} with one seed's timing.

\begin{itemize}
  \item \textbf{The largest convnet in the column finishes last among
    them}, and by a clear margin: $82.27\%$ against $89.99 \pm 0.32$ for
    ResNet-34 and $89.96\%$ for EfficientNet-B0, from $1.3\times$ and
    $6.9\times$ their parameters. Three things contribute. ConvNeXt-T
    carries 27.83M parameters against 9,469 training images, which is not
    the data scale that capacity wants. The recipe here is the shared one,
    AdamW at $10^{-3}$ with cosine and label smoothing, which is
    deliberately \emph{not} the recipe ConvNeXt was designed around. And
    its layer scale starts at the paper's $10^{-6}$, which on this short
    schedule is worth three points on its own (above).
  \item \textbf{That is the chapter's thesis arriving as a result rather
    ...
    epochs, reaches $81.53\%$ on ImageNet-1k and beats every other
    network in this book. The architecture was never the variable.
  \item \textbf{It is the slowest per step}, at 188 ms against
    ResNet-34's 120 and EfficientNet-B0's 103. The depthwise $7 \times 7$
    and the channel LayerNorms touch every spatial position twice per
    block, and the $4\times$ expansion makes its $1 \times 1$s the largest
    matmuls in the column.
  \item \textbf{The MLIR is mid-pack at 1{,}019\,KB}, above ResNet-34's 729
...
example earlier in this chapter, and the recipe runs on both paths ---
the reference first, then the verified path, then the two side by side.
...
def convNeXtTinyImagenetConfig : TrainConfig where
  ...
  cnxInit           := true         -- ConvNeXt _init_weights: trunc_normal(0.02) on every conv
  cosineDecay       := true
...
remains, and the schedule is no longer short of it either. The run below is the full 300-epoch tier, trained and
evaluated on timm's antialiased bicubic resampler and validation
protocol.
...
GPU & Epochs & Per epoch & Wall-clock & Val top-1 & Val top-5 \\
\hline
4$\times$ 3060 & 300 & $15.3$ min & $76.5$ hr & $\mathbf{81.53\%}$ & $\mathbf{95.50\%}$ \\
\hline
\end{tabular}
\end{center}

\noindent Both figures are the exponential moving average of the
weights, scored over all $50{,}000$ validation images. The raw weights
score $81.51\%$ / $95.50\%$, so weight averaging is worth $+0.03$ points
here --- the cosine schedule anneals the learning rate to zero over the
final epochs, and weights that have stopped moving leave the average
nothing to remove.

The full 300-epoch tier (\texttt{convnext-tiny-imagenet full},
\texttt{convNeXtTinyImagenetConfigFull}) reached $\mathbf{81.53\%}$ top-1
and $\mathbf{95.50\%}$ top-5 on the held-out validation pass. That is the
strongest result in this book's sweep by a wide margin, ahead of
EfficientNet-B0 ($77.15\%$) and ResNet-34 ($74.16\%$), and it lands within
$\sim$0.6\% of ConvNeXt-T's $82.1\%$ paper headline from a from-scratch
Lean$\to$JAX-codegen reimplementation. Two candidate explanations for
that residual can be ruled out from this run: weight averaging is
measured above and accounts for none of it, and the evaluation crop is
already timm's $0.875$. What is left is augmentation and schedule
detail, not the architecture and not the eval protocol.
...
full 300-epoch schedule (bf16, 4$\times$ 3060), every point the EMA weights
scored over all $50{,}000$ validation images. Neither curve turns over:
both rise through the schedule and flatten across the final $\sim$40
epochs, where the cosine has annealed the learning rate to zero.
...
-- above was measured on. Batch is 64 PER DEVICE, the reference's
-- global 256 over four replicas.
...
27,827,818 to \textbf{28,589,128}. Three kernel \texttt{\#guard}s hold
...
\medskip\noindent\textbf{What it has not done is run.} No verified
ImageNet result exists for this network yet, which is why every accuracy
above comes from the reference. What is measured is the throughput, and the
...
\hline
4$\times$ 4060 Ti (CUDA)  & $206 \to 131$ & $34.4 \to 21.9$ & $\sim$175 $\to$ $\sim$112 h \;($7.3 \to 4.7$ d) & TBD \\
4$\times$ 3060 (CUDA)     & ---           & ---             & ---                                           & TBD \\
\hline
\end{tabular}
\end{center}
...
from 9 blocks to 27 and touches nothing else; \textbf{B} does that \emph{and}
...
ConvNeXt-S & $27$ blocks, $96$ wide & $50{,}222{,}152$ & $255.2 \to 105.3$\,ms \;($\mathbf{2.42\times}$) \\
ConvNeXt-B & $27$ blocks, $128$ wide& $88{,}589{,}416$ & $378.6 \to 158.0$\,ms \;($2.40\times$) \\
\hline
\end{tabular}
\end{center}

\noindent Every number in this section was measured in one session on
4$\times$ RTX 4060 Ti (16\,GB, CUDA 12.9). Counts are \texttt{\#guard}ed against
$50.22$\,M and $88.59$\,M.

\medskip\noindent \textbf{A full schedule}, four cards, $300$ epochs, totals
including \evalckptnote. Trainer steps, all measured on real ImageNet over $40$
steps:
...
\hline
ConvNeXt-T & $128$ & $10{,}009$ & $206 \to 131$ & $175 \to 112$\,h \;($7.3 \to 4.7$\,d) \\
ConvNeXt-S & $128$ & $10{,}009$ & $345 \to 192$ & $291 \to 163$\,h \;($12.1 \to 6.8$\,d) \\
ConvNeXt-B & $128$ & $10{,}009$ & $521 \to 301$ & $438 \to 254$\,h \;($18.2 \to 10.6$\,d) \\
\hline
\end{tabular}
\end{center}
...
The destination is Theorem~\ref{thm:vitTiny_has_vjp_correct} at the
end of the chapter. It says: the whole ViT-Tiny --- patch embedding,
twelve transformer blocks with their own parameters, final LayerNorm
and head --- has a backward that equals its Jacobian transpose at every
input, assuming only \(\varepsilon > 0\) at the LayerNorms. The proof
composes everything this chapter introduces, plus most of what the
previous chapters introduced, into a tower of \texttt{vjpMat\_comp}
applications with \texttt{vjp\_comp} at the joins. Read the proof's
dependency list and you see a tour of Part 1: dense, residual, layer
norm, GELU, softmax, matrix multiply, plus the sixteen matrix-machinery
...
PJRT_FFI_RESIDENT=1 ./.lake/build/bin/vit-verified-adam data
\end{verbatim}
...
Attention is where you would expect this to break. What ran is
\texttt{pretty(provenGraph)}, the file
\texttt{verified\_mlir/\allowbreak vit\_adam\_train\_step.mlir}, and every
softmax in it backpropagates by Theorem~\ref{thm:softmax_has_vjp}, every
scaled dot-product attention by
Theorems~\ref{thm:sdpa_back_Q_correct}--\ref{thm:sdpa_back_V_correct},
every LayerNorm by Chapter~\ref{chap:layernorm}'s, every GELU by that
chapter's, and every residual skip by Chapter~\ref{chap:residual}'s
additive fan-in.

\medskip\noindent\textbf{And the fold holds at every depth.}
\texttt{vitForwardKV\_has\_vjp} chains the patch embedding, $k$
transformer blocks with their own parameters, the final LayerNorm and
the head through \texttt{vjp\_comp} for every $k$, and
\texttt{vitTiny\_has\_vjp\_correct} is its instance at this network's
shape --- twelve blocks, $224/16$, ten classes --- carried back to the
forward the spec denotes. Its only hypothesis is $\varepsilon > 0$ at
the LayerNorms. It is a single global
...
\prosesection{Why this chapter has thirty-seven theorems}
...
(\textbf{Matrix-level machinery}, sixteen theorems). Every one of them
...
The second section (\textbf{Attention proofs}, twenty-one theorems)
...
backward}. Once we land Theorem~\ref{thm:vitTiny_has_vjp_correct}, the
...
\begin{theorem}[3D VJP chain rule]
  ...
  \leanok
  \uses{ax:pdiv, def:hasvjp3, thm:pdiv3_comp}
...
\begin{theorem}[3D additive fan-in]
  ...
  \leanok
  \uses{ax:pdiv, ax:pdiv_add, def:hasvjp3}
...
\begin{theorem}[Row-wise softmax VJP on a matrix]
  ...
  \leanok
  \uses{ax:pdiv, def:hasvjp, def:hasvjpmat, thm:rowwise_has_vjp_mat,
      thm:softmax_has_vjp}
...
\begin{theorem}[SDPA backward wrt Q]
  ...
  \leanok
  \uses{ax:pdiv, ax:rowSoftmax_flat_diff, def:hasvjpmat, thm:matmul_right_const_has_vjp,
      thm:rowSoftmax_has_vjp_mat, thm:scalarScale_has_vjp, thm:vjpMat_comp}
...
\begin{theorem}[SDPA backward wrt K]
  ...
  \leanok
  \uses{ax:pdiv, ax:pdiv_comp, ax:pdiv_softmax, ax:rowSoftmax_flat_diff, def:hasvjp,
      def:hasvjpmat, thm:matmul_left_const_has_vjp, thm:matmul_right_const_has_vjp,
      thm:pdivMat_matmul_left_const, thm:pdivMat_matmul_right_const,
      thm:pdivMat_scalarScale, thm:pdivMat_transpose, thm:rowSoftmax_has_vjp_mat,
      thm:rowwise_has_vjp_mat, thm:scalarScale_has_vjp, thm:softmax_has_vjp,
      thm:transpose_has_vjp, thm:vjpMat_comp}
...
\begin{theorem}[SDPA backward wrt V]
  ...
  \leanok
  \uses{ax:pdiv, def:hasvjpmat, thm:matmul_left_const_has_vjp}
...
\begin{theorem}[Multi-head SDPA VJP]
  ...
  \leanok
  \uses{ax:pdiv, ax:rowSoftmax_flat_diff, def:hasvjpmat, thm:dense_has_vjp,
      thm:rowwise_has_vjp_mat, thm:sdpa_back_K_correct, thm:sdpa_back_Q_correct,
      thm:sdpa_back_V_correct, thm:vjpMat_comp}
...
\begin{theorem}[Per-token LayerNorm lifted to a matrix]
  ...
  \leanok
  \uses{ax:bnIstdBroadcast_diff, def:hasvjpmat, thm:layerNorm_has_vjp,
      thm:rowwise_has_vjp_mat}
\end{theorem}
...
\begin{theorem}[Per-token dense lifted to a matrix]
  ...
  \leanok
  \uses{def:hasvjpmat, thm:dense_has_vjp, thm:rowwise_has_vjp_mat}
\end{theorem}
...
\begin{theorem}[Per-token GELU lifted to a matrix]
  ...
  \leanok
  \uses{ax:geluScalar, def:hasvjpmat, thm:gelu_has_vjp, thm:rowwise_has_vjp_mat}
\end{theorem}
...
\begin{theorem}[Transformer MLP sublayer VJP]
  ...
  \leanok
  \uses{ax:geluScalar, def:hasvjpmat, thm:dense_per_token_has_vjp_mat,
      thm:gelu_per_token_has_vjp_mat, thm:vjpMat_comp}
  \(\HasVJPMat\) of
...
\begin{theorem}[Transformer attention sublayer with residual VJP]
  ...
  \leanok
  \uses{ax:bnIstdBroadcast_diff, ax:mhsa_has_vjp_mat, ax:mhsa_layer_flat_diff,
      def:hasvjpmat, thm:biPathMat_has_vjp, thm:identityMat_has_vjp,
      thm:layerNorm_per_token_has_vjp_mat, thm:vjpMat_comp}
...
\begin{theorem}[Transformer MLP sublayer with residual VJP]
  ...
  \leanok
  \uses{ax:bnIstdBroadcast_diff, ax:geluScalar, def:hasvjpmat, thm:biPathMat_has_vjp,
      thm:identityMat_has_vjp, thm:layerNorm_per_token_has_vjp_mat,
      thm:transformerMlp_has_vjp_mat, thm:vjpMat_comp}
  \Assume{} \(\varepsilon > 0\) \hfill[\texttt{h\(\varepsilon\)}]\\
  \Prove{} \(\HasVJPMat\) of
...
\begin{theorem}[Transformer block VJP]
  ...
  \leanok
  \uses{ax:bnIstdBroadcast_diff, ax:geluScalar, ax:mhsa_layer_flat_diff, def:hasvjpmat,
      thm:transformerAttnSublayer_has_vjp_mat, thm:transformerMlpSublayer_has_vjp_mat,
      thm:vjpMat_comp}
...
\begin{theorem}[Transformer tower, any depth]
  ...
  \leanok
  \uses{ax:bnIstdBroadcast_diff, ax:geluScalar, ax:mhsa_layer_flat_diff, def:hasvjpmat,
      thm:identityMat_has_vjp, thm:transformerBlock_has_vjp_mat, thm:vjpMat_comp}
...
\begin{theorem}[ViT body: the grand finale]
  ...
  \leanok
  \uses{ax:bnIstdBroadcast_diff, ax:geluScalar, ax:mhsa_layer_flat_diff, def:hasvjpmat,
      thm:layerNorm_per_token_has_vjp_mat, thm:transformerTower_has_vjp_mat,
      thm:vjpMat_comp}
...
Chapter~\ref{chap:tensor} is a proved theorem; one link remains, the
network itself. \Qed{}
\end{proof}

\begin{theorem}[ViT-Tiny, the whole network]
  \label{thm:vitTiny_has_vjp_correct}
  \lean{Proofs.vitTiny_has_vjp_correct}
  \leanok
  \uses{ax:bnIstdBroadcast_diff, ax:geluScalar, ax:mhsa_has_vjp_mat,
      ax:mhsa_layer_flat_diff, ax:patchEmbed_flat_diff, ax:patchEmbed_flat_has_vjp,
      ax:pdiv, ax:pdiv_reindex, def:hasvjp, def:hasvjpmat, thm:biPathMat_has_vjp,
      thm:dense_has_vjp, thm:identityMat_has_vjp, thm:identity_has_vjp,
      thm:layerNorm_has_vjp, thm:rowwise_has_vjp_mat, thm:transformerMlp_has_vjp_mat,
      thm:vjpMat_comp, thm:vjp_comp}
  \Assume{}
  \begin{enumerate}
    \item \(\varepsilon > 0\) \hfill[\texttt{h\(\varepsilon\)}]
  \end{enumerate}
  \Prove{} the backward of \texttt{vitForwardKV\_has\_vjp} at ViT-Tiny's
  shape --- a \(3 \times 224 \times 224\) image, \(16 \times 16\) patches
  (\(N = 196\) plus the CLS token), \(d = 192 = 3 \times 64\), MLP
  \(768\), \emph{twelve} blocks with distinct parameters, ten classes ---
  equals the \(\pdiv\)-contracted Jacobian of the flat forward
  \texttt{vitForwardKV} at every input, cotangent and index. The
  statement is generic in the weights.
\end{theorem}

\begin{proof}
\leanok
\begin{enumerate}
  \item \texttt{vitForwardKV\_has\_vjp}: for every depth \(k\), the flat
    ...
    (Theorem~\ref{ax:patchEmbed_flat_has_vjp}), the tower by induction
    on \(k\) with the block (Theorem~\ref{thm:transformerBlock_has_vjp_mat})
    bridged to the flat vector as the step, the per-token final LayerNorm
    (Theorem~\ref{thm:layerNorm_per_token_has_vjp_mat}, bridged the same
    way), and the classifier (Theorem~\ref{thm:dense_has_vjp} on the CLS
    row).
    \pf{Three applications of \texttt{vjp\_comp}
    (Theorem~\ref{thm:vjp_comp}); every factor is smooth, so the
    differentiability side conditions need only assumption 1.}
  \item \texttt{vitForwardKV\_has\_vjp\_correct}: the composed
    backward's \texttt{correct} field, read out at any depth.
  \item \Qed{}
    \pf{Instantiate 2 at \(k = 12\) and the dimensions above.}
\end{enumerate}
\end{proof}
...
EfficientNet-B0  &  4.02M  & 1{,}316\,KB & 103 ms   & 41 min   & 89.96\% & 98.45\% \\
ConvNeXt-T       & 27.83M  & 1{,}019\,KB & 188 ms   & 1.2 h    & 82.27\% & 96.51\% \\
...
network and the lowest-accuracy by more than thirteen points. That gap
...
the same architecture on 1.28M images reaches $72.31\%$ over a
...
(Theorems~\ref{thm:sdpa_back_Q_correct}--\ref{thm:sdpa_back_V_correct},
\texttt{sdpa\_back\_Q/K/V}), all built on the softmax backward, which
couples every position to every other (Theorem~\ref{thm:softmax_has_vjp}). Because $Q$, $K$, and $V$ are three linear
...
residual) through \texttt{vjpMat\_comp}.
...
on both paths --- the reference first, then the verified path, then the
two side by side --- and this is the chapter where the pair is cleanest,
because a transformer has no BatchNorm to normalize differently.

\prosesection{The reference (JAX)}

The reference trainer is \texttt{jax/MainVitImagenet.lean}, quoted as it
stands today:
...
--    reference driver; the verified path's entry point is trainAdamSched,
--    in the next section.
...
\noindent The two recipes differ in exactly one field, and the trainers
they generate differ in exactly one function: \texttt{init\_params}, 59
initialization sites, with the schedule, augmentation, EMA and
evaluation identical byte-for-byte.

\textbf{Gradient clipping is not optional.} At the DeiT learning rate
(peak $5\times 10^{-4}$ for batch 512) the model collapses to chance the
moment warmup ramps past ${\sim}1.6\times 10^{-4}$, train loss pinned at
$\ln(1000)\approx 6.9$ and never recovering. \texttt{gradClipNorm := 1.0}
is the whole fix; the same LR then trains through warmup to convergence.
...
LayerNorm/softmax that bf16 leaves alone.
...
GPU & Precision & Per epoch & Epochs & Total & Val top-1 & Val top-5 \\
\hline
4$\times$ 3060 (CUDA) & bf16 & 6.8 min & 300 & 34.2 hr & $\mathbf{72.31\%}$ & $\mathbf{91.12\%}$ \\
\hline
\end{tabular}
\end{center}

\noindent Global batch 512 as $4 \times 128$ replicas, the EMA weights
scored over all $50{,}000$ validation images; one attempt, no restarts.
DeiT-Ti without distillation publishes $72.2\%$ / $91.1\%$, so this is
$+0.11$ and $+0.02$, and at $n = 50{,}000$ the Wilson 95\% interval on
this run is $[71.92, 72.70]$: on the paper, not past it in any meaningful
sense.
...
\begin{axis}[
    ...
    every axis plot/.append style={line width=1pt, mark size=0.8pt},
    title style={font=\small},
    title={DeiT-Ti recipe, 300 epochs --- reference},
]
\addplot[blue, mark=*, mark options={fill=blue}] coordinates {
...
\noindent The run, sampled every 5 epochs. The slow start is Mixup and
CutMix blending the labels, compounded by the EMA shadow lagging the live
weights; after that it climbs to the end of the schedule without an
overfitting turn.

\textbf{Where this still differs from the paper.} The number matches.
The recipe does not yet match exactly, and the honest list is short but
not empty.

\emph{What matches.} Architecture (patch16, $d{=}192$, 12 blocks, 3
heads, 5{,}717{,}416 parameters); schedule (300 epochs, 5-epoch warmup,
cosine to zero); optimizer (AdamW, weight decay $0.05$, no decay on
norms, biases, CLS or positional embedding); loss (cross-entropy with
label smoothing $0.1$); the augmentation stack (Mixup $0.8$, CutMix
$1.0$, \texttt{rand-m9-mstd0.5-inc1} with the SolarizeAdd$+$Invert
op-set and per-op apply probability $0.5$, Random-Erase $p{=}0.25$,
repeated augmentation $3\times$); stochastic depth $0.1$; and model EMA
at $0.99996$. Weight initialization now matches as well --- every
transformer \texttt{Linear} gets $\mathrm{trunc\_normal}(\sigma{=}0.02)$
and the patch-embed convolution PyTorch's \texttt{Conv2d} default ---
which is what \texttt{vitInit} in the listing above selects. It is worth
saying why that one mattered: Xavier-uniform scales as $1/\sqrt{d}$
against timm's \emph{fixed} $0.02$, so the generic path was
$1.8\times$ too wide at ViT-B, $2.6\times$ at ViT-S and $3.6\times$ at
ViT-Ti, while the patch embed came out ${\sim}6\times$ too \emph{narrow}
(it divided by the output fan $d\cdot p^2$ rather than the input fan
$c\cdot p^2$). The smallest model was the worst case.

\emph{What does not match.} Four things, in rough order of how much
they could matter:

\begin{enumerate}
\item \textbf{Batch $512$, not DeiT's $1024$.} The learning rate is
  scaled consistently for it ($5\times 10^{-4}$ at $512$ is DeiT's own
  $\mathrm{lr} = 5\times10^{-4}\cdot B/512$), so this is a
  smaller-batch run of the same recipe rather than a mis-tuned one ---
  but it is not the paper's batch, and repeated augmentation interacts
  with batch size by construction.
\item \textbf{Gradient clipping at global norm $1.0$ is ours.} DeiT does
  not clip: its \texttt{main.py} defaults \texttt{--clip-grad} to
  \texttt{None}. Ours went in because without it the model collapsed to
  chance the moment warmup ramped past ${\sim}1.6\times 10^{-4}$ --- but
  that was observed on the Xavier initialization, before \texttt{vitInit}
  narrowed the weights $3.6\times$, and whether the collapse survives the
  init fix is untested. Until it is, the clip stays and is an addition.
\item \textbf{RandomResizedCrop and Random-Erasing are a from-scratch
  \texttt{tf.data} reimplementation}, not timm's code, so they agree by
  specification rather than bit-for-bit. Random-Erase fills with zero
  where timm fills with noise.
\item \textbf{Geometric RandAugment interpolates bilinearly}, where
  timm resolves bicubic from the model's data config. Measured, this is a
  bound rather than a gap: on a $21^\circ$ rotation our bilinear warp sits
  $0.56$ / $2.53$ of $255$ from PIL's bicubic on a smooth / noisy image,
  and PIL's own bilinear sits $0.39$ / $2.41$ from it, so we are as close
  as a bilinear implementation can be. TensorFlow's own \texttt{BICUBIC}
  is a third resampler, ${\sim}30$ from both, so asking for it would make
  this worse, not better.
\end{enumerate}

\noindent None of these is a bug, and the loss curve is textbook DeiT. All
four are properties of the recipe and of the shim rather than of this
trainer, and the verified path inherits every one of them: it runs the
same $4\times128$, clips at the same $1.0$ inside the render, and streams
the same \texttt{tf.data} pipeline. But ``matches the paper's number'' and
``matches the paper'' are different claims, and only the first is
established here, on either path.

The further $+2.3$ points to DeiT-Ti's $74.5\%$ headline are a different
thing entirely, the \emph{distillation} variant (a second
``distillation token'' trained against a strong CNN teacher), which we
do not implement.
...
def vitImagenetConfig : VerifiedConfig where
  epochs    := 300
  batchSize := 128
  vitInit   := true  -- timm/DeiT init, as the reference's `deit-init` recipe above
\end{verbatim}
...
here they compose.} Weight-decay exclusion, grad clipping and stochastic
depth are spelled \texttt{wx}, \texttt{clip} and \texttt{drop}, EMA is a
variant too, and \texttt{vitin\_\allowbreak emadp128x4\allowbreak wxclipdropbf16} is the render
that carries all four at once on top of AdamW and the four-way
all-reduce. It is the artifact the run used, and it is what makes the
pair below one-variable in recipe. Mixup, CutMix and repeated
augmentation never enter the graph at all, because they are data-side
and ride the shim. It runs with:
...
GPU & Precision & Per epoch & Epochs & Total & Val top-1 & Val top-5 \\
\hline
4$\times$ 3060 (CUDA) & bf16 & 9.6 min & 300 & 48.0 hr & $\mathbf{72.35\%}$ & $\mathbf{91.22\%}$ \\
\hline
\end{tabular}
\end{center}

\noindent Global batch 512 as $4 \times 128$ replicas, the EMA weights
scored over the same $50{,}000$ images.
...
Python in the step   & yes                      & none \\
ms/step              & 161                      & 199 \\
per epoch            & 6.8 min                  & 9.6 min \\
total                & 34.2 hr                  & 48.0 hr \\
\hline
precision            & bf16                     & bf16 \\
global batch         & 512 ($4\times128$)       & 512 ($4\times128$) \\
steps/epoch          & 2{,}502                  & 2{,}502 \\
EMA (scored)         & 0.99996                  & 0.99996 \\
\hline
\textbf{val top-1}   & $\mathbf{72.31\%}$       & $\mathbf{72.35\%}$ \\
\textbf{val top-5}   & $\mathbf{91.12\%}$       & $\mathbf{91.22\%}$ \\
\hline
\end{tabular}
\end{center}

\noindent Both columns are ViT-Tiny on the DeiT-Ti recipe, 300 epochs, on
the same four RTX 3060 cards, scored over the same $50{,}000$ validation
images. Rows above the first rule differ; rows between the rules are held
equal by construction.

\medskip\noindent\textbf{The two paths agree to $0.04$ points on top-1 and
$0.10$ on top-5}, inside the floor \S\ref{sec:r34_pjrt} set ($\pm0.39$ at
$72\%$ on $50{,}000$ images), at one seed each. Nothing sits above the
rule but the lowering itself and what it costs: precision is equal, the
batch is the reference's own $512$, and there is no BatchNorm to
normalize over a different group. This is the one-variable pair in the
book.

\medskip\noindent\textbf{The curves converge; they do not merely end together.}
The verified path runs half a point \emph{behind} through warmup, is level by
epoch~100, and stays within a fifth of a point for the remaining two hundred:

\begin{center}
\begin{tabular}{@{}l r r r r r r r@{}}
\hline
epoch & 5 & 50 & 100 & 150 & 200 & 250 & 300 \\
\hline
reference (JAX)       & 14.07 & 57.36 & 63.62 & 66.99 & 69.40 & 71.57 & 72.31 \\
verified path (PJRT)  & 14.32 & 56.86 & 63.63 & 66.86 & 69.31 & 71.36 & 72.35 \\
$\Delta$              & $+0.25$ & $-0.50$ & $+0.01$ & $-0.13$ & $-0.09$ & $-0.21$ & $+0.04$ \\
\hline
\end{tabular}
\end{center}
...
\addlegendentry{reference (JAX)}
\addplot[red, dashed, mark=o] coordinates {
(5,14.32) (10,29.97) (15,38.93) (20,44.20) (25,47.74) (30,50.38) (35,52.54) (40,54.36) (45,55.75) (50,56.86) (55,57.77) (60,58.72) (65,59.61) (70,60.36) (75,61.00) (80,61.65) (85,62.22) (90,62.68) (95,63.19) (100,63.63) (105,63.94) (110,64.23) (115,64.72) (120,65.03) (125,65.37) (130,65.62) (135,65.90) (140,66.22) (145,66.51) (150,66.86) (155,67.07) (160,67.35) (165,67.60) (170,67.87) (175,68.20) (180,68.39) (185,68.61) (190,68.90) (195,69.12) (200,69.31) (205,69.52) (210,69.79) (215,69.99) (220,70.12) (225,70.42) (230,70.63) (235,70.79) (240,71.08) (245,71.23) (250,71.36) (255,71.51) (260,71.67) (265,71.81) (270,71.96) (275,72.07) (280,72.17) (285,72.26) (290,72.31) (295,72.32) (300,72.35)
};
\addlegendentry{verified path (PJRT)}
\end{axis}
\end{tikzpicture}
...
\noindent One recipe, two paths to XLA, sampled every 5 epochs and scored
over all $50{,}000$ validation images.

\medskip\noindent\textbf{The remaining $48.0$ against $34.2$ hours is the data
feed, not the renderer.} With the shim removed the verified step is
$153$\,ms --- a $38.5$-hour schedule, within $13\%$ of the reference --- and
the fed step is $199$. The gap is the tfds shim streaming $602$\,KB per image
of fp32 through a subprocess pipe, $308$\,MB per step at global $512$, which
is host work the JAX trainer does in-process. Two knobs close most of it and
both are throughput-only: producer count, and \texttt{tf.data} op determinism,
which serializes the augmentation map and costs $5.3\times$ on the producers.
Turning it off took this run from $69$ to $48$ hours; it moves the order of
random draws and nothing else, and the reference has never paid it either.
The same accounting holds for every verified ImageNet run in this book,
and the convnets' gaps are read against it.

\medskip\noindent A caution on reading the two side by side: the \emph{training}
loss is not the metric to compare. It runs ${\sim}0.47$ apart at epoch 50 where
top-1 agrees to a tenth of a point, because label-smoothed cross-entropy sums
$\log p$ over all $1{,}000$ classes and is hypersensitive to the
small-probability tail, which top-1 cannot see. Held at fixed accuracy, varying
only that tail moves the reported loss by $0.6$. The validation accuracies are
the comparison; the loss curves are not.
...
MLP at $4D$ throughout. Depth never moves.
...
them, at four replicas because S and B have no single-device render at all. All
three run $128$ per device, so four cards is DeiT's own global $512$ and the
recipe's $5\times10^{-4}$ is the rate that batch was set for --- which took four
\texttt{\#eval} lines rather than a renderer feature, because the batch was
never the obstacle.
...
verified, 4 cards & global & steps/epoch & ms/step & 300 epochs \\
\hline
ViT-Ti & $512$ & $2{,}502$ & $224 \to 153$   & $50 \to 35$\,h \;($2.1 \to 1.5$\,d) \\
ViT-S  & $512$ & $2{,}502$ & $507 \to 296$   & $109 \to 65$\,h \;($4.5 \to 2.7$\,d) \\
ViT-B  & $512$ & $2{,}502$ & $1369 \to 799$  & $289 \to 170$\,h \;($12.0 \to 7.1$\,d) \\
\hline
\end{tabular}
\end{center}

\noindent\textbf{Read these as compute floors, not as schedules.} A $40$-step
window drains a \texttt{SHIM PREFETCH} queue that the producers filled during
compile, so it times the graph with the data feed all but removed.
\S\ref{sec:vit_phase4} is the check: ViT-Ti's bf16 figure here is $153$\,ms/step
and $35$ hours, and the completed run measured $153$\,ms/step with the shim
removed but $199$ fed, for $48.0$ hours. The ratio is not a constant --- it is
whatever the host pipeline costs for that recipe --- so ViT-S and ViT-B should
be expected to run over these figures by a margin nobody has measured.
...
\opchiprow{cBr}{\PB}{-7.1}{bottleneck, denseBlock, cspBlock, fpn, unetDown, unetUp}
...
Twenty-four \texttt{Layer} constructors exist only for the bestiary. The two
plain ones, \texttt{.separableConv} (Xception, QANet) and
\texttt{.fireModule} (SqueezeNet), are described in their entries; the
twenty-two bundled ones are defined here. Two of these are codegen-backed,
\texttt{.unetDown} and \texttt{.unetUp}, which the segmentation and diffusion
demos train through; the rest emit \texttt{// UNSUPPORTED}, and their goal
is pedagogical shape and parameter accounting, not training runs.
...
new primitives: the two paths are two \texttt{NetSpec}s below and the
...
to ${\sim}79.8\%$ top-1. The JAX reference trainer
...
\subsection{Object detection --- demo: R34+FPN on VisDrone}
...
focal objectness, focal class CE; $448$\,px input, 30 epochs, photometric
and box-aware scale augmentation. VisDrone is the harder half of detection:
drone-altitude imagery where a median image holds \emph{seventy} objects and
many are $2$--$5$\,px after the resize --- the regime where a single coarse
grid structurally cannot emit the objects, and what makes the multi-scale head
of Definition~\ref{layer:fpnModule} necessary rather than decorative. Scored
against VisDrone's \emph{uncapped} validation ground truth --- all $38{,}759$
boxes, not the 56-box-truncated training record, which drops $34.9\%$ of them
and is not the benchmark's protocol. The catalog entry below keeps the
paper-exact YOLOv1 \emph{shape} for the historical record; this is the trained
detector beside it.
...
\begin{tabular}{l c c c}
\hline
 & mAP@0.5 & recall & class-agnostic AP \\
\hline
\textbf{R34+FPN, 30\,ep, +scale aug} & \textbf{0.2363} & \textbf{0.769} & \textbf{0.487} \\
R34+FPN, 12\,ep, +photometric aug & 0.1961 & 0.749 & 0.441 \\
R34+FPN, 50\,ep, +photometric aug & 0.1674 & 0.703 & 0.429 \\
R34+FPN, 12\,ep, no aug & 0.1526 & 0.682 & 0.393 \\
R34+FPN, 50\,ep, no aug & 0.1243 & 0.669 & 0.369 \\
PyTorch replica, same architecture & 0.1532 & 0.677 & 0.400 \\
...
above, the R34+FPN's boxes below, at the top row of the table (30 epochs, scale
augmentation, mAP@0.5 $0.2363$). Box colour is the class, keyed along the bottom
edge, and each frame's box count sits in its corner. The night market and the
street market are where the misses live --- the small and the rare, exactly what
the per-class spread below predicts.}
\label{fig:visdrone_fpn}
\end{figure}

\noindent Figure~\ref{fig:visdrone_fpn} is the top row on four validation
frames. \textbf{Augmentation and schedule length are one decision, not two,
and the top five rows only mean something together.} Quadrupling the schedule
with augmentation off \emph{costs} $19\%$ --- ordinary overfitting on $6{,}471$
images. Photometric augmentation recovers it, but at that strength 12 epochs
still beats 50. Augmentation that changes object \emph{scale} inverts the
ordering again: it makes 30 epochs worth $44\%$ more than 12. A stronger
augmentation needs a longer schedule to absorb, so \textbf{the optimum epoch
count is a property of the augmentation pack} --- there is no schedule that can
be tuned once and carried across packs.

\medskip\noindent \textbf{The result worth reading is not the mean.} Per-class
AP runs from car down to bicycle, a spread of $19\times$, and scale augmentation
narrowed it from $43\times$ by lifting exactly the classes that were worst:
awning-tricycle by $59\%$, bicycle by $41\%$, tricycle by $40\%$, against car's
$7\%$. That the fix was data rather than objective is the substantive point.
...
\emph{and} rare, and an averaged mAP is precisely the statistic that hides it.

\medskip\noindent The stack beats the hand-written PyTorch replica of
\emph{this same architecture} by $54\%$, and runs at 65\,fps on one RTX 4060 Ti
--- or $55.9$\,fps on a 25\,W Jetson Orin Nano under TensorRT, with a
uint8 input path and the clocks pinned, which is the deployment this dataset
implies. The replica, not YOLOv8, is the yardstick: it
...
to the field is recipe, not architecture}, and one item of that recipe closes
$31\%$ of it.
...
\subsection{Industrial inspection --- demo: the same detector on NEU-DET steel defects}

The detector of the previous section, unchanged --- backbone, neck, heads,
loss, ImageNet bootstrap, scorer --- on the dataset the industrial-inspection
literature reports on. NEU-DET (Song \& Yan 2013; boxes He et al.\ 2020) is
...
single $14\times14$ grid should \emph{not} collapse, and running both heads on
...
a fixed seed. Epochs are chosen on the validation $360$ and the numbers below
are the test $360$ at that epoch, scored by the VisDrone protocol against
uncapped ground truth. The single-grid arm is the same ResNet-34 and the same
...
\begin{tabular}{l c c c}
\hline
NEU-DET test & mAP@0.5 & recall & class-agnostic AP \\
\hline
\textbf{R34+FPN, the VisDrone recipe} (e28) & \textbf{0.623} & 0.992 & 0.625 \\
R34+FPN, photometric augmentation off (e30) & 0.630 & 0.987 & 0.640 \\
R34+FPN, no ImageNet bootstrap (e26) & 0.555 & 0.986 & 0.555 \\
\textbf{R34, single $14\times14$ grid} (e28) & \textbf{0.607} & 0.917 & 0.609 \\
...
\hline
\textbf{R34+FPN}, 30\,ep, +scale aug & \textbf{0.2363} & 0.769 & 0.487 \\
\textbf{R34, single $14\times14$ grid}, 12\,ep & \textbf{0.0391} & 0.184 & 0.107 \\
\hline
\end{tabular}
\end{center}
...
\noindent \textbf{The second table is the result.} On steel the single grid
matches the multi-scale head, $0.607$ against $0.623$; on drones it is six
times behind, $0.039$ against $0.236$, with recall $0.18$ against $0.77$ ---
same backbone, same bootstrap, same scorer, same lowerer in all four cells.
The FPN pays for itself only where objects are small; on NEU-DET it runs three
heads for one level's worth of work, and the head that collapses on VisDrone
is not wrong, it is wrong \emph{for drones}. Recall is not the limit on steel:
the FPN finds $99\%$ of defects and the mean is held down by two classes,
crazing at $0.29$ and rolled-in scale at $0.41$ against $0.66$--$0.84$ for the
other four --- the diffuse textures whose ``box'' is most of the crop, the
worst two in every published NEU-DET table and the worst two for both heads
here. The ImageNet prefix is worth $+0.07$ at $1{,}080$ training images, a gap
that has not closed by epoch 30. Photometric augmentation is within the noise
floor the VisDrone table established ($+0.007$, $n=1$).
...
steps against VisDrone's $24{,}000$, and both arms are still rising at epoch
30. The anchor priors are a second, smaller part of it: three per level fit
...
\subsection{People watching --- demo: Arabic sign-language letters on ArASL}
...
$95.7\%$ of those neighbours carry the test image's own label; under
...
gravitational-wave demo's, unchanged: the ordinary train step with integer
...
five minutes on one card. The chapter's linear and MLP rungs run through
...
the validation tenth and the numbers below are the test tenth at that
epoch, three seeds for the chapter net, with a $95\%$ Wilson interval on
...
\subsection{Agriculture --- demo: lab leaves to field leaves on PlantVillage and PlantDoc}
...
--- in 28 classes that all map onto PlantVillage's. The net here is
...
is never seen in training except by the last two rows of the table, which
...
PlantVillage ships with every image.
...
the same leaves on a black background & \textbf{60} & \textbf{96} \\
...
Shapley value: images where the background \emph{helps} & 88\% & 71\% \\
...
the leaf, $85\%$ of it. The counterfactuals say otherwise. Put the same leaf
on black and the net loses forty points; show it the background with the
...
\emph{helps} in nine images out of ten, a little each time. A map of where
...
found, and repairs it: the leaves on black go from $60\%$ to $96\%$, the
...
seed noise. Field labels are what work: ten per class buy seven points, all
...
\begin{figure}[H]
\centering
\includegraphics[width=0.62\textwidth]{demos/brats_r34_unet.png}
...
\begin{itemize}
  \item \textbf{The label is a tensor, not a scalar.} Classifiers emit
    ...
    backward is the same one Ch~\ref{chap:tensor} derived, fired $HW$
    times in parallel.
  \item \textbf{The network has to give spatial resolution back.}
    ...
    only architectural piece this section's backbones add to the Part~1
    kit.
...
Diffusion, in the diffusion subsection below, \S\ref{sec:bestiary_diffusion}),
and the same two primitives
...
\subsection{Reinforcement learning --- demo: DQN on Blackjack}
\label{sec:bestiary_rl}

Networks that learn from the rewards of their own actions rather than from a
labelled set. The architectural side is small --- an MLP here, residual CNN
blocks in the AlphaGo family below --- and the complexity lives in the outer
loop: how experience is gathered, what the target is, and how you would ever
know the policy is good.

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{demos/blackjack_chart.png}
\caption{The exact hit/stick policy for blackjack under the Sutton \& Barto
rules, player total against the dealer's showing card, hard hands and soft.
Rings mark the twelve cells where the trained DQN disagrees with the theorem,
dots the eight where tabular Q does. Both learners miss on the knife edges:
hard 12 against a 4, where hitting and sticking differ by a quarter of a cent
a hand, and the soft-18 row, where the casino card is wrong too.}
\label{fig:blackjack_chart}
\end{figure}

\paragraph{DQN on blackjack.} The \texttt{demos/MainBlackjackDqn.lean} trainer
is Mnih et al.'s DQN (2015,
\href{https://www.nature.com/articles/nature14236}{Nature}) on the blackjack of
Sutton \& Barto's Example~5.1, the game written in Lean
(\texttt{LeanMlir/Blackjack.lean}) with its RNG threaded through so every hand
replays from a seed. The Q-function is a three-layer \texttt{.dense} net on a
29-float one-hot of the observation (player total, dealer's card, usable ace),
$6{,}210$ parameters. Three things change relative to everything trained so far:

\begin{itemize}
  \item \textbf{There is no dataset.} Transitions come from playing the game
    $\varepsilon$-greedily against the current network and are kept in a
    replay ring of 20k; each gradient step samples 128 of them. The book's
    other demos read their data from disk; this one manufactures it.
  \item \textbf{The target is the model's own prediction, one step later.}
    $y_a = r + \gamma \max_{a'} Q_{\text{target}}(s', a')$, with the target
    network a copy of the online parameters refreshed every 500 updates. The
    loss is the squared-error block the DDPM demo trains with
    (\S~\ref{sec:bestiary_diffusion}): the host runs the eval graph on the
    batch, writes $y = Q(s,\cdot)$, overwrites the taken action's slot with the
    Bellman target, and hands $y$ in. The gradient is then zero on the untaken
    slot and a per-sample TD error on the taken one. Zero new codegen.
  \item \textbf{The instrument is a solved game.} Blackjack under these rules is
    a 200-state, two-action MDP with known odds, so value iteration gives the
    optimal policy and the \emph{exact} mean reward of any policy, in
    milliseconds. Every thousand updates the greedy policy is read off the
    network over all 280 observations in one batched forward and scored
    exactly. The ceiling of the training curve is a theorem about the rules,
    not a number somebody measured.
\end{itemize}

\paragraph{Every arm scored against the theorem.} Mean reward per hand; exact
by value iteration, Monte Carlo over $10^6$ hands with its standard error,
and agreement with the optimum over the 200 decision states. The two value
columns agree within two standard errors on every row, which is the check that
the simulator and the DP describe the same game:

\begin{center}
\begin{tabular}{@{}l r r r@{}}
\hline
policy & exact & Monte Carlo, $10^6$ hands & agrees / 200 \\
\hline
random                                   & $-0.3942$ & $-0.3956 \pm 0.0009$ & 110 \\
threshold heuristic (hit below 18, $p=0.8$) & $-0.2403$ & $-0.2395 \pm 0.0009$ & 164 \\
published casino table                   & $-0.0967$ & $-0.0962 \pm 0.0010$ & 162 \\
tabular Q, $10^6$ hands                  & $-0.0440$ & $-0.0425 \pm 0.0010$ & 192 \\
DQN, Double, 200k updates                & $-0.0476$ & $-0.0471 \pm 0.0010$ & 188 \\
exact optimum                            & $-0.0431$ & $-0.0419 \pm 0.0010$ & 200 \\
\hline
\end{tabular}
\end{center}

\noindent The published table is the strategy card the first book shipped as
its ceiling. It sits four cents a hand below the optimum
because it is for a different game --- doubling, splitting and a dealer who
plays differently --- and it ignores the usable ace entirely. Tabular Q needs
a visit-count step size to get past $177/200$; a constant one chases the
moving bootstrap target forever. Figure~\ref{fig:blackjack_chart} is the
policy itself, cell by cell.


\paragraph{The DQN's own table.} One seed per arm, exact value at the end of
training; the gate is the DQN within $0.01$ of tabular Q's exact value, which
every 200k-update arm passes:

\begin{center}
\begin{tabular}{@{}l r r r r@{}}
\hline
arm & updates & hands & exact & agrees / 200 \\
\hline
plain                          &  50k &  34k & $-0.0578$ & 175 \\
plain                          & 200k & 133k & $-0.0503$ & 185 \\
Double DQN                     & 200k & 132k & $-0.0476$ & 188 \\
plain, lr $10^{-3} \to 10^{-4}$ & 200k & 132k & $-0.0530$ & 178 \\
Double, lr $10^{-3} \to 10^{-4}$ & 200k & 132k & $-0.0503$ & 187 \\
\hline
\end{tabular}
\end{center}

\noindent Double DQN (van Hasselt et al.\ 2016,
\href{https://arxiv.org/abs/1509.06461}{arXiv:1509.06461}: argmax from the
online net, value from the target) is the best of the four, and the
learning-rate decay does nothing, because the late-training jitter is not in
the step size: it is the greedy readout flipping knife-edge cells whose two
actions are worth the same to three decimals. The 1000-update readouts move by
$\pm 0.005$ late in every arm, so the gap between the first two 200k rows is
inside that. Ten minutes on one card at 6\% utilisation, each.

The entries below are the same idea at the scale that made it famous:
the network is bigger, the outer loop is self-play and tree search, and the
target is still the model's own later estimate.
...
\subsection{Natural language processing --- demo: TinyGPT on Shakespeare}
...
\begin{tikzpicture}[x=0.68cm, y=0.68cm, font=\small,
  ...
  arrow/.style={->, >=stealth, thick, gray}]
  % the sampler's own first line: "ROMEO:\nI" -- eight char tokens, position 0..7
  \def\toks{R,O,M,E,O,:,{\textbackslash n},I}
  \def\nexts{O,M,E,O,:,{\textbackslash n},I,?}
...
\end{tikzpicture}
\caption{The causal mask on the sampler's own first eight characters. Rows are the
...
4 layers, 2 heads). Ten thousand Adam steps take three minutes on one
RTX 4060 Ti through XLA, compile included, and land at 2.28 bits/char
on tinyshakespeare's held-out split --- against 3.56 for a bigram table
and 6.02 for guessing uniformly over the 65 characters. Sampling after
that produces recognizable Shakespeare cadence, real character names
(NORTHUMBERLAND, QUEEN MARGARET, MERCUTIO, BRUTUS) and multi-line
dialog. Three things change relative to the image trainers shown
throughout Part~1, and they capture what NLP training \emph{is}:
...
\begin{itemize}
  ...
  \item \textbf{Loss is per-token, averaged over the sequence.} Image
    ...
    Ch~\ref{chap:tensor} derived, applied $T$ times in parallel.
...
StableHLO-through-XLA pipeline. The bestiary entries below all reduce to
...
\subsection{Diffusion --- demo: DDPM on MNIST}
\label{sec:bestiary_diffusion}

Generative models that predict the noise added to a clean image, with inference
iterating the reverse (noise in, image out). The architecture is a UNet ---
often with cross-attention bolted in for text conditioning. The novelty lives in
the noise schedule and the sampling loop, not in layer design.

\paragraph{DDPM on MNIST.} The \texttt{demos/MainMnistDdpmTrain.lean} trainer
(its CIFAR-10, sinusoidal-time and attention variants are archived beside it)
...
backbone is \emph{literally} the segmentation UNet of
...
\begin{itemize}
  \item \textbf{The target is the noise, and it is self-supervised.} Every model
    ...
    the same squared-error backward from Part~1
    (\texttt{lossKind := .floatTargetMse}).
  \item \textbf{The backbone is the segmentation UNet, conditioned on $t$.} One
    ...
    image channel plus one time channel), and the CIFAR variant embeds it with
    the positional-encoding primitive NeRF introduced. Diffusion adds
    \emph{only} the conditioning.
  \item \textbf{Inference is an iterative loop, not one forward pass.} What that
    loop costs is the second table below.
\end{itemize}
...
\emph{bracketed at both ends}, since a metric that only ever reports ``bad'' is
...
\hline
real MNIST, scored as if generated & 10/10 & $99.40\%$ & $0.0020$ \;($1\times$)  & $0.122$ \\
the model                          & 10/10 & $90.94\%$ & $0.0237$ \;($15\times$) & $0.138$ \\
...
that its weights loaded. Its first run caught a broken checkpoint --- pixels
over $[-4.9, 11.9]$ where the data is $[0,1]$, from a trainer that never centred
to $[-1,1]$ and defaulted to 3 epochs against a documented 50.
...
\paragraph{The continuous-time view, and which solver to use.} Everything above
is discrete: a schedule tabulated at $t = 0 \dots T{-}1$ and a fixed reverse
recipe. Score-SDE (Song et al.\ 2021,
...
$-\hat\varepsilon(x,t)/\sigma_t$, so the $\varepsilon$-predicting network the
trainer above already produces \emph{is} a score model up to a scale factor.
...
\caption{DDPM on MNIST, both directions. Top: the forward process takes a
training digit to Gaussian noise, one column per step of the schedule. Bottom:
the trained reverse process, started from fresh noise, arrives at a digit the
training set never contained --- the sampling the table above scores by function
evaluations.}
\label{fig:ddpm_mnist}
\end{figure}

\medskip\noindent Which makes the third row the instructive one. \emph{The
...
section --- the previous paragraph's warning arriving from a second direction.
...
The shape carries all the way up: the bestiary DDPM entry below is the same UNet
at CIFAR scale, and Stable Diffusion is the same UNet on VAE latents with text
...
\subsection{Physics --- demo: Boltzmann generator on M\"uller--Brown}
\label{sec:bestiary_boltzmann}

Networks whose target is a physical law rather than a labelled set: a
Boltzmann density here, a PDE residual, a solution operator and the
atmosphere six hours ahead in the entries below. The architectural side is
...
job. The network is the 2-D toy demo's MLP
(\texttt{demos/MainDiffusion2d.lean}, $18{,}178$ parameters, the sincos time
channel at $H = W = 1$), the loss is that demo's rank-2 squared-error block,
...
\begin{itemize}
  \item \textbf{The ground truth is a formula.} $U$ has a closed form, so the
    ...
    $360 \times 360$ grid. Every column of the tables below is exact for the
    first row.
...
$p_{20} / p_\theta$ recovers the quadrature row to the last digit. At NFE 2
well B empties while the populations still look plausible, which is why
$\langle U \rangle$ and $\Delta F$ are columns.
...
Energy distance as a multiple of the floor, with Liu's straightness at NFE 50:
...
$4.9\times$ the floor where the independent coupling needs ten. The price is
the ceiling: both straightened arms plateau above it at NFE 50, because their
training set is the first model's output, biases included.

\subsubsection*{Boltzmann generator (\texttt{Bestiary/BoltzmannGenerator.lean})}
...
\subsubsection*{Physics-informed neural network (\texttt{Bestiary/PINN.lean})}
...
Boltzmann generator above is the physics this book can train, its physics
...
\subsubsection*{Fourier neural operator (\texttt{Bestiary/FNO.lean})}
\emph{One} new primitive, not shown. Li et al.\ (2021,
\href{https://arxiv.org/abs/2010.08895}{arXiv:2010.08895}) learn the solution
\emph{operator} of a PDE family rather than one solution: initial condition
...
\subsubsection*{FourCastNet (\texttt{Bestiary/FourCastNet.lean})}
...
\emph{overcounts}: attention at 768 spends $4 \times 768^2 \approx 2.36$\,M
...
\subsection{Signal processing --- demo: gravitational-wave detection on LIGO strain}
...
search's threshold, PyCBC's coherent search on the same windows, and the
CNN. (d) The same on real O3a strain, where glitches move the filter's
threshold from $5.9$ to $10.4$ and the CNN's barely at all. The CNN drawn in
...
has a theorem for a ceiling the way blackjack has value
iteration and the Boltzmann generator has quadrature. Advanced LIGO's strain
...
noise it is the Neyman--Pearson optimum, and its detection probability at
threshold $\rho^\ast$ for a signal of optimal SNR $\rho$ is a Marcum
$Q$-function. Three things change relative to the CIFAR demo:
...
\begin{itemize}
  \item \textbf{The data is real detector noise with the signal we chose.}
    ...
    file's own median-Welch PSD over 20--500\,Hz. Half the windows carry an
  ...
  \item \textbf{The instrument is a closed form, and the filter is checked
    ...
    agree on one PSD. The search statistic then maximises over the window
    and the $\pm 10$\,ms inter-detector delay, exactly as a search must, and
    the CNN, which sees no time, is compared to that.
  \item \textbf{The network is Chapter~\ref{chap:bn}'s CIFAR net, unchanged.}
    \texttt{CIFAR-CNN8-wide-BN} as that chapter spells it: eight
    convolutions in four conv-conv-pool stages at 16, 16, 32, 32 channels
    into the $2 \times 512$ head, $0.83$\,M parameters. The two edits are
    the two-channel stem and the two-way output. The input is a
...
exceeds it. The last column is the SNR at which each row crosses one half:
...
PyCBC coherent search, true template          & $0.26$ & $0.58$ & $0.90$ & $0.97$ & $1.00$ & $1.00$ & $1.00$ & $1.00$ & $\mathbf{5.27}$ \\
...
\noindent The filter sits on its closed form and the CNN trails it by $1.5$
units of SNR. Gabbard et al.\ 2018
...
training set; a log-power spectrogram discards the phase the filter
integrates, and this is the demo's number, not the architecture's ceiling.
...
veto for exactly that reason. The same rows on the strain itself, with each
model trained on one noise set and scored on both:
...
PyCBC coherent search       & $\mathbf{5.27}$ & $10.32$ & --- \\
\hline
\end{tabular}
\end{center}

\noindent Each network is best on the noise it trained on, by a few tenths.
The filter loses five units of SNR to glitches at $10^{-2}$, and at
...
teaches the glitch tail. % TODO Phase 4: the catalogue table (which GWTC
% events the trained model flags, beside their published SNR) and the
% Gravity Spy glitch row.
...
sequence: 3D scene reconstruction, multimodal embedding, scientific. Several of these (NeRF, CLIP) have essentially
...
\textbf{500\,GB of free space} to prepare it in one pass. From there
...
top-1 and top-5 in this book faithfully follows the validation protocol of
\texttt{timm}'s ``ResNet Strikes Back''
(\href{https://arxiv.org/abs/2110.00476}{arXiv:2110.00476}), so that a
number here and a number there are answers to the same question.
Concretely, four things are matched, and each uses \texttt{timm} as a
reference:
...
\begin{itemize}
  ...
  \item \textbf{ArASL} is the people-watching dataset: $54{,}049$ grey
    ...
    The chapter-4 CNN (\texttt{arasl-signs}) trains on it twice, under the
  ...
  \item \textbf{GWOSC O3a strain} is the signal-processing dataset: the
    ...
    cut into two-second windows, half of them given an injected
    binary-black-hole chirp, and the CNN of \S\ref{sec:bestiary_signal}
...
that script is where they come from.
...
GWOSC O3a strain & GWOSC & Abbott et al., 2023 &
  ...
  \texttt{preprocess\_gw.py} $\to$ \texttt{data/gw/}  \\
\hline
\end{tabular}
\end{center}
...
redistributes nothing --- the same posture as BraTS below. The download
...
training $1{,}080$ alone (\texttt{scripts/neu\_anchors.py}). It writes the
...
list by leaf id, so no leaf straddles a line. The audit of the ArASL build
runs on it and finds $0.1\%$ of consecutive photographs within six grey
levels and $1.3\%$ of random-split test images with a near-twin in training:
the four photographs of a leaf are different poses. Beside the test tenth it
...
73 theorems, sandboxed, verifying each one's axiom closure statically:

\begin{verbatim}
tests/comparator/run.sh
\end{verbatim}

It splits in three. \texttt{Challenge.lean}
...
Thirty-nine more live in \texttt{ChallengeArch.lean} and do import the project,
...
The last 21 are \texttt{ChallengeTier.lean}, the layer this book's second
half rests on: the step ties, the codegen faithfulness results, the whole-net
back-chains, the data-parallel results, the float bridge and the three
certificate theorems. Their statements run to hundreds of lines apiece, so the
file is printed from each declaration's own type
(\texttt{scripts/gen\_comparator\_tier.py}) rather than transcribed, and the
solution proves each one with the project theorem itself. With the four that
\texttt{ChallengeArch.lean} already holds, these are exactly the declarations
...
PJRT_FFI_RESIDENT=1 .lake/build/bin/resnet34-verified-adam data
\end{verbatim}
    \texttt{PJRT\_FFI\_RESIDENT=1} keeps the parameters on the device
    between steps. Every transcript in this book ran that way and the tiers
    below set it; without it the same run is slower --- 1.3--2$\times$ on
    the Imagenette nets, about 5$\times$ on MNIST and CIFAR --- and its
    numerics are unchanged. Use
    \texttt{CUDA\_VISIBLE\_DEVICES} / \texttt{HIP\_VISIBLE\_DEVICES} to
...
\begin{verbatim}
lake run mnist        # verified MNIST: linear, MLP, CNN              (~30 s)
lake run cifar        # ch.4 cifar8w: SGD/momentum/Adam x bn/no-bn   (~5 min)
lake run imagenette   # the Part-I nets at 224^2, 80-epoch AdamW      (~6 h)
lake run imagenet     # ImageNet-1k on 4 GPUs: bare plans, `start` runs (Track 4)
\end{verbatim}

The three times are XLA wall-clocks on one RTX 4060 Ti, resident; the fourth
tier is weeks of four-card time and has Track 4 to itself. The Imagenette
figure is the five chapter nets at their chapters' times, about 3.7 hours
between them, plus ResNet-50 (about an hour and a half) and MobileNetV4
(under an hour), which ride in the same tier. Each trainer's transcript is
teed to \texttt{runs/<date>-<trainer>/}. Each tier also has an
\texttt{-iree} twin --- see \emph{The second lowerer} at the end. The tiers
...
renderer as their Imagenette counterparts, in the chapter order Track 3
uses. A row is a \emph{(network, recipe)}
...
\texttt{resnet50-imagenet-verified}     & \texttt{a3} (RSB-A3, train@$160$)  & \texttt{r50-a3-wxclip-bf16-4gpu} \\
...
\noindent Six of these jobs have run and are the ones behind their chapters'
numbers; the ConvNeXt-T row is configured and has not finished, so
Chapter~\ref{chap:layernorm} carries the reference only. \texttt{scripts/jobs/} also carries \emph{axis} siblings that
this table does not list --- an fp32 peer of each of the ResNet-34,
ResNet-50 2018, RSB-A3 and ViT rows, and an earlier A3 artifact --- which differ from their
row in precision or in one optimizer knob, never in the recipe, plus
\texttt{selftest}, which exercises the supervisor without a GPU. Each config's
...
render rather than a wrong knob. That is the one-card shape. The four-card one,
with every knob taken from the job's own config, is \texttt{lake run
r34-default-bf16-4gpu once} --- the next section.
...
lake run r34-default-bf16-4gpu plan   # the precheck and the wall-clock on file; runs nothing
lake run r34-default-bf16-4gpu once   # one foreground run with the job's env, no restarts
...
\noindent \texttt{lake run imagenet} is the tier itself, the fourth of the four.
Bare, it only plans, because its twelve rows --- the table above and the side
quests below --- are weeks of four-card time; \texttt{start} is the confirmation.
...
collected here rather than in the table above because \emph{none of them has
been trained} --- not because they cannot start. Five of the seven have job
configs, launch like any Track-4 row and ride in \texttt{lake run imagenet}
right after their chapter's row; only the two ResNet-50 recipes have none. The
status column says what each one is missing.
...
\texttt{convnext-s-imagenet-verified}  & ---               & \texttt{cnxs-default-4gpu}, untrained & \S\ref{sec:convnext_sb} \\
\texttt{convnext-b-imagenet-verified}  & ---               & \texttt{cnxb-default-4gpu}, untrained & \S\ref{sec:convnext_sb} \\
\texttt{vit-s-imagenet-verified}       & ---               & global $512$; \texttt{vits-default-g512-4gpu}, untrained & \S\ref{sec:vit_sb} \\
\texttt{vit-b-imagenet-verified}       & ---               & global $512$; \texttt{vitb-default-g512-4gpu}, untrained & \S\ref{sec:vit_sb} \\
\end{tabular}
\end{center}
...
\texttt{mnist-iree}, and so on). It additionally needs \texttt{iree-compile}
on \texttt{\$PATH}, the runtime built per
\texttt{historical/\allowbreak IREE\_BUILD.md}, and
\texttt{ffi/\allowbreak libiree\_ffi.so}. Data-parallel runs are XLA-only --- collectives
...
\begin{figure}[ht]
\centering
\begin{tikzpicture}[scale=0.7293, transform shape,
...
\node[group] (matkit) at (8.0, 0) {
  ...
  \texttt{pdivMat\_comp} \\
  \texttt{matmul\_left\_const\_has\_vjp} \\
...
\node[final] (r34) at (-9.0, -8.0) {
  ...
  \texttt{resnet34ForwardB\_full\_} \\
  \texttt{has\_vjp\_at\_correct}
};
...
\node[final] (enet) at (-2.5, -9.4) {
  ...
  \texttt{efficientnetForwardB\_full\_has\_vjp\_correct}
};
...
\node[final] (vit) at (5.75, -9.4) {
  \textbf{Full ViT-Tiny (12 blocks)} \\
  \texttt{vitTiny\_has\_vjp\_correct}
};
...
The Verified VJP Proofs suite proves the \textbf{45 VJP contracts and
6 witnesses} of the table in \emph{How this book is organized}, their
restatements at fixed dimensions and over the committed specs, and the
two smooth-point forms that global theorems superseded, each asserting
backward \(= \sum_j \pdiv f\,x\,i\,j \cdot dy_j\), on
top of the foundation \(\pdiv\) calculus and the
differentiability/forward/witness machinery they rest on, plus the
bestiary's 45 architecture families --- and \textbf{zero project
...
zero project axioms. It runs 73 theorems (the foundation rules,
every chapter's headline Jacobian, the public
\texttt{*\_has\_vjp\_correct} wrappers, three smooth-point
...
and the 21-theorem tie / faithfulness / certificate tier)
...
The 73 are not the whole proof suite --- \texttt{tests/AuditAxioms.lean}
prints the axiom closure of 1{,}374 declarations, and the same allowlist
applies to all of them, because every theorem in the project closes the
same way. What the 73 add is a second, non-elaborator opinion, and they
include every declaration \texttt{formalization.yaml} puts forward as a
result, together with the calculus floor those rest on.
...
\begin{itemize}
  \item \textbf{Denoted IR and a bridge theorem.} The emitted backward (and
    ...
    \texttt{conv\_back\_bridge\_1to2}, \texttt{bn\_back\_bridge}, and
    \texttt{relu\_back\_bridge}, and each closes under the same three axioms
    as the rest of the project.
  \item \textbf{A computable printer.} \texttt{Back} carries real-valued
    operands and cannot be printed as it is, so \texttt{IRPrint.lean} defines
    a mirror of it, \texttt{Hlo}: the same node structure, with SSA names and
    shapes in place of values, printed one \texttt{stablehlo} operation per
    node: \texttt{dotGeneral}\,$W$ becomes
    \texttt{stablehlo.dot\_general}, a \texttt{selectPos} node becomes
    \texttt{compare GT 0} plus \texttt{select}, and so on. That the mirror
    is built node for node from the bridged graph is a reading of two
    definitions, not a theorem. The tier where the printed text is the
    denoted graph's own printout, with the parse-back proved
    (\texttt{StableHLO.roundtrip}), is the verified trainers' StableHLO
    tier, below.
  \item \textbf{An execution oracle.} A Python harness
    (\texttt{LeanMlir/Proofs/check\_ir\_codegen.py}) regenerates the
    \texttt{.mlir} from the printer, compiles it with IREE, runs it, and
    diffs the result against an independent NumPy reference. The CPU run
    (\texttt{llvm-cpu}) is the correctness gate; the GPU leg is ROCm-only
    and best-effort, and where it has run (a Radeon RX 7900 XTX, gfx1100)
    the proof-backed graph matched the reference to roughly $10^{-6}$.
\end{itemize}
...
\begin{itemize}
  \item \emph{Proven} (Lean, three axioms): the IR's denotation equals the
    proven derivative.
  \item \emph{Read, not proved}: that the printed mirror is, node for node,
    the bridged IR.
...
\emph{tie} theorem per network (\texttt{mlp\_train\_step\_tied\_certified},
\texttt{r34\_net\_tiedB}, \texttt{vit\_net\_tied\_certified} and their peers) then
...
(\texttt{vitBlockBackV\_eq\_transformerBlockV\_vjp} and its peers). The axiom
...
(\texttt{jax/Jax/Codegen.lean}, \(\sim 3700\) lines) produces an
...
run. Fourteen cases each agree with JAX autodiff on the step-2 loss, to
between \(2 \times 10^{-7}\) and \(10^{-4}\), the wider ones where a
max-pool or a depthwise reduction changes the summation order: dense,
dense+ReLU, conv, conv+BN, conv+max-pool, residual, depthwise, global
average pool, attention, and the SE-gated MBConv, bottleneck, MBConv-V3,
fused-MBConv and UIB blocks. Any future hand-derived VJP added to
...
oracle} (\S~\ref{sec:verified_codegen}) regenerates the per-operator and
small-network graphs (linear, MLP, CNN, convolution, max-pool, BatchNorm,
the loss cotangent and a ConvNeXt block) from the printer, compiles them
for CPU, and diffs the result against an independent NumPy reference,
which is the one check that reaches past the proofs to IREE's lowering
itself. And the
...
layer, where the oracles agree to float32 precision. It is now a theorem layer, and
...
per-operator: \texttt{FloatBridge.lean} budgets every operator the
networks are built from, and the certified backward ties say each net's
hand-written gradient chain is the certified one. The
...
\(e_{\exp}\) is precisely the constant the margin probe pins on the
device at 1--2 ULP (\emph{Measured against proven}, below), so the
deductive and empirical layers meet at a named interface instead of a
hand-wave.
...
\texttt{(M : FloatModel)} --- the per-operator lemmas, the block and stage
chains, the MNIST descent chain --- so none of them is a statement about
binary32 that happens to be proved; each is a statement about \emph{any}
...
their gradient nodes in \texttt{Bf16GradNodes.lean}; \texttt{Bf16Fold.lean}
and \texttt{E4M3Fold.lean} prove, for the linear net, that the mixed-precision
