...
% Placed at the head of every section that quotes an ImageNet accuracy. The
% emitters moved onto timm's validation protocol and resampler after all of
% these were measured (appendix A), so the numbers and the protocol the book
% describes are one re-run out of step. One macro, so the caveat cannot be
% attached to some result tables and forgotten on others.
...
\textbf{[TODO: run with timm tweaks.]} Both rows above were measured
before the trainers moved onto timm's validation protocol and its
resampler (appendix~\ref{app:data}), and before two optimizer
corrections read out of timm's own source: LAMB's gradient clip, which
timm enables by default and we did not, and its guard excluding the
\texttt{no\_weight\_decay} group from layer adaptation, which we had
been applying to every parameter. All four are in the emitters now,
none has been run, and none of the numbers here reflects any of them.
...
path, with these deltas}, not as ``RSB-A3 reproduced''.

\medskip\noindent\emph{Closed since that run, in the emitters and
\textbf{not} exercised.} Every one changes the trajectory, so none of them
can be compared to the $77.43\%$ except by a fresh run --- which is
required in any case, since the Posterize correction below moved the
training distribution for both paths at once. The artifact that now most
nearly implements the recipe is
\texttt{resnet50in160\_lambaccdp8x64wxclipbce\_train\_step}, and it has
never taken a step.

\begin{itemize}
  \item \texttt{wdExcludeNormBias}, the largest single delta: BN
    $\gamma/\beta$ and every bias were decayed at $\mathrm{wd} = 0.02$
    where timm skips them. Ported from the ConvNeXt and ViT renders as
    predicted --- a second decay constant $\%\mathtt{wdz}$ bound to zero,
    since every optimizer takes the decay as an operand \emph{name}. The
    split is 54 decayed / 107 excluded of 161, and both halves decompose
    structurally: 53 conv weights $+$ 1 dense weight against 53 BN layers
    $\times$ 2 $+$ 1 dense bias.
  \item \textbf{LAMB's gradient clip}, which timm enables by default
    (\texttt{Lamb.max\_grad\_norm} $= 1.0$) and which no LAMB artifact
    here had --- so every LAMB number in this document, including the
    $77.43\%$, was produced by
    \texttt{Lamb(max\_grad\_norm=None)}, a different optimizer from the
    one the recipe names. Two orderings had to be right rather than one:
    the clip follows the all-reduce (clipping partial per-replica
    gradients clips nothing in particular) \emph{and} follows the
    accumulation, because the reference clips the mean over the $k$
    micro-batches. The graph never materializes that mean --- the $1/k$
    is folded into the moment coefficients --- so the fold runs on the
    accumulated sum against a threshold scaled to $kC$, which is exactly
    the reference's factor by
    $\min(1, kC/(\|G\| + k\varepsilon)) = \min(1, C/(\|G\|/k +
    \varepsilon))$. That identity is proved rather than asserted
    (\texttt{Proofs.clipFactor\_accum}).
  \item The \texttt{no\_weight\_decay} group is no longer layer-adapted,
    matching timm's guard: an excluded parameter takes a plain Adam step
    at trust $= 1$. On the verified side this \emph{removed} an op rather
    than adding one, since feeding the existing zero constant lands on an
    already-proved lemma.
  \item BN running-statistic momentum is now compensated for
    accumulation. Evaluation-only, and plausibly means the $77.43\%$ is
    slightly \emph{understated}.
  \item Evaluation now scores all $50{,}000$ validation images, which is
    timm's denominator. \textbf{Every ImageNet top-1 quoted in this
    document, the $77.43\%$ included, is over $49{,}920$.}
  \item RandAugment's Posterize kept one MSB too few at RSB's own $m = 6$,
    and the trainers have since moved onto timm's validation protocol and
    resampler (appendix~\ref{app:data}).
\end{itemize}

\medskip\noindent\emph{Still outstanding}, in rough order of expected
effect:

\begin{itemize}
  \item Ghost-BN group is 64 against the reference's 512, because no
    collective touches the batch statistics. Needs a cross-replica
    reduction or a larger per-replica batch (which is baked into the
    signature). \emph{Weaker evidence than it was:} the phase-3 row above
    puts a true single-forward $2048$ at $77.00\%$ against Ghost-BN's
    $77.22\%$, so across $512 \to 2048$ the BN group is worth nothing and
    is possibly slightly negative. That does not settle $64 \to 512$,
    which is the range this row sits in, but it is the only direct
    measurement of the axis and it does not point at a lever.
  \item fp32, not bf16, which is most of the $2\times$ wall-clock gap and
    is a throughput lever rather than a fidelity one.
  \item $0.08\%$ of each epoch is dropped (5000 of 5004 micro-batches,
    since $8 \nmid 5004$). Structural to $k$: changing it changes the
    effective batch.
  \item Mixup $\lambda$ is drawn host-side from NumPy, so agreement with
    the reference is distributional, never per-step. Not fixable without
    reimplementing the generator.
  \item \emph{Verification, not recipe:} the gradient check's tier-1B
    identity is not decided under BCE. \textbf{The earlier diagnosis here
    was wrong and is worth correcting}, because it was recorded from a
    single sample: tier-1B does \emph{not} degrade into a cancellation
    residual. Its value under BCE is one of the stablest numbers in the
    report ($7.57$--$7.71 \times 10^{-4}$ over repeated runs, a $2\%$
    spread), and at the worst site the two inner products do not oppose
    each other at all, so there is no cancellation to blame. What failed
    was a fixed tolerance calibrated at CE, where the same residual is
    $7 \times 10^{-5}$ --- against a separation from the control
    population that stays at $42$--$45\times$. A second, independent
    fault \emph{was} found and fixed: the control verdict rested on the
    minimum over 21 sites, the least reproducible number in the report,
    so the answer depended on which run one happened to do. It now uses
    the tenth percentile and is stable under both losses. Tier-1A and
    tier-2 pass throughout, and nothing has ever indicated a fault in the
    render itself.
\end{itemize}

\noindent The optimizer is also now tied to the reference at the
\emph{update}, which nothing previously checked: the adjoint machinery
above compares the two implementations at the \emph{gradient}, while the
optimizer was two hand-maintained implementations that happened to agree.
One step of each on the same $(\theta, g, m, v, G)$ agrees to
$\sim\!10^{-7}$ across six variants, and reverting each of the clip's
design decisions moves that to $10^{-1}$.
...
It did not always. The block table this book rendered and proved against
used to be MobileNetV4-Conv-Small at 4.1M parameters, so the ImageNet
spec was Conv-S carrying a 1000-class head and the $75.48\%$ above was a
different network's number. Both the verified spec and its \texttt{jax/}
peer now carry the Conv-M table: 21 UIB blocks in place of 14, and a head
that goes $256 \to 960 \to 1280$ in two convolutions in place of one. The
spec is still built the way \S\ref{sec:mnv2_phase4} built MobileNetV2's
...
\textbf{So the $75.48\%$ above is now this network's target.} It is not
...
\hline
4$\times$ 4060 Ti (CUDA), global 256 & 177 & 14.8 & 24.6 h (1.0 d) & TBD \\
\hline
\end{tabular}
\end{center}

\noindent {\small Measured 2026-08-14, fp32, with the driver's own
steady-state probe (median of 112 steps after 8 warmup). The four-card
number needed MobileNetV4's data-parallel train step, which this book does
not ship: its collectives have no \texttt{shard-check} peer, so nothing
ties them. It was rendered locally, timed, and deleted. The wall clock is
therefore real and the artifact it was measured on is not in the tree. For
scale, one card takes $126$\,ms/step and $70.1$ hours, a four-card speedup
of $2.85\times$.}

\medskip\noindent The previous printing of this row estimated
$\sim$12.3 min/epoch and $\sim$20 h by scaling this network's single-card
throughput by MobileNetV2's four-card efficiency. That was $8\%$ optimistic.
A second and more mechanical-looking estimate, scaling MobileNetV2's
measured all-reduce cost by Conv-M's $2.77\times$ larger gradients, gave
$41.8$ h and was wrong by $70\%$. Most of the four-card overhead is not
all-reduce at all. At global 256 the host feeds four times as many images
per step, and that cost grows with the batch rather than with the model.
...
Chapter~\ref{chap:depthwise} could not.} Both networks have a fold proved
at full depth, and the two folds are not equally strong.
...
BatchNorm and nothing else assumed. It holds at every input. So for this
network the accurate sentence is the strong one: every operation in the
graph that just trained carries a proved backward, and their composition
into the whole network carries one too.

What buys that is the activation. Swish is smooth everywhere, so the
statement can quantify over all inputs. MobileNetV2's relu6 has two
kinks, so Chapter~\ref{chap:depthwise}'s seventeen-block fold has to name
a point and assume the thirty-five activations there stay away from them.
What separates the two chapters now is that hypothesis rather than the
depth.
...
\noindent \textbf{[TODO: run with timm tweaks.]} The protocol above is
what the trainers emit today. Every ImageNet accuracy printed in this
book predates it and was measured under the older preprocessing, so the
numbers and the protocol description are one re-run out of step. They
...
\begin{itemize}
  \item \textbf{Oxford-IIIT Pets} backs both the UNet segmenter
    (\texttt{unet-pets-train}, via trimaps) and the YOLO detector
    (\texttt{yolov1-pets-train-bootstrap}, via head boxes tiled into
    $2 \times 2$ mosaics).
  \item \textbf{MSD Task01 (BraTS)} carries segmentation to real medical
    imaging. It is four-modality brain-tumour MRI, where the thin-class
    collapse a Pets trimap merely tolerates becomes the entire problem
  ...
  \item \textbf{VisDrone-DET2019} is where detection leaves the household
    ...
    of them per image, so the single $7\times7$ grid that suffices for Pets
    head boxes collapses outright and the multi-scale FPN head of
    Definition~\ref{layer:fpnModule} becomes necessary rather than
    decorative.
...
(\texttt{data/pets\_mosaic\_bal/}), and the same \texttt{download\_pets.sh}
...
\texttt{IREE\_BACKEND} and \texttt{IREE\_CHIP}, for example
...
XLA-only, and the IREE shim refuses a DP entry point outright rather than
...
\begin{itemize}
  \item \textbf{\texttt{no lowerer shim could be loaded}.} Neither
    ...
    Build the PJRT one with the \texttt{gcc} line in Track 2, which needs
    nothing but libc.
  ...
  \item \textbf{\texttt{iree-compile failed} on ResNet-50.} Expected.
    ...
    size. R50 is an XLA-only net, so drop \texttt{LEAN\_MLIR\_LOWERER=iree}.
  \item \textbf{A stale \texttt{ffi/libiree\_ffi.so}} reports as
    \texttt{not a usable shim}. The loader requires every entry point the Lean
    side calls, so a shim built before one was added is rejected whole rather
    than failing at the call. Rebuild it per \texttt{IREE\_BUILD.md}, which is
    two \texttt{gcc} lines against the runtime archives. The file is a build
    output and is not tracked, so a fresh clone starts without one.
  \item \textbf{First IREE compile appears stuck.} It isn't. ResNet-34's
    full training-step compile takes 10--15 minutes. Watch RAM, and if it is
...
constant-level dependency explorer, in the repo's
...
\begin{itemize}
  ...
  \item \emph{Trusted} (validated numerically, not proven): that the
    ...
    compiles the text correctly, and that \texttt{float32} approximates
    $\mathbb{R}$. The last of those is now a conditional theorem validated on
    real silicon (\S~\ref{sec:float32}) rather than a bare assumption.
\end{itemize}
...
mask and no logit drift past \(\delta\) (\S~\ref{sec:float32}). It is
...
where the oracles agree to 1--2 ULP. It is now a theorem layer, and it has
...
at MNIST-CNN \(0.015\), CIFAR-8 \(2.6\), MobileNetV2 \(0.30\), ResNet-34 \(0.17\),
