...
including $37.5$\,s per epoch of evaluation and checkpointing:

\begin{center}
\begin{tabular}{@{}l r r r@{}}
\hline
verified, 4 cards & global & steps/epoch & 300 epochs \\
\hline
ViT-Ti & $512$ & $2{,}502$  & $51.5 \to 37.1$\,h \;(measured) \\
ViT-S  & $512$ & $2{,}502$  & $134 \to 90$\,h \;($5.6 \to 3.7$\,d, derived) \\
ViT-B  & $128$ & $10{,}009$ & --- \\
\hline
\end{tabular}
\end{center}

\noindent \textbf{ViT-B gets no projected total.} Tiny is the only ViT with a
measured trainer step, and the trainer costs a near-constant multiple of the
graph --- which is what lets S's row be derived, since S runs at Tiny's batch.
B does not: at a quarter the batch, Tiny's multiplier returns a $1.12\times$
trainer speedup, \emph{below} B's own $1.40\times$ device ratio, which no
overhead model may do. An additive model gives $1.37\times$ and a total a
hundred hours lower, and two defensible models that far apart is not an estimate.
