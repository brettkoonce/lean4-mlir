...
\hline
ConvNeXt-T & $9$ blocks, $96$ wide  & $28.6$\,M        & $157.9 \to 121.1$\,ms \;($1.30\times$) \\
ConvNeXt-S & $27$ blocks, $96$ wide & $50{,}222{,}152$ & $268.4 \to 192.7$\,ms \;($\mathbf{1.39\times}$) \\
ConvNeXt-B & $27$ blocks, $128$ wide& $88{,}589{,}416$ & $395.7 \to 294.3$\,ms \;($1.34\times$) \\
\hline
\end{tabular}
\end{center}

\noindent Counts are \texttt{\#guard}ed against $50.22$\,M and $88.59$\,M. T and
B ran as controls and returned their committed $157.8$ and $396.0$\,ms, which is
what makes S's $1.39\times$ a measurement rather than a claim about itself.
...
\medskip\noindent \textbf{What bf16 buys: S is the fastest of the three}, which
``deeper costs more'' does not predict either. Depth adds blocks at widths bf16
already suits, where B's widening moves every stage. \S\ref{sec:vit_sb} finds
the same shape on ViT for the same reason. Note how much of it the trainer eats,
though --- a $1.39\times$ graph is a $1.25\times$ run, because the shim feed and
the all-reduce do not get faster.
...
\hline
ConvNeXt-T & $128$ & $10{,}009$ & $217 \to 184$ & $184 \to 157$\,h \;($7.7 \to 6.5$\,d) \\
ConvNeXt-S & $128$ & $10{,}009$ & $366 \to 292$ & $308 \to 247$\,h \;($12.8 \to 10.3$\,d) \\
ConvNeXt-B & $128$ & $10{,}009$ & $534 \to 446$ & $449 \to 375$\,h \;($18.7 \to 15.6$\,d) \\
\hline
\end{tabular}
\end{center}

\noindent T and B are measured and S is derived, from a trainer cost that is a
near-constant multiple of the graph ($1.37\times$/$1.35\times$ fp32,
$1.519\times$/$1.516\times$ bf16) and reproduces T's and B's published totals to
within $0.5$\,h. Two direct probes of S have since returned $368 \to 301$ and
$367 \to 304$\,ms against the $366 \to 292$ derived here, so the model holds on
this net. \S\ref{sec:vit_sb} is where it does not, and says why.
...
$166.5$ hours compute-only, so the ${\sim}2\times$ gap to the rows above is the
batch and not the lowerer.
