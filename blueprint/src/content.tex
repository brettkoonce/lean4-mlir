...
\begin{theorem}[MLP VJP at smooth inputs]
  ...
  \leanok
  \uses{thm:dense_has_vjp, thm:relu_has_vjp, thm:vjp_comp, ax:mlp_has_vjp}
  \Assume{}
  \begin{enumerate}
...
\begin{theorem}[MNIST CNN VJP at smooth inputs]
  ...
  \leanok
  \uses{ax:conv2d_has_vjp3, ax:maxPool2_has_vjp3, thm:dense_has_vjp, thm:relu_has_vjp, thm:vjp_comp}
  \Assume{}
  \begin{enumerate}
...
\begin{theorem}[ViT-Tiny, the whole network]
  ...
  \leanok
  \uses{ax:patchEmbed_flat_has_vjp, thm:transformerBlock_has_vjp_mat, thm:layerNorm_per_token_has_vjp_mat, thm:dense_has_vjp, thm:vjp_comp}
  \Assume{}
  \begin{enumerate}
    \item \(\varepsilon > 0\) \hfill[\texttt{h\(\varepsilon\)}]
  \end{enumerate}
...
StableHLO-through-XLA pipeline; the same trainer at $8.5$\,M parameters and
a 4096-token BPE vocabulary runs on the 2\,GB TinyStories corpus
(\texttt{tinystories}, Appendix~\ref{app:data}). The bestiary entries below
all reduce to
...
TinyStories & TS & Eldan \& Li, 2023 &
  ...
  \texttt{download\_tinystories.sh}\newline $\to$ \texttt{data/tinystories/}  \\[2pt]
...
vocabulary keeps the embedding gather and the head matmul cheap and
the tokenizer auditable. \texttt{preprocess\_visdrone.py} writes three
...
The last 21 are \texttt{ChallengeTier.lean}, and they are the layer this book's
second half rests on: the step ties, the codegen faithfulness results, the
whole-net back-chains, the data-parallel results, the float bridge and the three
certificate theorems. They are machine-generated from each declaration's own type
(\texttt{scripts/gen\_comparator\_tier.py}) because the statements run to
hundreds of lines apiece --- \texttt{cnx\_net\_tiedGB} alone is 186 --- and a
pair of files that must stay bit-identical is not a thing to transcribe by hand.
They are exactly the declarations \texttt{formalization.yaml} advertises,
so the set is auditable by diffing two lists rather than by trusting this
paragraph.
...
prints the axiom closure of 1{,}380 declarations on every proof-path
push, and the same allowlist applies to all of them, because every
theorem in the project closes the same way. What the 73 add is a
second, non-elaborator opinion, and since 2026-09-20 they cover every
declaration \texttt{formalization.yaml} puts forward as a result,
together with the calculus floor those rest on.
