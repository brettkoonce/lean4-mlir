...
the cotangent). Across Chapters 2--10 there are \textbf{33 VJP
...
that support them are additional machinery, not counted here ---
Chapter~2 is entirely that kit, which is why it contributes zero
contracts: it builds the tools the other chapters' VJPs are made of.
...
\textbf{Chapter 8 (EfficientNet)} contributes 3 VJP contracts: the
squeeze-and-excitation block (\(\texttt{seBlock\_has\_vjp}\) ---
global-avg-pool plus two 1\(\times\)1 convs plus the elementwise
product, every piece already proved elsewhere), the smooth
\texttt{sigmoid} gate activation, and the whole-network
\(\texttt{efficientnet\_has\_vjp\_at}\) over MBConv blocks with a
full-spatial SE gate.

\textbf{Chapter 9 (ConvNeXt)} contributes 5 VJP contracts: GELU
(Jacobian via \(\fderiv\) and \texttt{Real.differentiable\_tanh}),
LayerNorm (reusing the BN proof template), Swish, the elementwise
...
\textbf{Chapter 10 (ViT)} contributes 7 VJP contracts (softmax-CE,
the three SDPA cotangents, MHSA, the transformer block, and ---
new in this revision --- the whole-network
\(\texttt{vit\_full\_has\_vjp\_correct}\) wrapper), resting on the
...
\hline
2 (Foundation)         & 0 & 0  & the \(\pdiv\) chain / fan-in / product kit (scaffolding) \\
3 (MNIST: 1D)          & 6 & 6  & Dense (weight/bias), ReLU, MLP ($+$ smooth-point \texttt{\_at}) \\
4 (MNIST: 2D)          & 3 & 9  & Conv, MaxPool ($+$ \texttt{\_at}) \\
5 (CIFAR: BN)          & 1 & 10 & BatchNorm input-gradient \\
6 (ResNet-34)          & 6 & 16 & Residual ($+$proj, $+$\texttt{\_at}), global-avg-pool, whole-net \texttt{cnn} \\
7 (MobileNetV2)        & 2 & 18 & Depthwise, whole-net \texttt{mobilenetv2} \\
8 (EfficientNet)       & 3 & 21 & SE (mult. fan-in), sigmoid, whole-net \texttt{efficientnet} \\
9 (ConvNeXt)           & 5 & 26 & LayerNorm, GELU, Swish, layer-scale, whole-net \texttt{convnext} \\
10 (ViT)               & 7 & 33 & matrix kit, Attention (SDPA/MHSA/block), whole-net \texttt{vit\_full} \\
\hline
                       & \textbf{33} & & \\
\hline
\end{tabular}
...
The Verified VJP Proofs suite proves \textbf{33 VJP correctness
...
zero project axioms. It runs 48 theorems --- the foundation rules,
...
The 48-theorem coverage is illustrative rather than exhaustive ---
