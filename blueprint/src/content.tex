...
\medskip\noindent\emph{Now in, none of it run.} Each changes the
trajectory, so comparison against the $77.43\%$ needs a fresh run. The
artifact that now most nearly implements the recipe is
\texttt{resnet50in160\_lambaccdp8x64wxclipbce\_train\_step}.

\begin{itemize}
  \item \texttt{wdExcludeNormBias} --- BN $\gamma/\beta$ and biases no
    longer decayed; 54 decayed / 107 excluded of 161. Largest single delta.
  \item \textbf{LAMB's gradient clip} (timm's \texttt{max\_grad\_norm}
    $= 1.0$ default). Every LAMB number here, the $77.43\%$ included, was
    produced without it. Applied after the all-reduce \emph{and} after the
  ...
  \item BN running-statistic momentum compensated for accumulation.
    Evaluation-only; plausibly means the $77.43\%$ is understated.
  \item Evaluation scores all $50{,}000$ images. \textbf{Every top-1 in
    this document is over $49{,}920$.}
  \item RandAugment Posterize, plus timm's validation protocol and
    resampler (appendix~\ref{app:data}).
  \item \emph{Verification:} the optimizer is now tied to the reference at
...
\begin{itemize}
  \item Ghost-BN group 64 against the reference's 512; no collective
    touches the batch statistics. \emph{Weaker than it looks:} phase~3
    above puts true-$2048$ at $77.00\%$ against Ghost-BN 512's $77.22\%$,
    so $512 \to 2048$ buys nothing. Untested at $64 \to 512$.
  \item fp32, not bf16 --- most of the $2\times$ wall clock, a throughput
    lever rather than a fidelity one.
  ...
  \item \emph{Verification, not recipe:} tier-1B is undecided under BCE.
    The cause is a CE-calibrated tolerance, \emph{not} the cancellation
    residual claimed here previously --- repeated runs refute that
    (tier-1B is stable to $2\%$, and its separation from the control
    population holds at $42$--$45\times$). Tier-1A and tier-2 pass.
\end{itemize}
