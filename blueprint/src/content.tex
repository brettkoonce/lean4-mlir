...
\noindent Counts are \texttt{\#guard}ed against $50.22$\,M and $88.59$\,M. These
are the graph's times, one card at batch $32$, and T and B ran as controls and
returned their committed $157.8$ and $396.0$\,ms --- which is what makes S's
$1.39\times$ a measurement rather than a claim about itself.

\medskip\noindent \textbf{Renderer cost tracks how many dimensions move, not how
many parameters.} Depth is already a loop, so S --- twenty-two million
parameters heavier than T --- came free. B's widening cost twenty-seven threaded
literals for half as much again. That is the whole of what ``not the same job at
all'' means here, and it is the opposite of what a parameter count predicts.

\medskip\noindent \textbf{S is also the fastest of the three in bf16}, which
...
Totals include $37.5$\,s per epoch of evaluation and checkpointing.
...
not fix it.} At global $128$ this is the only net here running $10{,}009$ steps
an epoch, double every other, and that is what the $300$-epoch schedule really
pays for. The obvious remedy fails on measurement rather than on memory: the
block interior's cost per \emph{image} is flat from batch $8$ to $128$
($1.03\times$ across that whole range), so doubling the batch halves the step
count and doubles the step. Memory was never the constraint.
\S\ref{sec:vit_sb} asks the same question of ViT and gets the opposite answer,
because there the per-image cost does improve.

\medskip\noindent \textbf{Status: rendered, not trained.} Artifacts emit, shapes
tie, counts are guarded, forty steps run under each size in both precisions. No
accuracy, and the numeric ties against the JAX peers have not been run.

\medskip\noindent \textbf{Both are launchable} ---
...
survives is the sign that whatever ran out was never the graph, and the configs
carry the rest.

\medskip\noindent The phase-2 peers, at twice the batch and so half the steps,
take $71.6$, $108.8$ and $166.5$ hours compute-only --- so the ${\sim}2\times$
gap to the rows above is the batch, not the lowerer.
...
\begin{tabular}{@{}l c c r@{}}
\hline
model & $D$ / heads & params & 4 cards, $128$/dev: fp32 $\to$ bf16 \\
\hline
ViT-Ti & $192$ / 3  & $5.7$\,M         & $171.6 \to 96.4$\,ms \;($1.78\times$) \\
ViT-S  & $384$ / 6  & $22{,}050{,}664$ & $464.0 \to 245.5$\,ms \;($\mathbf{1.89\times}$) \\
ViT-B  & $768$ / 12 & $86{,}567{,}656$ & $1295.6 \to 745.3$\,ms \;($1.74\times$) \\
\hline
\end{tabular}
\end{center}
...
recipe's $5\times10^{-4}$ is the rate that batch was set for.

\medskip\noindent \textbf{B is the one net here that will not fit a default
allocator.} PJRT's CUDA plugin reserves three quarters of a card and B's fp32
graph wants more; raised to $0.97$ it runs at $93\%$ of what it then gets, and
left alone it dies on its first step. Everything else in the table fits either
way. That is the whole of why $128$ per device took four \texttt{\#eval} lines
rather than a renderer feature: the batch was never the obstacle.

\medskip\noindent \textbf{ViT-S's $1.89\times$ is the largest bf16 payoff in the
book}, ahead of ResNet-50's $1.66\times$ measured the same way. Tiny's matmuls
are narrow enough to underuse the tensor cores even in bf16; S's $D = 384$ suits
them; B pays a $346$\,MB \emph{fp32} all-reduce every step, a fixed cost in both
arms. The one op where bf16 \emph{loses} --- the stem weight gradient, at
$0.19\times$ for want of a bf16 kernel at a $209\times209$ window --- is kept
out of the emit at every size, which is why all three ratios exceed one.
...
dimension literals. \texttt{ViTRender.lean}, the per-example renderer, is still
pinned at Tiny by roughly $154$ of them, so there is no Imagenette demo at
either size.

\medskip\noindent \textbf{A full schedule.} Trainer steps, all measured on real
ImageNet over $40$ steps at four cards, $300$ epochs, totals including
...
\medskip\noindent \textbf{Status: rendered, not trained.} The only accuracy
calibration is this repo's ViT-Ti at $70.28\%$ against DeiT-Ti's $72.2\%$, a
$1.9$-point recipe gap (\S\ref{sec:vit_imagenet}). DeiT quotes $79.8\%$ for S
and $81.8\%$ for B; everything missing from that $1.9$ is \emph{regularization},
and S has four times Tiny's capacity, so the gap plausibly widens.

\medskip\noindent \textbf{Both are launchable} ---
\texttt{vits-\allowbreak default-\allowbreak g512-\allowbreak 4gpu} and
\texttt{vitb-\allowbreak default-\allowbreak g512-\allowbreak 4gpu}, the only
side quests in the book with job configs. They differ in one line: B's raises
the allocator fraction and its driver will not start without it, since the error
it prevents reads as a card being too small and B is the net where that reading
is wrong. S's does not, and an early version did --- it looked like free
headroom for a graph that already fits, and the same setting turns out to kill
ConvNeXt's bf16 arms outright (\S\ref{sec:convnext_sb}). Raise it for a graph
that does not otherwise fit; leave it alone for one that does.

\medskip\noindent The phase-2 peers, same box at global $512$, cost $25.8$,
$60.4$ and $146.1$ hours compute-only. \textbf{B is the only one that is not a
single forward}: it peaks at $11.41$ against that same default arena and dies,
so it runs as $4\times128$ accumulation. Whether it too fits once the arena is
raised has not been measured.
...
Sampling is then whatever you choose to integrate --- the reverse SDE with
Euler--Maruyama, or its deterministic twin the probability-flow ODE --- and the
choice is a sampler swap on frozen weights, not a different model.
...
\hline
DDIM                          & $\mathbf{0.0874}$ & $\mathbf{0.0437}$ & $\mathbf{0.0237}$ & $\mathbf{0.0188}$ \\
...
\noindent DDIM wins at every budget, and the reason is structural rather than
incidental. The reverse process is stiff --- $\beta(t)$ diverges as $t \to 1$
--- and DDIM integrates that linear part \emph{exactly}, where an explicit
solver has to resolve it numerically and its step size pays for it. The
Euler--Maruyama arm \emph{diverges} below $200$ evaluations (pixel mean $2.90$
against the data's $0.13$, two classes of ten covered) while posting the highest
confidence in this section, which is the previous paragraph's warning arriving
from a second direction.

That is a statement about this discretisation rather than about stochastic
sampling: the stable discrete form of the reverse SDE is ancestral sampling,
DDIM at $\eta = 1$, which is not run here.
