...
this network's result. Nothing has been run on the verified path since
the conversion, so the row below still has no measurement in it, and what
blocks one is unchanged by the conversion: there is no data-parallel
render, so the spec is single-device while every other ImageNet row in
this book was measured at $4\times$.
