...
chapter. We do it on a trivial network (one matrix multiply: the
MNIST linear classifier), because the network is not the point ---
building the whole machinery is. Once that machinery exists, every later
...
the chain closes. (The underlying forward and witness \texttt{def}s, the
...
the models but the author, who is answerable for the rest.
