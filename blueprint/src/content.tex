...
tfds streaming. What follows documents that trainer: how a paper-faithful
ImageNet recipe is assembled, and what each choice in it is for.

\textbf{Compute budget.} Four NVIDIA RTX 4060 Ti, consumer hardware,
CUDA/cuDNN, streaming through tfds with sharding via
\texttt{Mesh + NamedSharding(P('batch'))}. At global batch 256 in bf16 one
epoch over the 1.28M-image training set takes about $10.5$ minutes, so the
full 90-epoch schedule lands at roughly $16$ wall-clock hours.

The parameter checkpoint (\texttt{jax\_r34\_imagenet.bin}, 87\,MB, 37
param groups) is written via the same \texttt{params\_to\_file} emit every
other path in this book consumes, so weights move between implementations
byte-for-byte with no conversion step.
...
4$\times$ 4060 Ti (CUDA)  & fp32 & $\sim$18.6 min & 90 & $\sim$27.9 hr & --- & --- \\
...
label smoothing $0.1$. The completed row is
\texttt{runs/r34in\_verified\_30ep\_2026-08-05/}, and its evidence is that both
lowerers were run from the same initialization on the same box: at epoch~5 the
two sets of weights scored \textbf{17{,}312} and \textbf{17{,}313} correct of
49{,}920, one image apart. The proof-carrying tier stops at Imagenette, so the
phrasing is \emph{one architecture, two independent lowerings, agreeing}, never
``proven at ImageNet scale''.

\medskip\noindent The 90-epoch row is the paper schedule and is a
\texttt{[TODO]}. It runs with:
...
\noindent \texttt{LEAN\_MLIR\_EPOCHS} sets the schedule rather than capping
it, which matters because the cosine anneals over \texttt{cfg.epochs}: capping
a 30-epoch config at 90 would run 30 epochs, while setting 90 re-shapes the
decay. The 7900 XTX row is blank rather than pending, because a verified trainer
does not currently survive ROCm (11 deaths in 11 attempts on gfx1100 against 6
clean in 6 on CUDA). The repro is in
...
4$\times$ 4060 Ti (fp32)  & Ghost-BN (64) & ---            & ---           & --- & --- \\
\hline
\end{tabular}
\end{center}

\noindent The first row is RSB-A3 at 100 epochs. The second is the same
network on the \emph{2018} recipe at 90 epochs, which is a \texttt{[TODO]} and
is the row that makes the comparison below a controlled one: it is bit-for-bit
...
\noindent \texttt{momdp64} is the heavy-ball render at four replicas of 64,
the same global 256 the ResNet-34 run uses, and \texttt{BASE\_LR\_U=100000} is
peak $0.1$ in the driver's micro-unit encoding. The single-device
\texttt{mom256} peer exists too but does not fit: it wants 14.6\,GiB for the
device-to-host copy on a 16\,GB card.

\medskip\noindent The A3 run logged zero thermal duty-cycle cooldowns and
