...
The previous two chapters trained MNIST classifiers with SGD, and a
four-convolution net was plenty. CIFAR-10 --- color $32 \times 32$
images, a genuinely harder problem --- rewards going deeper, and depth
is exactly where BatchNorm earns its keep. So this chapter's demo
\textbf{doubles the depth}: an eight-convolution network (twice the
four-conv MNIST CNN), trained two ways --- with BN and without --- to
watch what BN does to the training curve.

The headline, stated up front so the graph below is no surprise:
\textbf{BN does not change where the network ends up; it changes how
fast it gets there.} Both nets converge to about the same test
accuracy. The BN net just reaches it in roughly half the epochs. BN is
an accelerator, not a capacity boost --- and this section measures
that directly, with the proof-rendered training step from this chapter
running on a GPU.
...
\subsection*{Training: same destination, BN gets there faster}

Both nets train with plain SGD at learning rate \textbf{0.02} for 40
epochs --- a stable rate at which \emph{both} learn, no schedule and no
tricks. Per-epoch test accuracy, captured from the verified runs
(\texttt{runs/cifar8\_verified\_lr0.02\_40ep.log} and its
\texttt{\_bn\_} sibling):

\begin{verbatim}
   no BN                       with BN
epoch  1:  31.6%            epoch  1:  35.0%
epoch  2:  35.0%            epoch  2:  42.3%    <- BN already +7
epoch  5:  47.6%            epoch  5:  52.9%
epoch 10:  53.4%            epoch 10:  58.4%    <- BN ~8 epochs ahead
epoch 20:  59.7%            epoch 20:  59.6%    <- the curves meet
epoch 40:  61.3%            epoch 40:  59.4%    <- no-BN nudges ahead
  best:    62.0%              best:    60.3%
\end{verbatim}

Neither net freezes; both climb to about the same place. The whole
difference is in the \emph{rate}. BN is already at 42\% after two
epochs (the no-BN net needs five), reaches the high-50s by epoch~10
(the no-BN net takes until about epoch~18), and has essentially
flattened by epoch~20. The no-BN net arrives at the same plateau a
dozen epochs later and, given the extra room, even edges slightly
ahead by the end. Same ceiling --- about 60\% --- and BN takes roughly
half the epochs to reach it.
...
\begin{axis}[
    ...
    xmin=0, xmax=41, ymin=30, ymax=65,
...
\addplot[blue, mark=*, mark options={fill=blue}] coordinates {
(1,34.97) (2,42.26) (3,48.91) (4,51.26) (5,52.94) (6,54.65) (7,56.20) (8,57.17) (9,57.31) (10,58.42) (11,58.79) (12,58.68) (13,58.87) (14,59.46) (15,58.89) (16,59.12) (17,59.09) (18,59.45) (19,59.44) (20,59.64) (21,59.65) (22,59.84) (23,59.79) (24,59.56) (25,60.05) (26,59.31) (27,59.63) (28,59.61) (29,59.80) (30,59.65) (31,59.51) (32,59.45) (33,58.37) (34,60.18) (35,59.75) (36,60.05) (37,59.46) (38,60.23) (39,60.29) (40,59.36)
};
\addlegendentry{with BN}
\addplot[orange, mark=*, mark options={fill=orange}] coordinates {
(1,31.60) (2,34.97) (3,40.43) (4,45.65) (5,47.60) (6,48.95) (7,50.29) (8,51.37) (9,51.42) (10,53.40) (11,54.16) (12,54.85) (13,55.28) (14,56.87) (15,57.11) (16,57.75) (17,58.21) (18,58.30) (19,58.94) (20,59.70) (21,59.88) (22,60.24) (23,60.17) (24,60.90) (25,59.86) (26,60.98) (27,60.85) (28,61.55) (29,60.99) (30,61.59) (31,62.02) (32,61.35) (33,61.58) (34,61.80) (35,61.31) (36,61.67) (37,61.36) (38,62.01) (39,61.46) (40,61.28)
};
...
\noindent CIFAR-10, 8-conv arch, SGD~0.02, 40 epochs, BN vs no-BN,
through the verified renderer
(\texttt{runs/cifar8\_\{,bn\_\}verified\_lr0.02\_40ep.log}). Both
curves climb to roughly the same accuracy; the BN curve is the same
shape shifted left.

The BN net leads from epoch one and is essentially done by epoch~20;
the no-BN net traces the same arc about a dozen epochs behind. Same
optimizer, same learning rate, same dataset, same architecture plus
the BN theorems we proved in this chapter. The three-term formula from
\S~\ref{thm:bnNormalize_has_vjp} is what buys the speed --- BN
standardizes each layer's input distribution (\(\mu = 0\),
\(\sigma = 1\)) and its backward pass returns a gradient that accounts
for how that standardization affected every other sample in the batch.
The result: gradients stay well-conditioned, every SGD step makes
reliable progress, and the network reaches its ceiling in half the
passes over the data.

\subsection*{Where the BN lift lives}

Why does normalizing buy speed? Each layer is tuned for the
distribution of its inputs --- but those inputs are the outputs of
every layer below, which are themselves changing every step. So each
layer chases a moving target: activations drift to scales where the
nonlinearity saturates or gradients blow up, and the step that helped
one layer wrecks the next. BN removes the moving part --- by pinning
every layer's input to mean-zero, unit-variance before the learnable
\(\gamma, \beta\) get a say, each layer always sees a well-conditioned
input no matter what the layers below just did. Ioffe \& Szegedy framed
this as reducing ``internal covariate shift''; later analysis
(Santurkar et al., 2018) argued the sharper reason is that
normalization \emph{smooths the loss landscape} --- the gradient stays
predictable from one step to the next. Either way, a smoother landscape
means each SGD step makes more reliable progress, and reliable progress
compounds across epochs: that is precisely the left-shifted curve the
graph measures.

BN is not buying capacity. Both specs top out around the same accuracy
--- the no-BN net gets there too, just later. What BN buys is the
\emph{rate}: the same destination in roughly half the epochs. (On
larger networks, and with true batch-statistic normalization, the
effect is stronger still --- it also opens up learning rates that are
unstable without it --- but at this depth and this stable rate, what we
can honestly measure and verify is the speed, so that is the claim we
make here.)

That speed is why every image architecture since 2015 bakes a
normalization layer in by default: on a multi-day ImageNet run,
reaching the same accuracy in half the passes over the data is the
difference between experimentally tractable and not. The proof suite we
built in this chapter is what mechanically guarantees the BN backward
computes the correct VJP at every one of those steps --- so the curve
above rests on a machine-checked claim, not a trusted kernel.
