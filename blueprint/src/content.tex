...
\section{The five layers, summarized}

Together, the five layers catch five different kinds of bugs:
...
\begin{itemize}
  ...
  \item \textbf{Verified code generation} (\S~\ref{sec:verified_codegen})
    fails if the StableHLO the GPU actually runs denotes something other
    than the proven derivative: the bridge theorems pin each emitted
    operator to its proof, and the IREE-versus-NumPy oracle confirms the
    graph executes correctly on hardware. This is the one layer about the
    \emph{emitted code} rather than the mathematics behind it --- the gap
    the other four sit upstream of.
\end{itemize}
