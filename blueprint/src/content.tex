...
The per-chapter detail lives where the work does --- each chapter names
its contracts as it proves them --- and how the claims are
\emph{checked} (the audits, the bridge theorems, the hardware oracles)
is Appendix~\ref{app:verification}'s subject. It reads most naturally
once you've been through the Foundations chapter, so it's safe to skip
on a first pass.
...
nothing else --- no thumb on the scale. Appendix~\ref{app:verification}
shows how that claim is independently audited.
...
GPU & Precision & Per epoch & Epochs & Total & Val top-1 & Val top-5 \\
\hline
6$\times$ 4060 Ti (CUDA) & bf16 & $\sim$7.5 min & 90 (cosine) & $\sim$11 hr & $68.77\%$ & $88.53\%$ \\
...
discussed below. Its per-epoch time is \emph{lower} than the
90-epoch run's despite using two fewer GPUs: that run predated the
pinned-environment rebuild, and the newer cuDNN selects better
bf16 kernels for the $1\times1$ convolutions than the stack the
six-GPU run used.)
...
The 90-epoch run above is a validation tier, and it is not merely a
shorter version of the paper recipe --- it differs on three rows at
once. The \texttt{full} recipe fixes all three: label smoothing
$0.1\to0$ (Sandler et al.\ used none), cosine $\to$ the paper's
exponential decay ($\times0.98$ per epoch), and $90\to350$ epochs.

The third change is what forces the first two. Under
...
collected by then. That is precisely why the short tier substituted
cosine: the paper's schedule is not a sensible thing to truncate. Only
over $350$ epochs does the exponential reach
$0.045 \times 0.98^{345} \approx 4.2\times10^{-5}$ and behave as
intended.

Run end to end on four 4060 Ti it lands at $\mathbf{71.44\%}$ top-1
/ $\mathbf{90.34\%}$ top-5 on the full 50{,}000-image validation
split --- $\mathbf{+2.67}$ over the 90-epoch tier, and
$\mathbf{0.56}$ points short of the paper's $72.0\%$:
...
Read against the 90-epoch curve above, the two tell the story of
the schedule directly. At epoch $90$ this run is at $66.31\%$ ---
$2.5$ points \emph{behind} the cosine tier at the same epoch,
because it is deliberately still hot. It passes the short tier's
final number around epoch $120$, and everything after that is
territory the 90-epoch schedule could not reach at all. The last
$150$ epochs are worth only $\sim0.7$ points, though: the curve is
flat from about epoch $250$, so the gap that remains is no longer
schedule.

That residual $0.56$ is the honest part of the result. The
hypothesis this chapter carried --- that the whole $-3.23$ gap was
training budget --- is now measured, and it was \emph{mostly} right:
schedule bought $2.67$ of the $3.23$. What is left is small enough
to sit inside the known recipe deviations rather than needing a new
explanation. Two are documented and shared across every net in this
book: the resize is not antialiased on either the train or eval path
(timm's is; worth $\sim0.2$ points), and the running-BN decay is
fixed at $0.99$ where PyTorch's default works out to $0.9$. Neither
is worth a retrain of the sweep to chase.
...
decay, no smoothing, 350 epochs) is the $71.44\%$ measured earlier in
this section, and it settles the expectation those rows carried: the
gap was training budget, not model or optimizer, for $2.67$ of its
$3.23$ points.
...
initialization behind --- and since a random network and a trained one
are not joined by a low-loss path, even a few percent of it puts the
average at \emph{chance}, not merely behind. Conv-M trips this where
longer runs do not: gradient accumulation leaves it only $312$ steps
per epoch, so $31{,}200$ total against a time constant of $10{,}000$.
The fix is TensorFlow's, which this family's reference implementations
assume --- ramp the decay, $d_t = \min\!\big(d,\ (1+t)/(10+t)\big)$.

\texttt{mobilenet-v4-imagenet} ships two tiers: \textbf{100 epochs}
(recipe \texttt{default} --- a reduced-regularization confidence
tier: wd $0.05$, dropout $0.1$, RandAugment m9, since the paper's
full pack underfits short schedules) and \textbf{500 epochs}
(recipe \texttt{full} --- the paper recipe end to end). The
100-epoch tier is run here; the $79.9\%$ headline is a 500-epoch
number:
...
\noindent The 100-epoch tier lands at $\mathbf{75.48\%}$ top-1 /
...
$350$, $16$ wall-clock hours against $45$. It also clears
ConvNeXt-T's 80-epoch $75.93\%$-class territory at a third of the
parameters, which is the comparison the UIB template is really
making: by 2024 the mobile line is not trading accuracy for size so
...
The tier above is \emph{detuned} by construction --- wd $0.05$ against
the paper's $0.1$, dropout $0.1$ against $0.2$, RandAugment $m9$
against $m15$ --- so its $75.48\%$ is not a reproduction of
$79.9\%$. Its accuracy is flat over the last ten epochs
...
(BraTS)} carries segmentation to real medical imaging --- four-modality
brain-tumour MRI, where the thin-class collapse a Pets trimap merely tolerates
becomes the entire problem (enhancing tumour is $\sim$0.5\,\% of voxels). Two
trainers read it: \texttt{unet-brats-train} (a from-scratch UNet at native
$240 \times 240$) and \texttt{unet-brats-r34} (the ResNet-34 encoder of
\S\ref{sec:bestiary_segmentation}, which needs the $224$ centre-cropped build
since its stride-32 backbone does not divide 240); \textbf{tiny
...
decoder-only GPT (\texttt{tinystories}). Detection then leaves the
household frame entirely: \textbf{VisDrone-DET2019} is drone-altitude
imagery where the objects are the problem --- a median box is $20 \times
25$\,px in the source frame, some 70 of them per image, so the single
$7\times7$ grid that suffices for Pets head boxes collapses outright and
the multi-scale FPN head of Definition~\ref{layer:fpnModule} becomes
necessary rather than decorative. \textbf{MS-COCO 2017} is the standard
detection benchmark alongside it, at the opposite end of the same axis
(photographic scale, 80 classes, 118k training images); the repository
ships its loader and the two standard subsets described below, though no
trainer in this book consumes it yet. Same fetch-and-unpack story as
...
tokenizer auditable. The two detection sets follow the same pattern:
\texttt{preprocess\_visdrone.py} and \texttt{preprocess\_coco.py} write
\emph{the same three record formats} --- single-grid, single-scale anchor,
and multi-scale FPN --- so one set of Lean loaders and one codegen serve
...
values untouched. It is also lossless here --- the 8-pixel border it removes
contains no brain voxels, these volumes being skull-stripped and centred.
...
\smallskip\noindent Two preprocessing details matter more than they look.
First, \textbf{COCO's category ids are not contiguous}: they run 1--90 with
ten gaps (12, 26, 29, 30, 45, 66, 68, 69, 71, 83) around 80 real classes, so
the obvious \texttt{id\,-\,1} remap silently shifts every class above the
first gap --- a relabelling that trains and evaluates without ever raising
an error. \texttt{preprocess\_coco.py} therefore derives the map from the
annotation file's own \texttt{categories} list rather than hardcoding it.
Second, \texttt{iscrowd} regions are skipped, exactly as VisDrone's
\texttt{score\,=\,0} ignored regions are, because COCO's own evaluation
ignores them.

\smallskip\noindent\textbf{The class-count ceiling.} Of the three C loaders
these formats share, two bake the class count into their record stride ---
\texttt{lean\_f32\_load\_voc\_dims} computes \texttt{30 * gridH * gridW}
($2\cdot5+20$) and \texttt{lean\_f32\_load\_voc\_anchor} computes
\texttt{A * 15} ($5+10$) --- while \texttt{lean\_f32\_load\_voc\_fpn} takes
the target length \texttt{ntot} as a parameter and is class-agnostic. So
\textbf{full 80-class COCO is FPN-only} on today's C code, and the
preprocessor refuses the other two formats at 80 classes rather than
writing a file that would be misparsed stride-wise and trained on as
garbage. Two standard subsets fit the narrower loaders: \texttt{--classes
voc20} (the 20 PASCAL-VOC categories, all present in COCO under slightly
different names) for the single-grid format, and \texttt{--classes vdmap},
which emits COCO directly into \emph{VisDrone's} 10-class index space so a
COCO-pretrained head transfers unchanged --- six of VisDrone's ten classes
have a COCO counterpart (person, bicycle, car, truck, bus, motorcycle) and
the remaining four stay empty.

\smallskip\noindent Both detection preprocessors write an \emph{uncapped}
ground-truth sidecar (\texttt{val.full\_gt.bin}) next to \texttt{val.bin},
in the same loop and therefore in the same record order. The training
record caps boxes at \texttt{MAX\_BBOXES}\,=\,56, which is harmless for
training --- a $14\times14$ grid encodes far fewer --- but scoring mAP
against a truncated ground truth is not the protocol of either benchmark,
so the scorer reads the sidecar instead. \emph{References:} Lin et al.,
...
\textbf{Cross-checked} (empirical): what cannot be proven --- IREE's
lowering, GPU transcendentals, \texttt{float32} rounding beyond its
theorem layer (\S~\ref{sec:float32}) --- is watched by independent
oracles that must agree before the hardware is believed. Each kind
catches what the others structurally cannot; this appendix walks the
mechanisms, grouped by kind.
...
architecture definitions in the bestiary, across 199 Lean proof files,
with zero project axioms. The axiom audit
(\texttt{tests/AuditAxioms.lean}) re-checks \textbf{1{,}457 theorems} in
all --- the VJP contracts plus the forward-graph faithfulness, render,
cotangent-chain, backward-tie, and \texttt{float32} results
(\S~\ref{sec:float32}) --- every one closing under the same three
core axioms: \(\texttt{\#print axioms vit\_full\_has\_vjp}\) shows only
Lean core (\texttt{propext}, \texttt{Classical.choice},
\texttt{Quot.sound}), nothing project-level beneath those. (Earlier
drafts axiomatized shortcuts for the kinked operators; every one has
...
graph accompanying this blueprint currently shows roughly a hundred nodes --- one
...
The proofs above answer whether the \emph{mathematics} is right --- the
...
training step, forward through backward through the parameter update --- and
that file is the printout of one graph whose denotation a faithfulness theorem
...
that site, threaded through the real forward activations --- each update one
composed equation rooted at the proven forward, not a quantifier over a free
cotangent. All twelve chapter networks are tied this way, from the MNIST
...
\section{Cross-checked: the hardware agrees}

Because IREE cannot itself be proven, and GPU transcendentals have no
IEEE specification, the last stretch --- from proven math to executed
...
the result against an independent NumPy reference --- the one check that
reaches past the proofs to IREE's lowering itself. And the
...
the run stays inside the float theorems' hypotheses --- no flipped ReLU
mask, no logit drift past \(\delta\) (\S~\ref{sec:float32}). It is
what keeps the conditional theorems honest about actual training rather
than hypothetical nets.

\bigskip

Each kind fails differently --- a deductive proof is blind to a
miscompile, an empirical diff is blind to a measure-zero kink, a
faithful bridge is blind to a wrong formula it renders perfectly --- and
overlaying them is the guarantee. Verified code generation straddles two
of the kinds: its bridge is structural, its oracle empirical. The
\texttt{float32} theorem layer, next, straddles the other pair: its
