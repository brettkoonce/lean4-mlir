...
On one 7900 XTX (ROCm, gfx1100), from
\texttt{runs/2026-08-11-linear-verified-xla/}. XLA's startup banner is
the only thing removed:

\begin{verbatim}
[pjrt_ffi] XLA backend: PJRT 0.112, 1 device(s)
[pjrt_ffi] compiled verified_mlir/linear_train_step.mlir
             (@linear_train_step, 2 outputs, 1 replica) in 782 ms
[pjrt_ffi] compiled verified_mlir/linear_fwd.mlir
             (@linear_fwd, 1 outputs, 1 replica) in 23 ms
MNIST-Linear via the VERIFIED renderer (pretty∘emit) → XLA/PJRT → GPU
  train 60000, test 10000; dense 784->10, bs 128, SGD
  epoch 1: test_acc = 8977/10000 = 89.770000% (306ms)
  epoch 2: test_acc = 9082/10000 = 90.820000% (240ms)
  epoch 3: test_acc = 9124/10000 = 91.240000% (241ms)
  epoch 4: test_acc = 9156/10000 = 91.560000% (257ms)
  epoch 5: test_acc = 9167/10000 = 91.670000% (270ms)
  epoch 6: test_acc = 9176/10000 = 91.760000% (271ms)
  epoch 7: test_acc = 9186/10000 = 91.860000% (271ms)
  epoch 8: test_acc = 9194/10000 = 91.940000% (272ms)
  epoch 9: test_acc = 9198/10000 = 91.980000% (270ms)
  epoch 10: test_acc = 9198/10000 = 91.980000% (269ms)
  epoch 11: test_acc = 9203/10000 = 92.030000% (268ms)
  epoch 12: test_acc = 9210/10000 = 92.100000% (272ms)
done (trained MNIST-Linear via the proof-rendered StableHLO).
\end{verbatim}

Twelve epochs at about 270\,ms each, and 92.10\% on the full
...
Here is the part that matters. That run did not execute a
...
same recipe to 98.57\%. That gap of about 6.5 points is the value of
...
Here is the real thing, abridged. Elisions are marked, and the full
function is \texttt{VerifiedNet.trainLinear}:
...
def VerifiedNet.trainLinear (net : VerifiedNet) (cfg : VerifiedConfig)
  ...
  let d1 := net.nClasses
    ...
    -- 1. One session per committed render. The second path is a legacy
  --    artifact-cache slot that XLA does not use.
  ...
  for ep in [0:cfg.epochs] do
    ...
    ...                       -- argmax the fwd session over evalImg
    IO.println s!"  epoch {ep+1}: test_acc = {correct}/{nEval} = ..."
\end{verbatim}
...
XLA compiles the StableHLO in process, which is the 782\,ms the run
reported, and the second argument is a legacy artifact-cache slot it does
not use.

Two names in this listing are historical residue rather than description.
The session type is called \texttt{IreeSession} and the cache slot ends
in \texttt{.vmfb}, both from an earlier backend. They are on the list to
rename. What the code opens is the XLA shim.
...
startup, in 782\,ms, and every batch after that is a single dispatched
call. Chapter~1's run does 468 of those per epoch and finishes an epoch
in about 270\,ms including a full pass over the 10,000 test images.

\textbf{5. Validation.} Every epoch, not every tenth, because a forward
pass over the test set is cheap next to the training epoch. The
...
threaded through the real forward, and each agrees on GPU to float rounding. There is one open gap. The theorems are over $\mathbb{R}$ and the GPU
runs \texttt{float32}. Part~II's bestiary is deliberately lighter,
