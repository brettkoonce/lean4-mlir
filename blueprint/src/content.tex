...
If you're planning how much of the book to read, here's the size of what
each chapter \emph{proves about gradients}. Each proves a handful of
\emph{VJP contracts} --- machine-checked theorems that a layer's backward
function computes the right gradient (formally
\(\texttt{backward}\,x\,dy\,i = \sum_j \pdiv f\,x\,i\,j \cdot dy_j\): the
backward equals that layer's \(\pdiv\)-Jacobian contracted against the
incoming cotangent, which the Foundations chapter builds up from scratch).
Across Chapters~1--9 there are \textbf{40 contracts} --- one per layer,
operator, and whole-network architecture --- plus \textbf{8 witnesses}:
concrete end-to-end instances (a specific net at specific weights), kept
in their own column as sanity checks rather than citable contracts. Even
Chapter~1's toolkit is used straight away by a proven model, so no chapter
is pure scaffolding.
...
That's the ladder at a glance. The per-chapter detail --- what each
contract actually is, how the smooth nets close at \emph{full}
architecture, and a dependency-spine figure tracing every network back to
the Chapter~1 calculus --- is rear-loaded into
Appendix~\ref{app:verification} (\S\ref{sec:chapter_census}). It reads
most naturally once you've been through the Foundations chapter, so it's
...
\(\partial\mathcal{L}/\partial\theta\) --- so before a single layer, we
need a rigorous, machine-checkable notion of ``the derivative of this
function at this point.''

Mathlib supplies it as the Fréchet derivative, \(\fderiv\): the
\emph{best linear approximation} --- the continuous linear map \(D\)
with \(f(x+h) \approx f(x) + D\,h\) --- defined over any normed space,
not just \(\R^n\). For a layer \(f : \Vect{n} \to \Vect{m}\),
...
\(\pdiv\,f\,x\,i\,j = \big(\fderiv\,\R\,f\,x\,(e_i)\big)_j\) --- apply
\(\fderiv\,\R\,f\,x\) to the \(i\)-th basis vector \(e_i\) and take
component \(j\). So the Jacobian and the gradient \(\nabla\mathcal{L}\)
...
Because \(\pdiv\) is \emph{defined} from \(\fderiv\) rather than
assumed, the structural rules --- chain, sum, product --- are theorems
proved from Mathlib's API, not axioms we ask you to trust. Earlier
drafts of this book took the other route and axiomatized those eight
rules as opaque facts; the current foundation flips it, and every
later-chapter axiom has been pruned the same way: \(\texttt{conv2d}\),
multi-head SDPA, patch embedding, the BN inverse-stddev broadcast,
GELU, the softmax Jacobian, and every transformer-level composition
chain are defined concretely and proved from the foundation. The cost
is mild --- a \(\Differentiable\) hypothesis propagates through each
chapter. The benefit is that
\(\texttt{\#print axioms vit\_full\_has\_vjp}\) bottoms out at Lean core
(\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}) and
nothing else --- no thumb on the scale. See
Appendix~\ref{app:verification} for the full elimination history.
...
\prosesection{The MLP wastes information; we can do better}
...
1$\times$ MI300X (ROCm)   & bf16 & $\sim$3.5 min & 90 & $\sim$5.4 hr & $\mathbf{73.95\%}$\textsuperscript{\dag} & $\mathbf{91.76\%}$ \\
\hline
\end{tabular}
\end{center}

\noindent ResNet-34 / ImageNet-1k: per-epoch throughput and final
validation for the completed runs (ROCm at 30 epochs, CUDA at the full
90; the MI300X row is a single-GPU batch-1024 run on gfx942). The A100
row is a wall-clock estimate, accuracies pending.
\textsuperscript{\dag}\,MI300X used batch 1024 / lr 0.4 (linear
large-batch scaling) vs batch 256 / lr 0.1 for the other rows, so its
$+1.9$-point top-1 over the 4060\,Ti bf16 row is the large-batch scaling,
not the hardware; a matched batch-256 run for an apples-to-apples
hardware comparison is still to do.
...
1$\times$ A100 (bf16, true-2048) & full-batch (2048) & $\sim$6.3 min & 100 & $\sim$10.5 hr & $\mathbf{77.00\%}$ & $\mathbf{93.15\%}$ \\
1$\times$ MI300X (bf16, true-2048) & full-batch (2048) & --- & 100 & $\sim$7.2 hr\textsuperscript{\dag} & --- & --- \\
\hline
\end{tabular}
\end{center}
...
\texttt{true-2048} row normalizes over the full $2048$. The
...
\textsuperscript{\dag}\,MI300X row is a projected single-device rerun
($\sim$7.2\,hr, no gradient accumulation); its per-epoch time and
accuracies are pending the run.

\textbf{What the A100 row tests.} Removing the Ghost-BN approximation
barely moves the needle: \texttt{true-2048} lands at $77.00\%$ against
Ghost-BN's $77.22\%$ --- a $\sim$0.2-point difference, and slightly
\emph{lower}, well inside run-to-run noise. Normalizing BatchNorm over
the full $2048$ neither helps nor hurts, so the BN-sharding regime is
\emph{not} where the residual $\sim$1 point to the paper's $78.1\%$
hides.

\textbf{Where it does hide: the resize.} That gap turns out to be an
evaluation artifact, not a training one. Both rows here are scored
through this repo's \texttt{tf.image.resize} preprocessing, whereas the
timm reference reports $78.1\%$ through PIL's bicubic resize --- and the
two resamplers are not interchangeable. Port these weights into a
standard PIL-resize eval and they fall to $\sim$74.4\%; port the
reference \texttt{a3\_in1k} weights \emph{into this pipeline} and
\emph{they} fall to $\sim$74\%. The ranking flips with the resize
library: each network scores best on the resampler it trained with,
because it partly fits that resampler's specific aliasing. So the honest
reading is that the two recipes sit within a couple of points, and which
one ``wins'' is decided by the eval pipeline --- not that this run is a
point short.

\textbf{[TODO --- resize/eval reconciliation.]} Put \texttt{true-2048}
and the timm reference on one ruler --- a single fixed resampler, or a
bit-exact \texttt{tf.image.resize} match in the reference eval --- before
reading anything into the $77.00\%$ vs $78.1\%$ gap.

\emph{Changed since the $76.66\%$ first run:} RandAugment is now
timm-literal (\texttt{rand-m6-mstd0.5-inc1}).
...
\begin{theorem}[Depthwise weight VJP]
  ...
  \leanok
  \uses{ax:depthwiseConv2d, ax:pdiv_add, ax:pdiv_const, ax:pdiv_mul, thm:pdiv_finset_sum, ax:pdiv_reindex, def:hasvjp3}
  Phase 7. \texttt{DepthwiseKernel} is definitionally
...
GPU & Precision & Per epoch & Epochs & Total & Val top-1 & Val top-5 \\
\hline
6$\times$ 4060 Ti (CUDA) & bf16 & $\sim$7.5 min & 90 & $\sim$11 hr & $\mathbf{68.77\%}$ & $\mathbf{88.53\%}$ \\
4$\times$ 4060 Ti (CUDA) & bf16 & --- & 350 & $\sim$46 hr & --- & --- \\
\hline
\end{tabular}
\end{center}

\noindent (The 350-epoch row is the paper-faithful full tier ---
the [TODO] below --- wall-clock projected, accuracies pending.)
The full 90-epoch run --- now on MobileNetV2's native
...
side of a fork: Google's TensorFlow/Inception training tradition ---
RMSProp with a huge $\varepsilon$, an exponential LR staircase, a
...
side by side --- and the base that the next chapter's 2019
EfficientNet recipe is built on. The \texttt{.train} entry point is
identical; every row is a one-line \texttt{NetSpec} or
\texttt{TrainConfig} change:
...
\hline
Backbone      & basic block, 21.8\,M   & inverted residual, 3.5\,M     & depthwise $3\times3$ at the wide point between $1\times1$s --- $1/6$ the parameters \\
Optimizer     & SGD + momentum $0.9$   & RMSProp                       & $\rho{=}0.9$, $\mu{=}0.9$, $\varepsilon{=}1.0$ --- the huge $\varepsilon$ keeps the adaptive step damped (B0 shrinks it to $10^{-3}$ and pays for it) \\
...
Epochs        & 90                     & $\sim$300--400                & the long-haul schedule; most of the $-3.2$-point gap above is this \\
Weight decay  & $10^{-4}$, all params  & $4\times10^{-5}$              & depthwise filters are 9 weights per channel --- $10^{-4}$ over-regularizes them \\
Label smoothing & $0.1$                & none                          & Sandler et al.\ used no smoothing at all \\
Augmentation  & RRC + hflip            & RRC + hflip                   & unchanged --- no learned policy, no mixing; the leanest recipe in this book \\
\hline
\end{tabular}
\end{center}
...
One honesty note: the $68.77\%$ run above is the 90-epoch validation
tier and predates two of these rows --- it ran the cosine schedule
with label smoothing $0.1$. The paper-faithful full tier (exponential
decay, no smoothing, 350 epochs) is the [TODO] above; the expectation
is that the remaining $\sim$3 points are training budget, not model
or optimizer.

The through-line of this fork: nothing here regularizes harder ---
...
depthwise positions optional --- one before the expansion, one after
it. Four instantiations fall out of that one template: both
depthwise convs on (\emph{ExtraDW}), only the leading one
(\emph{ConvNeXt} --- this chapter's sandwich rearranged into
Chapter~\ref{chap:layernorm}'s block shape), neither (a plain
\emph{FFN} --- two $1\times1$s), or only the inner one (exactly
...
\texttt{NetSpec} --- and the same \texttt{.train} entry point as
every other net in this book. What changed from 2018 to 2024 is at
...
Paper top-1   & $72.0\%$               & $79.9\%$                      & non-distilled; distillation pushes the family higher still \\
...
Weight decay  & $4\times10^{-5}$       & $0.1$, skip norm/bias         & $2500\times$ stronger, applied selectively --- AdamW makes heavy decay usable \\
Augmentation  & RRC + hflip            & + RandAugment (N2, m15, p0.7) & heavy learned augmentation; still no Mixup/CutMix for Conv-M \\
Label smoothing & none                 & $0.1$                         & standard by 2024 \\
Stochastic depth & none                & drop-path $0.075$             & linear ramp over the 21 UIB blocks \\
EMA           & none                   & decay $0.9999$                & weight averaging (with the BN buffers shadowed, per Ch~\ref{chap:se}'s lesson) \\
\hline
\end{tabular}
\end{center}
...
\textbf{[TODO --- run w/ config.]} \texttt{mobilenet-v4-imagenet},
two tiers: \textbf{100 epochs} (recipe \texttt{default} --- a
reduced-regularization confidence tier: wd $0.05$, dropout $0.1$,
RandAugment m9, since the paper's full pack underfits short
schedules) and \textbf{500 epochs} (recipe \texttt{full} --- the
paper recipe end to end). Expect low-to-mid 70s from the 100-epoch
tier; the $79.9\%$ headline is a 500-epoch number. A throughput
bench puts Conv-M at $\sim$9 minutes per epoch on the four-GPU box:
...
GPU & Precision & Per epoch & Epochs & Total & Val top-1 & Val top-5 \\
\hline
4$\times$ 4060 Ti (CUDA) & bf16 & $\sim$9 min & 100 & $\sim$15 hr & --- & --- \\
4$\times$ 4060 Ti (CUDA) & bf16 & $\sim$9 min & 500 & $\sim$75 hr & --- & --- \\
\hline
\end{tabular}
\end{center}

\noindent (Wall-clock projected from the measured per-epoch
throughput; accuracies pending the runs. The 500-epoch tier is the
longest single run this book has queued --- roughly the ConvNeXt-T
300-epoch run's $\sim$75 hours, at a third the per-epoch cost over
$1.7\times$ the epochs.)
...
\hline
6$\times$ 4060 Ti & 80 (SGD) & $\sim$8.0 min & $\sim$10.5 hr & $72.31\%$ & $90.31\%$ \\
4$\times$ 4060 Ti & 350 (RMSProp) & $\sim$8.6 min & $\sim$55.5 hr & $\mathbf{76.80\%}$ & $\mathbf{93.26\%}$ \\
\hline
\end{tabular}
\end{center}

\noindent (CUDA, bf16. Two recipes: the 80-epoch SGD validation tier,
and the faithful 350-epoch RMSProp full run --- the paper recipe end to
end, discussed below.) The 80-epoch run reached
$\mathbf{72.31\%}$ top-1 / $\mathbf{90.31\%}$ top-5 on the full
50{,}000-image validation split --- a strong result for this tier,
edging ResNet-34's $72.0\%$ at roughly a quarter of the parameters
($5.3$M vs $21.8$M). It sits under EfficientNet's headline ${\sim}77\%$
because it is a stripped validation tier: SGD, crop/flip only, 80 epochs.
The faithful recipe --- RMSProp, AutoAugment, stochastic depth, EMA,
350 epochs --- closes that gap (the 350-epoch row above, curve and story
below), but getting RMSProp to train at all took two bug fixes. The
80-epoch validation curve:
...
with. Two differences --- $\varepsilon$ added \emph{outside} the square
root ($\sqrt{\mathrm{sq}}+\varepsilon$) rather than inside
($\sqrt{\mathrm{sq}+\varepsilon}$), and the mean-square accumulator
initialized to $0$ rather than $1$ --- inflate the effective step by up
to $\sim$30$\times$ when the running mean-square is small, which is
exactly the erosion observed. This also explains the MobileNet-V2
...
weight EMA (decay $0.9999$) on, evaluation blew up --- validation loss to
$10^{8}$ --- because the EMA shadowed only the weights, and eval then
...
the paper recipe end to end: RMSProp at $0.016$, AutoAugment, stochastic
depth, EMA, exponential LR decay --- reached $\mathbf{76.80\%}$ top-1 /
$\mathbf{93.26\%}$ top-5, against EfficientNet-B0's paper
$77.1\%\,/\,93.3\%$: within $0.3\%$ top-1 and essentially exact on top-5.
A faithful, from-scratch reproduction with no hacks --- real RMSProp,
real LR, real EMA. It ran $\sim$55.5\,hr on four 4060 Ti (350 epochs plus
eleven 30-minute thermal rests, zero AER). The validation curve --- note
the deliberate slow start, the weight EMA at $0.9999$ lagging near-init
weights until $\sim$epoch 8 before catching up sharply:
...
live weights, not a training failure; final $76.80\%\,/\,93.26\%$ matches
the paper's $77.1\%\,/\,93.3\%$.
...
\texttt{.train} entry point is identical; every row is a one-line
\texttt{NetSpec} or \texttt{TrainConfig} change:
...
Epochs        & 90                     & 350                           & the long-schedule regime the regularizers need --- $+4.5$ top-1 over the 80-epoch validation tier \\
...
fp32) --- and, unlike every later recipe in this ladder, no gradient
clipping.
...
depth, and averaged weights supplying the regularization --- and a
...
measures that claim directly --- a CutMix-vs-bare-vs-RandAugment
ablation lifts ConvNeXt-T's Imagenette accuracy by 2.9 points
without touching a single weight --- and finishes by putting the
full recipe side-by-side with the 2018 ResNet-34 recipe of
...
\begin{itemize}
  ...
  \item \textbf{84.94\% is below EnetB0's 87.58\%} on the same
    ...
    where the modernization actually lives. The Data Augmentation
    section below layers those in one at a time --- with the right
    knob, ConvNeXt-T's accuracy at this scale passes EnetB0.
...
\label{sec:convnext_aug_ablation}
...
Every element of the paper recipe is now wired and on: the faithful
patchify stem (\texttt{.convNextStem}: $4\times4$ stride-4 conv $\to$
channel-LayerNorm, no BN/ReLU), \textbf{stochastic depth} (drop-path 0.1)
and \textbf{EMA} (decay 0.9999) --- together ${<}5\%$ per epoch ---
\textbf{decoupled weight decay that excludes the norm/bias/LayerScale
1-D params} (\texttt{wdExcludeNormBias}, timm's \texttt{no\_weight\_decay}),
and the \textbf{full DeiT-style augmentation pack}: Mixup $\alpha0.8$,
CutMix $\alpha1.0$, Random Erasing $p0.25$, and geometric RandAugment
($N{=}2$, $M{=}9$, \texttt{mstd0.5-inc1}). The peak LR is
$2.5\mathrm{e}{-}4$ (the $4\mathrm{e}{-}3$@4096 official value linearly
scaled to batch 256) over a 20-epoch warmup. No architectural or recipe
deviation from the paper remains --- the only knob still short of the
headline is the schedule: the run below is 80 epochs, the validation tier
of an $80\to300$ ladder.
...
per epoch --- the most expensive per-epoch of any net in this book,
...
\textbf{thermal duty cycle} --- a 30-minute cooldown after epochs 30 and
60, each resuming bit-for-bit from a full-state checkpoint (params $+$
optimizer $+$ EMA $+$ step, so the cosine schedule and Adam moments are
...
GPU & Epochs & Per epoch & Wall-clock & Val top-1 & Val top-5 \\
\hline
4$\times$ 4060 Ti & 80 & $\sim$14.9 min & $\sim$20 hr$^{\dagger}$ & $\mathbf{78.13\%}$ & $\mathbf{94.05\%}$ \\
4$\times$ 4060 Ti & 300 & $\sim$14.9 min & $\sim$75 hr$^{\ddagger}$ & $\mathbf{81.10\%}$ & $\mathbf{95.37\%}$ \\
\hline
\end{tabular}
\end{center}

\noindent ($^{\dagger}$CUDA, bf16; $\sim$20\,hr pure training ---
$\sim$21\,hr actual wall-clock (measured $19{:}35\to16{:}37$, $21$\,h
$01$\,m) including the two 30-minute thermal rests. $^{\ddagger}$The
300-epoch run measured $\sim$75\,hr pure training / $\sim$80\,hr actual
wall-clock across nine thermal rests plus one benign watchdog restart --- a
corrected CPU-cache event mis-read as a PCIe fault, with no real hardware
fault the entire run.)
The 80-epoch run reached $\mathbf{78.13\%}$ top-1 / $\mathbf{94.05\%}$
top-5 on the held-out validation pass --- the strongest result in this
book's sweep by a widening margin, ahead of EfficientNet-B0 ($72.3\%$)
and ResNet-34 ($72.1\%$). It also clears the prior base-augmentation
80-epoch run ($75.93\%$) by $+2.2\%$: the full DeiT pack, weight-decay
masking, and 20-epoch warmup are now all in, and the only remaining gap
to ConvNeXt-T's $\sim$82\% headline is the 300-epoch schedule, not the
architecture or the recipe. The validation curve tells the augmentation
story directly --- a deliberately slow start (Mixup and CutMix blend
labels, so the net under-trains for the first $\sim$20 epochs, compounded
by the EMA shadow lagging the live weights), then a climb that, unlike
the base-augmentation run, never plateaus:
...
(bf16, 4$\times$ 4060 Ti, full DeiT augmentation pack). The two
30-minute thermal rests (after epochs 30 and 60) are invisible here:
each resumes bit-for-bit, so accuracy is continuous across them.
...
top-1 climbed monotonically across all 80 epochs with no erosion or
...
\textbf{300-epoch canonical run --- done.} The full tier
(\texttt{convnext-tiny-imagenet full}, \texttt{convNeXtTinyImagenetConfigFull}:
identical config to the $78.13\%$ run above, only \texttt{epochs := 300})
reached $\mathbf{81.10\%}$ top-1 / $\mathbf{95.37\%}$ top-5 --- $+2.97\%$
top-1 over the 80-epoch run, and within $\sim$1\% of ConvNeXt-T's $82.1\%$
paper headline, from a from-scratch Lean$\to$JAX-codegen reimplementation.
The 80-epoch curve's refusal to plateau was the right tell: the extra 220
epochs of cosine decay converted directly into accuracy rather than
overfitting, exactly as the heavy-augmentation regime predicts. The
residual $\sim$1\% is the usual EMA-eval / test-crop / augmentation-detail
territory, not the architecture or the recipe. Wall-clock $\sim$80\,hr on
the four clean GPUs across nine duty-cycle rests, with zero real hardware
faults the entire run.
...
resuming bit-for-bit. Against the 80-epoch curve above, the extra 220
epochs of cosine decay are pure gain.
...
2018$\to$2022 jump with the block modernized in step --- and since
``the recipe is the thesis'' is this architecture's whole argument,
the diff below is the chapter in table form. The \texttt{.train}
entry point is identical; every row is a one-line \texttt{NetSpec} or
\texttt{TrainConfig} change:
...
Epochs        & 90                     & 300 (80 = validation tier)    & heavy regularization converts the extra schedule straight into accuracy ($+2.97$ top-1 from 80$\to$300) \\
...
Augmentation  & RRC + hflip            & full DeiT pack                & RandAugment + Mixup + CutMix + Random Erasing --- where the modernization actually lives (Section~\ref{sec:convnext_aug_ablation}) \\
...
The augmentation row is the full DeiT pack that
Section~\ref{sec:convnext_aug_ablation} measured knob by knob at
Imagenette scale. A small-data practitioner cloning this recipe
should reach for CutMix first and treat the rest as optional.
...
\begin{theorem}[Multi-head SDPA VJP]
  ...
  Phase 3 (Apr 2026), de-axiomatized Phase 8.
...
\begin{theorem}[Patch-embedding VJP]
  ...
  Phase 6b (Apr 2026): de-opaqued in 6a --- the concrete \texttt{def}
  unfolds to conv-with-stride + CLS prepend + positional embed.
...
\nolinkurl{image-net.org} download is account-gated and slow; in practice
the two LSVRC-2012 tarballs are most easily fetched from Academic
Torrents
...
and 50{,}000 validation images across 64 shards (6.74\,GB) --- the exact
layout every ImageNet trainer in this book consumes. Budget disk
...
pipeline --- parallel shard reads, JPEG decode, prefetch, all engineered
to scale --- carries the loading, and we inherit that performance work
for free.
...
head boxes tiled into $2 \times 2$ mosaics); \textbf{MSD Task01
(BraTS)} carries the same UNet to real medical imaging
(\texttt{unet-brats-train}) --- four-modality brain-tumour MRI, where the
thin-class collapse a Pets trimap merely tolerates becomes the entire
problem (enhancing tumour is $\sim$0.5\,\% of voxels); \textbf{tiny
Shakespeare} --- a single 1\,MB character stream --- is what the TinyGPT
char-level transformer (\texttt{tinygpt-shakespeare}) trains on; and
\textbf{TinyStories} --- Eldan \& Li's synthetic short-story corpus,
$\sim$2\,GB --- is its scaled-up sibling, byte-level-BPE tokenized to a
deliberately small 4096-token vocabulary for the $\sim$8.5\,M-parameter
decoder-only GPT (\texttt{tinystories}). Same fetch-and-unpack story as
the table above, with one extra step: each is tokenized or tiled into the
trainer's input format by the matching \texttt{preprocess\_*.py} script
before the first \texttt{lake exe} run.
...
TinyStories & TS & Eldan \& Li, 2023 &
  ...
  \texttt{download\_tinystories.sh} $\to$ \texttt{data/tinystories/} \\
\hline
\end{tabular}
\end{center}
...
download feeds both the segmenter and the detector. For TinyStories,
...
token streams (\texttt{train.bin}/\texttt{val.bin}); the small vocabulary
keeps the in-graph one-hot embedding and head matmuls cheap and the
tokenizer auditable.
\normalsize

\medskip\noindent\textbf{Building the BraTS dataset.} Brain-tumour
segmentation has two public faces, and the choice between them is a
reproducibility decision. \emph{BraTS~2021 proper} (1{,}251 cases) is
gated behind a free Synapse account \emph{and} a signed research-use
agreement, so no script can fetch it --- a demo whose first step is ``go
register on Synapse'' is a demo readers do not run. The \textbf{Medical
...
7.6\,GB tarball over unauthenticated S3. That is what
\texttt{download\_brats.sh} fetches --- and it splits the file into
parallel ranged \texttt{curl}s, since S3 throttles per connection, taking
the download from $\sim$3\,h single-stream to $\sim$12\,min. The cost of
the choice, stated plainly: 484 volumes rather than 1{,}251, and numbers
that are \emph{not} comparable to a BraTS~2021 leaderboard --- this is a
real segmentation result on real medical data that any reader can
reproduce, not a challenge entry.

\smallskip\noindent Two preprocessing details are load-bearing.
\texttt{preprocess\_brats.py} ships a \textbf{dependency-free NIfTI-1
reader} (numpy only): a \texttt{pip install nibabel} is a reader-hostile
first step under PEP~668's externally-managed-environment lock, so the
$\sim$40-line header parser is cross-validated against nibabel~5.4.2 on
nine cases (column-major, big-endian, \texttt{scl\_slope}/\texttt{scl\_inter})
instead. And the two decisions that would silently ruin the numbers if
done the obvious way: intensities are z-scored over \emph{brain voxels
only} (BraTS volumes are skull-stripped, so $\sim$83\,\% of each is exact
zero and whole-volume statistics would crush the tissue contrast), and
the train/val split is \emph{by patient, not by slice} (adjacent axial
slices of one brain are near-duplicates, so a slice-level split leaks
memorized neighbours across the boundary). Volumes are quantized to
uint8 over a $\pm5\sigma$ window and the C loader inverts it, matching
...
\paragraph{How much lives below the named layer.} The dependency
graph accompanying this blueprint currently shows 106 nodes --- one
per named theorem or definition, hand-linked by
\texttt{\textbackslash uses} annotations. The compiled environment is
bigger: just over 5{,}000 project constants once compiler-generated
names are filtered out. That gap is not padding; it is the substrate
the named layer stands on. Measured mechanically (with a
constant-level dependency explorer; see the repo's
\texttt{atlas-integration} branch), the ViT capstone
\texttt{vit\_body\_has\_vjp\_mat} alone transitively depends on 101
project constants --- and only 25 of those are named theorems in this
book. The other 76 are the working parts a reader never needs by
name: \texttt{Mat} and its flatten/unflatten round-trip lemmas, the
\texttt{HasVJP}/\texttt{HasVJPMat} structures themselves, forward
functions, reindexing glue. Every one of the 101 type-checks
sorry-free on the same three-axiom trust kernel as the named layer
--- the audits above make no distinction between spine and substrate,
which is the point: the blueprint's curation is pedagogical, not
epistemic.

\section{What each chapter proves}
\label{sec:chapter_census}

This expands the contract count from ``How this book is organized'' into
the per-chapter detail. Chapter by chapter, here is what each network
contributes --- the \textbf{40 contracts} and \textbf{8 witnesses}
summarized in that chapter's table, now spelled out.
...
stem) with its committed-config variant.
