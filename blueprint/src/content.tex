...
\medskip\noindent The 7900 XTX row carries a projected wall-clock for the
two-card ROCm box and no accuracies, and the blanks there are not waiting on
scheduling. A verified trainer does not currently survive a run on ROCm: a
command-buffer fault kills it, at 11 deaths in 11 attempts on gfx1100 against 6
clean runs in 6 on CUDA at the identical JAX version. The repro and the upstream
report are in
\texttt{upstream-issues/2026-08-jax-rocm-command-buffer-launchgraph-segv/}. The
row stays because the schedule is worth knowing and because the fault is
somebody's bug to fix rather than a property of the architecture, but until it
is fixed those two columns cannot be filled.
...
zero PCIe/AER interruptions. There is no ROCm peer row here for the same
reason the one in \S\ref{sec:r34_phase4} is empty, which is the
command-buffer fault recorded there. Every verified number in this chapter is
CUDA.
