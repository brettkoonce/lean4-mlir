...
not the full $2048$; a single big-memory card would hold the whole batch and
normalize over all $2048$ at once. Same network, same effective
batch. That is the BN statistic group, and it is the one axis on which these
two runs are not directly comparable to \texttt{timm}'s own:
...
Each wall-clock includes three 30-minute thermal duty-cycle cooldowns --- the
4060\,Ti box is duty-cycled on long runs --- and the A3 run logged zero
PCIe/AER interruptions across all 100 epochs.

\medskip\noindent The first row is the \textbf{2018 recipe} on the same
network; its verified peer remains a \texttt{[TODO]}. It is what makes
the recipe comparison symmetric: both paths get both recipes, so a difference
...
\hline
4$\times$ 4060 Ti (fp32)  & 2018 (90 ep)    & Ghost-BN (64) & \withheld      & \withheld     & \withheld & \withheld \\
4$\times$ 4060 Ti (fp32)  & RSB-A3 (100 ep) & Ghost-BN (64) & $\sim$19.2 min & $\sim$32.1 hr & $\mathbf{77.91\%}$ & $\mathbf{93.84\%}$ \\
\hline
\end{tabular}
\end{center}

\noindent The first row is the same network on the \emph{2018} recipe, a
...
\noindent Three things bite on that run, all of them silent, all three
measured on its ResNet-34 twin in 2026 August. \emph{Rebuild the binary
first} --- an executable older than the timm-protocol change reproduces the
pre-re-run number with nothing in the log to say so. \emph{Clear any
checkpoint from a different schedule} --- resuming a shorter run's state
into the 90-epoch cosine fuses two learning-rate curves.
And note \texttt{LEAN\_MLIR\_EPOCHS} \emph{sets} the schedule where
\texttt{LEAN\_MLIR\_MAX\_EPOCHS} only caps it, so the latter cannot reach
this tier from a shorter default; \texttt{totalSteps} is what the cosine
anneals over. \texttt{scripts/jobs/r34-imagenet-90ep-4gpu.conf} encodes all
three as a refusing PRECHECK and is the template to copy.

\textbf{How to quote this.} It lands $0.35$ points behind the JAX
...
these deltas}, rather than ``RSB-A3 reproduced''.
...
\medskip\noindent\emph{Now in, and in the $77.91\%$ above.} Each changes
the trajectory, which is why the earlier $77.43\%$ is superseded rather than
comparable: the evaluation, the network and two optimizer terms all moved
between them.
...
\begin{itemize}
  ...
  \item \textbf{LAMB's gradient clip} (timm's \texttt{max\_grad\_norm}
    $= 1.0$ default). The superseded $77.43\%$ was produced without it;
    the $77.91\%$ has it. Applied after the all-reduce \emph{and} after the
...
what the trainers emit today. \textbf{Chapter~\ref{chap:residual} has been
re-run against it} --- both ResNet-34 and ResNet-50, on both the phase-2 and
the verified paths --- and its numbers score all $50{,}000$ validation images.
Every other ImageNet accuracy in this book predates the protocol and was
measured under the older preprocessing, so those numbers and this description
are one re-run out of step. Those sections say so in one line at their head;
a section with no such line has been re-run.
