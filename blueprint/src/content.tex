...
\hline
Chapter & VJP contracts & Cumulative & New primitive / result \\
\hline
2 (Foundation)         & 4 & 4  & \texttt{mnistLinear} (linear classifier), dense weight/bias-grad, softmax-CE \\
3 (MNIST: 1D)          & 4 & 8  & ReLU, MLP ($+$ smooth-point \texttt{\_at}) \\
4 (MNIST: 2D)          & 3 & 11 & Conv, MaxPool ($+$ \texttt{\_at}) \\
5 (CIFAR: BN)          & 1 & 12 & BatchNorm input-gradient \\
6 (ResNet-34)          & 6 & 18 & Residual ($+$proj, $+$\texttt{\_at}), global-avg-pool, whole-net \texttt{cnn} \\
7 (MobileNetV2)        & 2 & 20 & Depthwise, whole-net \texttt{mobilenetv2} \\
8 (EfficientNet)       & 4 & 24 & swish, SE (mult. fan-in), sigmoid, whole-net \texttt{efficientnet} \\
9 (ConvNeXt)           & 4 & 28 & GELU, LayerNorm, layer-scale, whole-net \texttt{convnext} \\
10 (ViT)               & 6 & 34 & matrix kit, Attention (SDPA/MHSA/block), whole-net \texttt{vit\_full} \\
\hline
                       & \textbf{34} & & \\
\hline
\end{tabular}
