...
\textbf{Compute budget.} On the four RTX 4060 Ti (CUDA, bf16, batch
256 across the four GPUs), steady-state throughput is
$\sim$102\,ms per step, about $8.9$ minutes per epoch --- so the
80-epoch validation tier projects to roughly 12 wall-clock hours,
and the full 300-epoch recipe to about 45 hours (a touch under two
days).
...
\hline
4$\times$ 4060 Ti (CUDA) & bf16 & $\sim$8.9 min & $\sim$12 hr & $\sim$45 hr & TODO & TODO \\
\hline
\end{tabular}
\end{center}

\noindent (Throughput is measured from steady-state ms/step; the run
is not yet trained to completion, so accuracy is pending.
EfficientNet's headline ${\sim}77\%$ comes from a far longer schedule
with RMSProp, AutoAugment, stochastic depth, and EMA. Of those, EMA
(decay 0.9999) and stochastic depth (drop-connect 0.2) are now on, and
cost ${<}5\%$ per epoch; RMSProp and the geometric half of RandAugment
remain off. So this 80-epoch tier validates the regularizers train
cleanly before the 300-epoch run; it will still sit under the paper
number until the schedule lengthens and the aug pack comes on.)
