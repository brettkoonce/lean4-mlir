...
fast and store it in half the memory, which is why every network from
Chapter~\ref{chap:residual} onward is trained that way. The question this
chapter would like to answer is whether that costs any accuracy.

\medskip\noindent It cannot answer it, and the reason is worth one
paragraph. The renderer has two convolution op families --- the
per-example one these MNIST and CIFAR chapters use, and the batched one
the ImageNet chapters use --- and bf16 twins exist only for the batched
family. BatchNorm has no bf16 twin in either. So bf16 here can only be
measured on the \emph{un-normalized} net, and against a baseline in a
different op family than the fp8 arm. Here is the one comparison
that is clean --- same net, same batched family, same constant rate, five
seeds, fp32 against bf16:
...
\hline
SGD (lr 0.1)   & fp32 & 71.31 & 70.74 & 70.04 & 71.13 & 69.02 & 70.74 \\
               & bf16 & 69.14 & 68.89 & 70.09 & 69.34 & 70.72 & 69.34 \\[2pt]
momentum       & fp32 & 71.14 & 73.45 & 74.25 & 74.21 & 73.85 & 73.85 \\
               & bf16 & \emph{10.0} & 73.29 & 72.14 & 74.47 & 74.41 & 73.29 \\[2pt]
AdamW          & fp32 & 71.56 & 74.23 & 72.76 & 73.53 & 73.70 & 73.53 \\
               & bf16 & 73.44 & 74.00 & 72.23 & 72.99 & 73.90 & 73.44 \\
\hline
\end{tabular}
\end{center}

\noindent Medians differ by $-1.40$, $-0.56$ and
$-0.09$, and every one of those is smaller than the spread
\emph{within} the corresponding fp32 row. One bf16 momentum seed
collapsed to chance; no fp32 seed did, and one run in five is not enough
to say whether that is bf16 or the un-normalized net, which the previous
section showed is the fragile part.

\medskip\noindent For scale, changing the op family at fixed fp32 ---
per-example to batched, the same arithmetic throughout --- moves the
answer by $+2.0$ on SGD and $+1.7$ on momentum. The lever we did not mean
to pull is bigger than the one we did.

\medskip\noindent What that licenses is narrow but real: bf16 introduces
no failure this net does not already have in fp32 --- the \texttt{NaN}
losses of the previous section appear in both, and in the same column ---
and where the comparison is clean at all it costs between nothing and a
point and a half, on cells whose own run-to-run range is wider than that.
It does not license the
claim Chapter~\ref{chap:residual} actually needs --- that a
\emph{normalized} net trains the same in bf16 --- and that measurement
waits on a bf16 BatchNorm render, which is where the next chapter starts.
An fp8 (E4M3) arm exists in the repository and is left out here for the
same reason: its render is a lowering probe with forward convolutions
only, not a trainable arm.
