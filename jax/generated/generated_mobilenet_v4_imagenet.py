...
def _imagenet_decode_random_crop_flip(image_bytes):
    ...
    img = _randaugment(img, 2, 9.000000, 0.000000)
    return img
...
def forward(params, x, bn, training, drop_key=None):
    ...
    x, _ne = uib_block(params, x, 3, 2, 3, 5, bn, bn_i, training)
    bn_out.extend(_ne); bn_i += len(_ne)
    x, _ne = uib_block(params, x, 7, 1, 3, 3, bn, bn_i, training)
    bn_out.extend(_ne); bn_i += len(_ne)
    x, _ne = uib_block(params, x, 11, 2, 3, 5, bn, bn_i, training)
    bn_out.extend(_ne); bn_i += len(_ne)
    x, _ne = uib_block(params, x, 15, 1, 3, 3, bn, bn_i, training)
    bn_out.extend(_ne); bn_i += len(_ne)
    x, _ne = uib_block(params, x, 19, 1, 3, 3, bn, bn_i, training)
    bn_out.extend(_ne); bn_i += len(_ne)
    x, _ne = uib_block(params, x, 23, 1, 3, 5, bn, bn_i, training)
    bn_out.extend(_ne); bn_i += len(_ne)
    x, _ne = uib_block(params, x, 27, 1, 3, 3, bn, bn_i, training)
    bn_out.extend(_ne); bn_i += len(_ne)
    x, _ne = uib_block(params, x, 31, 1, 3, 0, bn, bn_i, training)
    bn_out.extend(_ne); bn_i += len(_ne)
    x, _ne = uib_block(params, x, 34, 1, 0, 0, bn, bn_i, training)
    bn_out.extend(_ne); bn_i += len(_ne)
    x, _ne = uib_block(params, x, 36, 1, 3, 0, bn, bn_i, training)
    bn_out.extend(_ne); bn_i += len(_ne)
    x, _ne = uib_block(params, x, 39, 2, 5, 5, bn, bn_i, training)
    bn_out.extend(_ne); bn_i += len(_ne)
    x, _ne = uib_block(params, x, 43, 1, 5, 5, bn, bn_i, training)
    bn_out.extend(_ne); bn_i += len(_ne)
    x, _ne = uib_block(params, x, 47, 1, 3, 5, bn, bn_i, training)
    bn_out.extend(_ne); bn_i += len(_ne)
    x, _ne = uib_block(params, x, 51, 1, 3, 5, bn, bn_i, training)
    bn_out.extend(_ne); bn_i += len(_ne)
    x, _ne = uib_block(params, x, 55, 1, 0, 0, bn, bn_i, training)
    bn_out.extend(_ne); bn_i += len(_ne)
    x, _ne = uib_block(params, x, 57, 1, 3, 0, bn, bn_i, training)
    bn_out.extend(_ne); bn_i += len(_ne)
    x, _ne = uib_block(params, x, 60, 1, 3, 5, bn, bn_i, training)
    bn_out.extend(_ne); bn_i += len(_ne)
    x, _ne = uib_block(params, x, 64, 1, 5, 5, bn, bn_i, training)
    bn_out.extend(_ne); bn_i += len(_ne)
    x, _ne = uib_block(params, x, 68, 1, 0, 0, bn, bn_i, training)
    bn_out.extend(_ne); bn_i += len(_ne)
    x, _ne = uib_block(params, x, 70, 1, 0, 0, bn, bn_i, training)
    bn_out.extend(_ne); bn_i += len(_ne)
    x, _ne = uib_block(params, x, 72, 1, 5, 0, bn, bn_i, training)
    bn_out.extend(_ne); bn_i += len(_ne)
    ...
    if drop_key is not None:
        x = x * jax.random.bernoulli(jax.random.fold_in(drop_key, 999983), 0.900000, x.shape).astype(x.dtype) / 0.900000
...
if __name__ == "__main__":
    ...
    if _trace_f:
        _hdr = {
            ...
            'config': {
                'lr': 0.004000, 'batch_size': 512, 'epochs': 100,
                'use_adam': True,
