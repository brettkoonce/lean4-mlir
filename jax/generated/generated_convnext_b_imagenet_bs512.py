"""ConvNeXt-B (ImageNet, bf16) — Generated by Lean 4 → JAX
Architecture: CNXStem(3→128,4x4)+LN → ConvNeXt3(c=128) → CNXDown(128→256) → ConvNeXt3(c=256) → CNXDown(256→512) → ConvNeXt27(c=512) → CNXDown(512→1024) → ConvNeXt3(c=1024) → GAP → LN(1024) → 1024→1000
Parameters: 88591464
...
def init_params_from_file(path):
    ...
    idx = 0
    params = []
    # CNXStem conv 4x4 3→128 (W, b)
    W = jnp.array(buf[idx:idx+6144].reshape(128, 3, 4, 4)); idx += 6144
    b = jnp.array(buf[idx:idx+128]); idx += 128
    params.append((W, b))
    # CNXStem LN 128 (γ, β)
    gamma = jnp.array(buf[idx:idx+128]); idx += 128
    beta = jnp.array(buf[idx:idx+128]); idx += 128
    params.append((gamma, beta))
    # ConvNeXt[0] DW 7x7 128ch (W, b)
    W = jnp.array(buf[idx:idx+6272].reshape(128, 1, 7, 7)); idx += 6272
    b = jnp.array(buf[idx:idx+128]); idx += 128
    params.append((W, b))
    # ConvNeXt[0] LN 128 (γ, β)
    gamma = jnp.array(buf[idx:idx+128]); idx += 128
    beta = jnp.array(buf[idx:idx+128]); idx += 128
    params.append((gamma, beta))
    # ConvNeXt[0] PW expand 128→512 (W, b)
    W = jnp.array(buf[idx:idx+65536].reshape(512, 128, 1, 1)); idx += 65536
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[0] PW project 512→128 (W, b)
    W = jnp.array(buf[idx:idx+65536].reshape(128, 512, 1, 1)); idx += 65536
    b = jnp.array(buf[idx:idx+128]); idx += 128
    params.append((W, b))
    # ConvNeXt[0] LayerScale 128 (γ,)
    g = jnp.array(buf[idx:idx+128]); idx += 128
    params.append((g,))
    # ConvNeXt[1] DW 7x7 128ch (W, b)
    W = jnp.array(buf[idx:idx+6272].reshape(128, 1, 7, 7)); idx += 6272
    b = jnp.array(buf[idx:idx+128]); idx += 128
    params.append((W, b))
    # ConvNeXt[1] LN 128 (γ, β)
    gamma = jnp.array(buf[idx:idx+128]); idx += 128
    beta = jnp.array(buf[idx:idx+128]); idx += 128
    params.append((gamma, beta))
    # ConvNeXt[1] PW expand 128→512 (W, b)
    W = jnp.array(buf[idx:idx+65536].reshape(512, 128, 1, 1)); idx += 65536
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[1] PW project 512→128 (W, b)
    W = jnp.array(buf[idx:idx+65536].reshape(128, 512, 1, 1)); idx += 65536
    b = jnp.array(buf[idx:idx+128]); idx += 128
    params.append((W, b))
    # ConvNeXt[1] LayerScale 128 (γ,)
    g = jnp.array(buf[idx:idx+128]); idx += 128
    params.append((g,))
    # ConvNeXt[2] DW 7x7 128ch (W, b)
    W = jnp.array(buf[idx:idx+6272].reshape(128, 1, 7, 7)); idx += 6272
    b = jnp.array(buf[idx:idx+128]); idx += 128
    params.append((W, b))
    # ConvNeXt[2] LN 128 (γ, β)
    gamma = jnp.array(buf[idx:idx+128]); idx += 128
    beta = jnp.array(buf[idx:idx+128]); idx += 128
    params.append((gamma, beta))
    # ConvNeXt[2] PW expand 128→512 (W, b)
    W = jnp.array(buf[idx:idx+65536].reshape(512, 128, 1, 1)); idx += 65536
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[2] PW project 512→128 (W, b)
    W = jnp.array(buf[idx:idx+65536].reshape(128, 512, 1, 1)); idx += 65536
    b = jnp.array(buf[idx:idx+128]); idx += 128
    params.append((W, b))
    # ConvNeXt[2] LayerScale 128 (γ,)
    g = jnp.array(buf[idx:idx+128]); idx += 128
    params.append((g,))
    # CNXDown LN 128 (γ, β)
    gamma = jnp.array(buf[idx:idx+128]); idx += 128
    beta = jnp.array(buf[idx:idx+128]); idx += 128
    params.append((gamma, beta))
    # CNXDown conv 2x2 128→256 (W, b)
    W = jnp.array(buf[idx:idx+131072].reshape(256, 128, 2, 2)); idx += 131072
    b = jnp.array(buf[idx:idx+256]); idx += 256
    params.append((W, b))
    # ConvNeXt[0] DW 7x7 256ch (W, b)
    W = jnp.array(buf[idx:idx+12544].reshape(256, 1, 7, 7)); idx += 12544
    b = jnp.array(buf[idx:idx+256]); idx += 256
    params.append((W, b))
    # ConvNeXt[0] LN 256 (γ, β)
    gamma = jnp.array(buf[idx:idx+256]); idx += 256
    beta = jnp.array(buf[idx:idx+256]); idx += 256
    params.append((gamma, beta))
    # ConvNeXt[0] PW expand 256→1024 (W, b)
    W = jnp.array(buf[idx:idx+262144].reshape(1024, 256, 1, 1)); idx += 262144
    b = jnp.array(buf[idx:idx+1024]); idx += 1024
    params.append((W, b))
    # ConvNeXt[0] PW project 1024→256 (W, b)
    W = jnp.array(buf[idx:idx+262144].reshape(256, 1024, 1, 1)); idx += 262144
    b = jnp.array(buf[idx:idx+256]); idx += 256
    params.append((W, b))
    # ConvNeXt[0] LayerScale 256 (γ,)
    g = jnp.array(buf[idx:idx+256]); idx += 256
    params.append((g,))
    # ConvNeXt[1] DW 7x7 256ch (W, b)
    W = jnp.array(buf[idx:idx+12544].reshape(256, 1, 7, 7)); idx += 12544
    b = jnp.array(buf[idx:idx+256]); idx += 256
    params.append((W, b))
    # ConvNeXt[1] LN 256 (γ, β)
    gamma = jnp.array(buf[idx:idx+256]); idx += 256
    beta = jnp.array(buf[idx:idx+256]); idx += 256
    params.append((gamma, beta))
    # ConvNeXt[1] PW expand 256→1024 (W, b)
    W = jnp.array(buf[idx:idx+262144].reshape(1024, 256, 1, 1)); idx += 262144
    b = jnp.array(buf[idx:idx+1024]); idx += 1024
    params.append((W, b))
    # ConvNeXt[1] PW project 1024→256 (W, b)
    W = jnp.array(buf[idx:idx+262144].reshape(256, 1024, 1, 1)); idx += 262144
    b = jnp.array(buf[idx:idx+256]); idx += 256
    params.append((W, b))
    # ConvNeXt[1] LayerScale 256 (γ,)
    g = jnp.array(buf[idx:idx+256]); idx += 256
    params.append((g,))
    # ConvNeXt[2] DW 7x7 256ch (W, b)
    W = jnp.array(buf[idx:idx+12544].reshape(256, 1, 7, 7)); idx += 12544
    b = jnp.array(buf[idx:idx+256]); idx += 256
    params.append((W, b))
    # ConvNeXt[2] LN 256 (γ, β)
    gamma = jnp.array(buf[idx:idx+256]); idx += 256
    beta = jnp.array(buf[idx:idx+256]); idx += 256
    params.append((gamma, beta))
    # ConvNeXt[2] PW expand 256→1024 (W, b)
    W = jnp.array(buf[idx:idx+262144].reshape(1024, 256, 1, 1)); idx += 262144
    b = jnp.array(buf[idx:idx+1024]); idx += 1024
    params.append((W, b))
    # ConvNeXt[2] PW project 1024→256 (W, b)
    W = jnp.array(buf[idx:idx+262144].reshape(256, 1024, 1, 1)); idx += 262144
    b = jnp.array(buf[idx:idx+256]); idx += 256
    params.append((W, b))
    # ConvNeXt[2] LayerScale 256 (γ,)
    g = jnp.array(buf[idx:idx+256]); idx += 256
    params.append((g,))
    # CNXDown LN 256 (γ, β)
    gamma = jnp.array(buf[idx:idx+256]); idx += 256
    beta = jnp.array(buf[idx:idx+256]); idx += 256
    params.append((gamma, beta))
    # CNXDown conv 2x2 256→512 (W, b)
    W = jnp.array(buf[idx:idx+524288].reshape(512, 256, 2, 2)); idx += 524288
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[0] DW 7x7 512ch (W, b)
    W = jnp.array(buf[idx:idx+25088].reshape(512, 1, 7, 7)); idx += 25088
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[0] LN 512 (γ, β)
    gamma = jnp.array(buf[idx:idx+512]); idx += 512
    beta = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((gamma, beta))
    # ConvNeXt[0] PW expand 512→2048 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(2048, 512, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+2048]); idx += 2048
    params.append((W, b))
    # ConvNeXt[0] PW project 2048→512 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(512, 2048, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[0] LayerScale 512 (γ,)
    g = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((g,))
    # ConvNeXt[1] DW 7x7 512ch (W, b)
    W = jnp.array(buf[idx:idx+25088].reshape(512, 1, 7, 7)); idx += 25088
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[1] LN 512 (γ, β)
    gamma = jnp.array(buf[idx:idx+512]); idx += 512
    beta = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((gamma, beta))
    # ConvNeXt[1] PW expand 512→2048 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(2048, 512, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+2048]); idx += 2048
    params.append((W, b))
    # ConvNeXt[1] PW project 2048→512 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(512, 2048, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[1] LayerScale 512 (γ,)
    g = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((g,))
    # ConvNeXt[2] DW 7x7 512ch (W, b)
    W = jnp.array(buf[idx:idx+25088].reshape(512, 1, 7, 7)); idx += 25088
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[2] LN 512 (γ, β)
    gamma = jnp.array(buf[idx:idx+512]); idx += 512
    beta = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((gamma, beta))
    # ConvNeXt[2] PW expand 512→2048 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(2048, 512, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+2048]); idx += 2048
    params.append((W, b))
    # ConvNeXt[2] PW project 2048→512 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(512, 2048, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[2] LayerScale 512 (γ,)
    g = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((g,))
    # ConvNeXt[3] DW 7x7 512ch (W, b)
    W = jnp.array(buf[idx:idx+25088].reshape(512, 1, 7, 7)); idx += 25088
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[3] LN 512 (γ, β)
    gamma = jnp.array(buf[idx:idx+512]); idx += 512
    beta = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((gamma, beta))
    # ConvNeXt[3] PW expand 512→2048 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(2048, 512, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+2048]); idx += 2048
    params.append((W, b))
    # ConvNeXt[3] PW project 2048→512 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(512, 2048, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[3] LayerScale 512 (γ,)
    g = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((g,))
    # ConvNeXt[4] DW 7x7 512ch (W, b)
    W = jnp.array(buf[idx:idx+25088].reshape(512, 1, 7, 7)); idx += 25088
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[4] LN 512 (γ, β)
    gamma = jnp.array(buf[idx:idx+512]); idx += 512
    beta = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((gamma, beta))
    # ConvNeXt[4] PW expand 512→2048 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(2048, 512, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+2048]); idx += 2048
    params.append((W, b))
    # ConvNeXt[4] PW project 2048→512 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(512, 2048, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[4] LayerScale 512 (γ,)
    g = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((g,))
    # ConvNeXt[5] DW 7x7 512ch (W, b)
    W = jnp.array(buf[idx:idx+25088].reshape(512, 1, 7, 7)); idx += 25088
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[5] LN 512 (γ, β)
    gamma = jnp.array(buf[idx:idx+512]); idx += 512
    beta = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((gamma, beta))
    # ConvNeXt[5] PW expand 512→2048 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(2048, 512, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+2048]); idx += 2048
    params.append((W, b))
    # ConvNeXt[5] PW project 2048→512 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(512, 2048, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[5] LayerScale 512 (γ,)
    g = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((g,))
    # ConvNeXt[6] DW 7x7 512ch (W, b)
    W = jnp.array(buf[idx:idx+25088].reshape(512, 1, 7, 7)); idx += 25088
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[6] LN 512 (γ, β)
    gamma = jnp.array(buf[idx:idx+512]); idx += 512
    beta = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((gamma, beta))
    # ConvNeXt[6] PW expand 512→2048 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(2048, 512, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+2048]); idx += 2048
    params.append((W, b))
    # ConvNeXt[6] PW project 2048→512 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(512, 2048, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[6] LayerScale 512 (γ,)
    g = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((g,))
    # ConvNeXt[7] DW 7x7 512ch (W, b)
    W = jnp.array(buf[idx:idx+25088].reshape(512, 1, 7, 7)); idx += 25088
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[7] LN 512 (γ, β)
    gamma = jnp.array(buf[idx:idx+512]); idx += 512
    beta = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((gamma, beta))
    # ConvNeXt[7] PW expand 512→2048 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(2048, 512, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+2048]); idx += 2048
    params.append((W, b))
    # ConvNeXt[7] PW project 2048→512 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(512, 2048, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[7] LayerScale 512 (γ,)
    g = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((g,))
    # ConvNeXt[8] DW 7x7 512ch (W, b)
    W = jnp.array(buf[idx:idx+25088].reshape(512, 1, 7, 7)); idx += 25088
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[8] LN 512 (γ, β)
    gamma = jnp.array(buf[idx:idx+512]); idx += 512
    beta = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((gamma, beta))
    # ConvNeXt[8] PW expand 512→2048 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(2048, 512, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+2048]); idx += 2048
    params.append((W, b))
    # ConvNeXt[8] PW project 2048→512 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(512, 2048, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[8] LayerScale 512 (γ,)
    g = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((g,))
    # ConvNeXt[9] DW 7x7 512ch (W, b)
    W = jnp.array(buf[idx:idx+25088].reshape(512, 1, 7, 7)); idx += 25088
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[9] LN 512 (γ, β)
    gamma = jnp.array(buf[idx:idx+512]); idx += 512
    beta = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((gamma, beta))
    # ConvNeXt[9] PW expand 512→2048 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(2048, 512, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+2048]); idx += 2048
    params.append((W, b))
    # ConvNeXt[9] PW project 2048→512 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(512, 2048, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[9] LayerScale 512 (γ,)
    g = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((g,))
    # ConvNeXt[10] DW 7x7 512ch (W, b)
    W = jnp.array(buf[idx:idx+25088].reshape(512, 1, 7, 7)); idx += 25088
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[10] LN 512 (γ, β)
    gamma = jnp.array(buf[idx:idx+512]); idx += 512
    beta = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((gamma, beta))
    # ConvNeXt[10] PW expand 512→2048 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(2048, 512, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+2048]); idx += 2048
    params.append((W, b))
    # ConvNeXt[10] PW project 2048→512 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(512, 2048, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[10] LayerScale 512 (γ,)
    g = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((g,))
    # ConvNeXt[11] DW 7x7 512ch (W, b)
    W = jnp.array(buf[idx:idx+25088].reshape(512, 1, 7, 7)); idx += 25088
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[11] LN 512 (γ, β)
    gamma = jnp.array(buf[idx:idx+512]); idx += 512
    beta = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((gamma, beta))
    # ConvNeXt[11] PW expand 512→2048 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(2048, 512, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+2048]); idx += 2048
    params.append((W, b))
    # ConvNeXt[11] PW project 2048→512 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(512, 2048, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[11] LayerScale 512 (γ,)
    g = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((g,))
    # ConvNeXt[12] DW 7x7 512ch (W, b)
    W = jnp.array(buf[idx:idx+25088].reshape(512, 1, 7, 7)); idx += 25088
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[12] LN 512 (γ, β)
    gamma = jnp.array(buf[idx:idx+512]); idx += 512
    beta = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((gamma, beta))
    # ConvNeXt[12] PW expand 512→2048 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(2048, 512, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+2048]); idx += 2048
    params.append((W, b))
    # ConvNeXt[12] PW project 2048→512 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(512, 2048, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[12] LayerScale 512 (γ,)
    g = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((g,))
    # ConvNeXt[13] DW 7x7 512ch (W, b)
    W = jnp.array(buf[idx:idx+25088].reshape(512, 1, 7, 7)); idx += 25088
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[13] LN 512 (γ, β)
    gamma = jnp.array(buf[idx:idx+512]); idx += 512
    beta = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((gamma, beta))
    # ConvNeXt[13] PW expand 512→2048 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(2048, 512, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+2048]); idx += 2048
    params.append((W, b))
    # ConvNeXt[13] PW project 2048→512 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(512, 2048, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[13] LayerScale 512 (γ,)
    g = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((g,))
    # ConvNeXt[14] DW 7x7 512ch (W, b)
    W = jnp.array(buf[idx:idx+25088].reshape(512, 1, 7, 7)); idx += 25088
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[14] LN 512 (γ, β)
    gamma = jnp.array(buf[idx:idx+512]); idx += 512
    beta = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((gamma, beta))
    # ConvNeXt[14] PW expand 512→2048 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(2048, 512, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+2048]); idx += 2048
    params.append((W, b))
    # ConvNeXt[14] PW project 2048→512 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(512, 2048, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[14] LayerScale 512 (γ,)
    g = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((g,))
    # ConvNeXt[15] DW 7x7 512ch (W, b)
    W = jnp.array(buf[idx:idx+25088].reshape(512, 1, 7, 7)); idx += 25088
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[15] LN 512 (γ, β)
    gamma = jnp.array(buf[idx:idx+512]); idx += 512
    beta = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((gamma, beta))
    # ConvNeXt[15] PW expand 512→2048 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(2048, 512, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+2048]); idx += 2048
    params.append((W, b))
    # ConvNeXt[15] PW project 2048→512 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(512, 2048, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[15] LayerScale 512 (γ,)
    g = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((g,))
    # ConvNeXt[16] DW 7x7 512ch (W, b)
    W = jnp.array(buf[idx:idx+25088].reshape(512, 1, 7, 7)); idx += 25088
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[16] LN 512 (γ, β)
    gamma = jnp.array(buf[idx:idx+512]); idx += 512
    beta = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((gamma, beta))
    # ConvNeXt[16] PW expand 512→2048 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(2048, 512, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+2048]); idx += 2048
    params.append((W, b))
    # ConvNeXt[16] PW project 2048→512 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(512, 2048, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[16] LayerScale 512 (γ,)
    g = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((g,))
    # ConvNeXt[17] DW 7x7 512ch (W, b)
    W = jnp.array(buf[idx:idx+25088].reshape(512, 1, 7, 7)); idx += 25088
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[17] LN 512 (γ, β)
    gamma = jnp.array(buf[idx:idx+512]); idx += 512
    beta = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((gamma, beta))
    # ConvNeXt[17] PW expand 512→2048 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(2048, 512, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+2048]); idx += 2048
    params.append((W, b))
    # ConvNeXt[17] PW project 2048→512 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(512, 2048, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[17] LayerScale 512 (γ,)
    g = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((g,))
    # ConvNeXt[18] DW 7x7 512ch (W, b)
    W = jnp.array(buf[idx:idx+25088].reshape(512, 1, 7, 7)); idx += 25088
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[18] LN 512 (γ, β)
    gamma = jnp.array(buf[idx:idx+512]); idx += 512
    beta = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((gamma, beta))
    # ConvNeXt[18] PW expand 512→2048 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(2048, 512, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+2048]); idx += 2048
    params.append((W, b))
    # ConvNeXt[18] PW project 2048→512 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(512, 2048, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[18] LayerScale 512 (γ,)
    g = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((g,))
    # ConvNeXt[19] DW 7x7 512ch (W, b)
    W = jnp.array(buf[idx:idx+25088].reshape(512, 1, 7, 7)); idx += 25088
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[19] LN 512 (γ, β)
    gamma = jnp.array(buf[idx:idx+512]); idx += 512
    beta = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((gamma, beta))
    # ConvNeXt[19] PW expand 512→2048 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(2048, 512, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+2048]); idx += 2048
    params.append((W, b))
    # ConvNeXt[19] PW project 2048→512 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(512, 2048, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[19] LayerScale 512 (γ,)
    g = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((g,))
    # ConvNeXt[20] DW 7x7 512ch (W, b)
    W = jnp.array(buf[idx:idx+25088].reshape(512, 1, 7, 7)); idx += 25088
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[20] LN 512 (γ, β)
    gamma = jnp.array(buf[idx:idx+512]); idx += 512
    beta = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((gamma, beta))
    # ConvNeXt[20] PW expand 512→2048 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(2048, 512, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+2048]); idx += 2048
    params.append((W, b))
    # ConvNeXt[20] PW project 2048→512 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(512, 2048, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[20] LayerScale 512 (γ,)
    g = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((g,))
    # ConvNeXt[21] DW 7x7 512ch (W, b)
    W = jnp.array(buf[idx:idx+25088].reshape(512, 1, 7, 7)); idx += 25088
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[21] LN 512 (γ, β)
    gamma = jnp.array(buf[idx:idx+512]); idx += 512
    beta = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((gamma, beta))
    # ConvNeXt[21] PW expand 512→2048 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(2048, 512, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+2048]); idx += 2048
    params.append((W, b))
    # ConvNeXt[21] PW project 2048→512 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(512, 2048, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[21] LayerScale 512 (γ,)
    g = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((g,))
    # ConvNeXt[22] DW 7x7 512ch (W, b)
    W = jnp.array(buf[idx:idx+25088].reshape(512, 1, 7, 7)); idx += 25088
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[22] LN 512 (γ, β)
    gamma = jnp.array(buf[idx:idx+512]); idx += 512
    beta = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((gamma, beta))
    # ConvNeXt[22] PW expand 512→2048 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(2048, 512, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+2048]); idx += 2048
    params.append((W, b))
    # ConvNeXt[22] PW project 2048→512 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(512, 2048, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[22] LayerScale 512 (γ,)
    g = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((g,))
    # ConvNeXt[23] DW 7x7 512ch (W, b)
    W = jnp.array(buf[idx:idx+25088].reshape(512, 1, 7, 7)); idx += 25088
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[23] LN 512 (γ, β)
    gamma = jnp.array(buf[idx:idx+512]); idx += 512
    beta = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((gamma, beta))
    # ConvNeXt[23] PW expand 512→2048 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(2048, 512, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+2048]); idx += 2048
    params.append((W, b))
    # ConvNeXt[23] PW project 2048→512 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(512, 2048, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[23] LayerScale 512 (γ,)
    g = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((g,))
    # ConvNeXt[24] DW 7x7 512ch (W, b)
    W = jnp.array(buf[idx:idx+25088].reshape(512, 1, 7, 7)); idx += 25088
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[24] LN 512 (γ, β)
    gamma = jnp.array(buf[idx:idx+512]); idx += 512
    beta = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((gamma, beta))
    # ConvNeXt[24] PW expand 512→2048 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(2048, 512, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+2048]); idx += 2048
    params.append((W, b))
    # ConvNeXt[24] PW project 2048→512 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(512, 2048, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[24] LayerScale 512 (γ,)
    g = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((g,))
    # ConvNeXt[25] DW 7x7 512ch (W, b)
    W = jnp.array(buf[idx:idx+25088].reshape(512, 1, 7, 7)); idx += 25088
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[25] LN 512 (γ, β)
    gamma = jnp.array(buf[idx:idx+512]); idx += 512
    beta = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((gamma, beta))
    # ConvNeXt[25] PW expand 512→2048 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(2048, 512, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+2048]); idx += 2048
    params.append((W, b))
    # ConvNeXt[25] PW project 2048→512 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(512, 2048, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[25] LayerScale 512 (γ,)
    g = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((g,))
    # ConvNeXt[26] DW 7x7 512ch (W, b)
    W = jnp.array(buf[idx:idx+25088].reshape(512, 1, 7, 7)); idx += 25088
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[26] LN 512 (γ, β)
    gamma = jnp.array(buf[idx:idx+512]); idx += 512
    beta = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((gamma, beta))
    # ConvNeXt[26] PW expand 512→2048 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(2048, 512, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+2048]); idx += 2048
    params.append((W, b))
    # ConvNeXt[26] PW project 2048→512 (W, b)
    W = jnp.array(buf[idx:idx+1048576].reshape(512, 2048, 1, 1)); idx += 1048576
    b = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((W, b))
    # ConvNeXt[26] LayerScale 512 (γ,)
    g = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((g,))
    # CNXDown LN 512 (γ, β)
    gamma = jnp.array(buf[idx:idx+512]); idx += 512
    beta = jnp.array(buf[idx:idx+512]); idx += 512
    params.append((gamma, beta))
    # CNXDown conv 2x2 512→1024 (W, b)
    W = jnp.array(buf[idx:idx+2097152].reshape(1024, 512, 2, 2)); idx += 2097152
    b = jnp.array(buf[idx:idx+1024]); idx += 1024
    params.append((W, b))
    # ConvNeXt[0] DW 7x7 1024ch (W, b)
    W = jnp.array(buf[idx:idx+50176].reshape(1024, 1, 7, 7)); idx += 50176
    b = jnp.array(buf[idx:idx+1024]); idx += 1024
    params.append((W, b))
    # ConvNeXt[0] LN 1024 (γ, β)
    gamma = jnp.array(buf[idx:idx+1024]); idx += 1024
    beta = jnp.array(buf[idx:idx+1024]); idx += 1024
    params.append((gamma, beta))
    # ConvNeXt[0] PW expand 1024→4096 (W, b)
    W = jnp.array(buf[idx:idx+4194304].reshape(4096, 1024, 1, 1)); idx += 4194304
    b = jnp.array(buf[idx:idx+4096]); idx += 4096
    params.append((W, b))
    # ConvNeXt[0] PW project 4096→1024 (W, b)
    W = jnp.array(buf[idx:idx+4194304].reshape(1024, 4096, 1, 1)); idx += 4194304
    b = jnp.array(buf[idx:idx+1024]); idx += 1024
    params.append((W, b))
    # ConvNeXt[0] LayerScale 1024 (γ,)
    g = jnp.array(buf[idx:idx+1024]); idx += 1024
    params.append((g,))
    # ConvNeXt[1] DW 7x7 1024ch (W, b)
    W = jnp.array(buf[idx:idx+50176].reshape(1024, 1, 7, 7)); idx += 50176
    b = jnp.array(buf[idx:idx+1024]); idx += 1024
    params.append((W, b))
    # ConvNeXt[1] LN 1024 (γ, β)
    gamma = jnp.array(buf[idx:idx+1024]); idx += 1024
    beta = jnp.array(buf[idx:idx+1024]); idx += 1024
    params.append((gamma, beta))
    # ConvNeXt[1] PW expand 1024→4096 (W, b)
    W = jnp.array(buf[idx:idx+4194304].reshape(4096, 1024, 1, 1)); idx += 4194304
    b = jnp.array(buf[idx:idx+4096]); idx += 4096
    params.append((W, b))
    # ConvNeXt[1] PW project 4096→1024 (W, b)
    W = jnp.array(buf[idx:idx+4194304].reshape(1024, 4096, 1, 1)); idx += 4194304
    b = jnp.array(buf[idx:idx+1024]); idx += 1024
    params.append((W, b))
    # ConvNeXt[1] LayerScale 1024 (γ,)
    g = jnp.array(buf[idx:idx+1024]); idx += 1024
    params.append((g,))
    # ConvNeXt[2] DW 7x7 1024ch (W, b)
    W = jnp.array(buf[idx:idx+50176].reshape(1024, 1, 7, 7)); idx += 50176
    b = jnp.array(buf[idx:idx+1024]); idx += 1024
    params.append((W, b))
    # ConvNeXt[2] LN 1024 (γ, β)
    gamma = jnp.array(buf[idx:idx+1024]); idx += 1024
    beta = jnp.array(buf[idx:idx+1024]); idx += 1024
    params.append((gamma, beta))
    # ConvNeXt[2] PW expand 1024→4096 (W, b)
    W = jnp.array(buf[idx:idx+4194304].reshape(4096, 1024, 1, 1)); idx += 4194304
    b = jnp.array(buf[idx:idx+4096]); idx += 4096
    params.append((W, b))
    # ConvNeXt[2] PW project 4096→1024 (W, b)
    W = jnp.array(buf[idx:idx+4194304].reshape(1024, 4096, 1, 1)); idx += 4194304
    b = jnp.array(buf[idx:idx+1024]); idx += 1024
    params.append((W, b))
    # ConvNeXt[2] LayerScale 1024 (γ,)
    g = jnp.array(buf[idx:idx+1024]); idx += 1024
    params.append((g,))
    # head LN 1024 (γ, β)
    gamma = jnp.array(buf[idx:idx+1024]); idx += 1024
    ...
    params.append((gamma, beta))
    # dense 1024 → 1000
    W = jnp.array(buf[idx:idx+1024000].reshape(1024, 1000).T); idx += 1024000
...
def params_to_file(params, path):
    ...
    f.write(np.asarray(gamma).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(beta).astype(np.float32).flatten().tobytes())
    # ConvNeXt[0] PW expand (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[0] PW project (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[0] LayerScale (γ,)
    (g,) = params[idx]; idx += 1
    f.write(np.asarray(g).astype(np.float32).flatten().tobytes())
    # ConvNeXt[1] DW 7x7 (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[1] LN (γ, β)
    ...
    f.write(np.asarray(gamma).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(beta).astype(np.float32).flatten().tobytes())
    # ConvNeXt[1] PW expand (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[1] PW project (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[1] LayerScale (γ,)
    (g,) = params[idx]; idx += 1
    f.write(np.asarray(g).astype(np.float32).flatten().tobytes())
    # ConvNeXt[2] DW 7x7 (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[2] LN (γ, β)
    ...
    f.write(np.asarray(gamma).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(beta).astype(np.float32).flatten().tobytes())
    # ConvNeXt[2] PW expand (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[2] PW project (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[2] LayerScale (γ,)
    (g,) = params[idx]; idx += 1
    f.write(np.asarray(g).astype(np.float32).flatten().tobytes())
    # CNXDown LN (γ, β)
    gamma, beta = params[idx]; idx += 1
    ...
    f.write(np.asarray(gamma).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(beta).astype(np.float32).flatten().tobytes())
    # ConvNeXt[3] PW expand (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[3] PW project (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[3] LayerScale (γ,)
    (g,) = params[idx]; idx += 1
    f.write(np.asarray(g).astype(np.float32).flatten().tobytes())
    # ConvNeXt[4] DW 7x7 (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[4] LN (γ, β)
    ...
    f.write(np.asarray(gamma).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(beta).astype(np.float32).flatten().tobytes())
    # ConvNeXt[4] PW expand (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[4] PW project (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[4] LayerScale (γ,)
    (g,) = params[idx]; idx += 1
    f.write(np.asarray(g).astype(np.float32).flatten().tobytes())
    # ConvNeXt[5] DW 7x7 (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[5] LN (γ, β)
    ...
    f.write(np.asarray(gamma).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(beta).astype(np.float32).flatten().tobytes())
    # ConvNeXt[5] PW expand (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[5] PW project (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[5] LayerScale (γ,)
    (g,) = params[idx]; idx += 1
    f.write(np.asarray(g).astype(np.float32).flatten().tobytes())
    # ConvNeXt[6] DW 7x7 (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[6] LN (γ, β)
    ...
    f.write(np.asarray(gamma).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(beta).astype(np.float32).flatten().tobytes())
    # ConvNeXt[6] PW expand (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[6] PW project (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[6] LayerScale (γ,)
    (g,) = params[idx]; idx += 1
    f.write(np.asarray(g).astype(np.float32).flatten().tobytes())
    # ConvNeXt[7] DW 7x7 (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[7] LN (γ, β)
    ...
    f.write(np.asarray(gamma).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(beta).astype(np.float32).flatten().tobytes())
    # ConvNeXt[7] PW expand (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[7] PW project (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[7] LayerScale (γ,)
    (g,) = params[idx]; idx += 1
    f.write(np.asarray(g).astype(np.float32).flatten().tobytes())
    # ConvNeXt[8] DW 7x7 (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[8] LN (γ, β)
    ...
    f.write(np.asarray(gamma).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(beta).astype(np.float32).flatten().tobytes())
    # ConvNeXt[8] PW expand (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[8] PW project (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[8] LayerScale (γ,)
    (g,) = params[idx]; idx += 1
    f.write(np.asarray(g).astype(np.float32).flatten().tobytes())
    # ConvNeXt[9] DW 7x7 (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[9] LN (γ, β)
    ...
    f.write(np.asarray(gamma).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(beta).astype(np.float32).flatten().tobytes())
    # ConvNeXt[9] PW expand (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[9] PW project (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[9] LayerScale (γ,)
    (g,) = params[idx]; idx += 1
    f.write(np.asarray(g).astype(np.float32).flatten().tobytes())
    # ConvNeXt[10] DW 7x7 (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[10] LN (γ, β)
    ...
    f.write(np.asarray(gamma).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(beta).astype(np.float32).flatten().tobytes())
    # ConvNeXt[10] PW expand (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[10] PW project (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[10] LayerScale (γ,)
    (g,) = params[idx]; idx += 1
    f.write(np.asarray(g).astype(np.float32).flatten().tobytes())
    # ConvNeXt[11] DW 7x7 (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[11] LN (γ, β)
    ...
    f.write(np.asarray(gamma).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(beta).astype(np.float32).flatten().tobytes())
    # ConvNeXt[11] PW expand (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[11] PW project (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[11] LayerScale (γ,)
    (g,) = params[idx]; idx += 1
    f.write(np.asarray(g).astype(np.float32).flatten().tobytes())
    # ConvNeXt[12] DW 7x7 (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[12] LN (γ, β)
    ...
    f.write(np.asarray(gamma).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(beta).astype(np.float32).flatten().tobytes())
    # ConvNeXt[12] PW expand (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[12] PW project (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[12] LayerScale (γ,)
    (g,) = params[idx]; idx += 1
    f.write(np.asarray(g).astype(np.float32).flatten().tobytes())
    # ConvNeXt[13] DW 7x7 (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[13] LN (γ, β)
    ...
    f.write(np.asarray(gamma).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(beta).astype(np.float32).flatten().tobytes())
    # ConvNeXt[13] PW expand (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[13] PW project (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[13] LayerScale (γ,)
    (g,) = params[idx]; idx += 1
    f.write(np.asarray(g).astype(np.float32).flatten().tobytes())
    # ConvNeXt[14] DW 7x7 (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[14] LN (γ, β)
    ...
    f.write(np.asarray(gamma).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(beta).astype(np.float32).flatten().tobytes())
    # ConvNeXt[14] PW expand (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[14] PW project (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[14] LayerScale (γ,)
    (g,) = params[idx]; idx += 1
    f.write(np.asarray(g).astype(np.float32).flatten().tobytes())
    # ConvNeXt[15] DW 7x7 (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[15] LN (γ, β)
    ...
    f.write(np.asarray(gamma).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(beta).astype(np.float32).flatten().tobytes())
    # ConvNeXt[15] PW expand (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[15] PW project (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[15] LayerScale (γ,)
    (g,) = params[idx]; idx += 1
    f.write(np.asarray(g).astype(np.float32).flatten().tobytes())
    # ConvNeXt[16] DW 7x7 (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[16] LN (γ, β)
    ...
    f.write(np.asarray(gamma).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(beta).astype(np.float32).flatten().tobytes())
    # ConvNeXt[16] PW expand (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[16] PW project (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[16] LayerScale (γ,)
    (g,) = params[idx]; idx += 1
    f.write(np.asarray(g).astype(np.float32).flatten().tobytes())
    # ConvNeXt[17] DW 7x7 (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[17] LN (γ, β)
    ...
    f.write(np.asarray(gamma).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(beta).astype(np.float32).flatten().tobytes())
    # ConvNeXt[17] PW expand (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[17] PW project (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[17] LayerScale (γ,)
    (g,) = params[idx]; idx += 1
    f.write(np.asarray(g).astype(np.float32).flatten().tobytes())
    # ConvNeXt[18] DW 7x7 (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[18] LN (γ, β)
    ...
    f.write(np.asarray(gamma).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(beta).astype(np.float32).flatten().tobytes())
    # ConvNeXt[18] PW expand (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[18] PW project (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[18] LayerScale (γ,)
    (g,) = params[idx]; idx += 1
    f.write(np.asarray(g).astype(np.float32).flatten().tobytes())
    # ConvNeXt[19] DW 7x7 (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[19] LN (γ, β)
    ...
    f.write(np.asarray(gamma).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(beta).astype(np.float32).flatten().tobytes())
    # ConvNeXt[19] PW expand (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[19] PW project (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[19] LayerScale (γ,)
    (g,) = params[idx]; idx += 1
    f.write(np.asarray(g).astype(np.float32).flatten().tobytes())
    # ConvNeXt[20] DW 7x7 (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[20] LN (γ, β)
    ...
    f.write(np.asarray(gamma).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(beta).astype(np.float32).flatten().tobytes())
    # ConvNeXt[20] PW expand (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[20] PW project (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[20] LayerScale (γ,)
    (g,) = params[idx]; idx += 1
    f.write(np.asarray(g).astype(np.float32).flatten().tobytes())
    # ConvNeXt[21] DW 7x7 (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[21] LN (γ, β)
    ...
    f.write(np.asarray(gamma).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(beta).astype(np.float32).flatten().tobytes())
    # ConvNeXt[21] PW expand (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[21] PW project (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[21] LayerScale (γ,)
    (g,) = params[idx]; idx += 1
    f.write(np.asarray(g).astype(np.float32).flatten().tobytes())
    # ConvNeXt[22] DW 7x7 (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[22] LN (γ, β)
    ...
    f.write(np.asarray(gamma).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(beta).astype(np.float32).flatten().tobytes())
    # ConvNeXt[22] PW expand (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[22] PW project (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[22] LayerScale (γ,)
    (g,) = params[idx]; idx += 1
    f.write(np.asarray(g).astype(np.float32).flatten().tobytes())
    # ConvNeXt[23] DW 7x7 (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[23] LN (γ, β)
    ...
    f.write(np.asarray(gamma).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(beta).astype(np.float32).flatten().tobytes())
    # ConvNeXt[23] PW expand (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[23] PW project (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[23] LayerScale (γ,)
    (g,) = params[idx]; idx += 1
    f.write(np.asarray(g).astype(np.float32).flatten().tobytes())
    # ConvNeXt[24] DW 7x7 (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[24] LN (γ, β)
    ...
    f.write(np.asarray(gamma).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(beta).astype(np.float32).flatten().tobytes())
    # ConvNeXt[24] PW expand (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[24] PW project (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[24] LayerScale (γ,)
    (g,) = params[idx]; idx += 1
    f.write(np.asarray(g).astype(np.float32).flatten().tobytes())
    # ConvNeXt[25] DW 7x7 (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[25] LN (γ, β)
    ...
    f.write(np.asarray(gamma).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(beta).astype(np.float32).flatten().tobytes())
    # ConvNeXt[25] PW expand (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[25] PW project (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[25] LayerScale (γ,)
    (g,) = params[idx]; idx += 1
    f.write(np.asarray(g).astype(np.float32).flatten().tobytes())
    # ConvNeXt[26] DW 7x7 (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[26] LN (γ, β)
    ...
    f.write(np.asarray(gamma).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(beta).astype(np.float32).flatten().tobytes())
    # ConvNeXt[26] PW expand (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[26] PW project (W, b)
    W, b = params[idx]; idx += 1
    f.write(np.asarray(W).astype(np.float32).flatten().tobytes())
    f.write(np.asarray(b).astype(np.float32).flatten().tobytes())
    # ConvNeXt[26] LayerScale (γ,)
    (g,) = params[idx]; idx += 1
    f.write(np.asarray(g).astype(np.float32).flatten().tobytes())
    # CNXDown LN (γ, β)
    gamma, beta = params[idx]; idx += 1
...
def forward(params, x, drop_key=None):
    ...
    dpkeys = (jax.random.split(drop_key, 36) if drop_key is not None else [None] * 36)
    x = convnext_stem(params, x, 0, 4)
    x = convnext_block(params, x, 2, dpkeys[0], 1.000000)
    x = convnext_block(params, x, 7, dpkeys[1], 0.985714)
    x = convnext_block(params, x, 12, dpkeys[2], 0.971429)
    x = convnext_downsample(params, x, 17)
    x = convnext_block(params, x, 19, dpkeys[3], 0.957143)
    x = convnext_block(params, x, 24, dpkeys[4], 0.942857)
    x = convnext_block(params, x, 29, dpkeys[5], 0.928571)
    x = convnext_downsample(params, x, 34)
    x = convnext_block(params, x, 36, dpkeys[6], 0.914286)
    x = convnext_block(params, x, 41, dpkeys[7], 0.900000)
    x = convnext_block(params, x, 46, dpkeys[8], 0.885714)
    x = convnext_block(params, x, 51, dpkeys[9], 0.871429)
    x = convnext_block(params, x, 56, dpkeys[10], 0.857143)
    x = convnext_block(params, x, 61, dpkeys[11], 0.842857)
    x = convnext_block(params, x, 66, dpkeys[12], 0.828571)
    x = convnext_block(params, x, 71, dpkeys[13], 0.814286)
    x = convnext_block(params, x, 76, dpkeys[14], 0.800000)
    x = convnext_block(params, x, 81, dpkeys[15], 0.785714)
    x = convnext_block(params, x, 86, dpkeys[16], 0.771429)
    x = convnext_block(params, x, 91, dpkeys[17], 0.757143)
    x = convnext_block(params, x, 96, dpkeys[18], 0.742857)
    x = convnext_block(params, x, 101, dpkeys[19], 0.728571)
    x = convnext_block(params, x, 106, dpkeys[20], 0.714286)
    x = convnext_block(params, x, 111, dpkeys[21], 0.700000)
    x = convnext_block(params, x, 116, dpkeys[22], 0.685714)
    x = convnext_block(params, x, 121, dpkeys[23], 0.671429)
    x = convnext_block(params, x, 126, dpkeys[24], 0.657143)
    x = convnext_block(params, x, 131, dpkeys[25], 0.642857)
    x = convnext_block(params, x, 136, dpkeys[26], 0.628571)
    x = convnext_block(params, x, 141, dpkeys[27], 0.614286)
    x = convnext_block(params, x, 146, dpkeys[28], 0.600000)
    x = convnext_block(params, x, 151, dpkeys[29], 0.585714)
    x = convnext_block(params, x, 156, dpkeys[30], 0.571429)
    x = convnext_block(params, x, 161, dpkeys[31], 0.557143)
    x = convnext_block(params, x, 166, dpkeys[32], 0.542857)
    x = convnext_downsample(params, x, 171)
    x = convnext_block(params, x, 173, dpkeys[33], 0.528571)
    x = convnext_block(params, x, 178, dpkeys[34], 0.514286)
    x = convnext_block(params, x, 183, dpkeys[35], 0.500000)
    x = global_avg_pool(x)
...
if __name__ == "__main__":
    print("ConvNeXt-B (ImageNet, bf16) · Lean 4 → JAX")
    print("  " + "CNXStem(3→128,4x4)+LN → ConvNeXt3(c=128) → CNXDown(128→256) → ConvNeXt3(c=256) → CNXDown(256→512) → ConvNeXt27(c=512) → CNXDown(512→1024) → ConvNeXt3(c=1024) → GAP → LN(1024) → 1024→1000")
    print()
    ...
    print("lr=0.000500  batch_size=" + str(BATCH_SIZE) +
          ...
          "  epochs=" + str(EPOCHS) + "  params=88591464")
    ...
    if _trace_f:
        _hdr = {
            ...
            'netspec_name': "ConvNeXt-B (ImageNet, bf16)",
            'config': {
