"""ViT-B/16 (ImageNet, bf16) — Generated by Lean 4 → JAX
Architecture: Patch(3→768,16x16) → Trans(12x[12h,768]) → 768→1000
Parameters: 86567656
...
def init_params(key):
    ...
    params = []
    # Patch embedding conv 3→768, 16x16  (PyTorch Conv2d default: U(±1/sqrt(fan_in)))
    key, k_ = random.split(key)
    scale = 1.0 / jnp.sqrt(768.0)
    params.append((random.uniform(k_, (768, 3, 16, 16), minval=-scale, maxval=scale),
                   jnp.zeros(768)))
    # CLS token
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768,)) * 0.02,))
    # Positional embedding (197, 768)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (197, 768)) * 0.02,))
    # Transformer block 0 LN1
    params.append((jnp.ones(768), jnp.zeros(768)))
    # Transformer block 0 Q 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 0 K 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 0 V 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 0 Out 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 0 LN2
    params.append((jnp.ones(768), jnp.zeros(768)))
    # Transformer block 0 MLP fc1 768→3072  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (3072, 768)) * 0.02, jnp.zeros(3072)))
    # Transformer block 0 MLP fc2 3072→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 3072)) * 0.02, jnp.zeros(768)))
    # Transformer block 1 LN1
    params.append((jnp.ones(768), jnp.zeros(768)))
    # Transformer block 1 Q 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 1 K 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 1 V 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 1 Out 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 1 LN2
    params.append((jnp.ones(768), jnp.zeros(768)))
    # Transformer block 1 MLP fc1 768→3072  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (3072, 768)) * 0.02, jnp.zeros(3072)))
    # Transformer block 1 MLP fc2 3072→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 3072)) * 0.02, jnp.zeros(768)))
    # Transformer block 2 LN1
    params.append((jnp.ones(768), jnp.zeros(768)))
    # Transformer block 2 Q 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 2 K 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 2 V 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 2 Out 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 2 LN2
    params.append((jnp.ones(768), jnp.zeros(768)))
    # Transformer block 2 MLP fc1 768→3072  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (3072, 768)) * 0.02, jnp.zeros(3072)))
    # Transformer block 2 MLP fc2 3072→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 3072)) * 0.02, jnp.zeros(768)))
    # Transformer block 3 LN1
    params.append((jnp.ones(768), jnp.zeros(768)))
    # Transformer block 3 Q 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 3 K 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 3 V 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 3 Out 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 3 LN2
    params.append((jnp.ones(768), jnp.zeros(768)))
    # Transformer block 3 MLP fc1 768→3072  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (3072, 768)) * 0.02, jnp.zeros(3072)))
    # Transformer block 3 MLP fc2 3072→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 3072)) * 0.02, jnp.zeros(768)))
    # Transformer block 4 LN1
    params.append((jnp.ones(768), jnp.zeros(768)))
    # Transformer block 4 Q 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 4 K 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 4 V 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 4 Out 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 4 LN2
    params.append((jnp.ones(768), jnp.zeros(768)))
    # Transformer block 4 MLP fc1 768→3072  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (3072, 768)) * 0.02, jnp.zeros(3072)))
    # Transformer block 4 MLP fc2 3072→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 3072)) * 0.02, jnp.zeros(768)))
    # Transformer block 5 LN1
    params.append((jnp.ones(768), jnp.zeros(768)))
    # Transformer block 5 Q 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 5 K 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 5 V 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 5 Out 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 5 LN2
    params.append((jnp.ones(768), jnp.zeros(768)))
    # Transformer block 5 MLP fc1 768→3072  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (3072, 768)) * 0.02, jnp.zeros(3072)))
    # Transformer block 5 MLP fc2 3072→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 3072)) * 0.02, jnp.zeros(768)))
    # Transformer block 6 LN1
    params.append((jnp.ones(768), jnp.zeros(768)))
    # Transformer block 6 Q 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 6 K 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 6 V 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 6 Out 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 6 LN2
    params.append((jnp.ones(768), jnp.zeros(768)))
    # Transformer block 6 MLP fc1 768→3072  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (3072, 768)) * 0.02, jnp.zeros(3072)))
    # Transformer block 6 MLP fc2 3072→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 3072)) * 0.02, jnp.zeros(768)))
    # Transformer block 7 LN1
    params.append((jnp.ones(768), jnp.zeros(768)))
    # Transformer block 7 Q 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 7 K 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 7 V 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 7 Out 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 7 LN2
    params.append((jnp.ones(768), jnp.zeros(768)))
    # Transformer block 7 MLP fc1 768→3072  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (3072, 768)) * 0.02, jnp.zeros(3072)))
    # Transformer block 7 MLP fc2 3072→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 3072)) * 0.02, jnp.zeros(768)))
    # Transformer block 8 LN1
    params.append((jnp.ones(768), jnp.zeros(768)))
    # Transformer block 8 Q 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 8 K 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 8 V 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 8 Out 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 8 LN2
    params.append((jnp.ones(768), jnp.zeros(768)))
    # Transformer block 8 MLP fc1 768→3072  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (3072, 768)) * 0.02, jnp.zeros(3072)))
    # Transformer block 8 MLP fc2 3072→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 3072)) * 0.02, jnp.zeros(768)))
    # Transformer block 9 LN1
    params.append((jnp.ones(768), jnp.zeros(768)))
    # Transformer block 9 Q 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 9 K 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 9 V 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 9 Out 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 9 LN2
    params.append((jnp.ones(768), jnp.zeros(768)))
    # Transformer block 9 MLP fc1 768→3072  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (3072, 768)) * 0.02, jnp.zeros(3072)))
    # Transformer block 9 MLP fc2 3072→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 3072)) * 0.02, jnp.zeros(768)))
    # Transformer block 10 LN1
    params.append((jnp.ones(768), jnp.zeros(768)))
    # Transformer block 10 Q 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 10 K 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 10 V 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 10 Out 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 10 LN2
    params.append((jnp.ones(768), jnp.zeros(768)))
    # Transformer block 10 MLP fc1 768→3072  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (3072, 768)) * 0.02, jnp.zeros(3072)))
    # Transformer block 10 MLP fc2 3072→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 3072)) * 0.02, jnp.zeros(768)))
    # Transformer block 11 LN1
    params.append((jnp.ones(768), jnp.zeros(768)))
    # Transformer block 11 Q 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 11 K 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 11 V 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 11 Out 768→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 768)) * 0.02, jnp.zeros(768)))
    # Transformer block 11 LN2
    params.append((jnp.ones(768), jnp.zeros(768)))
    # Transformer block 11 MLP fc1 768→3072  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (3072, 768)) * 0.02, jnp.zeros(3072)))
    # Transformer block 11 MLP fc2 3072→768  (timm trunc_normal std=0.02)
    key, k_ = random.split(key)
    params.append((random.normal(k_, (768, 3072)) * 0.02, jnp.zeros(768)))
    # Final LN
    params.append((jnp.ones(768), jnp.zeros(768)))
...
def init_params_from_file(path):
    ...
    idx = 0
    params = []
    # patchEmbed conv W, b (3→768, 16×16)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 3, 16, 16)); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # patchEmbed cls token [768]
    cls = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((cls,))
    # patchEmbed positional embedding [197, 768]
    pos = jnp.array(buf[idx:idx+151296].reshape(197, 768)); idx += 151296
    params.append((pos,))
    # trans[0] LN1 (γ, β)
    gamma = jnp.array(buf[idx:idx+768]); idx += 768
    beta = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((gamma, beta))
    # trans[0] Q 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[0] K 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[0] V 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[0] Output 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[0] LN2 (γ, β)
    ...
    beta = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((gamma, beta))
    # trans[0] fc1 768→3072 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(768, 3072).T); idx += 2359296
    b = jnp.array(buf[idx:idx+3072]); idx += 3072
    params.append((W, b))
    # trans[0] fc2 3072→768 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(3072, 768).T); idx += 2359296
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[1] LN1 (γ, β)
    gamma = jnp.array(buf[idx:idx+768]); idx += 768
    beta = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((gamma, beta))
    # trans[1] Q 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[1] K 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[1] V 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[1] Output 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[1] LN2 (γ, β)
    ...
    beta = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((gamma, beta))
    # trans[1] fc1 768→3072 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(768, 3072).T); idx += 2359296
    b = jnp.array(buf[idx:idx+3072]); idx += 3072
    params.append((W, b))
    # trans[1] fc2 3072→768 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(3072, 768).T); idx += 2359296
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[2] LN1 (γ, β)
    ...
    beta = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((gamma, beta))
    # trans[2] Q 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[2] K 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[2] V 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[2] Output 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[2] LN2 (γ, β)
    ...
    beta = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((gamma, beta))
    # trans[2] fc1 768→3072 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(768, 3072).T); idx += 2359296
    b = jnp.array(buf[idx:idx+3072]); idx += 3072
    params.append((W, b))
    # trans[2] fc2 3072→768 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(3072, 768).T); idx += 2359296
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[3] LN1 (γ, β)
    ...
    beta = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((gamma, beta))
    # trans[3] Q 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[3] K 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[3] V 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[3] Output 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[3] LN2 (γ, β)
    ...
    beta = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((gamma, beta))
    # trans[3] fc1 768→3072 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(768, 3072).T); idx += 2359296
    b = jnp.array(buf[idx:idx+3072]); idx += 3072
    params.append((W, b))
    # trans[3] fc2 3072→768 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(3072, 768).T); idx += 2359296
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[4] LN1 (γ, β)
    ...
    beta = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((gamma, beta))
    # trans[4] Q 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[4] K 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[4] V 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[4] Output 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[4] LN2 (γ, β)
    ...
    beta = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((gamma, beta))
    # trans[4] fc1 768→3072 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(768, 3072).T); idx += 2359296
    b = jnp.array(buf[idx:idx+3072]); idx += 3072
    params.append((W, b))
    # trans[4] fc2 3072→768 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(3072, 768).T); idx += 2359296
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[5] LN1 (γ, β)
    ...
    beta = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((gamma, beta))
    # trans[5] Q 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[5] K 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[5] V 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[5] Output 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[5] LN2 (γ, β)
    ...
    beta = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((gamma, beta))
    # trans[5] fc1 768→3072 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(768, 3072).T); idx += 2359296
    b = jnp.array(buf[idx:idx+3072]); idx += 3072
    params.append((W, b))
    # trans[5] fc2 3072→768 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(3072, 768).T); idx += 2359296
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[6] LN1 (γ, β)
    ...
    beta = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((gamma, beta))
    # trans[6] Q 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[6] K 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[6] V 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[6] Output 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[6] LN2 (γ, β)
    ...
    beta = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((gamma, beta))
    # trans[6] fc1 768→3072 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(768, 3072).T); idx += 2359296
    b = jnp.array(buf[idx:idx+3072]); idx += 3072
    params.append((W, b))
    # trans[6] fc2 3072→768 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(3072, 768).T); idx += 2359296
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[7] LN1 (γ, β)
    ...
    beta = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((gamma, beta))
    # trans[7] Q 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[7] K 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[7] V 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[7] Output 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[7] LN2 (γ, β)
    ...
    beta = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((gamma, beta))
    # trans[7] fc1 768→3072 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(768, 3072).T); idx += 2359296
    b = jnp.array(buf[idx:idx+3072]); idx += 3072
    params.append((W, b))
    # trans[7] fc2 3072→768 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(3072, 768).T); idx += 2359296
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[8] LN1 (γ, β)
    ...
    beta = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((gamma, beta))
    # trans[8] Q 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[8] K 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[8] V 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[8] Output 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[8] LN2 (γ, β)
    ...
    beta = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((gamma, beta))
    # trans[8] fc1 768→3072 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(768, 3072).T); idx += 2359296
    b = jnp.array(buf[idx:idx+3072]); idx += 3072
    params.append((W, b))
    # trans[8] fc2 3072→768 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(3072, 768).T); idx += 2359296
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[9] LN1 (γ, β)
    ...
    beta = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((gamma, beta))
    # trans[9] Q 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[9] K 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[9] V 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[9] Output 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[9] LN2 (γ, β)
    ...
    beta = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((gamma, beta))
    # trans[9] fc1 768→3072 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(768, 3072).T); idx += 2359296
    b = jnp.array(buf[idx:idx+3072]); idx += 3072
    params.append((W, b))
    # trans[9] fc2 3072→768 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(3072, 768).T); idx += 2359296
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[10] LN1 (γ, β)
    ...
    beta = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((gamma, beta))
    # trans[10] Q 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[10] K 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[10] V 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[10] Output 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[10] LN2 (γ, β)
    ...
    beta = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((gamma, beta))
    # trans[10] fc1 768→3072 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(768, 3072).T); idx += 2359296
    b = jnp.array(buf[idx:idx+3072]); idx += 3072
    params.append((W, b))
    # trans[10] fc2 3072→768 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(3072, 768).T); idx += 2359296
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[11] LN1 (γ, β)
    ...
    beta = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((gamma, beta))
    # trans[11] Q 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[11] K 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[11] V 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[11] Output 768→768 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(768, 768).T); idx += 589824
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans[11] LN2 (γ, β)
    ...
    beta = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((gamma, beta))
    # trans[11] fc1 768→3072 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(768, 3072).T); idx += 2359296
    b = jnp.array(buf[idx:idx+3072]); idx += 3072
    params.append((W, b))
    # trans[11] fc2 3072→768 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(3072, 768).T); idx += 2359296
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # trans final LN (γ, β)
    ...
    params.append((gamma, beta))
    # dense 768 → 1000
    W = jnp.array(buf[idx:idx+768000].reshape(768, 1000).T); idx += 768000
...
def forward(params, x, drop_key=None):
    ...
    x = mm(x, params[0][0].reshape(768, -1).T) + params[0][1]
    cls = jnp.broadcast_to(params[1][0], (x.shape[0], 1, 768))
    x = jnp.concatenate([cls, x], axis=1)
    x = x + params[2][0]
    x = transformer_block(params, x, 3, 12, dpkeys[0], 1.000000)
    x = transformer_block(params, x, 11, 12, dpkeys[1], 0.990909)
    x = transformer_block(params, x, 19, 12, dpkeys[2], 0.981818)
    x = transformer_block(params, x, 27, 12, dpkeys[3], 0.972727)
    x = transformer_block(params, x, 35, 12, dpkeys[4], 0.963636)
    x = transformer_block(params, x, 43, 12, dpkeys[5], 0.954545)
    x = transformer_block(params, x, 51, 12, dpkeys[6], 0.945455)
    x = transformer_block(params, x, 59, 12, dpkeys[7], 0.936364)
    x = transformer_block(params, x, 67, 12, dpkeys[8], 0.927273)
    x = transformer_block(params, x, 75, 12, dpkeys[9], 0.918182)
    x = transformer_block(params, x, 83, 12, dpkeys[10], 0.909091)
    x = transformer_block(params, x, 91, 12, dpkeys[11], 0.900000)
...
if __name__ == "__main__":
    print("ViT-B/16 (ImageNet, bf16) · Lean 4 → JAX")
    print("  " + "Patch(3→768,16x16) → Trans(12x[12h,768]) → 768→1000")
    print()
    ...
    MICRO_BATCH = (128 // n_devices) * n_devices or n_devices
    ...
    print("lr=0.000500  batch_size=" + str(BATCH_SIZE) +
          ...
          "  epochs=" + str(EPOCHS) + "  params=86567656")
    ...
    if _trace_f:
        _hdr = {
            ...
            'netspec_name': "ViT-B/16 (ImageNet, bf16)",
            'config': {
                'lr': 0.000500, 'batch_size': 128, 'epochs': 300,
                'use_adam': True,
