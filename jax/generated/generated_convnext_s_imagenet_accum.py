"""ConvNeXt-S (ImageNet, bf16) — Generated by Lean 4 → JAX
Architecture: CNXStem(3→96,4x4)+LN → ConvNeXt3(c=96) → CNXDown(96→192) → ConvNeXt3(c=192) → CNXDown(192→384) → ConvNeXt27(c=384) → CNXDown(384→768) → ConvNeXt3(c=768) → GAP → LN(768) → 768→1000
Parameters: 50223688
...
def init_params_from_file(path):
    ...
    idx = 0
    params = []
    # CNXStem conv 4x4 3→96 (W, b)
    W = jnp.array(buf[idx:idx+4608].reshape(96, 3, 4, 4)); idx += 4608
    ...
    # ConvNeXt[0] DW 7x7 96ch (W, b)
    W = jnp.array(buf[idx:idx+4704].reshape(96, 1, 7, 7)); idx += 4704
    b = jnp.array(buf[idx:idx+96]); idx += 96
    params.append((W, b))
    # ConvNeXt[0] LN 96 (γ, β)
    gamma = jnp.array(buf[idx:idx+96]); idx += 96
    beta = jnp.array(buf[idx:idx+96]); idx += 96
    params.append((gamma, beta))
    # ConvNeXt[0] PW expand 96→384 (W, b)
    W = jnp.array(buf[idx:idx+36864].reshape(384, 96, 1, 1)); idx += 36864
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[0] PW project 384→96 (W, b)
    W = jnp.array(buf[idx:idx+36864].reshape(96, 384, 1, 1)); idx += 36864
    b = jnp.array(buf[idx:idx+96]); idx += 96
    params.append((W, b))
    # ConvNeXt[0] LayerScale 96 (γ,)
    g = jnp.array(buf[idx:idx+96]); idx += 96
    params.append((g,))
    # ConvNeXt[1] DW 7x7 96ch (W, b)
    W = jnp.array(buf[idx:idx+4704].reshape(96, 1, 7, 7)); idx += 4704
    b = jnp.array(buf[idx:idx+96]); idx += 96
    params.append((W, b))
    # ConvNeXt[1] LN 96 (γ, β)
    gamma = jnp.array(buf[idx:idx+96]); idx += 96
    beta = jnp.array(buf[idx:idx+96]); idx += 96
    params.append((gamma, beta))
    # ConvNeXt[1] PW expand 96→384 (W, b)
    W = jnp.array(buf[idx:idx+36864].reshape(384, 96, 1, 1)); idx += 36864
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[1] PW project 384→96 (W, b)
    W = jnp.array(buf[idx:idx+36864].reshape(96, 384, 1, 1)); idx += 36864
    b = jnp.array(buf[idx:idx+96]); idx += 96
    params.append((W, b))
    # ConvNeXt[1] LayerScale 96 (γ,)
    g = jnp.array(buf[idx:idx+96]); idx += 96
    params.append((g,))
    # ConvNeXt[2] DW 7x7 96ch (W, b)
    W = jnp.array(buf[idx:idx+4704].reshape(96, 1, 7, 7)); idx += 4704
    b = jnp.array(buf[idx:idx+96]); idx += 96
    params.append((W, b))
    # ConvNeXt[2] LN 96 (γ, β)
    gamma = jnp.array(buf[idx:idx+96]); idx += 96
    beta = jnp.array(buf[idx:idx+96]); idx += 96
    params.append((gamma, beta))
    # ConvNeXt[2] PW expand 96→384 (W, b)
    W = jnp.array(buf[idx:idx+36864].reshape(384, 96, 1, 1)); idx += 36864
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[2] PW project 384→96 (W, b)
    W = jnp.array(buf[idx:idx+36864].reshape(96, 384, 1, 1)); idx += 36864
    b = jnp.array(buf[idx:idx+96]); idx += 96
    params.append((W, b))
    # ConvNeXt[2] LayerScale 96 (γ,)
    g = jnp.array(buf[idx:idx+96]); idx += 96
    params.append((g,))
    # CNXDown LN 96 (γ, β)
    gamma = jnp.array(buf[idx:idx+96]); idx += 96
    beta = jnp.array(buf[idx:idx+96]); idx += 96
    params.append((gamma, beta))
    # CNXDown conv 2x2 96→192 (W, b)
    W = jnp.array(buf[idx:idx+73728].reshape(192, 96, 2, 2)); idx += 73728
    b = jnp.array(buf[idx:idx+192]); idx += 192
    params.append((W, b))
    # ConvNeXt[0] DW 7x7 192ch (W, b)
    W = jnp.array(buf[idx:idx+9408].reshape(192, 1, 7, 7)); idx += 9408
    b = jnp.array(buf[idx:idx+192]); idx += 192
    params.append((W, b))
    # ConvNeXt[0] LN 192 (γ, β)
    gamma = jnp.array(buf[idx:idx+192]); idx += 192
    beta = jnp.array(buf[idx:idx+192]); idx += 192
    params.append((gamma, beta))
    # ConvNeXt[0] PW expand 192→768 (W, b)
    W = jnp.array(buf[idx:idx+147456].reshape(768, 192, 1, 1)); idx += 147456
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # ConvNeXt[0] PW project 768→192 (W, b)
    W = jnp.array(buf[idx:idx+147456].reshape(192, 768, 1, 1)); idx += 147456
    b = jnp.array(buf[idx:idx+192]); idx += 192
    params.append((W, b))
    # ConvNeXt[0] LayerScale 192 (γ,)
    g = jnp.array(buf[idx:idx+192]); idx += 192
    params.append((g,))
    # ConvNeXt[1] DW 7x7 192ch (W, b)
    W = jnp.array(buf[idx:idx+9408].reshape(192, 1, 7, 7)); idx += 9408
    b = jnp.array(buf[idx:idx+192]); idx += 192
    params.append((W, b))
    # ConvNeXt[1] LN 192 (γ, β)
    gamma = jnp.array(buf[idx:idx+192]); idx += 192
    beta = jnp.array(buf[idx:idx+192]); idx += 192
    params.append((gamma, beta))
    # ConvNeXt[1] PW expand 192→768 (W, b)
    W = jnp.array(buf[idx:idx+147456].reshape(768, 192, 1, 1)); idx += 147456
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # ConvNeXt[1] PW project 768→192 (W, b)
    W = jnp.array(buf[idx:idx+147456].reshape(192, 768, 1, 1)); idx += 147456
    b = jnp.array(buf[idx:idx+192]); idx += 192
    params.append((W, b))
    # ConvNeXt[1] LayerScale 192 (γ,)
    g = jnp.array(buf[idx:idx+192]); idx += 192
    params.append((g,))
    # ConvNeXt[2] DW 7x7 192ch (W, b)
    W = jnp.array(buf[idx:idx+9408].reshape(192, 1, 7, 7)); idx += 9408
    b = jnp.array(buf[idx:idx+192]); idx += 192
    params.append((W, b))
    # ConvNeXt[2] LN 192 (γ, β)
    gamma = jnp.array(buf[idx:idx+192]); idx += 192
    beta = jnp.array(buf[idx:idx+192]); idx += 192
    params.append((gamma, beta))
    # ConvNeXt[2] PW expand 192→768 (W, b)
    W = jnp.array(buf[idx:idx+147456].reshape(768, 192, 1, 1)); idx += 147456
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # ConvNeXt[2] PW project 768→192 (W, b)
    W = jnp.array(buf[idx:idx+147456].reshape(192, 768, 1, 1)); idx += 147456
    b = jnp.array(buf[idx:idx+192]); idx += 192
    params.append((W, b))
    # ConvNeXt[2] LayerScale 192 (γ,)
    g = jnp.array(buf[idx:idx+192]); idx += 192
    params.append((g,))
    # CNXDown LN 192 (γ, β)
    gamma = jnp.array(buf[idx:idx+192]); idx += 192
    beta = jnp.array(buf[idx:idx+192]); idx += 192
    params.append((gamma, beta))
    # CNXDown conv 2x2 192→384 (W, b)
    W = jnp.array(buf[idx:idx+294912].reshape(384, 192, 2, 2)); idx += 294912
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[0] DW 7x7 384ch (W, b)
    W = jnp.array(buf[idx:idx+18816].reshape(384, 1, 7, 7)); idx += 18816
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[0] LN 384 (γ, β)
    gamma = jnp.array(buf[idx:idx+384]); idx += 384
    beta = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((gamma, beta))
    # ConvNeXt[0] PW expand 384→1536 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(1536, 384, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+1536]); idx += 1536
    params.append((W, b))
    # ConvNeXt[0] PW project 1536→384 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(384, 1536, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[0] LayerScale 384 (γ,)
    g = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((g,))
    # ConvNeXt[1] DW 7x7 384ch (W, b)
    W = jnp.array(buf[idx:idx+18816].reshape(384, 1, 7, 7)); idx += 18816
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[1] LN 384 (γ, β)
    gamma = jnp.array(buf[idx:idx+384]); idx += 384
    beta = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((gamma, beta))
    # ConvNeXt[1] PW expand 384→1536 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(1536, 384, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+1536]); idx += 1536
    params.append((W, b))
    # ConvNeXt[1] PW project 1536→384 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(384, 1536, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[1] LayerScale 384 (γ,)
    g = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((g,))
    # ConvNeXt[2] DW 7x7 384ch (W, b)
    W = jnp.array(buf[idx:idx+18816].reshape(384, 1, 7, 7)); idx += 18816
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[2] LN 384 (γ, β)
    gamma = jnp.array(buf[idx:idx+384]); idx += 384
    beta = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((gamma, beta))
    # ConvNeXt[2] PW expand 384→1536 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(1536, 384, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+1536]); idx += 1536
    params.append((W, b))
    # ConvNeXt[2] PW project 1536→384 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(384, 1536, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[2] LayerScale 384 (γ,)
    g = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((g,))
    # ConvNeXt[3] DW 7x7 384ch (W, b)
    W = jnp.array(buf[idx:idx+18816].reshape(384, 1, 7, 7)); idx += 18816
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[3] LN 384 (γ, β)
    gamma = jnp.array(buf[idx:idx+384]); idx += 384
    beta = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((gamma, beta))
    # ConvNeXt[3] PW expand 384→1536 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(1536, 384, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+1536]); idx += 1536
    params.append((W, b))
    # ConvNeXt[3] PW project 1536→384 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(384, 1536, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[3] LayerScale 384 (γ,)
    g = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((g,))
    # ConvNeXt[4] DW 7x7 384ch (W, b)
    W = jnp.array(buf[idx:idx+18816].reshape(384, 1, 7, 7)); idx += 18816
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[4] LN 384 (γ, β)
    gamma = jnp.array(buf[idx:idx+384]); idx += 384
    beta = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((gamma, beta))
    # ConvNeXt[4] PW expand 384→1536 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(1536, 384, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+1536]); idx += 1536
    params.append((W, b))
    # ConvNeXt[4] PW project 1536→384 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(384, 1536, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[4] LayerScale 384 (γ,)
    g = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((g,))
    # ConvNeXt[5] DW 7x7 384ch (W, b)
    W = jnp.array(buf[idx:idx+18816].reshape(384, 1, 7, 7)); idx += 18816
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[5] LN 384 (γ, β)
    gamma = jnp.array(buf[idx:idx+384]); idx += 384
    beta = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((gamma, beta))
    # ConvNeXt[5] PW expand 384→1536 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(1536, 384, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+1536]); idx += 1536
    params.append((W, b))
    # ConvNeXt[5] PW project 1536→384 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(384, 1536, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[5] LayerScale 384 (γ,)
    g = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((g,))
    # ConvNeXt[6] DW 7x7 384ch (W, b)
    W = jnp.array(buf[idx:idx+18816].reshape(384, 1, 7, 7)); idx += 18816
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[6] LN 384 (γ, β)
    gamma = jnp.array(buf[idx:idx+384]); idx += 384
    beta = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((gamma, beta))
    # ConvNeXt[6] PW expand 384→1536 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(1536, 384, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+1536]); idx += 1536
    params.append((W, b))
    # ConvNeXt[6] PW project 1536→384 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(384, 1536, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[6] LayerScale 384 (γ,)
    g = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((g,))
    # ConvNeXt[7] DW 7x7 384ch (W, b)
    W = jnp.array(buf[idx:idx+18816].reshape(384, 1, 7, 7)); idx += 18816
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[7] LN 384 (γ, β)
    gamma = jnp.array(buf[idx:idx+384]); idx += 384
    beta = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((gamma, beta))
    # ConvNeXt[7] PW expand 384→1536 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(1536, 384, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+1536]); idx += 1536
    params.append((W, b))
    # ConvNeXt[7] PW project 1536→384 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(384, 1536, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[7] LayerScale 384 (γ,)
    g = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((g,))
    # ConvNeXt[8] DW 7x7 384ch (W, b)
    W = jnp.array(buf[idx:idx+18816].reshape(384, 1, 7, 7)); idx += 18816
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[8] LN 384 (γ, β)
    gamma = jnp.array(buf[idx:idx+384]); idx += 384
    beta = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((gamma, beta))
    # ConvNeXt[8] PW expand 384→1536 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(1536, 384, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+1536]); idx += 1536
    params.append((W, b))
    # ConvNeXt[8] PW project 1536→384 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(384, 1536, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[8] LayerScale 384 (γ,)
    g = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((g,))
    # ConvNeXt[9] DW 7x7 384ch (W, b)
    W = jnp.array(buf[idx:idx+18816].reshape(384, 1, 7, 7)); idx += 18816
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[9] LN 384 (γ, β)
    gamma = jnp.array(buf[idx:idx+384]); idx += 384
    beta = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((gamma, beta))
    # ConvNeXt[9] PW expand 384→1536 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(1536, 384, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+1536]); idx += 1536
    params.append((W, b))
    # ConvNeXt[9] PW project 1536→384 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(384, 1536, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[9] LayerScale 384 (γ,)
    g = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((g,))
    # ConvNeXt[10] DW 7x7 384ch (W, b)
    W = jnp.array(buf[idx:idx+18816].reshape(384, 1, 7, 7)); idx += 18816
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[10] LN 384 (γ, β)
    gamma = jnp.array(buf[idx:idx+384]); idx += 384
    beta = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((gamma, beta))
    # ConvNeXt[10] PW expand 384→1536 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(1536, 384, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+1536]); idx += 1536
    params.append((W, b))
    # ConvNeXt[10] PW project 1536→384 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(384, 1536, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[10] LayerScale 384 (γ,)
    g = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((g,))
    # ConvNeXt[11] DW 7x7 384ch (W, b)
    W = jnp.array(buf[idx:idx+18816].reshape(384, 1, 7, 7)); idx += 18816
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[11] LN 384 (γ, β)
    gamma = jnp.array(buf[idx:idx+384]); idx += 384
    beta = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((gamma, beta))
    # ConvNeXt[11] PW expand 384→1536 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(1536, 384, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+1536]); idx += 1536
    params.append((W, b))
    # ConvNeXt[11] PW project 1536→384 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(384, 1536, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[11] LayerScale 384 (γ,)
    g = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((g,))
    # ConvNeXt[12] DW 7x7 384ch (W, b)
    W = jnp.array(buf[idx:idx+18816].reshape(384, 1, 7, 7)); idx += 18816
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[12] LN 384 (γ, β)
    gamma = jnp.array(buf[idx:idx+384]); idx += 384
    beta = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((gamma, beta))
    # ConvNeXt[12] PW expand 384→1536 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(1536, 384, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+1536]); idx += 1536
    params.append((W, b))
    # ConvNeXt[12] PW project 1536→384 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(384, 1536, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[12] LayerScale 384 (γ,)
    g = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((g,))
    # ConvNeXt[13] DW 7x7 384ch (W, b)
    W = jnp.array(buf[idx:idx+18816].reshape(384, 1, 7, 7)); idx += 18816
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[13] LN 384 (γ, β)
    gamma = jnp.array(buf[idx:idx+384]); idx += 384
    beta = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((gamma, beta))
    # ConvNeXt[13] PW expand 384→1536 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(1536, 384, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+1536]); idx += 1536
    params.append((W, b))
    # ConvNeXt[13] PW project 1536→384 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(384, 1536, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[13] LayerScale 384 (γ,)
    g = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((g,))
    # ConvNeXt[14] DW 7x7 384ch (W, b)
    W = jnp.array(buf[idx:idx+18816].reshape(384, 1, 7, 7)); idx += 18816
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[14] LN 384 (γ, β)
    gamma = jnp.array(buf[idx:idx+384]); idx += 384
    beta = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((gamma, beta))
    # ConvNeXt[14] PW expand 384→1536 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(1536, 384, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+1536]); idx += 1536
    params.append((W, b))
    # ConvNeXt[14] PW project 1536→384 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(384, 1536, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[14] LayerScale 384 (γ,)
    g = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((g,))
    # ConvNeXt[15] DW 7x7 384ch (W, b)
    W = jnp.array(buf[idx:idx+18816].reshape(384, 1, 7, 7)); idx += 18816
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[15] LN 384 (γ, β)
    gamma = jnp.array(buf[idx:idx+384]); idx += 384
    beta = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((gamma, beta))
    # ConvNeXt[15] PW expand 384→1536 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(1536, 384, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+1536]); idx += 1536
    params.append((W, b))
    # ConvNeXt[15] PW project 1536→384 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(384, 1536, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[15] LayerScale 384 (γ,)
    g = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((g,))
    # ConvNeXt[16] DW 7x7 384ch (W, b)
    W = jnp.array(buf[idx:idx+18816].reshape(384, 1, 7, 7)); idx += 18816
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[16] LN 384 (γ, β)
    gamma = jnp.array(buf[idx:idx+384]); idx += 384
    beta = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((gamma, beta))
    # ConvNeXt[16] PW expand 384→1536 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(1536, 384, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+1536]); idx += 1536
    params.append((W, b))
    # ConvNeXt[16] PW project 1536→384 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(384, 1536, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[16] LayerScale 384 (γ,)
    g = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((g,))
    # ConvNeXt[17] DW 7x7 384ch (W, b)
    W = jnp.array(buf[idx:idx+18816].reshape(384, 1, 7, 7)); idx += 18816
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[17] LN 384 (γ, β)
    gamma = jnp.array(buf[idx:idx+384]); idx += 384
    beta = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((gamma, beta))
    # ConvNeXt[17] PW expand 384→1536 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(1536, 384, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+1536]); idx += 1536
    params.append((W, b))
    # ConvNeXt[17] PW project 1536→384 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(384, 1536, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[17] LayerScale 384 (γ,)
    g = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((g,))
    # ConvNeXt[18] DW 7x7 384ch (W, b)
    W = jnp.array(buf[idx:idx+18816].reshape(384, 1, 7, 7)); idx += 18816
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[18] LN 384 (γ, β)
    gamma = jnp.array(buf[idx:idx+384]); idx += 384
    beta = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((gamma, beta))
    # ConvNeXt[18] PW expand 384→1536 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(1536, 384, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+1536]); idx += 1536
    params.append((W, b))
    # ConvNeXt[18] PW project 1536→384 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(384, 1536, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[18] LayerScale 384 (γ,)
    g = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((g,))
    # ConvNeXt[19] DW 7x7 384ch (W, b)
    W = jnp.array(buf[idx:idx+18816].reshape(384, 1, 7, 7)); idx += 18816
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[19] LN 384 (γ, β)
    gamma = jnp.array(buf[idx:idx+384]); idx += 384
    beta = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((gamma, beta))
    # ConvNeXt[19] PW expand 384→1536 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(1536, 384, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+1536]); idx += 1536
    params.append((W, b))
    # ConvNeXt[19] PW project 1536→384 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(384, 1536, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[19] LayerScale 384 (γ,)
    g = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((g,))
    # ConvNeXt[20] DW 7x7 384ch (W, b)
    W = jnp.array(buf[idx:idx+18816].reshape(384, 1, 7, 7)); idx += 18816
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[20] LN 384 (γ, β)
    gamma = jnp.array(buf[idx:idx+384]); idx += 384
    beta = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((gamma, beta))
    # ConvNeXt[20] PW expand 384→1536 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(1536, 384, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+1536]); idx += 1536
    params.append((W, b))
    # ConvNeXt[20] PW project 1536→384 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(384, 1536, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[20] LayerScale 384 (γ,)
    g = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((g,))
    # ConvNeXt[21] DW 7x7 384ch (W, b)
    W = jnp.array(buf[idx:idx+18816].reshape(384, 1, 7, 7)); idx += 18816
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[21] LN 384 (γ, β)
    gamma = jnp.array(buf[idx:idx+384]); idx += 384
    beta = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((gamma, beta))
    # ConvNeXt[21] PW expand 384→1536 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(1536, 384, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+1536]); idx += 1536
    params.append((W, b))
    # ConvNeXt[21] PW project 1536→384 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(384, 1536, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[21] LayerScale 384 (γ,)
    g = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((g,))
    # ConvNeXt[22] DW 7x7 384ch (W, b)
    W = jnp.array(buf[idx:idx+18816].reshape(384, 1, 7, 7)); idx += 18816
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[22] LN 384 (γ, β)
    gamma = jnp.array(buf[idx:idx+384]); idx += 384
    beta = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((gamma, beta))
    # ConvNeXt[22] PW expand 384→1536 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(1536, 384, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+1536]); idx += 1536
    params.append((W, b))
    # ConvNeXt[22] PW project 1536→384 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(384, 1536, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[22] LayerScale 384 (γ,)
    g = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((g,))
    # ConvNeXt[23] DW 7x7 384ch (W, b)
    W = jnp.array(buf[idx:idx+18816].reshape(384, 1, 7, 7)); idx += 18816
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[23] LN 384 (γ, β)
    gamma = jnp.array(buf[idx:idx+384]); idx += 384
    beta = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((gamma, beta))
    # ConvNeXt[23] PW expand 384→1536 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(1536, 384, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+1536]); idx += 1536
    params.append((W, b))
    # ConvNeXt[23] PW project 1536→384 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(384, 1536, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[23] LayerScale 384 (γ,)
    g = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((g,))
    # ConvNeXt[24] DW 7x7 384ch (W, b)
    W = jnp.array(buf[idx:idx+18816].reshape(384, 1, 7, 7)); idx += 18816
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[24] LN 384 (γ, β)
    gamma = jnp.array(buf[idx:idx+384]); idx += 384
    beta = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((gamma, beta))
    # ConvNeXt[24] PW expand 384→1536 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(1536, 384, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+1536]); idx += 1536
    params.append((W, b))
    # ConvNeXt[24] PW project 1536→384 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(384, 1536, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[24] LayerScale 384 (γ,)
    g = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((g,))
    # ConvNeXt[25] DW 7x7 384ch (W, b)
    W = jnp.array(buf[idx:idx+18816].reshape(384, 1, 7, 7)); idx += 18816
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[25] LN 384 (γ, β)
    gamma = jnp.array(buf[idx:idx+384]); idx += 384
    beta = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((gamma, beta))
    # ConvNeXt[25] PW expand 384→1536 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(1536, 384, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+1536]); idx += 1536
    params.append((W, b))
    # ConvNeXt[25] PW project 1536→384 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(384, 1536, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[25] LayerScale 384 (γ,)
    g = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((g,))
    # ConvNeXt[26] DW 7x7 384ch (W, b)
    W = jnp.array(buf[idx:idx+18816].reshape(384, 1, 7, 7)); idx += 18816
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[26] LN 384 (γ, β)
    gamma = jnp.array(buf[idx:idx+384]); idx += 384
    beta = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((gamma, beta))
    # ConvNeXt[26] PW expand 384→1536 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(1536, 384, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+1536]); idx += 1536
    params.append((W, b))
    # ConvNeXt[26] PW project 1536→384 (W, b)
    W = jnp.array(buf[idx:idx+589824].reshape(384, 1536, 1, 1)); idx += 589824
    b = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((W, b))
    # ConvNeXt[26] LayerScale 384 (γ,)
    g = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((g,))
    # CNXDown LN 384 (γ, β)
    gamma = jnp.array(buf[idx:idx+384]); idx += 384
    beta = jnp.array(buf[idx:idx+384]); idx += 384
    params.append((gamma, beta))
    # CNXDown conv 2x2 384→768 (W, b)
    W = jnp.array(buf[idx:idx+1179648].reshape(768, 384, 2, 2)); idx += 1179648
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # ConvNeXt[0] DW 7x7 768ch (W, b)
    W = jnp.array(buf[idx:idx+37632].reshape(768, 1, 7, 7)); idx += 37632
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # ConvNeXt[0] LN 768 (γ, β)
    gamma = jnp.array(buf[idx:idx+768]); idx += 768
    beta = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((gamma, beta))
    # ConvNeXt[0] PW expand 768→3072 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(3072, 768, 1, 1)); idx += 2359296
    b = jnp.array(buf[idx:idx+3072]); idx += 3072
    params.append((W, b))
    # ConvNeXt[0] PW project 3072→768 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(768, 3072, 1, 1)); idx += 2359296
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # ConvNeXt[0] LayerScale 768 (γ,)
    g = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((g,))
    # ConvNeXt[1] DW 7x7 768ch (W, b)
    W = jnp.array(buf[idx:idx+37632].reshape(768, 1, 7, 7)); idx += 37632
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # ConvNeXt[1] LN 768 (γ, β)
    gamma = jnp.array(buf[idx:idx+768]); idx += 768
    beta = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((gamma, beta))
    # ConvNeXt[1] PW expand 768→3072 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(3072, 768, 1, 1)); idx += 2359296
    b = jnp.array(buf[idx:idx+3072]); idx += 3072
    params.append((W, b))
    # ConvNeXt[1] PW project 3072→768 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(768, 3072, 1, 1)); idx += 2359296
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # ConvNeXt[1] LayerScale 768 (γ,)
    g = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((g,))
    # ConvNeXt[2] DW 7x7 768ch (W, b)
    W = jnp.array(buf[idx:idx+37632].reshape(768, 1, 7, 7)); idx += 37632
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # ConvNeXt[2] LN 768 (γ, β)
    gamma = jnp.array(buf[idx:idx+768]); idx += 768
    beta = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((gamma, beta))
    # ConvNeXt[2] PW expand 768→3072 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(3072, 768, 1, 1)); idx += 2359296
    b = jnp.array(buf[idx:idx+3072]); idx += 3072
    params.append((W, b))
    # ConvNeXt[2] PW project 3072→768 (W, b)
    W = jnp.array(buf[idx:idx+2359296].reshape(768, 3072, 1, 1)); idx += 2359296
    b = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((W, b))
    # ConvNeXt[2] LayerScale 768 (γ,)
    g = jnp.array(buf[idx:idx+768]); idx += 768
    params.append((g,))
    # head LN 768 (γ, β)
    gamma = jnp.array(buf[idx:idx+768]); idx += 768
...
def forward(params, x, drop_key=None):
    ...
    x = convnext_block(params, x, 7, dpkeys[1], 0.988571)
    x = convnext_block(params, x, 12, dpkeys[2], 0.977143)
    x = convnext_downsample(params, x, 17)
    x = convnext_block(params, x, 19, dpkeys[3], 0.965714)
    x = convnext_block(params, x, 24, dpkeys[4], 0.954286)
    x = convnext_block(params, x, 29, dpkeys[5], 0.942857)
    x = convnext_downsample(params, x, 34)
    x = convnext_block(params, x, 36, dpkeys[6], 0.931429)
    x = convnext_block(params, x, 41, dpkeys[7], 0.920000)
    x = convnext_block(params, x, 46, dpkeys[8], 0.908571)
    x = convnext_block(params, x, 51, dpkeys[9], 0.897143)
    x = convnext_block(params, x, 56, dpkeys[10], 0.885714)
    x = convnext_block(params, x, 61, dpkeys[11], 0.874286)
    x = convnext_block(params, x, 66, dpkeys[12], 0.862857)
    x = convnext_block(params, x, 71, dpkeys[13], 0.851429)
    x = convnext_block(params, x, 76, dpkeys[14], 0.840000)
    x = convnext_block(params, x, 81, dpkeys[15], 0.828571)
    x = convnext_block(params, x, 86, dpkeys[16], 0.817143)
    x = convnext_block(params, x, 91, dpkeys[17], 0.805714)
    x = convnext_block(params, x, 96, dpkeys[18], 0.794286)
    x = convnext_block(params, x, 101, dpkeys[19], 0.782857)
    x = convnext_block(params, x, 106, dpkeys[20], 0.771429)
    x = convnext_block(params, x, 111, dpkeys[21], 0.760000)
    x = convnext_block(params, x, 116, dpkeys[22], 0.748571)
    x = convnext_block(params, x, 121, dpkeys[23], 0.737143)
    x = convnext_block(params, x, 126, dpkeys[24], 0.725714)
    x = convnext_block(params, x, 131, dpkeys[25], 0.714286)
    x = convnext_block(params, x, 136, dpkeys[26], 0.702857)
    x = convnext_block(params, x, 141, dpkeys[27], 0.691429)
    x = convnext_block(params, x, 146, dpkeys[28], 0.680000)
    x = convnext_block(params, x, 151, dpkeys[29], 0.668571)
    x = convnext_block(params, x, 156, dpkeys[30], 0.657143)
    x = convnext_block(params, x, 161, dpkeys[31], 0.645714)
    x = convnext_block(params, x, 166, dpkeys[32], 0.634286)
    x = convnext_downsample(params, x, 171)
    x = convnext_block(params, x, 173, dpkeys[33], 0.622857)
    x = convnext_block(params, x, 178, dpkeys[34], 0.611429)
    x = convnext_block(params, x, 183, dpkeys[35], 0.600000)
    x = global_avg_pool(x)
...
if __name__ == "__main__":
    print("ConvNeXt-S (ImageNet, bf16) · Lean 4 → JAX")
    print("  " + "CNXStem(3→96,4x4)+LN → ConvNeXt3(c=96) → CNXDown(96→192) → ConvNeXt3(c=192) → CNXDown(192→384) → ConvNeXt27(c=384) → CNXDown(384→768) → ConvNeXt3(c=768) → GAP → LN(768) → 768→1000")
    print()
    ...
    print("lr=0.000250  batch_size=" + str(BATCH_SIZE) +
          ...
          "  epochs=" + str(EPOCHS) + "  params=50223688")
    ...
    if _trace_f:
        _hdr = {
            ...
            'netspec_name': "ConvNeXt-S (ImageNet, bf16)",
            'config': {
                'lr': 0.000250, 'batch_size': 128, 'epochs': 300,
                'use_adam': True,
