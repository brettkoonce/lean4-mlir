...
_TRAIN_SIZE = 160   # RSB-A3 train resolution; eval stays _IMG_SIZE (forward infers from flat len)
...
def _imagenet_decode_random_crop_flip(image_bytes):
    ...
    img = _randaugment(img, 2, 6.000000, 0.500000)
    return img
...
def _cutmix(x, y, key):
    ...
    x4 = x.reshape(B, 3, 160, 160)
    cut = jnp.sqrt(1.0 - lam)
    cw = (cut * 160).astype(jnp.int32); ch = (cut * 160).astype(jnp.int32)
    cx = jax.random.randint(k2, (), 0, 160); cy = jax.random.randint(k3, (), 0, 160)
    x1 = jnp.clip(cx - cw // 2, 0, 160); x2 = jnp.clip(cx + cw // 2, 0, 160)
    y1c = jnp.clip(cy - ch // 2, 0, 160); y2c = jnp.clip(cy + ch // 2, 0, 160)
    mx = (jnp.arange(160) >= x1) & (jnp.arange(160) < x2)
    my = (jnp.arange(160) >= y1c) & (jnp.arange(160) < y2c)
    mask = (my[:, None] & mx[None, :]).astype(x.dtype)
    x4m = x4 * (1.0 - mask) + jnp.flip(x4, 0) * mask
    lam_adj = 1.0 - jnp.sum(mask) / (160 * 160)
...
if __name__ == "__main__":
    ...
    print("lr=0.008000  batch_size=" + str(BATCH_SIZE) +
    ...
    if _trace_f:
        _hdr = {
            ...
            'config': {
                'lr': 0.008000, 'batch_size': 2048, 'epochs': 100,
                'use_adam': False,
    ...
    for epoch in range(_start_epoch, EPOCHS):
        ...
        for _ in range(steps_per_epoch):
            ...
            params, opt_state, bn_state, loss = train_step(params, opt_state, bn_state, x, y, lr)
