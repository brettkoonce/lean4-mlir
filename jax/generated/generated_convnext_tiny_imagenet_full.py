"""ConvNeXt-T (ImageNet, bf16) — Generated by Lean 4 → JAX
Architecture: CNXStem(3→96,4x4)+LN → ConvNeXt3(c=96) → CNXDown(96→192) → ConvNeXt3(c=192) → CNXDown(192→384) → ConvNeXt9(c=384) → CNXDown(384→768) → ConvNeXt3(c=768) → GAP → LN(768) → 768→1000
Parameters: 28589128
...
def forward(params, x, drop_key=None):
    ...
    dpkeys = (jax.random.split(drop_key, 18) if drop_key is not None else [None] * 18)
    x = convnext_stem(params, x, 0, 4)
    x = convnext_block(params, x, 2, dpkeys[0], 1.000000)
    x = convnext_block(params, x, 7, dpkeys[1], 0.994118)
    x = convnext_block(params, x, 12, dpkeys[2], 0.988235)
    x = convnext_downsample(params, x, 17)
    x = convnext_block(params, x, 19, dpkeys[3], 0.982353)
    x = convnext_block(params, x, 24, dpkeys[4], 0.976471)
    x = convnext_block(params, x, 29, dpkeys[5], 0.970588)
    x = convnext_downsample(params, x, 34)
    x = convnext_block(params, x, 36, dpkeys[6], 0.964706)
    x = convnext_block(params, x, 41, dpkeys[7], 0.958824)
    x = convnext_block(params, x, 46, dpkeys[8], 0.952941)
    x = convnext_block(params, x, 51, dpkeys[9], 0.947059)
    x = convnext_block(params, x, 56, dpkeys[10], 0.941176)
    x = convnext_block(params, x, 61, dpkeys[11], 0.935294)
    x = convnext_block(params, x, 66, dpkeys[12], 0.929412)
    x = convnext_block(params, x, 71, dpkeys[13], 0.923529)
    x = convnext_block(params, x, 76, dpkeys[14], 0.917647)
    x = convnext_downsample(params, x, 81)
    x = convnext_block(params, x, 83, dpkeys[15], 0.911765)
    x = convnext_block(params, x, 88, dpkeys[16], 0.905882)
    x = convnext_block(params, x, 93, dpkeys[17], 0.900000)
    x = global_avg_pool(x)
    x = head_layer_norm(x, params[98][0], params[98][1])
    x = mm(x, params[99][0].T) + params[99][1]
    return x
...
if __name__ == "__main__":
    print("ConvNeXt-T (ImageNet, bf16) · Lean 4 → JAX")
    print("  " + "CNXStem(3→96,4x4)+LN → ConvNeXt3(c=96) → CNXDown(96→192) → ConvNeXt3(c=192) → CNXDown(192→384) → ConvNeXt9(c=384) → CNXDown(384→768) → ConvNeXt3(c=768) → GAP → LN(768) → 768→1000")
    print()
    ...
    print("lr=0.000250  batch_size=" + str(BATCH_SIZE) +
          ...
          "  epochs=" + str(EPOCHS) + "  params=28589128")
    ...
    if _trace_f:
        _hdr = {
            ...
            'netspec_name': "ConvNeXt-T (ImageNet, bf16)",
            'config': {
