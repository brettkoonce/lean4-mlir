...
RUNLOG=/tmp/mnv2_90ep.log                           # full training stdout (current attempt)
MASTER=/tmp/mnv2_90ep_master.log                    # supervisor narration (persists across attempts)
...
echo "[sup] $(date '+%F %T') START 90-epoch MNv2 bf16 on GPUs $DEVS" | tee -a "$MASTER"
