#!/usr/bin/env bash
# Supervised 30-epoch MobileNetV2-ImageNet bf16 run on the 4 clean GPUs (0,2,3,4).
...
# - Stops when training prints "Done." (epoch 30 complete) or after a
...
RUNLOG=/tmp/mnv2_30ep.log                           # full training stdout (current attempt)
MASTER=/tmp/mnv2_30ep_master.log                    # supervisor narration (persists across attempts)
MAX_ATTEMPTS=60

echo "[sup] $(date '+%F %T') START 30-epoch MNv2 bf16 on GPUs $DEVS" | tee -a "$MASTER"
...
while [ "$attempt" -lt "$MAX_ATTEMPTS" ]; do
  ...
  if [ "$result" = "done" ]; then
    echo "[sup] $(date '+%T') ✅ TRAINING COMPLETE (30 epochs). final=${CKPT_BASE}.bin" | tee -a "$MASTER"
    exit 0
  fi
