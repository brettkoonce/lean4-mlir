...
# committed rsb-faithful config (regenerate via: resnet50-imagenet rsb-faithful) lives in
...
while [ "$attempt" -lt "$MAX_ATTEMPTS" ]; do
  ...
  while kill -0 "$PYPID" 2>/dev/null; do
    # Fatal-only: BadTLP (the GPU PCIe storm that cascades to a host reset) or an
    # UNcorrected/Fatal AER/MCE. Deliberately NOT "Hardware Error" — that also
    # matches benign *corrected* DRAM ECC ("Corrected error, no action required"),
    # which the hardware already fixed and must NOT kill a healthy run. Can't filter
    # on "corrected" (substring of "unCORRECTED"), so exclude "no action required".
  ...
  pkill -9 -f "generated_resnet50_imagenet_rsbfaithful.py" 2>/dev/null
