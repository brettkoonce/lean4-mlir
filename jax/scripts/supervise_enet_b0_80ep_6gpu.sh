#!/usr/bin/env bash
# Supervised 80-epoch EfficientNet-B0-ImageNet bf16 run on the 4 clean GPUs (0,2,3,4).
# This is the *validation* tier — bump EPOCHS to 300 in MainEfficientNetImagenet.lean
...
DEVS="0,1,2,3,4,5"
PY=.lake/build/generated_efficientnet_b0_imagenet.py
CKPT_BASE=/home/skoonce/enet_b0_imagenet_bf16          # -> _e{N}.bin per epoch, .bin final
SPE=5083                                            # 6-GPU: batch 252 (6x42) -> 1281167//252
RUNLOG=/tmp/enet_b0_80ep_6gpu.log                           # full training stdout (current attempt)
MASTER=/tmp/enet_b0_80ep_6gpu_master.log                    # supervisor narration (persists across attempts)
...
echo "[sup] $(date '+%F %T') START 80-epoch ENet-B0 bf16 on GPUs $DEVS (6-GPU, post-Gen3)" | tee -a "$MASTER"
...
while [ "$attempt" -lt "$MAX_ATTEMPTS" ]; do
  ...
  pkill -9 -f "generated_efficientnet_b0_imagenet.py" 2>/dev/null
  sleep 15
done
