# The survey this repository's README used to be (snapshot, 2026-09-08)

The README was rewritten as a tour on 2026-09-08 (`planning/archive/tour_realignment.md` §3); these are
the sections it dropped, verbatim from the last survey version, kept as the record of the three phases,
the pipeline, the cross-backend checks and the per-tier verification argument as they stood. The live
statement of what is and is not verified is the book's *On Verification* appendix; the numbers here are
the older recipe's and are superseded by the chapters.
...
Tested with Lean 4.32.0 / Lake 5.0.0, IREE built from source against
ROCm 7.2.0 / gfx1100.
