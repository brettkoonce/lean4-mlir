...
outer=[("StableHLO.batchMap B (convNextStageChK 3 w.s1)",
       ...
       ("StableHLO.batchMap B (convNextStageChK 9 w.s3)",
        "⟨cnxStageBAt B 9 w.s3 h3 (cnxSavedB5 B w x),\n     batchMap_differentiableAt _ _ (fun _ => (convNextStageChK_differentiable 9 w.s3 h3).differentiableAt)⟩")]
body=("vjpCompDiffAt (StableHLO.batchMap B (cnxSavedA0 w))\n"
...
for i in range(min(k,len(outer))):
    s,wt=outer[i]; body=f"vjpCompDiffAt _ ({s}) x\n    ({body})\n    ({wt})"
