...
| **CIFAR** ✅ **DONE 2026-06-28** (`cifar-pgd`) | `genCifarPgdStep`: 4 conv input-VJPs + **2** maxpool-backs + conv1-VJP, mirrored from `cifar_train_step.mlir` | 7-layer conv-aware **product**: **L = 942K** (cert vacuous) | **medium+** (done) | clean **70.0%** → L∞ ε=0.1 → **0%** (collapse). Depth-cliff: linear 5.3 → mlp 39 → cnn 3196 → cifar 942K. best-θ rescues the late SGD divergence. `runs/pgd_cifar_phase3.log` |
