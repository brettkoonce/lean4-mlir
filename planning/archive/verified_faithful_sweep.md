...
> `chanLNTensor3`'s two conjugations are inverse permutations (`chanRowsPerm`), so the output-side one moves onto
> the cotangent as its inverse (`pdiv_reindexOut_contract`, generic). Everything else is re-threading: `cnxCotD`
> becomes `chanLNTensor3Back` (the certified VJP, `chanLNTensor3Back_eq_chanLN_vjp`), the head goes to ViT's
> vector-LN at `N = 1`, and the stem LN joins the thread. **The four even-kernel weight grads are no longer a
> gap** — the 3 downsample 2×2/s2 weights are `convStridedWeightSgd` (kernel-generic since `sWGradGeom` split the
> odd/even padding cases) and the stem 4×4/s4 is `convStride4WeightGrad`, so the capstone now covers **all 182**
> parameters: 181 as `θ − lr·∂Loss/∂θ`, `psW` at its gradient (its SGD wrap is hand-written text, a §5 carve-out).
> `lake build Certs` green (3956), `lake env lean tests/AuditAxioms.lean` 3-axiom clean, `docstring-checkrefs` and
> `check_audit_coverage.py` green. Den-level, no renderer/.mlir change.
>
...
| **convnext** (full [3,3,9,3], 182 tied params) | ✅ **whole net** den-composed: all 18 ConvNeXt blocks + 3 downsamples + GAP→LN→dense head + the stem with its LN threaded at the real `convNextTrainStepFaithfulV` forward activations (`cnxStemFwdO`/`cnxBlockFwdChO`/`cnxDownFwdChO`), cotangent composed from the loss `g` down through dense (`dense_has_vjp`) + head-LN + GAP (`globalAvgPoolFlat_has_vjp`) + every block's backward, with the **identity-skip fan-in `+ dyOut`** at each of the 18 block merges + the LN-back at each of the 3 downsamples; capstone `cnx_net_tied_certified` bundles all 182 params' ties + the dense total-loss fold + `cnxLossCot_den` | ◐ level-2: cotangents at correctly-threaded SSAs (block backward rendered hand-written, not `SHlo`); **GELU mask** (smooth, no kink — `geluScalarDeriv`) + **channel LayerNorm** γ/β (`Vec c`, re-stated 2026-09-05) + **per-channel layer-scale** γ (`Vec c`, `chanIdx` broadcast) are the new content vs mnv2's relu6/per-channel-BN; `psW` ties at its gradient | **✅ TIED** |
...
5. _**VJP witness upgrade (separate §4 track) — ✅ FULL-DEPTH DONE (2026-06-19)**.
   ...
   **every** input (no nonnegativity bookkeeping ResNet's `idBlk2` needed). 15 such blocks shift by
   ...
   (224×224, real channel/width schedule — Item D in `planning/archive/whole_network_backward.md`; the witness
