...
`planning/archive/xla_pjrt_ladder.md`; this file is the short version — state, next moves, and the things
...
| — | *↓ the batched-index thread (§0.2 ▶2), 2026-08-03 — ConvNeXt at `N := B`* |
| `11d6102b`…`5dac8b52` | ConvNeXt's forward, backward sites, `convNextBackAllB`, the AdamW tail — 7 increments, tie 16 → 34 forms |
| — | *↓ the stochastic-depth thread (§0.10), 2026-08-03 — the first FEATURE in a while* |
| *(this thread)* | **ConvNeXt's SD render on the batched chain**: 18 sites, 5 artifacts, keep = 1 bit-exact on 83,478,846 floats, and a gradient defect the gates could not have caught |
...
§0.10 and §0.11 are the two SD records; `planning/archive/stochastic_depth.md` §7e-f is the gate detail.
...
`planning/archive/imagenet_sweep.md` records efficientnetin **310** and vitin **424** ms/step on 4 GPUs. Measured
...
### §0.10 ✅ **ConvNeXt's STOCHASTIC-DEPTH RENDER — DONE 2026-08-03, on the batched chain**

`planning/archive/stochastic_depth.md` §7e is the full gate record. **18 sites, one per block** (`[3,3,9,3]`,
rate 0.1), on the **residual branch only**: `dropPathB` between `layerScaleCh` and the `addVB` in
`fwdBlockB`, and the **same constructor on the cotangent** between `dy` and `layerScaleCh` in
...
| gate | result | control |
|---|---|---|
| the drop-free chain unchanged (`sd := false` emits no `pretty` call) | `convnext-fwd-b-tie` byte-identical, gradMap 180/180 | — |
| `verified_mlir/` diff, writers **FORCED** with `lake env lean` | **0 lines** across all 118 committed artifacts | — |
| **keep = 1 ⇒ `adamdrop` trains what `adam` trains** (3 steps, det shim) | **0 of 83,478,846** floats, against a **bit-exact** A-vs-A floor | real ramp **0.399**; broken conv-VJP flip **0.0343** |
| **the misplacement control** | `misplace_drop_sites.py` → **rc=1** | *is* the control |
| gate B, `droppath-tie --net convnext` | floor 320/320, B1 320/320, **B3 0.259**, **B4 1.104** | the misplaced render |
| gate A, `droppath-tie --op` at `B ≠ n` | bit-exact 40/40 | C1 1.29, C2 35/40 |
| **DP mask shard**, `drop-shard-check convnext` (2 replicas) | ① **83,478,846/83,478,846** bit-identical, ②a `%loss` MOVED | `DROP_FAULT=replicate` → **81.8M** move, rel 1.00005; `PJRT_DP_NO_MASK_SHARD=1` → arity refusal |
| prefix audit | `*_drop_fwd` ⊂ `*_adamdrop`, **1580 lines**, both scales | — |

#### ⚠⚠ THE ONE DEFECT NO GATE IN THAT TABLE WOULD HAVE CAUGHT

**LayerScale's γ gradient reads the cotangent AT THE DROP SITE.** It is `Σ (cot ⊙ p)` at the
cotangent of the LayerScale *output*, which the drop scales — so it needs `s ⊙ dy`, not `dy`. Every
other block gradient descends from `cot_p` and inherits the scale for free; `%…lg` is the one fed
`dy` directly. Feeding it the undropped cotangent type-checks, trains and descends: **18 of 180
gradients wrong by a per-example factor, on the very parameter stochastic depth acts through.**

⚠ **And it is invisible to this feature's whole gate set.** At keep = 1 the two cotangents are
*equal*, so the keep = 1 tie cannot see it; the placement controls only touch the forward; the byte
tie compares against a render that has no drop at all. It was found by tracing operands. Carry
forward: **when an op is spliced into a chain, list every consumer of the value it displaced** —
`bwdBlockB` now returns `dyd` for exactly that reason.

#### ⚠ What this leaves owed

* **The keep = 1 tie doubles as the ConvNeXt batched-chain numeric tie** — and that is worth
  reading twice. `convnext_adam_train_step` is the **per-example** renderer's artifact and
  `convnext_adamdrop` is the **batched** chain's; they differ by the 78 conv-VJP `transpose`/
  `reverse` lines *and* the 36 drop ops. At keep = 1 they produce **bit-identical `[θ|m|v]`** after
  3 AdamW steps, and `scripts/perturb_conv_vjp.py` proves the comparison reads those 78 lines
  (0.0343). ⚠ That is the evidence §0.3's blocked-swap row wanted, obtained without IREE — but the
  swap itself is still not made, because it would move bytes in the artifact behind the 84.41%
  80-epoch run and nothing needs it yet. **Whoever makes it: this tie is the license.**
* **No long run.** SD is a recipe-parity item — `stochastic_depth.md` §8: ConvNeXt's reference
  75.93% is the EMA shadow's, at 300 epochs with the full aug pack. The 3-step gates say the render
  is right, not that it helps. §0.1.
* **ViT still has no SD**, and cannot until its batched-index move lands (§0.2 ▶3).
...
####      **AND ITS PAYOFF IS BANKED**: stochastic depth is built on that chain and gated (§0.10).
...
tail shared with the per-example renderer, and **the drop render that was the whole point of the
move now exists on it**. The increment log and every finding are below; §0.10 is the SD record.
...
#### ▶ 3. **WHAT IS ACTUALLY NEXT: ViT's batched-index move**, then two smaller items

▶ **ViT's `N := B` move is the last large item in this file**, and stochastic depth is now the
reason to do it rather than a side effect: EfficientNet and ConvNeXt both have SD, `vitTinyImagenetConfig`
sets `dropPath 0.1`, and ViT is the only net that cannot express a per-example mask. ~10 of
ConvNeXt's forms were shared and are built; what is ViT-only is `patchEmbed`, `clsPad`/`clsSlice`,
`headPad`/`headSlice`, `posEmbedGrad`, `rowDenseWeightGrad`, `softmaxRowBack`, `denseRow` — plus
**`matmulF`, the schedule risk** (attention has BOTH operands per-example, where every batched binary
in the kit is pointwise-same-shape; it needs a `batchMap2`-shaped combinator, its own VJP and a
`dot_general` with a batching dimension). ⚠ **ViT's SD is 24 sites, not 12** — each block drops the
attention and MLP branches independently (`ka, km = split(drop_key)`) while sharing one `keep_i`, so
emitting one mask per BLOCK halves the noise and is invisible to every structural check
(`stochastic_depth.md` §6.3). Cost `matmulF` separately before starting.
...
|---|---|---|
| ~~⛔⛔ **EVERY ImageNet TOP-1 THIS REPO HAS EVER PRINTED WAS ARGMAX OVER THE FIRST 10 LOGITS**~~ ✅ **FIXED 2026-08-04** | `F32.argmax10`'s C loop was `for (i = 1; i < 10; i++)` — a **literal**, with no `n` in the signature for anything to check it against. ⚠ **It was correct on every net the repo GATES**: Imagenette, CIFAR-10 and MNIST are all 10-class, so the constant agreed with the net everywhere a test looked, and it was wrong on exactly the tier that has no accuracy gate — the 1000-class ImageNet trainers, where it confined every prediction to labels 0..9. **Caught on the live R34/ImageNet 4-GPU run, and only because the number was compared against the JAX reference**: epoch 1 read 0.85% and epoch 2 read 0.94% while train loss fell 6.13 → 4.77. That asymmetry — 1.37 nats of learning for 0.09 points of top-1 — is the signature. The arithmetic closes it exactly: tfds val is 49,920 imgs / 1000 classes ⇒ **~499 reachable**, and 424 and 471 of them were right, i.e. **85% then 94% on the 10-way sub-problem it was really being asked**. Re-scored with the fix, the epoch-2 net is **~4.4%**, consistent with its own loss (smoothed CE 4.77 ⇒ p_true ≈ 1%). **Training was healthy throughout; only the readout was wrong.** Fix: `F32.argmaxN (ba) (off) (n)`, 23 call sites each passing the class count that is *already* the multiplier in its own `off` expression, so the two cannot drift; `argmax10` DELETED so nothing can call it again. New gate `lake build argmax-check` — 2 gates + a control that re-runs the 10-wide window and requires it to MISS. ⚠ **JAX-side numbers are unaffected** (their eval is generated Python). ⚠⚠ **This is §4b's own lesson recurring one layer down — *a parameter no audit reads is a parameter that can be wrong forever* — except here there was no parameter at all.** | `ffi/f32_helpers.c`, `tests/TestArgmaxN.lean` |
| ▶ **PREFETCH THE SHIM READ — scoped, not built.** The next throughput lever, and the one residency EXPOSES | The step is two blocking calls back to back: `readShimBatchRR` (154 MB off a pipe) then the invoke. Nothing drains the pipe during compute, and **a batch is 154 MB where a pipe's buffer is 64 KB** (this box caps `pipe-max-size` at 1 MB, still 0.6% of a batch) — so the producer can run ahead by essentially nothing and spends most of its life blocked in `write()`. That is why one producer measures 258% CPU on a 32-core box at load 5.6 while the GPUs idle: **capacity is irrelevant when the consumer pulls one batch and then walks away.** The repo's own docstring already says a single producer does ~1,530 img/s and "R34/ImageNet at bs256 needs only ~380", so this was never a producer-throughput problem. ⭐ **Residency makes this WORTH MORE, not less**: with params resident the step is 360 ms of which ~105 is compute (their measured DP figure) and ~5 is params, leaving **~250 ms of image path as the dominant term**. Fix is a depth-1 double buffer — issue the read for step i+1 via `IO.asTask (prio := .dedicated)` BEFORE invoking step i; step becomes `max(read, rest)` instead of `read + rest`, estimated **360 → ~170–200 ms** (~15 h → ~8 h). ⚠ **Depth 1 is not a simplification, it is the correctness condition, twice over**: a pipe is a stream so two concurrent reads on one handle interleave and corrupt the batch; and the resident path carries a generation counter (`res_gen`), so steps must stay strictly in order. With `SHIM_WORKERS=n` the natural depth is n — one in flight per handle, never two on the same one. ⚠ Scope it to the shim path only; the Imagenette/CIFAR branches augment host-side off `augSeed` in the loop. ⚠ **No threading exists anywhere in this repo today** (`IO.asTask` appears zero times), so this introduces a concurrency primitive to a codebase that has none. The gate is cheap and obvious: N steps with and without prefetch, loss sequence **bit-identical** — same handle, same order, same bytes, only the timing moves. ▶ Measure `LEAN_MLIR_BENCH_SYNTH=1` first to split `t_read` from `t_rest` instead of inferring it | `VerifiedTrain.lean` step loop |
| ~~⛔ **THE R34/ImageNet JOB RAN AT 26 h BECAUSE IT NEVER SET `PJRT_FFI_RESIDENT=1`**~~ ✅ **FIXED 2026-08-04** | §2d.3's device-resident parameters are **opt-in**: `resident_enabled()` in `ffi/pjrt_ffi.c` reads `PJRT_FFI_RESIDENT` and **defaults to 0**. The recorded **~16 h / 386 ms/step** was measured with it; `scripts/jobs/r34-default-4gpu.conf` never set it, so every launch of this job silently took the **copying** path. ⭐ **Confirmed on the live run before changing anything**: measured **~600 ms/step**, against this exact shape's recorded pair of **596 copying / 386 resident** — sitting on the copying number to within noise, and `/proc/<pid>/environ` carried only `PJRT_PLUGIN` and `PJRT_REPLICAS`. Without it every step ships **261 MB of `[θ\|m\|v]` to all four replicas and back** (param transport **55.5%** of the step vs 1.2% resident; compute unchanged, 105.6 → 104.8 ms). Fixed by setting it in `ENV_EXTRA` **and adding a `PRECHECK` that refuses to launch without it** — verified with its own control (config minus the line ⇒ `⛔ PRECHECK failed`). ⚠ The ON case DOES announce itself — `[pjrt_ffi] RESIDENT: … holds 330 parameter tensors (249.5 MB) on 4 devices` — but the OFF case prints **nothing at all**, so the failure is an ABSENT line in a 5,000-line log, which is exactly how a benchmark and its production config diverged unnoticed for a week. *An announcement only on the good path is not an announcement.* Re-gated post-stem-pool: `scripts/residency_gate_all.sh r34` **PASS, 255,477,624 bytes bit-identical**, both controls firing. ⚠⚠ **General form, and it is the third instance today: a setting with no output and no gate is a setting that is silently wrong.** ⚠ The other four nets' job configs (when written) need the same line and the same precheck | `scripts/jobs/*.conf`, §2d.3 |
| ~~⛔ **THE VERIFIED PATH AND ITS JAX ORACLE NEVER AGREED ON INIT, OR ON BN MOMENTUM**~~ ✅ **BOTH FIXED 2026-08-04** | Found by chasing the residual gap left after the argmax fix: at epoch 3 this read **9.97%** where the reference read **37.0%**, both still inside the shared 5-epoch warmup (identical lr), so it was not a schedule artifact — and the gap was **narrowing** (1.8 → 1.07 → 0.87 nats), which is the signature of a slow start rather than a defect. Two causes, both measured off the two sources: ⭐ **(1) BN running-stat EMA was 10× too fast.** `F32.ema` computes `(1−m)·running + m·batch`, and the driver passed **`m = 0.1`** against the reference's `momentum=0.99` ⇒ **`m = 0.01`**. Ours averaged ~10 batches where the reference averages ~100. **EVAL-ONLY — it never touched a gradient**, which is exactly why the loss curve looked healthier than the top-1, and it bit hardest early when activation statistics still move fast. Fixed at BOTH sites (`trainAdamSched` and the fp8 variant). ⭐ **(2) `mkParam` used `2/fan_in` for every weight**, where `jax/Jax/Codegen.lean` emits **He fan-OUT** for convs (`emitConvBnInit`: `uniform(±√(6/fan_out))`, torchvision's `mode='fan_out'` for ResNet) and **Glorot** for dense (`emitDenseInit`: `±√(6/(fan_in+fan_out))`). Identical wherever `ic == oc`; the gaps are the R34 stem (fan_in **147** vs fan_out **3136** — 4.6× in σ), each stage-entry conv and 1×1 projection (2×), and every classifier (`2/512` vs `2/1512`, 1.7× in σ). ⚠ **This moves the init of EVERY verified net**, so no recorded accuracy is seed-reproducible any more — but it changes **no committed artifact**, init being host-side. ⚠ The DISTRIBUTION is still unmatched on purpose: `heInit` sums three uniforms (Bates-3, ≈normal) vs JAX's single uniform — variance matched, shape not, and torchvision uses a normal so neither side is canonical there. ⚠⚠ **`zeroGamma` is NOT the explanation and was checked**: `emitConvBnInit`'s flag is passed only for BOTTLENECK blocks, so it is a ResNet-50 concern and R34 inits γ=1 on both sides | `VerifiedTrain.lean` `mkParam` + the BN EMA |
| ⚠ **BN running statistics are NOT in the checkpoint** — found while fixing the row above | The checkpoint is exactly `[θ\|m\|v]` (3 × 21,797,672 floats for R34/ImageNet, byte-checked), so a resume restores weights and optimizer state but **restarts the BN running stats from init**. On this box the supervisor restarts on AER and thermal events, so this fires in normal operation, and the first eval after any restart under-reads until the stats re-converge. Measured incidentally: re-scoring the epoch-2 weights through a fresh process gave **4.4% against near-init stats**, i.e. a LOWER BOUND on that net's true accuracy. ⛔ Not fixed — it changes the checkpoint format, which is a §2n call | `VerifiedTrain.lean` ~1286–1320 |
...
### §0.8 ✅ **GRAD CLIP — BUILT AND GATED 2026-08-02.** `planning/archive/grad_clip.md` §11 is the record
...
⚠⚠ **FULLY SCOPED 2026-08-02 — `planning/archive/grad_clip.md` is the spec and SUPERSEDES this section.**
...
(`planning/archive/ema.md`: `emarms` does not start with `"rms"`).
...
audit **96 artifacts, one writer each** · build 3,911 green. Full write-up in `planning/archive/ema.md`.
...
* **Two specs**, `planning/archive/ema.md` and `planning/archive/stochastic_depth.md`, both written before building
...
* **`planning/archive/recipe_gaps.md` is new and is the plan.** Its headline: **ResNet-34 is already at
...
artifact untouched, all 6 GPUs idle, `scripts/jobs/r34-default-4gpu.conf` correct.
...
⚠ Note `r34-default-4gpu.conf` uses `DEVS="0,2,3,4"`, **not** 0-3 — it excludes the AER-unclean
...
### ▶ Then, in value order — **superseded by `planning/archive/recipe_gaps.md`, read that first**
...
1. **bf16** — and its case just got much stronger, twice: measured **×1.76 on ares** (three runs;
   ...
   sessions, needs `conv_close_mixed`; `planning/archive/bf16_renderer.md`.
...
# GRAD CLIP — global-norm clipping (`recipe_gaps.md` v1.4b, `planning/archive/grad_clip.md`). Two SHlo ops,
...
**This makes §2d.3 the top lever, ahead of bf16** (`planning/archive/bf16_renderer.md`, which is 4-6
...
* **A long run is now supervisable**: `scripts/supervise.sh r34-default-4gpu` (engine +
...
check it against (`planning/archive/bf16_renderer.md`, 4-6 sessions, needs `conv_close_mixed`). The
