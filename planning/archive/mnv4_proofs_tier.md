...
| ⚠ **no quoted number**: Conv-S Imagenette 87.36% (superseded spec, kept in `historical/RESULTS.md` as such); Conv-M has no Imagenette run; the verified ImageNet port (`mobilenetv4-imagenet-verified`, `scripts/jobs/mnv4-default-4gpu.conf`) has not run; the JAX Conv-M reference is 75.51% (outside the repo, `/home/skoonce/mnv4_convm_100ep`) | `historical/RESULTS.md` §MobileNetV4-Conv-S; `VerifiedNets.lean` ("no Imagenette accuracy run of its own yet"); memory |
