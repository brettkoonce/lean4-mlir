...
* **EMA** — ⚠ **SCOPED 2026-08-02, `planning/archive/ema.md`, and the "~a day, driver-only" estimate here
...
* ~~**grad clip** needs the **global** norm across every parameter gradient, then a rescale — a
  ...
  fold is an ordinary tree. `planning/archive/grad_clip.md`. The original note follows.
  **grad clip** needs the **global** norm across every parameter gradient, then a rescale — a
...
* ~~**stochastic depth** is architectural: a per-block Bernoulli mask plus its VJP, and a train/eval
  ...
  `planning/archive/stochastic_depth.md`, and this tiering is wrong in BOTH directions.** The math is
...
**bf16 / bf16Conv.** 4-6 sessions, needs `conv_close_mixed`, and `planning/archive/bf16_renderer.md`
...
`planning/archive/ema.md` — scoped, then built. **Zero new `SHlo` ops**: `adamMNext β₁ m g = β₁·m + (1−β₁)·g`
...
### v1.4b — ✅ **grad clip: DONE, BOTH NETS, 2026-08-02.** ⚠ **SCOPED 2026-08-02, `planning/archive/grad_clip.md`, and the Tier E entry below is WRONG BY A TIER.**

`planning/archive/grad_clip.md` §11 is the record: **two `SHlo` ops, no new proof machinery, no driver
...
`planning/archive/stochastic_depth.md`. One `SHlo` op whose **VJP is `layerScale_has_vjp` verbatim** (no new
...
`conv_close_mixed`; `planning/archive/bf16_renderer.md`; 4-6 sessions.
