...
and a priority. Companion to `planning/archive/muon.md` (Muon is Thread 1's first landed point).
...
- **Render-faithful (`den=`):** the whole family is `dot_general` + NS ⇒ reachable, same tech as
  the Muon tie (`planning/archive/muon.md §8.6`).
...
this ladder is now a concrete plan: **`planning/archive/natural_gradient.md`** — steepest descent under a metric
...
cert assumes — is now a concrete plan: **`planning/archive/power_iteration_lipschitz.md`**. Power iteration is
...
The *deterministic-local* sibling is **`planning/archive/ibp_certification.md`** — verified Interval Bound
...
- In-repo: `planning/archive/muon.md` (Thread 1's landed first point), `Tensor.lean` (`pdiv`/`HasVJP`),
