...
Successor to `planning/archive/detector_pjrt_port.md`, which ported **one** demo (the VisDrone FPN
...
`planning/archive/brats_demo.md` — the FINAL VERDICT table, the wave-2 ablation, the
...
on mismatched image/mask pairs every epoch. `planning/archive/post_shuffle_fix.md` §1
...
§1, `demos/README.md`, `planning/archive/r34_brats_retrain.md`, and blueprint §10.2.3,
...
monotonically. `planning/archive/brats_demo.md` now carries a STOP banner; `post_shuffle_fix.md`
...
* `planning/archive/detector_pjrt_port.md` — the precedent. §9 is the outcome, the two things that plan got
  wrong, and the gate design this document reuses wholesale.
* `planning/archive/xla_pjrt_ladder.md` — had the "specialised entry points are the blocker" call right
