...
the batch-divisible subset with live weights.** `planning/archive/paper_faithfulness.md:153`
...
| gap | status | evidence |
|---|---|---|
| **Data parallel** | ✅ **RENDERED AND TIED 2026-08-27** | Was "not rendered", then "rendered but untied", now neither. `mnv4in_adamdp64` + its bf16 peer are 4-replica renders, and both halves of the tie are green: `tests/TestMnv4DpCheck.lean` (fp32 `bnstat` bit-exact 67,904/67,904, gradient 8.45e-7; bf16 bit-exact on all 9,715,512 floats) and the `mnv4in` row in `tests/TestShardCheck.lean` (TEST 1.10e-6 vs CONTROL 2.00). Both go red on a sum-not-mean render. → `scripts/jobs/mnv4-default-4gpu.conf`, a **measured 25.6 h** at 4×bs64 f32 (177 ms/step, `bf16_renderer.md` §21.1), 18.6 h in bf16. ⚠⚠ **The 63.5 h this row used to quote is a Conv-S figure** — 114 ms/step measured before the Conv-M conversion (4.1M → 9.7M), so it never described this network. No single-card Conv-M number has been measured. ▶ `runs/2026-08-27-mnv4-dp-shard-gates/` |
...
| **bf16** | ✅ **rendered** (this row was stale) | `mnv4in_adam64bf16` and `mnv4in_adamdp64bf16` are on disk (2026-08-25). MNv4 was the first net to need the SYMMETRIC-pad `depthwiseStrided` bf16 family — 47 call sites, 3 new ops (`bf16_renderer.md` §13). Measured **1.40×** at 4×bs64, 177 → 126 ms/step. The "0 bf16 artifacts" claim was true when written and is not now |
...
▶ **SCOPED 2026-08-14 in `planning/archive/mnv2_fold_and_mnv4_convm.md`**, with the measurement
...
⚠ `planning/archive/paper_faithfulness.md:26` and `:120` still say **DEFERRED** and are now stale on
...
marker collisions (`planning/archive/ema.md`) are the reason the markers are spelled `drop` and `do`
