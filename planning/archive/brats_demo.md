...
and `ca83835`. Ledger of everything it invalidated: `planning/archive/post_shuffle_fix.md` §1.
...
2. ✅ **Pets is done — and it was the same bug.** Re-run 2026-08-26: mIoU
   ...
   *opening premise* of this document's original version, which cited Pets'
   ...
   `planning/archive/unet_demo_v2.md` is **unresolved**, not merely inconclusive.
...
5. **3D / 2.5D** — `planning/archive/unet3d.md`. Cheapest real win is 2.5D (stack 2k+1
...
* `blueprint/src/content.tex` §10.2.3 — the published write-up. Already correct
  ...
  appendix**.
* `planning/archive/post_shuffle_fix.md` — §1 the invalidation ledger, §1a the decisive
  test and its answer.
* `planning/archive/r34_brats_retrain.md` — the post-fix R34 retrain.
* `planning/archive/demo_xla_port.md` §4.5 — the XLA port and the 2026-08-26 sweep.
* `planning/archive/unet_demo_v2.md` — the Pets-side seg stack (⚠ void, see §5.2).
* `planning/archive/unet3d.md` — the volumetric follow-on.
...
- **IREE lowers rank-5 fine — MEASURED, not assumed.** A hand-written
  ...
  Kept at `planning/archive/conv3d_spike.mlir` so the claim is re-runnable:
  `.venv/bin/iree-compile planning/archive/conv3d_spike.mlir
...
Phase 7  D      2.5D                                    (Gate D) → gates planning/archive/unet3d.md
...
precedent). It is its own workstream with its own doc — `planning/archive/unet3d.md`
— gated on Gate D
