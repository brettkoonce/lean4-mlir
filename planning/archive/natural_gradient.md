...
- Files: `LeanMlir/Proofs/NaturalGradient.lean` (imports `MuonGeometry`), root in `lakefile.lean`,
  `#print axioms` in `tests/AuditAxioms.lean`. Companion to `planning/archive/muon_geometry.md`,
  `planning/archive/shampoo.md`, `math_threads.md` Thread 1.
...
`planning/archive/power_iteration_lipschitz.md` (both reuse the spectral `√`/eigen engine) — together they are
