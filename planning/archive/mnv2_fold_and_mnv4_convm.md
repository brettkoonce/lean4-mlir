...
(`planning/archive/chapter_makeover.md`). They are in one doc because they share a shape:
...
backward's dispatch, which is what the ties in `planning/archive/mnv4_convm_ties_todo.md` are still owed
...
Both jobs are governed by `planning/archive/chapter_makeover.md` §5, and two of its rules matter
