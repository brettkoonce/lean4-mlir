...
[[math-threads-demo-first]], `planning/archive/muon.md`, `planning/archive/shampoo.md`.
...
- **Next session — only optional/heavy forks remain:**
  ...
  - **(b) Newton–Schulz convergence** — formalize the implementation's NS iteration → `UVᵀ` (the
    ...
    `planning/archive/muon_ns_convergence.md`** — key insight: NS acts only on singular values, so matrix
