...
`planning/archive/muon_geometry.md`, `LeanMlir/Proofs/MuonGeometry.lean`). L1–L6 proved that the polar factor
...
`den=` Newton–Schulz scaffold; design notes in `planning/archive/muon.md` §"Newton–Schulz".
...
sign / polar iteration family), Jordan 2024 (Muon), `planning/archive/muon.md`.
...
- **Jordan's tuned quintic `(3.4445, −4.7750, 2.0315)`**: tuned for *fast approach to a band near 1
  ...
  (fixed 5 steps, "rough is fine; we recompute next step anyway", `planning/archive/muon.md:91`).
...
5. Commit per-phase (the ladder's per-rung commit rhythm), update `planning/archive/muon_geometry.md` §4 +
