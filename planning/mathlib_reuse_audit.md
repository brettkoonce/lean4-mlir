...
## Status (2026-09-19, main `b8124630`, 11 ahead of origin, + the staged row)
...
| *(staged)* | **§6 live misc** (−16): `ResNet34LivePC.flatConv_diag_id` takes a bias (`b`, `hb`) and absorbs `MobileNetV2SealRealistic`'s `flatConv_diag_id_b`; LivePC's `relu_const_pos` goes for the identical `Proofs.relu_const_pos` in ResNet34.lean |
