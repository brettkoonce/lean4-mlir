...
## Status (2026-09-18, main `45a6eb27`, 2 ahead of origin, + the staged row)

**Landed** — about 8.3k lines out, every pinned theorem name and statement unchanged:
...
2. **§7 `BnPairTiedB`** (~1,200, verified on `mnv2Stride1TiedB`) — the BN γ/β conjunct pair, proven by
   ...
   **Decided 2026-09-18: (a), fold into the defs** (neither `formalization.yaml` nor the book shows them).
   MobileNet/EfficientNet family staged (38 pairs, −509); next the ResNet family — `ResNet50StepTieB` (11)
   and `ResNet34StepTieB` (6), ~−290 by the dry run. The fused-SGD twin (`bnGammaSgd{,B}` pairs in
   `EfficientNetStepTie`, `Cifar8BnStepTie`, `CifarBnStepTie`) is a different pair and a later batch.
