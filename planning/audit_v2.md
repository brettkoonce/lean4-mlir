...
3. ◐ **§2.5 batched back-link kit** (parts 1–2: stages, stage layers, back links, leaf ties, pool/head layers, seal pieces; left: the sync kit, the small-net kit, the `c·h·w` seam's other spellings) — `EnetTiePoC`'s `reassocB`/`cInB`/`bnBackB`/… (in the retired
