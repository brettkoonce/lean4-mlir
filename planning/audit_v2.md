...
3. ◐ **§2.5 batched back-link kit** (part 1 landed: stages + back links; left: `ConvBackCertifiedTie` mv, `comp_ok_of`/`r34PoolLayer` → CertifiedChain/HeadLayers, seal pieces → BatchSealKit, the sync kit, the small-net kit, the `c·h·w` seam's other spellings) — `EnetTiePoC`'s `reassocB`/`cInB`/`bnBackB`/… (in the retired
