...
- Coordinates are standardised for the network: `z = (x - c)/s` with
  ...
  Same reason `scripts/datasets/preprocess_toy2d.py` scales its targets to unit radius.
...
`scripts/datasets/preprocess_boltzmann.py [n] [outdir=data/boltzmann]`, numpy only:
...
`scripts/datasets/preprocess_boltzmann.py`, no download".
