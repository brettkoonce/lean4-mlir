...
| **CNN conv2/1 bias** | `cnn_conv{2,1}_bias_grad_close` ✅ | `cnn_conv{2,1}_bias_float_sgd_descends` ✅ | ✅ CLOSED (Increment 5) |
| deep nets / joint step | — | — | OUT OF SCOPE (honest stop) |

**The whole §3 CNN descent program is now CLOSED** — every conv weight AND bias of the
Chapter-4 MNIST CNN is a float-faithful descent step (gradient accuracy *proven*, not
assumed). Only the honest-stop line below remains open by design.
...
**Increment 5 — the conv biases. ✅ DONE (3-axiom clean, audited).** Both bias rungs landed
exactly as planned, the strictly-easier (sum-not-dot) twins of the weight rungs:
- **One new core**, as predicted: `FloatModel.sum_perturbed_close` — the `M.sum` peer of
  `dot_perturbed_close` (mirror its proof with `M.sum`/`sum_close`; the `a·` input factor drops
  out because the bias Jacobian carries no input window). Everything else is reuse.
- **conv2-bias**: `cnnConv2BiasFloatGrad` (`fun o => M.sum (cotWin c̃Conv o)`), bridge
  `cnn_conv2_bias_loss_gradAt_reluMask` (`head3_cot_reluMask` + Kronecker `Finset.sum_eq_single`
  + `convBiasGrad_eq_sum`), budget `cnnConv2BiasGradBudget` (= `cnnConv2GradBudget` with `a·`
  stripped), grad-close `cnn_conv2_bias_grad_close` (reuses `cnn_conv2_cot_close` at the EXACT
  conv-2 input `x₁`, `aX2=a`/`eX2=0`), wiring `cnn_conv2_bias_float_sgd_descends`.
- **conv1-bias**: the same five decls one conv-backward deeper, reusing `cnn_conv2_cot_close` at
  the FLOAT conv-2 input `relu(z̃₁)` + `convTap_back_close` + `mask_scalar_close`, closed by
  `sum_perturbed_close`. The def/bridge cotangents were generated by transforming the conv1-WEIGHT
  expressions (`(Kernel4.unflatten u)`→`W₁`) to keep the deep-softmax parens exact.
- **Gotchas confirmed:** the off-by-one deep-softmax paren (the `(Kernel4.unflatten v)`→`W₂` swap
  removes a balanced pair but the OUTER close-count is unchanged — needs the same 9/13 closes
  before `label`); the bias bridge needs `convBiasGrad_eq_sum _ o` + a final `one_mul` to clear
  the `1 *` Kronecker factor. The wiring's `0 ≤ budget` falls out for free as
  `le_trans (abs_nonneg _) (hgh ⟨0, hc⟩)` (no need to re-derive the nonneg chain). All 11 decls in
  `tests/AuditAxioms.lean`; build + coverage clean. *Effort: as estimated (≈½ each weight rung).*

**Original Increment 5 cold-start (for reference).** `cnn_conv2_bias_*` then `cnn_conv1_bias_*` —
strictly easier than the weight rungs (no input window, no kernel mass): the bias Jacobian is the
**Kronecker channel indicator** (`conv2d_bias_pdiv`), so the bias gradient is a bare **spatial
sum** of the conv-output cotangent (`∑ₛ cotWin cConv o`, `convBiasGrad_eq_sum` at
SgdDescentCnn.lean:1438) — a SUM, not a dot.
