...
User rules for this pass: the demos in the book's TOC are the canonical list; ROCm-era and Pets
material is old lab work and is pruned, not archived.

* **Deleted (52):** all ROCm/AMD-era scripts, including the (H)-marked ones (`jax_*_bench.py`: the
  upstream-issues README now says they are in git history); the BraTS probes nothing cites
  (crop guard, overfit subset, oversample, region dice, `run_brats_ablation.sh`); all eleven
  FPN/VisDrone investigation probes; Pets' `yolo_map.py` / `yolo_render.py`; the three mocks;
  `bf16_probe_4gpu.sh`, `queue_r50_a3_pair.sh`, `bf16_boundary_probe.py`, `geo_aug_pil_diff.py`,
  `lipschitz_cert_power_iter.py`, both `emit_*_tikz.py` (they wrote only into `runs/`);
  `rope_test.sh` + `run_tinystories_8k.sh` after folding them into
  `lm_demos_modernization.md` §5.
* **Moved to `historical/` (2):** `run_smooth_scorecard.sh` (the protocol run behind the committed
  `SmoothingCPScorecard` data; the generated Lean names it by basename, unchanged) and
  `lipschitz_cert_rationalize.py` (the `LipschitzCertInstance` weights; `TrainedMlpWitness.lean`
  and its generator now say `historical/…`; regenerated byte-identical, `lake build Certs` green).
* **Kept, TOC demo (BraTS):** `brats_class_weights.py`, `brats_r34_fd_probe.py`,
  `brats_r34_ab.py` + `run_brats_r34_ab.sh` (cited from Lean, `demos/README.md`, the 2.5D plan);
  their ROCm env defaults are now CUDA.
* **Decided:** `convnext_forward_tie.py` cited from `jax/MainConvNeXtImagenet.lean` as the
  mnv2/mnv4/enet siblings are from theirs; `gw_compare.py` cited in `gw_detection_demo.md` §5;
  `check_pinned_env.py --allow-cpu` runs in jax.yml's convention job, whose jax is now the
