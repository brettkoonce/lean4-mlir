...
| 8 | Tests and roots on retired artifacts | `ConvLossFold` deleted in 2e; the regen-script line dropped in the sweep; `TestResnet34Train`, `TestResnet34TrainPC` and `ResNet50BlocksCertified` deleted with R.A | still open: `tests/TestMobilenetV2Train.lean` (its `mobilenetv2_train_step.mlir` is gone) and `TestMobilenetV2TrainPC` (a live smoke of the committed AdamW bytes plus a /tmp SGD render) — decide with M.2 |
