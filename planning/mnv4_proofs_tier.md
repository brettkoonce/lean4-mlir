...
in `planning/proofs_tier_to_paper_nets.md`'s table with nothing at the net level, and the last
...
`planning/mnv4_verified.md` §8d records closing the same day and nobody edited. The row copied
...
| ⚠ **empirical ties are OWED for Conv-M**: the forward tie (1.423e-6) and gradient tie (0/147) were at the Conv-S table; `mnv4_forward_tie.py`'s reference `.py` is stale and `grad_tie.py`'s `NETS["mnv4"]` still says `nparams=158, nstats=104` (must be 233 / 154) | `planning/mnv4_convm_ties_todo.md` (2026-08-14, still open) |
...
(b) ⚠ **ASK, then run the two owed ties** (`planning/mnv4_convm_ties_todo.md` §1–2b has the
...
`planning/float_budget_numbers.md` closed the float-budget thread as vacuous by user decision on
...
| session | new | edited |
|---|---|---|
| 0 | — | `Foundation/MobileNetV4BackB0.lean` (header, `mnv4Stage14`), `formalization.yaml` (three rows), `planning/proofs_tier_to_paper_nets.md` §3.6, `scripts/grad_tie.py` (233/154) with the tie runs |
