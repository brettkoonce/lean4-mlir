...
/-! Tiny DDPM trainer on CIFAR-10 — sincos t-embedding variant.

    Same architecture as `cifar-ddpm-train` (base 80 plain conv, no
    bottleneck attention), but swaps the single-channel `t/T_max`
    tile for a multi-channel sincos time embedding (Vaswani / NeRF
...
def nFreq : Nat := 4
def inCh  : Nat := 3 + 2 * nFreq  -- 3 RGB + 8 sincos t-channels = 11

def tinyCifarDdpm : NetSpec where
  name := "DDPM UNet sincos-t base80 (CIFAR 32x32x3)"
  imageH := 32
  imageW := 32
  layers := [
    .unetDown inCh 80,              -- 11 input channels (3 RGB + 8 sincos t)
    .unetDown 80 160,
