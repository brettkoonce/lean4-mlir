...
/-! FPN multi-scale YOLO detector on VisDrone at 448 (detection brick #3,
    ...
    **Shared body for both backends** (`planning/detector_pjrt_port.md`). The two
    exes are thin roots over `runYolov1VisdroneFpn` below and differ only in which
    shim they link: `MainYolov1VisdroneFpn` → `ireeLink`, `MainYolov1VisdroneFpnXla`
    → `xlaLink`. Nothing above the shim moves, so the graph both run is the same
    emitted StableHLO; only the trusted lowerer differs.
    ...
    Usage:
      ...
      HIP_VISIBLE_DEVICES=1 .lake/build/bin/yolov1-visdrone-fpn-xla data/visdrone_fpn
    ⚠ `IREE_BACKEND` is inert on the XLA path — PJRT never reads it.
-/
